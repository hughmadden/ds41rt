...
def validate_exl3_expert_snapshot(
    ...
    """Read and fail closed once on a calibrated DS4RT EXL3 snapshot."""
    ...
    for key, expected in expected_top.items():
        if quant.get(key) != expected:
            raise ValueError(
                f"DS4RT EXL3 requires quantization_config.{key}={expected!r}, "
                f"got {quant.get(key)!r}"
            )
    calibration = quant.get("calibration") or {}
    swiglu_limit = float(raw["swiglu_limit"])
    ds4rt = quant.get("ds4rt") or {}
    ...
    if not calibration_valid:
        raise ValueError(
            "DS4RT EXL3 requires a recognized checkpoint-bound bounded DeepSeek "
    ...
    for key, expected in expected_ds4rt.items():
        if ds4rt.get(key) != expected:
            raise ValueError(
                f"DS4RT EXL3 requires ds4rt.{key}={expected!r}, "
                f"got {ds4rt.get(key)!r}"
            )
    source = ds4rt.get("quantizer_source") or {}
    expected_source = {
    ...
    if source != expected_source:
        raise ValueError("DS4RT EXL3 quantizer provenance does not match its recipe")
...
def read_exl3_expert_config(snapshot: str | Path) -> NativeExpertConfig:
    """Read and fail closed on a calibrated DS4RT EXL3 contract."""
...
def _load_exl3_expert_layer(
    ...
    *,
    ...
    if device not in {"cuda", "cuda:0"}:
        raise ValueError(f"DS4RT permits only physical GPU 0, got {device!r}")
    if quant_mode != "w4a16":
