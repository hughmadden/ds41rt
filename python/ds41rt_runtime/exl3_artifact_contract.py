"""Fail-closed contract for DS41RT's GPTQModel-native EXL3 publication."""
...
ARTIFACT_SCHEMA = "ds41rt-deepseek-v4-gptqmodel-artifact-v1"
PLAN_FILE = "ds41rt-gptqmodel-plan.json"
RUN_FILE = "ds41rt-gptqmodel-run.json"
ARTIFACT_FILE = "ds41rt-gptqmodel-artifact.json"
LEDGER_FILE = "ds41rt-exl3-error-ledger.jsonl"
LEDGER_MANIFEST_FILE = "ds41rt-exl3-error-ledger.manifest.json"
CANONICAL_ASSEMBLY_FILE = "ds41rt-exl3-canonical-assembly.json"
CANONICAL_ASSEMBLY_SCHEMA = "ds41rt-exl3-canonical-hybrid-assembly-v1"
COMPOSITE_ASSEMBLY_SCHEMA = "ds41rt-exl3-canonical-hybrid-assembly-v2"
PROJECTION_ASSEMBLY_SCHEMA = (
    "ds41rt-exl3-canonical-hybrid-projection-assembly-v1"
)
...
QUANT_CONFIG_ASSEMBLY_SCHEMA = (
    "ds41rt-exl3-canonical-quant-config-assembly-v1"
)
BLOCK_AUDIT_SET_SCHEMA = "ds41rt-exl3-independent-block-audit-set-v1"
COMPOSITE_BLOCK_AUDIT_SET_SCHEMA = (
    "ds41rt-exl3-independent-composite-block-audit-set-v1"
)
...
ZERO_ROUTE_RECOVERY_CONFIG = {
    "contract": "ds41rt.exl3-zero-route-recovery",
...
def _inline_mixed_policies(
    ...
    compatibility = meta.get("ds41rt_inline_mixed") if isinstance(meta, dict) else None
    published_namespaces = (
        meta.get("ds41rt_inline_mixed_namespaces")
...
def validate_gptqmodel_native_exl3(
    ...
    *,
    ...
    provenance = meta.get("ds41rt_error_ledger")
    if (
...
def _validate_canonical_tensor_storage(
    ...
    provenance = quant.get("meta", {}).get("ds41rt_error_ledger")
    family_join = (
...
def _valid_recovery_sample_accounting(
    ...
    return (
        recovery.get("schema") == "ds41rt.exl3-zero-route-recovery"
...
def _validate_error_ledger(
    ...
    if (
        manifest.get("schema") != "ds41rt.exl3-error-ledger"
    ...
    try:
        for line_number, line in enumerate(ledger_payload.splitlines(), 1):
            ...
            if (
                record.get("schema") != "ds41rt.exl3-error-ledger"
...
def validate_gptqmodel_publication(
    ...
    *,
    ...
    if (
        ...
        or plan.get("ledger_provenance") != quant["meta"]["ds41rt_error_ledger"]
