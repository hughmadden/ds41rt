#!/usr/bin/env python3
"""Benchmark SparkInfer MLA query projection against DS4RT's native routes.

The candidate uses four non-copying H=16 views of GLM-5's H=64 tensors.  The
two controls reproduce the native launch structures used by DS4RT:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--native-library",
        type=Path,
        default=Path("native/build-cuda-sparkinfer-routed/libds4rt_native.so"),
    )
...
def configure_native(path: Path) -> NativeApi:
    ...
    matmul = library.ds4rt_cuda_matmul_bf16_strided_batched_cublas_async
    matmul.argtypes = (
        pointer,
        pointer,
        pointer,
    ...
    copy_2d = library.ds4rt_copy_d2d_2d_async
    copy_2d.argtypes = (
        DeviceBuffer,
        size,
        DeviceBuffer,
    ...
    transpose = library.ds4rt_cuda_transpose_rows_heads_bf16_async
    transpose.argtypes = (pointer, pointer, size, size, size, pointer)
    transpose.restype = ctypes.c_int

    compose = library.ds4rt_cuda_mla_compose_absorbed_query_bf16_async
    compose.argtypes = (
        pointer,
        pointer,
        pointer,
...
def check_status(api: NativeApi, status: int, action: str) -> None:
    ...
    api.library.ds4rt_last_error(message, len(message))
    raise RuntimeError(
...
def main() -> None:
    ...
    # weights.  The unused V rows remain in place, exactly as in DS4RT.
    weights = (
        torch.randn(
    ...
    result = {
        "schema": "ds4rt-sparkinfer-mla-query-projection-benchmark-v1",
