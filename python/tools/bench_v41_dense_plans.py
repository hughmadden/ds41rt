#!/usr/bin/env python3
"""Probe query-B AOT tile choices against native quantization/GEMM on official weights."""
import argparse
import ctypes as C
...
def main():
    ...
    weight = load('layers.0.attn.wq_b.weight')
    scale = load('layers.0.attn.wq_b.scale')
    n,k = weight.shape
    assert (n,k)==(32768,1280)
    ...
    report = {'scope':__doc__,'native_sha256':hashlib.sha256(args.native_lib.read_bytes()).hexdigest(),
        ...
        'shape':[n,k],'sms':sms,'eviction_bytes':256<<20,'cases':[],'hardware':[]}
    ...
    for capacity in [1,16]:
        ...
        for tile in [(16,64),(16,128),(32,64),(32,128)]:
            ...
            try:
                ...
                assert slices==1
                plans[capacity,tile] = (quant,compiled)
                print('COMPILED',capacity,tile,flush=True)
            finally: dense._select_default_dense_gemm_plan = selector
    ...
    try:
        for capacity in [1,16]:
            ...
            outputs = [torch.empty((capacity,n),dtype=torch.bfloat16,device='cuda') for _ in range(5)]
            arenas = [torch.empty_like(scratch) for _ in range(4)]
            ...
            for rows in ([1] if capacity==1 else [1,5,6,16]):
                ...
                for index,tile in enumerate([(16,64),(16,128),(32,64),(32,128)]):
                    quant,gemm = plans[capacity,tile]; arena = arenas[index]; output = outputs[index+1]
                    def run(quant=quant,gemm=gemm,arena=arena,output=output):
                        ...
                        gemm(ptr(address+info.values,cutlass.Float8E4M3FN),ptr(weight.data_ptr(),cutlass.Float8E4M3FN),
                             ...
                             ptr(output.data_ptr(),cutlass.BFloat16),ptr(address+info.values,cutlass.Float8E4M3FN),
                    ...
                    runs.append(run); names.append(f'{tile[0]}x{tile[1]}')
                graphs = []; x.normal_()
                for run in runs:
                ...
                for repeat in range(6):
                    order = list(range(5)); order = order[repeat%5:]+order[:repeat%5]
    ...
    report['limitations']=['Official weights with synthetic finite activations; bounded numerical comparison to native output, not full-model quality.',
        'Component graph timing includes quantization and GEMM; eviction work is outside events.',
