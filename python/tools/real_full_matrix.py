...
MODEL_ID = DEFAULT_FLASH_FULL_MODEL_ID
DS4_USER_PREFIX = f"{DS4_BOS}{DS4_USER}"
...
def default_tokenizer_path() -> Path:
    return default_flash_tokenizer_path()
...
def load_snapshot(
    ...
    if not rendered_prefix.startswith(DS4_USER_PREFIX):
        raise ValueError(
            "snapshot does not begin with the supported non-thinking DeepSeek "
            "V4 user template"
        )
...
def render_messages(messages: list[dict[str, str]]) -> str:
    return render_nonthinking_messages(messages)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "packed KV snapshot root; required in snapshot mode so the "
    ...
    parser.add_argument(
        "--tokenizer",
        type=Path,
        help="tokenizer.json; defaults to the selected local Flash snapshot",
    )
...
def main() -> int:
    ...
    for case in args.case:
        ...
        if args.mode == "snapshot":
            cached_message = snapshot_rendered_prefix[len(DS4_USER_PREFIX) :]
            messages = [
