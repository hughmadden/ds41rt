...
from deepseek_v4_benchmark import (
    ...
    default_flash_tokenizer_path,
    render_simple_user_prompt,
)
...
FIXTURES = {
    "count-13": Fixture(761, 25, 13),
    "count-50": Fixture(946, 99, 50),
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tokenizer",
        type=Path,
        help="DeepSeek tokenizer used to derive the pinned token-count contract.",
    )
...
def prompt_content(fixture: Fixture) -> str:
    return (
...
def expected_content(fixture: Fixture) -> str:
...
def main() -> None:
    ...
    content = prompt_content(fixture)
    expected = expected_content(fixture)
    try:
        tokenizer_path = args.tokenizer or default_flash_tokenizer_path()
    except FileNotFoundError as error:
        raise SystemExit(str(error)) from error
    tokenizer = Tokenizer.from_file(str(tokenizer_path))
    expected_prompt_tokens = len(
        tokenizer.encode(
            render_simple_user_prompt(content), add_special_tokens=False
        ).ids
    )
    expected_completion_tokens = len(
        tokenizer.encode(expected, add_special_tokens=False).ids
    )
    if expected_completion_tokens != fixture.max_tokens:
        raise SystemExit(
            f"fixture {args.fixture} expected text encodes to "
            f"{expected_completion_tokens} tokens, not its {fixture.max_tokens}-token "
            "generation budget"
        )
    payload = json.dumps(
        {
    ...
    summary = {
        ...
        "all_prompt_token_counts_match": all(
            sample["prompt_tokens"] == expected_prompt_tokens for sample in samples
        ),
        "expected_completion_tokens": expected_completion_tokens,
        "all_completion_token_counts_match": all(
            sample["completion_tokens"] == expected_completion_tokens
