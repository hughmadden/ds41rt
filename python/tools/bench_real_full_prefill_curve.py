...
def comma_separated_nonnegative_ints(value: str) -> list[int]:
    try:
        values = [int(item) for item in value.split(",")]
    except ValueError as error:
        raise argparse.ArgumentTypeError("values must be comma-separated integers") from error
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--prefix-tokens",
        type=comma_separated_nonnegative_ints,
        default=list(DEFAULT_PREFIXES),
        help="cached-prefix targets; zero measures an exact-row fresh prefill",
    )
    parser.add_argument(
        "--suffix-rows",
...
def main() -> None:
    ...
    for target_prefix in args.prefix_tokens:
        if target_prefix == 0:
            ...
            for repeat in range(args.repeats):
                ...
                if repeat % 3 == 1:
                    ...
                    random.Random(args.nonce_seed + repeat).shuffle(order)
                orders.append(order)
            for pass_index, order in enumerate(orders):
                for suffix_rows in order:
                    ...
                    def fresh_metric(source_text: str) -> tuple[int, dict[str, Any]]:
                        messages = [
                    ...
                    _, planned_rows, extra = closest_slice(
                        ...
                        build=fresh_metric,
                        target=suffix_rows,
                        initial_high=suffix_rows,
                    )
                    if planned_rows != suffix_rows:
                        raise RuntimeError(
                            f"could not construct exact {suffix_rows}-row fresh "
                            f"prompt; got {planned_rows}"
                        )
                    samples.append(
                        {
            ...
            plans.append(
                {
                    ...
                    "prefix_content": None,
                    "seed_prompt_tokens": 0,
                    "planned_probe_prefix_tokens": 0,
                    "prefix_nonce": None,
                    "samples": samples,
                }
            )
        ...
        # The first suffix publishes the one-token merged-user separator into
        # the radix tree. Seed one token below the reported context target and
        # make the uncounted suffix warmup populate that stable separator.
        ...
        for plan in plans:
            if plan["target_prefix_tokens"] > 0:
                ...
                seed_record = {
                    ...
                    "prefill_ms": float(
                        seed_result["metrics"].get("prefill_ms") or 0.0
                    ),
                }
