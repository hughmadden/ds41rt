...
def paired_by_m(
    ...
    for (physical_m, chain_id), paths in by_chain.items():
        ...
        coordinator_layers = tuple(
            float(value) for value in coordinator["layer_ms"]
        )
        spark_layers = tuple(float(value) for value in spark["layer_ms"])
        if not coordinator_layers or len(coordinator_layers) != len(spark_layers):
            raise ValueError(
                f"M={physical_m} {chain_id} has incomplete or mismatched "
                "per-layer samples"
            )
        result[physical_m].append(
            ReplayPair(
                ...
                coordinator_layer_ms=coordinator_layers,
                spark_layer_ms=spark_layers,
            )
        )
...
def summarize(
    ...
    missing_geometry = [key for key in geometry_keys if key not in manifest]
    if missing_geometry:
        raise ValueError(
            f"result manifest lacks model geometry fields {missing_geometry}"
        )
    expected_layer_samples = len(manifest["routed_layers"])
    if expected_layer_samples < 1:
        raise ValueError("result manifest has no routed layers")
    rows = []
    ...
    for physical_m, replay_pairs in pairs_by_m.items():
        ...
        if any(
            len(pair.coordinator_layer_ms) != expected_layer_samples
            or len(pair.spark_layer_ms) != expected_layer_samples
            for pair in replay_pairs
        ):
            raise ValueError(
                f"M={physical_m} has a measurement whose layer samples do not "
                f"match the manifest's {expected_layer_samples} routed layers"
            )
