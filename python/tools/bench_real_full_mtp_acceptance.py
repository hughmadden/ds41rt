...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", default=DEFAULT_FLASH_MODEL_ID)
    parser.add_argument(
        "--max-tokens",
        type=int,
...
def positional_acceptance(
    draft_lengths: list[int], accepted_lengths: list[int]
) -> dict[int, dict[str, float | int]]:
    """Summarize selected-draft accuracy at each one-based proposal depth."""
    if len(draft_lengths) != len(accepted_lengths):
        raise ValueError("draft and accepted length samples must have equal length")
    for drafts, accepted in zip(draft_lengths, accepted_lengths, strict=True):
        if drafts < 0 or accepted < 0 or accepted > drafts:
            raise ValueError(
                f"invalid speculative lengths: drafts={drafts} accepted={accepted}"
            )
    summary = {}
    for position in range(1, max(draft_lengths, default=0) + 1):
        attempts = sum(drafts >= position for drafts in draft_lengths)
        accepted = sum(length >= position for length in accepted_lengths)
        summary[position] = {
            "attempts": attempts,
            "accepted": accepted,
...
def cycle_latency_by_physical_m(
    draft_lengths: list[int], cycle_ms: list[float]
) -> dict[int, dict[str, float | int]]:
    """Summarize complete speculative-cycle latency by target batch rows."""
    if len(draft_lengths) != len(cycle_ms):
        raise ValueError("draft length and cycle latency samples must have equal length")
    samples: dict[int, list[float]] = {}
    for drafts, latency_ms in zip(draft_lengths, cycle_ms, strict=True):
        if drafts < 0 or not math.isfinite(latency_ms) or latency_ms <= 0.0:
            raise ValueError(
                f"invalid speculative cycle: drafts={drafts} latency_ms={latency_ms}"
            )
        samples.setdefault(drafts + 1, []).append(latency_ms)
    summary = {}
    for physical_m, values in sorted(samples.items()):
...
def main() -> None:
    ...
    positional = positional_acceptance(
        [
            length
            for summary in summaries
            for length in summary["draft_lengths"]
        ],
        [
            length
            for summary in summaries
    ...
    cycle_latency = cycle_latency_by_physical_m(
        [length for summary in summaries for length in summary["draft_lengths"]],
        [latency for summary in summaries for latency in summary["verify_cycle_ms"]],
    )
    ...
    aggregate = {
        ...
        "physical_m_cycle_latency_ms": cycle_latency,
        "accepted_draft_length_histogram": dict(sorted(accepted_histogram.items())),
        "draft_position_acceptance": positional,
