...
DEFAULT_MODEL = DEFAULT_FLASH_MODEL_ID
DEFAULT_HIDDEN_SIZE = 4096
DEFAULT_TOP_K = 6
DEFAULT_ROUTED_EXPERTS = 256
DEFAULT_FIRST_ROUTED_LAYER = 0
DEFAULT_LAST_ROUTED_LAYER = 42
DEFAULT_QUANTIZATION_RECIPE = "deepseek_v4_native_fp4_fp8_mixed_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--routed-experts", type=int, default=DEFAULT_ROUTED_EXPERTS
    )
    parser.add_argument(
        "--first-routed-layer", type=int, default=DEFAULT_FIRST_ROUTED_LAYER
    )
    parser.add_argument(
        "--last-routed-layer", type=int, default=DEFAULT_LAST_ROUTED_LAYER
    )
    parser.add_argument(
        "--quantization-recipe", default=DEFAULT_QUANTIZATION_RECIPE
    )
    parser.add_argument(
        "--trace-log",
        type=Path,
...
def parse_route_line(
    line: str, top_k: int, routed_experts: int
) -> RouteFragment | None:
    ...
    for raw_row in raw_routes.split(","):
        ...
        if row_index in indexed_routes:
            raise ValueError(f"route trace repeats row index {row_index}")
        experts = tuple(int(expert) for expert in raw_experts.split("+"))
        if len(experts) != top_k:
            raise ValueError(
                f"row {row_index} has {len(experts)} routes instead of top-{top_k}"
            )
        if len(set(experts)) != len(experts):
            raise ValueError(f"row {row_index} repeats an expert ID")
        if any(expert < 0 or expert >= routed_experts for expert in experts):
            raise ValueError(
                f"row {row_index} contains an expert outside "
                f"0..{routed_experts - 1}"
            )
...
def read_trace_delta(
    trace: BinaryIO, start_offset: int, top_k: int, routed_experts: int
) -> tuple[list[RouteFragment], int]:
    ...
    for line_number, line in enumerate(delta.splitlines(), start=1):
        try:
            fragment = parse_route_line(line, top_k, routed_experts)
...
def verify_cycles(
    fragments: Iterable[RouteFragment],
    expected_physical_ms: list[int],
    ...
    verify_fragments = [
        ...
        and fragment.layer_id in routed_layers
    ]
    expected_fragments = len(expected_physical_ms) * len(routed_layers)
    ...
    for cycle_index, expected_m in enumerate(expected_physical_ms):
        start = cycle_index * len(routed_layers)
        cycle = verify_fragments[start : start + len(routed_layers)]
        actual_layers = tuple(fragment.layer_id for fragment in cycle)
        if actual_layers != routed_layers:
            raise ValueError(
...
def main() -> None:
    ...
    if args.hidden_size < 1 or args.hidden_size % 16:
        raise SystemExit("--hidden-size must be a positive multiple of 16")
    if args.top_k < 1:
        raise SystemExit("--top-k must be positive")
    if args.routed_experts < args.top_k:
        raise SystemExit("--routed-experts must be at least --top-k")
    if args.first_routed_layer < 0 or args.last_routed_layer < args.first_routed_layer:
        raise SystemExit("routed layer bounds must define a nonempty nonnegative range")
    if not args.quantization_recipe:
        raise SystemExit("--quantization-recipe must be nonempty")
    routed_layers = tuple(
        range(args.first_routed_layer, args.last_routed_layer + 1)
    )
