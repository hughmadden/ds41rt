...
_SPARSE_MLA_TOPK = 2048
_SPARSE_MLA_PAGE_SIZE = 64
_TARGET_ENV = "DS41RT_B12X_MLA_CAPTURE_TARGET"
...
_SPARKINFER_GLM_H64_QUERY_POLICY_ENV = (
    "DS41RT_SPARKINFER_GLM_H64_BF16_QUERY_PROJECTION"
)
...
def plan_sparkinfer_glm_h64_bf16_query_projection(
    *,
    ...
    if log_key not in _SPARKINFER_GLM_H64_QUERY_LOGGED_PLANS:
        print(
            "ds41rt_sparkinfer_glm_h64_query_projection "
...
@cache
def _ds41rt_packed_fp8_mla_exact_grouped() -> Any:
    native_path = os.environ.get("DS41RT_NATIVE_LIB")
    if not native_path:
        raise RuntimeError(
            "exact grouped packed-FP8 MLA requires DS41RT_NATIVE_LIB"
        )
...
def _packed_fp8_mla_exact_grouped_chunks(
    ...
    setting = os.environ.get(
        "DS41RT_REAL_FULL_PACKED_FP8_MLA_EXACT_GROUPED", "auto"
    ).strip().lower()
    if setting in {"0", "false", "no", "off"}:
        return None
    if setting == "auto":
        try:
            _ds41rt_packed_fp8_mla_exact_grouped()
        except (AttributeError, OSError, RuntimeError):
            return None
    elif setting not in {"1", "true", "yes", "on"}:
        raise ValueError(
            "DS41RT_REAL_FULL_PACKED_FP8_MLA_EXACT_GROUPED must be auto or a "
...
def _run_flashinfer_packed_fp8_mla_prefill(
    ...
    topk = int(kwargs.get("topk", _SPARSE_MLA_TOPK))
    heads = int(kwargs["heads"])
    nope_dim = int(kwargs["nope_dim"])
    rope_dim = int(kwargs["rope_dim"])
    ...
    if topk not in (128, 512, 1024, _SPARSE_MLA_TOPK):
        raise ValueError(
            "packed FP8 MLA prefill requires topk in "
            f"(128, 512, 1024, {_SPARSE_MLA_TOPK}), got {topk}"
        )
    ...
    with torch.cuda.device(device_id), torch.cuda.stream(stream):
        ...
        kv = _u8_tensor(buffers["kv"], (kv_pages, _SPARSE_MLA_PAGE_SIZE, 656))
        ...
        mid_lse = None
        if query_rows <= 64:
            splits = (topk + _SPARSE_MLA_PAGE_SIZE - 1) // _SPARSE_MLA_PAGE_SIZE
        ...
        if query_rows <= 8:
            ...
            if query_rows == 1:
                ...
                status = _ds41rt_packed_fp8_mla_exact_grouped()(
...
def _run_sparkinfer_nvfp4_mla(
    ...
    **kwargs: Any,
    ...
    with torch.cuda.device(device_id), torch.cuda.stream(stream):
        ...
        kv = _u8_tensor(buffers["kv"], (kv_pages, _SPARSE_MLA_PAGE_SIZE, 432))
        indices = _i32_tensor(buffers["indices"], (query_rows, topk))
        topk_length = _i32_tensor(buffers["topk_length"], (query_rows,))
        output = _bf16_tensor(
            buffers["output"], (query_rows, heads, nope_dim)
        ...
        if decode:
            splits = topk // _SPARSE_MLA_PAGE_SIZE
            mid_out = _bf16_tensor(
                buffers["mid_out"], (query_rows, heads, splits, nope_dim)
            )
            mid_lse = _f32_tensor(
            ...
            run_unified_decode(
                ...
                sm_scale=scale,
                latent_scale=1.0,
                swa_page_size=_SPARSE_MLA_PAGE_SIZE,
                out=output,
            ...
            run_unified_prefill(
                ...
                sm_scale=scale,
                latent_scale=1.0,
                page_block_size=_SPARSE_MLA_PAGE_SIZE,
                output=output,
                lse_out=out_lse,
...
def capture_mla_rope_attention(ctx: dict[str, Any], **kwargs: Any) -> None:
    """Capture the GLM_NSA SparkInfer MLA kernel from DS41RT raw pointer metadata.
