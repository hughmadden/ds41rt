...
_TARGET_ENV = "DS4RT_B12X_MLA_CAPTURE_TARGET"
...
_SPARKINFER_GLM_H64_QUERY_POLICY_ENV = (
    "DS4RT_SPARKINFER_GLM_H64_BF16_QUERY_PROJECTION"
)
...
def plan_sparkinfer_glm_h64_bf16_query_projection(
    *,
    ...
    if log_key not in _SPARKINFER_GLM_H64_QUERY_LOGGED_PLANS:
        print(
            "ds4rt_sparkinfer_glm_h64_query_projection "
...
@cache
def _ds4rt_packed_fp8_mla_exact_grouped() -> Any:
    native_path = os.environ.get("DS4RT_NATIVE_LIB")
    if not native_path:
        raise RuntimeError(
            "exact grouped packed-FP8 MLA requires DS4RT_NATIVE_LIB"
        )
...
def _packed_fp8_mla_exact_grouped_chunks(
    ...
    setting = os.environ.get(
        "DS4RT_REAL_FULL_PACKED_FP8_MLA_EXACT_GROUPED", "auto"
    ).strip().lower()
    if setting in {"0", "false", "no", "off"}:
        return None
    if setting == "auto":
        try:
            _ds4rt_packed_fp8_mla_exact_grouped()
        except (AttributeError, OSError, RuntimeError):
            return None
    elif setting not in {"1", "true", "yes", "on"}:
        raise ValueError(
            "DS4RT_REAL_FULL_PACKED_FP8_MLA_EXACT_GROUPED must be auto or a "
    ...
    with torch.cuda.device(device_id), torch.cuda.stream(stream):
        ...
        if query_rows <= 8:
            ...
            if query_rows == 1:
                ...
                status = _ds4rt_packed_fp8_mla_exact_grouped()(
...
def capture_mla_rope_attention(ctx: dict[str, Any], **kwargs: Any) -> None:
    """Capture the GLM_NSA SparkInfer MLA kernel from DS4RT raw pointer metadata.
