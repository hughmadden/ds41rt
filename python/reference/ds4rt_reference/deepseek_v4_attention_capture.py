...
def qualify_deepseek_v4_compressed_mla_contract(**kwargs: Any) -> bool:
    """Fail closed if the pinned SparkInfer public surface drifts from DS4RT."""
    ...
    if int(sparkinfer_chunks) != contract.max_chunks_per_row:
        raise RuntimeError(
            ...
            f"DS4RT={contract.max_chunks_per_row} SparkInfer={sparkinfer_chunks}"
        )
    scratch_plan = plan(
        Caps(
            device="cpu",
    ...
    if int(scratch_spec.nbytes) != contract.scratch_bytes:
        raise RuntimeError(
            ...
            f"DS4RT={contract.scratch_bytes} SparkInfer={scratch_spec.nbytes}"
        )
    return True
