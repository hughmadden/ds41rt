...
__global__ void ds4_dspark_prompt_metadata_kernel(
  ...
  constexpr size_t kRotaryDim = 64;
  constexpr size_t kPairs = kRotaryDim / 2;
  ...
  const size_t row = idx / kPairs;
  const size_t pair = idx % kPairs;
  const size_t absolute_position = absolute_position_start + row;
  if (pair == 0) {
    positions[row] = static_cast<uint32_t>(row);
    main_slots[row] = static_cast<uint32_t>(request_slot * kPageSize +
                                            absolute_position % kRingSlots);
  }
  const float angle =
      static_cast<float>(absolute_position) *
      powf(theta, -2.0f * static_cast<float>(pair) / static_cast<float>(kRotaryDim));
  sincosf(angle, cos_sin_cache + row * kRotaryDim + kPairs + pair,
          cos_sin_cache + row * kRotaryDim + pair);
}

__global__ void ds4_dspark_proposal_embedding_kernel(
    const uint16_t* embedding, uint16_t* proposal_residual,
    size_t anchor_token_id, size_t noise_token_id, size_t hidden_size) {
  constexpr size_t kProposalRows = 5;
  constexpr size_t kHcMult = 4;
  const size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  const size_t total = kProposalRows * kHcMult * hidden_size;
  if (idx >= total) {
    return;
  }
  const size_t row = idx / (kHcMult * hidden_size);
  const size_t col = idx % hidden_size;
  const size_t token_id = row == 0 ? anchor_token_id : noise_token_id;
  proposal_residual[idx] = embedding[token_id * hidden_size + col];
}

__global__ void ds4_dspark_proposal_metadata_kernel(
    uint32_t* draft_token_ids, uint32_t* positions, uint32_t* main_slots,
    float* cos_sin_cache, size_t anchor_token_id, size_t noise_token_id,
    size_t request_slot, size_t main_context_end, float theta) {
  constexpr size_t kProposalRows = 5;
  constexpr size_t kRotaryDim = 64;
  constexpr size_t kPairs = kRotaryDim / 2;
  constexpr size_t kPageSize = 256;
  constexpr size_t kProposalSlotStart = 128;
  const size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  const size_t total = kProposalRows * kPairs;
  if (idx >= total) {
    return;
  }
  const size_t row = idx / kPairs;
  const size_t pair = idx % kPairs;
  const size_t absolute_position = main_context_end + row;
  if (pair == 0) {
    draft_token_ids[row] =
        static_cast<uint32_t>(row == 0 ? anchor_token_id : noise_token_id);
    positions[row] = static_cast<uint32_t>(row);
    main_slots[row] = static_cast<uint32_t>(request_slot * kPageSize +
                                            kProposalSlotStart + row);
  }
  const float angle =
      static_cast<float>(absolute_position) *
      powf(theta, -2.0f * static_cast<float>(pair) /
                      static_cast<float>(kRotaryDim));
  sincosf(angle, cos_sin_cache + row * kRotaryDim + kPairs + pair,
          cos_sin_cache + row * kRotaryDim + pair);
}

__global__ void ds4_dspark_proposal_selection_kernel(
    int32_t* selected_indices, int32_t* selected_lengths,
    size_t request_slot, size_t cache_window_start,
    size_t main_context_end) {
  constexpr size_t kProposalRows = 5;
  constexpr size_t kPageSize = 256;
  constexpr size_t kRingSlots = 128;
  constexpr size_t kSelectionWidth = 133;
  const size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  const size_t total = kProposalRows * kSelectionWidth;
  if (idx >= total) {
    return;
  }
  const size_t row = idx / kSelectionWidth;
  const size_t column = idx % kSelectionWidth;
  const size_t main_count = main_context_end - cache_window_start;
  const size_t selection_length = main_count + kProposalRows;
  const size_t page_base = request_slot * kPageSize;
  if (column < main_count) {
    const size_t start_slot = cache_window_start % kRingSlots;
    const size_t wrapped = start_slot + main_count > kRingSlots
                               ? start_slot + main_count - kRingSlots
                               : 0;
    const size_t slot = column < wrapped
                            ? column
                            : start_slot + column - wrapped;
    selected_indices[idx] = static_cast<int32_t>(page_base + slot);
  } else if (column < selection_length) {
    selected_indices[idx] = static_cast<int32_t>(
        page_base + kRingSlots + column - main_count);
  } else {
    selected_indices[idx] = -1;
  }
  if (column == 0) {
    selected_lengths[row] = static_cast<int32_t>(selection_length);
  }
}
...
extern "C" ds41rt_status_t ds41rt_cuda_ds4_dspark_prompt_metadata_async(
    ...
    size_t absolute_position_start, size_t request_slot, size_t rows, float theta,
    void* cuda_stream) {
  constexpr size_t kPairs = 32;
  constexpr size_t kMaxU32 = static_cast<size_t>(std::numeric_limits<uint32_t>::max());
  if (positions == nullptr || main_slots == nullptr || cos_sin_cache == nullptr || rows == 0 ||
  ...
  ds4_dspark_prompt_metadata_kernel<<<static_cast<int>(blocks), 256, 0, stream>>>(
      positions, main_slots, cos_sin_cache, absolute_position_start, request_slot, rows, theta);
  return status_from_cuda(cudaGetLastError());
}
...
extern "C" ds41rt_status_t ds41rt_cuda_ds4_dspark_prepare_proposal_async(
  ...
  constexpr size_t kHcMult = 4;
  constexpr size_t kPageSize = 256;
  constexpr size_t kLastProposalSlot = 132;
  constexpr size_t kRingSlots = 128;
  constexpr size_t kSelectionWidth = 133;
  constexpr size_t kMaxU32 =
      static_cast<size_t>(std::numeric_limits<uint32_t>::max());
  constexpr size_t kMaxI32 =
      static_cast<size_t>(std::numeric_limits<int32_t>::max());
  if (embedding == nullptr || draft_token_ids == nullptr ||
      proposal_residual == nullptr || positions == nullptr ||
      main_slots == nullptr || cos_sin_cache == nullptr ||
      selected_indices == nullptr || selected_lengths == nullptr ||
      vocab_size == 0 || hidden_size == 0 || anchor_token_id >= vocab_size ||
      noise_token_id >= vocab_size || !isfinite(theta) || theta <= 0.0f ||
      cache_window_start >= main_context_end ||
      main_context_end - cache_window_start > kRingSlots ||
      main_context_end > kMaxU32 - (kProposalRows - 1) ||
      anchor_token_id > kMaxU32 || noise_token_id > kMaxU32 ||
      request_slot > (kMaxU32 - kLastProposalSlot) / kPageSize ||
  ...
  cudaStream_t stream = reinterpret_cast<cudaStream_t>(cuda_stream);
  (void)cudaGetLastError();
  ds4_dspark_proposal_embedding_kernel<<<
  ...
  ds4_dspark_proposal_selection_kernel<<<3, 256, 0, stream>>>(
      selected_indices, selected_lengths, request_slot, cache_window_start,
      main_context_end);
  return status_from_cuda(cudaGetLastError());
}
