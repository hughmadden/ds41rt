...
__global__ void store(const uint8_t* values,const uint8_t* scales,const uint64_t* destinations,
  ...
  cache[dst*512+t]=values[row*512+t];cache[dst*512+t+256]=values[row*512+t+256];
  if(t<16)cache_scales[dst*16+t]=scales[row*16+t];
}
}
extern "C" int32_t ds41rt_v41_kv_pack(const uint16_t* input,const float* frequencies,
  ...
  pack<<<rows,256,0,reinterpret_cast<cudaStream_t>(stream)>>>(reinterpret_cast<const __nv_bfloat16*>(input),frequencies,values,scales);
...
extern "C" int32_t ds41rt_v41_kv_store(const uint8_t* values,const uint8_t* scales,
  ...
  store<<<rows,256,0,reinterpret_cast<cudaStream_t>(stream)>>>(values,scales,destinations,cache,cache_scales,capacity);
  return cudaGetLastError();
}
