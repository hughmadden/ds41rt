...
__global__ void write_window(const __nv_bfloat16* source, const ds41rt_v41_kv_write_t* writes,
    __nv_bfloat16* ring, uint32_t source_rows, uint32_t slots) {
  ...
  ring[output_row * 512 + col] = __float2bfloat16_rn(float(quantized) * scale);
}
...
extern "C" int32_t ds41rt_v41_dspark_cache_write(const uint16_t* source,
    const ds41rt_v41_kv_write_t* writes, uint16_t* ring, int32_t source_rows,
    int32_t slots, void* stream) {
  if (source_rows < 1 || source_rows > 4096 || slots < 1 || slots > 16) return cudaErrorInvalidValue;
  const uint64_t input_bytes = uint64_t(source_rows) * 1024, ring_bytes = uint64_t(slots) * 128 * 1024;
  if (!span(source,input_bytes,2) || !span(writes,384,8) || !span(ring,ring_bytes,2) ||
  ...
  write_window<<<dim3(128,16),512,0,reinterpret_cast<cudaStream_t>(stream)>>>(
      ...
      reinterpret_cast<__nv_bfloat16*>(ring),source_rows,slots);
