...
typedef enum ds4rt_status_t {
  DS4RT_STATUS_OK = 0,
  DS4RT_STATUS_INVALID_ARGUMENT = 1,
  DS4RT_STATUS_BUFFER_TOO_SMALL = 2,
  DS4RT_STATUS_CUDA_UNAVAILABLE = 3,
  DS4RT_STATUS_ALLOCATION_FAILED = 4,
  DS4RT_STATUS_COPY_FAILED = 5,
  DS4RT_STATUS_INTERNAL_ERROR = 6,
  DS4RT_STATUS_RDMA_UNAVAILABLE = 7,
  DS4RT_STATUS_NCCL_UNAVAILABLE = 8,
} ds4rt_status_t;

typedef enum ds4rt_xgrammar_kind_t {
  DS4RT_XGRAMMAR_JSON_OBJECT = 1,
  DS4RT_XGRAMMAR_JSON_SCHEMA = 2,
  DS4RT_XGRAMMAR_STRUCTURAL_TAG = 3,
} ds4rt_xgrammar_kind_t;

ds4rt_status_t ds4rt_xgrammar_compiler_create(
...
ds4rt_status_t ds4rt_xgrammar_compiler_destroy(void* compiler);
ds4rt_status_t ds4rt_xgrammar_compile(
...
ds4rt_status_t ds4rt_xgrammar_grammar_destroy(void* grammar);
ds4rt_status_t ds4rt_xgrammar_matcher_create(
    const void* grammar, void** out_matcher, char* error, size_t error_bytes);
ds4rt_status_t ds4rt_xgrammar_matcher_fork(
    const void* matcher, void** out_matcher, char* error, size_t error_bytes);
ds4rt_status_t ds4rt_xgrammar_matcher_destroy(void* matcher);
ds4rt_status_t ds4rt_xgrammar_matcher_fill_bitmask(
    void* matcher, uint32_t* bitmask, size_t bitmask_words, int* out_needs_mask,
    char* error, size_t error_bytes);
ds4rt_status_t ds4rt_xgrammar_matcher_accept_token(
    void* matcher, uint32_t token_id, int* out_accepted, char* error, size_t error_bytes);
ds4rt_status_t ds4rt_xgrammar_matcher_is_completed(
    const void* matcher, int* out_completed, char* error, size_t error_bytes);

typedef enum ds4rt_device_buffer_flags_t {
  DS4RT_DEVICE_BUFFER_FLAG_NONE = 0,
  DS4RT_DEVICE_BUFFER_FLAG_HOST_FALLBACK = 1,
  DS4RT_DEVICE_BUFFER_FLAG_MANAGED = 2,
  DS4RT_DEVICE_BUFFER_FLAG_MAPPED_HOST = 4,
} ds4rt_device_buffer_flags_t;

typedef enum ds4rt_host_buffer_flags_t {
  DS4RT_HOST_BUFFER_FLAG_NONE = 0,
  DS4RT_HOST_BUFFER_FLAG_PINNED = 1,
  DS4RT_HOST_BUFFER_FLAG_HOST_FALLBACK = 2,
  DS4RT_HOST_BUFFER_FLAG_MAPPED = 4,
} ds4rt_host_buffer_flags_t;

typedef struct ds4rt_cuda_device_info_t {
  int device_id;
  int cuda_available;
...
} ds4rt_cuda_device_info_t;

typedef struct ds4rt_device_buffer_t {
  void* ptr;
  size_t bytes;
  int device_id;
  uint64_t flags;
} ds4rt_device_buffer_t;

typedef struct ds4rt_host_buffer_t {
  void* ptr;
  size_t bytes;
  uint64_t flags;
} ds4rt_host_buffer_t;

typedef enum ds4rt_route_shard_wire_dtype_t {
  DS4RT_ROUTE_SHARD_WIRE_BF16 = 1,
  DS4RT_ROUTE_SHARD_WIRE_FP8_E4M3_ROW_SCALED = 2,
  DS4RT_ROUTE_SHARD_WIRE_NVFP4_E2M1_FP8_E4M3 = 3,
} ds4rt_route_shard_wire_dtype_t;

typedef enum ds4rt_route_shard_local_dtype_t {
  DS4RT_ROUTE_SHARD_LOCAL_F32 = 1,
  DS4RT_ROUTE_SHARD_LOCAL_BF16 = 2,
} ds4rt_route_shard_local_dtype_t;

typedef struct ds4rt_route_shard_reduction_buffers_t {
  ds4rt_device_buffer_t local;
  ds4rt_device_buffer_t peers[3];
  ds4rt_device_buffer_t output_f32;
} ds4rt_route_shard_reduction_buffers_t;

typedef struct ds4rt_route_shard_fp8_rail_reduction_buffers_t {
  ds4rt_device_buffer_t local_bf16;
  ds4rt_device_buffer_t peer_rail0[3];
  ds4rt_device_buffer_t peer_rail1[3];
  ds4rt_device_buffer_t output_fp8;
} ds4rt_route_shard_fp8_rail_reduction_buffers_t;

typedef struct ds4rt_nvfp4_route_batched_metadata_t {
  ...
  float up_scale_2;
  float down_scale_2;
} ds4rt_nvfp4_route_batched_metadata_t;

typedef struct ds4rt_b12x_spark_w4a16_moe_buffers_t {
  ds4rt_device_buffer_t input;
  ds4rt_device_buffer_t w13_weight;
  ds4rt_device_buffer_t w2_weight;
  ds4rt_device_buffer_t fc1_output;
  ds4rt_device_buffer_t activated;
  ds4rt_device_buffer_t output;
  ds4rt_device_buffer_t w13_scale;
  ds4rt_device_buffer_t w2_scale;
  ds4rt_device_buffer_t w13_global_scale;
  ds4rt_device_buffer_t w2_global_scale;
  ds4rt_device_buffer_t packed_route_indices;
  ds4rt_device_buffer_t block_expert_ids;
  ds4rt_device_buffer_t packed_route_count;
  ds4rt_device_buffer_t topk_weights;
  ds4rt_device_buffer_t fc1_scratch;
  ds4rt_device_buffer_t fc2_scratch;
  ds4rt_device_buffer_t locks;
} ds4rt_b12x_spark_w4a16_moe_buffers_t;
...
typedef struct ds4rt_ds4_flash_spark_w4a16_moe_buffers_t {
  ds4rt_device_buffer_t input;
  ds4rt_device_buffer_t w13_weight;
  ds4rt_device_buffer_t w2_weight;
  ds4rt_device_buffer_t fc1_output;
  ds4rt_device_buffer_t activated;
  ds4rt_device_buffer_t routed_output;
  ds4rt_device_buffer_t output;
  ds4rt_device_buffer_t w13_scale;
  ds4rt_device_buffer_t w2_scale;
  ds4rt_device_buffer_t w13_global_scale;
  ...
  ds4rt_device_buffer_t fc2_scratch;
  ds4rt_device_buffer_t locks;
} ds4rt_ds4_flash_spark_w4a16_moe_buffers_t;
...
typedef struct ds4rt_ds4_flash_spark_exl3_k2_moe_buffers_t {
  ds4rt_device_buffer_t input;
  ds4rt_device_buffer_t w13_trellis;
  ds4rt_device_buffer_t w2_trellis;
  ds4rt_device_buffer_t gate_suh;
  ds4rt_device_buffer_t up_suh;
  ds4rt_device_buffer_t intermediate_rotations;
  ds4rt_device_buffer_t down_svh;
  ds4rt_device_buffer_t expert_map;
  ds4rt_device_buffer_t dummy_scale;
  ds4rt_device_buffer_t trellis_lut;
  ds4rt_device_buffer_t global_scale;
  ds4rt_device_buffer_t topk_ids;
  ds4rt_device_buffer_t topk_weights;
  ds4rt_device_buffer_t rotation_gate;
  ds4rt_device_buffer_t rotation_up;
  ds4rt_device_buffer_t fc1_output;
  ds4rt_device_buffer_t activated;
  ds4rt_device_buffer_t routed_output;
  ds4rt_device_buffer_t output_f32;
  ds4rt_device_buffer_t output_bf16;
  ds4rt_device_buffer_t packed_route_indices;
  ds4rt_device_buffer_t block_expert_ids;
  ds4rt_device_buffer_t packed_route_count;
  ds4rt_device_buffer_t expert_counts;
  ds4rt_device_buffer_t expert_offsets;
  ds4rt_device_buffer_t fc1_scratch;
  ds4rt_device_buffer_t fc2_scratch;
  ds4rt_device_buffer_t workspace;
} ds4rt_ds4_flash_spark_exl3_k2_moe_buffers_t;
...
typedef struct ds4rt_ds4_flash_spark_exl3_mixed_moe_buffers_t {
  ds4rt_device_buffer_t input;
  ds4rt_device_buffer_t tier0_w13_trellis;
  ds4rt_device_buffer_t tier0_w2_trellis;
  ds4rt_device_buffer_t tier1_w13_trellis;
  ds4rt_device_buffer_t tier1_w2_trellis;
  ds4rt_device_buffer_t dummy_scale;
  ds4rt_device_buffer_t tier0_global_scale;
  ds4rt_device_buffer_t tier1_global_scale;
  ds4rt_device_buffer_t gate_suh;
  ds4rt_device_buffer_t up_suh;
  ds4rt_device_buffer_t intermediate_rotations;
  ds4rt_device_buffer_t down_svh;
  ...
  ds4rt_device_buffer_t topk_ids;
  ds4rt_device_buffer_t mapped_topk_ids;
  ds4rt_device_buffer_t topk_weights;
  ds4rt_device_buffer_t rotation_gate;
  ds4rt_device_buffer_t rotation_up;
  ds4rt_device_buffer_t fc1_output;
  ...
  ds4rt_device_buffer_t fc2_scratch;
  ds4rt_device_buffer_t workspace;
} ds4rt_ds4_flash_spark_exl3_mixed_moe_buffers_t;
...
typedef struct ds4rt_ds4_pro_spark_exl3_k2_moe_buffers_t {
  ds4rt_device_buffer_t input;
  ds4rt_device_buffer_t w13_trellis;
  ds4rt_device_buffer_t w2_trellis;
  ds4rt_device_buffer_t gate_suh;
  ...
  ds4rt_device_buffer_t fc2_scratch;
  ds4rt_device_buffer_t workspace;
} ds4rt_ds4_pro_spark_exl3_k2_moe_buffers_t;
...
typedef struct ds4rt_ds4_flash_route_pack_buffers_t {
  ds4rt_device_buffer_t topk_ids;
  ds4rt_device_buffer_t packed_route_indices;
  ds4rt_device_buffer_t block_expert_ids;
  ds4rt_device_buffer_t packed_route_count;
  ds4rt_device_buffer_t expert_counts;
  ds4rt_device_buffer_t expert_offsets;
} ds4rt_ds4_flash_route_pack_buffers_t;
...
typedef struct ds4rt_ds4_flash_shared_expert_fp8_buffers_t {
  ds4rt_device_buffer_t input;
  ds4rt_device_buffer_t w1_weight;
  ds4rt_device_buffer_t w1_scale_mma;
  ds4rt_device_buffer_t w3_weight;
  ds4rt_device_buffer_t w3_scale_mma;
  ds4rt_device_buffer_t w2_weight;
  ds4rt_device_buffer_t w2_scale_mma;
  ds4rt_device_buffer_t gate;
  ds4rt_device_buffer_t up;
  ds4rt_device_buffer_t activated;
  ds4rt_device_buffer_t output;
  ds4rt_device_buffer_t alpha;
} ds4rt_ds4_flash_shared_expert_fp8_buffers_t;
...
typedef struct ds4rt_ds4_pro_shared_expert_fp8_prefill_buffers_t {
  ds4rt_ds4_flash_shared_expert_fp8_buffers_t base;
  ds4rt_device_buffer_t input_q_values;
  ds4rt_device_buffer_t input_q_scale_rows;
  ds4rt_device_buffer_t input_q_scale_mma;
  ds4rt_device_buffer_t activated_q_values;
  ds4rt_device_buffer_t activated_q_scale_rows;
  ds4rt_device_buffer_t activated_q_scale_mma;
} ds4rt_ds4_pro_shared_expert_fp8_prefill_buffers_t;

typedef struct ds4rt_b12x_coordinator_w4a16_buffers_t {
  ds4rt_device_buffer_t input;
  ds4rt_device_buffer_t weight;
  ds4rt_device_buffer_t output;
  ds4rt_device_buffer_t scale;
  ds4rt_device_buffer_t global_scale;
  ds4rt_device_buffer_t packed_route_indices;
  ds4rt_device_buffer_t block_expert_ids;
  ds4rt_device_buffer_t packed_route_count;
  ds4rt_device_buffer_t topk_weights;
  ds4rt_device_buffer_t c_tmp;
  ds4rt_device_buffer_t locks;
} ds4rt_b12x_coordinator_w4a16_buffers_t;

typedef struct ds4rt_cuda_graph_capture_info_t {
  void* graph;
  void* graph_exec;
  size_t node_count;
...
} ds4rt_cuda_graph_capture_info_t;

typedef struct ds4rt_bf16_summary_t {
  double checksum;
  uint64_t values;
...
typedef struct ds4rt_rdma_device_info_t {
  int rdma_enabled;
  int device_count;
  ...
  char status[128];
} ds4rt_rdma_device_info_t;

typedef struct ds4rt_rdma_host_buffer_plan_t {
  ...
  int span_aligned;
  int rdma_enabled;
} ds4rt_rdma_host_buffer_plan_t;

typedef struct ds4rt_rdma_register_probe_t {
  size_t bytes;
  int registered;
  uint32_t lkey;
  uint32_t rkey;
  char device_name[128];
} ds4rt_rdma_register_probe_t;

typedef struct ds4rt_rdma_rc_qp_probe_t {
  int rdma_enabled;
  int created;
  uint32_t port_num;
  uint32_t qp_num;
  ...
  char device_name[128];
  char status[128];
} ds4rt_rdma_rc_qp_probe_t;

typedef struct ds4rt_rdma_rc_send_recv_probe_t {
  int rdma_enabled;
  int completed;
  int payload_matches;
  uint32_t port_num;
  ...
  char device_name[128];
  char status[128];
} ds4rt_rdma_rc_send_recv_probe_t;

typedef struct ds4rt_rdma_rc_protocol_v2_loopback_probe_t {
  int rdma_enabled;
  int completed;
  ...
  char device_name[128];
  char status[128];
} ds4rt_rdma_rc_protocol_v2_loopback_probe_t;

typedef struct ds4rt_rdma_rc_endpoint_info_t {
  int rdma_enabled;
  void* handle;
  uint32_t port_num;
  uint32_t qp_num;
  ...
  char device_name[128];
  char status[128];
} ds4rt_rdma_rc_endpoint_info_t;

typedef struct ds4rt_rdma_rc_endpoint_buffer_view_t {
  void* host_ptr;
  void* device_ptr;
  size_t bytes;
  int device_id;
  uint64_t host_flags;
} ds4rt_rdma_rc_endpoint_buffer_view_t;

typedef struct ds4rt_rdma_rc_completion_stats_t {
  ...
  uint32_t poll_iterations;
  char status[128];
} ds4rt_rdma_rc_completion_stats_t;

ds4rt_status_t ds4rt_native_version(char* out, size_t out_len);
ds4rt_status_t ds4rt_cuda_device_info(int device_id, ds4rt_cuda_device_info_t* out);
ds4rt_status_t ds4rt_alloc_host_buffer(size_t bytes, ds4rt_host_buffer_t* out);
ds4rt_status_t ds4rt_cuda_host_buffer_device_alias(ds4rt_host_buffer_t host,
                                                    ds4rt_device_buffer_t* out);
ds4rt_status_t ds4rt_free_host_buffer(ds4rt_host_buffer_t* buf);
ds4rt_status_t ds4rt_alloc_device_buffer(size_t bytes, ds4rt_device_buffer_t* out);
ds4rt_status_t ds4rt_alloc_managed_device_buffer(size_t bytes, ds4rt_device_buffer_t* out);
ds4rt_status_t ds4rt_free_device_buffer(ds4rt_device_buffer_t* buf);
ds4rt_status_t ds4rt_cuda_stream_create(void** out_cuda_stream);
ds4rt_status_t ds4rt_cuda_stream_destroy(void* cuda_stream);
ds4rt_status_t ds4rt_cuda_stream_synchronize(void* cuda_stream);
ds4rt_status_t ds4rt_cuda_stream_wait_event(void* cuda_stream, void* cuda_event);
ds4rt_status_t ds4rt_cuda_event_create(void** out_cuda_event);
ds4rt_status_t ds4rt_cuda_event_destroy(void* cuda_event);
ds4rt_status_t ds4rt_cuda_event_record(void* cuda_event, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_event_synchronize(void* cuda_event);
ds4rt_status_t ds4rt_cuda_event_elapsed_ms(void* start_event, void* end_event, float* out_ms);
ds4rt_status_t ds4rt_cuda_graph_begin_capture(void* cuda_stream);
ds4rt_status_t ds4rt_cuda_graph_end_capture(void* cuda_stream, void** out_cuda_graph_exec);
ds4rt_status_t ds4rt_cuda_graph_end_capture_retained(
    void* cuda_stream, ds4rt_cuda_graph_capture_info_t* out);
ds4rt_status_t ds4rt_cuda_graph_launch(void* cuda_graph_exec, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_graph_exec_update(void* cuda_graph_exec, void* cuda_graph);
ds4rt_status_t ds4rt_cuda_graph_destroy(void* cuda_graph);
ds4rt_status_t ds4rt_cuda_graph_exec_destroy(void* cuda_graph_exec);
ds4rt_status_t ds4rt_cuda_graph_update_rmsnorm_bf16_node(
...
ds4rt_status_t ds4rt_cuda_graph_update_layernorm_affine_f32_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index, ds4rt_device_buffer_t x,
    ds4rt_device_buffer_t weight, ds4rt_device_buffer_t bias, ds4rt_device_buffer_t out,
    int rows, int hidden, float eps);
ds4rt_status_t ds4rt_cuda_graph_update_layernorm_affine_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index, ds4rt_device_buffer_t x,
    ds4rt_device_buffer_t weight, ds4rt_device_buffer_t bias, ds4rt_device_buffer_t out,
    int rows, int hidden, float eps);
ds4rt_status_t ds4rt_cuda_graph_update_linear_bf16_node(
...
ds4rt_status_t ds4rt_cuda_graph_update_embedding_lookup_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
    ds4rt_device_buffer_t embedding, ds4rt_device_buffer_t token_ids,
    ds4rt_device_buffer_t out, size_t rows, size_t vocab, size_t hidden);
ds4rt_status_t ds4rt_cuda_graph_update_lm_head_argmax_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
...
ds4rt_status_t ds4rt_cuda_graph_update_lm_head_sample_topk_topp_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
    ds4rt_device_buffer_t hidden, ds4rt_device_buffer_t lm_head,
...
ds4rt_status_t ds4rt_cuda_graph_update_router_topk_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
...
ds4rt_status_t ds4rt_cuda_graph_update_silu_gated_mlp_rows_bf16_down_stride_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index, ds4rt_device_buffer_t x,
...
ds4rt_status_t ds4rt_cuda_graph_update_residual_add_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
    ds4rt_device_buffer_t residual, ds4rt_device_buffer_t delta, ds4rt_device_buffer_t out,
    size_t count);
ds4rt_status_t ds4rt_cuda_graph_update_residual_add_f32_delta_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
    ds4rt_device_buffer_t residual, ds4rt_device_buffer_t delta_f32, ds4rt_device_buffer_t out,
    size_t count);
ds4rt_status_t ds4rt_cuda_graph_update_residual_add_shared_f32_delta_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
    ds4rt_device_buffer_t residual, ds4rt_device_buffer_t shared_delta,
    ds4rt_device_buffer_t routed_delta_f32, ds4rt_device_buffer_t out, size_t count);
ds4rt_status_t ds4rt_cuda_graph_update_causal_attention_bf16_node(
...
ds4rt_status_t ds4rt_cuda_graph_update_rope_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
...
ds4rt_status_t ds4rt_cuda_graph_update_mla_rope_attention_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
...
ds4rt_status_t ds4rt_cuda_graph_update_mla_rope_attention_bf16_suffix_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
    ds4rt_device_buffer_t q_nope, ds4rt_device_buffer_t q_rope,
    ds4rt_device_buffer_t k_nope, ds4rt_device_buffer_t k_rope, ds4rt_device_buffer_t v,
...
ds4rt_status_t ds4rt_cuda_graph_update_mla_kv_cache_unpack_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
...
ds4rt_status_t ds4rt_cuda_graph_update_mla_kv_projected_split_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
...
ds4rt_status_t ds4rt_cuda_graph_update_f32_to_bf16_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
    ds4rt_device_buffer_t src, ds4rt_device_buffer_t dst, size_t count);
ds4rt_status_t ds4rt_cuda_graph_update_scatter_add_rows_bf16_to_f32_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
    ds4rt_device_buffer_t src, ds4rt_device_buffer_t row_indices, ds4rt_device_buffer_t dst,
    size_t dst_rows, size_t rows, size_t row_width);
ds4rt_status_t ds4rt_cuda_graph_update_kv_cache_write_bytes_node(
    void* cuda_graph, void* cuda_graph_exec, size_t kernel_node_index,
    ds4rt_device_buffer_t src, ds4rt_device_buffer_t cache, size_t cache_offset_bytes,
    size_t bytes);
ds4rt_status_t ds4rt_copy_h2d(ds4rt_device_buffer_t dst, const void* src, size_t bytes);
ds4rt_status_t ds4rt_copy_d2h(void* dst, ds4rt_device_buffer_t src, size_t bytes);
ds4rt_status_t ds4rt_copy_d2d(ds4rt_device_buffer_t dst, ds4rt_device_buffer_t src, size_t bytes);
ds4rt_status_t ds4rt_copy_h2d_async(ds4rt_device_buffer_t dst, const void* src, size_t bytes,
                                    void* cuda_stream);
ds4rt_status_t ds4rt_copy_h2d_batch_async(const ds4rt_device_buffer_t* dsts,
                                          const void* const* srcs, const size_t* bytes,
                                          size_t count, void* cuda_stream);
ds4rt_status_t ds4rt_copy_h2d_2d_async(ds4rt_device_buffer_t dst, size_t dst_pitch_bytes,
                                       const void* src, size_t src_pitch_bytes,
                                       size_t width_bytes, size_t rows, void* cuda_stream);
ds4rt_status_t ds4rt_copy_d2h_async(void* dst, ds4rt_device_buffer_t src, size_t bytes,
                                    void* cuda_stream);
ds4rt_status_t ds4rt_copy_d2d_async(ds4rt_device_buffer_t dst, ds4rt_device_buffer_t src,
                                    size_t bytes, void* cuda_stream);
ds4rt_status_t ds4rt_copy_d2d_2d_async(ds4rt_device_buffer_t dst, size_t dst_pitch_bytes,
                                       ds4rt_device_buffer_t src, size_t src_pitch_bytes,
                                       size_t width_bytes, size_t rows, void* cuda_stream);
ds4rt_status_t ds4rt_last_error(char* out, size_t out_len);
ds4rt_status_t ds4rt_nccl_unique_id_bytes(size_t* out_bytes);
ds4rt_status_t ds4rt_nccl_get_unique_id(void* out, size_t out_bytes);
ds4rt_status_t ds4rt_nccl_comm_init_rank(const void* unique_id, size_t unique_id_bytes,
                                         int world_size, int rank, void** out_handle);
ds4rt_status_t ds4rt_nccl_gather_u8_async(void* handle, ds4rt_device_buffer_t send,
                                           ds4rt_device_buffer_t recv, size_t bytes, int root,
                                           void* cuda_stream);
ds4rt_status_t ds4rt_nccl_row_all_to_all_u8_async(
    void* handle, ds4rt_device_buffer_t send, ds4rt_device_buffer_t recv, size_t rows,
    size_t row_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_nccl_all_reduce_bf16_async(void* handle, ds4rt_device_buffer_t send,
                                                ds4rt_device_buffer_t recv, size_t values,
                                                void* cuda_stream);
ds4rt_status_t ds4rt_nccl_reduce_bf16_async(void* handle, ds4rt_device_buffer_t send,
                                            ds4rt_device_buffer_t recv, size_t values, int root,
                                            void* cuda_stream);
ds4rt_status_t ds4rt_nccl_comm_destroy(void* handle);
ds4rt_status_t ds4rt_rdma_device_info(ds4rt_rdma_device_info_t* out);
ds4rt_status_t ds4rt_rdma_plan_host_buffer_registration(
    const void* ptr, size_t bytes, size_t alignment, ds4rt_rdma_host_buffer_plan_t* out);
ds4rt_status_t ds4rt_rdma_register_host_buffer_probe(void* ptr, size_t bytes,
                                                     ds4rt_rdma_register_probe_t* out);
ds4rt_status_t ds4rt_rdma_create_rc_qp_probe(uint32_t port_num, uint32_t send_wr,
                                             uint32_t recv_wr, uint32_t max_sge,
                                             ds4rt_rdma_rc_qp_probe_t* out);
ds4rt_status_t ds4rt_rdma_rc_send_recv_loopback_probe(uint32_t port_num, size_t bytes,
                                                      ds4rt_rdma_rc_send_recv_probe_t* out);
ds4rt_status_t ds4rt_rdma_rc_protocol_v2_loopback_probe(
...
ds4rt_status_t ds4rt_rdma_rc_endpoint_create(
...
ds4rt_status_t ds4rt_rdma_rc_endpoint_create_with_buffer_flags(
    uint32_t port_num, uint32_t local_psn, size_t send_frame_bytes, size_t recv_frame_bytes,
    size_t send_registered_span_bytes, size_t recv_registered_span_bytes, uint32_t max_send_wr,
    uint32_t max_recv_wr, uint32_t max_sge, uint64_t host_buffer_flags,
    ds4rt_rdma_rc_endpoint_info_t* out);
ds4rt_status_t ds4rt_rdma_rc_endpoint_create_on_device_with_buffer_flags(
    ...
    uint64_t host_buffer_flags, ds4rt_rdma_rc_endpoint_info_t* out);
ds4rt_status_t ds4rt_rdma_rc_endpoint_buffer_view(
    void* handle, int receive_buffer, ds4rt_rdma_rc_endpoint_buffer_view_t* out);
ds4rt_status_t ds4rt_rdma_rc_endpoint_connect(void* handle, uint32_t remote_qp_num,
                                               uint32_t remote_psn, uint32_t remote_lid,
                                               const char* remote_gid_hex);
ds4rt_status_t ds4rt_rdma_rc_endpoint_post_recv(void* handle, size_t bytes, uint64_t wr_id);
ds4rt_status_t ds4rt_rdma_rc_endpoint_post_recv_at(void* handle, size_t offset_bytes,
                                                   size_t bytes, uint64_t wr_id);
ds4rt_status_t ds4rt_rdma_rc_endpoint_post_send_at(void* handle, size_t offset_bytes,
                                                   size_t bytes, uint64_t wr_id);
ds4rt_status_t ds4rt_rdma_rc_endpoint_send(void* handle, const void* frame, size_t bytes,
                                           uint64_t wr_id);
ds4rt_status_t ds4rt_rdma_rc_endpoint_send_at(void* handle, const void* frame,
                                              size_t offset_bytes, size_t bytes,
                                              uint64_t wr_id);
ds4rt_status_t ds4rt_rdma_rc_endpoint_send_parts_at(
...
ds4rt_status_t ds4rt_rdma_rc_endpoint_poll(void* handle, uint32_t expected_send_completions,
                                           uint32_t expected_recv_completions,
                                           uint32_t max_poll_iterations,
                                           ds4rt_rdma_rc_completion_stats_t* out);
ds4rt_status_t ds4rt_rdma_rc_endpoint_poll_with_timeout(
    ...
    ds4rt_rdma_rc_completion_stats_t* out);
ds4rt_status_t ds4rt_rdma_rc_endpoint_try_poll(
    void* handle, uint32_t max_send_completions, uint32_t max_recv_completions,
    ds4rt_rdma_rc_completion_stats_t* out);
ds4rt_status_t ds4rt_rdma_rc_endpoint_copy_recv(void* handle, void* out, size_t out_bytes,
                                                 size_t bytes);
ds4rt_status_t ds4rt_rdma_rc_endpoint_copy_recv_at(void* handle, void* out, size_t out_bytes,
                                                   size_t offset_bytes, size_t bytes);
ds4rt_status_t ds4rt_rdma_rc_endpoint_destroy(void* handle);

ds4rt_status_t ds4rt_cuda_rmsnorm_f32(const float* x, const float* weight, float* out,
                                      int rows, int hidden, float eps);
ds4rt_status_t ds4rt_cuda_rmsnorm_f32_async(const float* x, const float* weight, float* out,
                                            int rows, int hidden, float eps, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_rmsnorm_bf16(const uint16_t* x, const uint16_t* weight, uint16_t* out,
                                       int rows, int hidden, float eps);
ds4rt_status_t ds4rt_cuda_rmsnorm_bf16_async(const uint16_t* x, const uint16_t* weight,
...
ds4rt_status_t ds4rt_cuda_layernorm_affine_f32_bf16(const float* x, const uint16_t* weight,
                                                    const uint16_t* bias, float* out, int rows,
                                                    int hidden, float eps);
ds4rt_status_t ds4rt_cuda_layernorm_affine_f32_bf16_async(
    const float* x, const uint16_t* weight, const uint16_t* bias, float* out, int rows,
    int hidden, float eps, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_layernorm_affine_bf16(const uint16_t* x, const uint16_t* weight,
                                                const uint16_t* bias, uint16_t* out, int rows,
                                                int hidden, float eps);
ds4rt_status_t ds4rt_cuda_layernorm_affine_bf16_async(
    const uint16_t* x, const uint16_t* weight, const uint16_t* bias, uint16_t* out, int rows,
    int hidden, float eps, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_silu_gated_mlp_f32(const float* x, const float* gate_weight,
                                             const float* up_weight, const float* down_weight,
                                             float* out, int hidden, int intermediate);
ds4rt_status_t ds4rt_cuda_silu_gated_mlp_rows_f32(
...
ds4rt_status_t ds4rt_cuda_silu_gated_mlp_rows_f32_async(
    const float* x, const float* gate_weight, const float* up_weight, const float* down_weight,
    float* out, size_t rows, size_t hidden, size_t intermediate, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_silu_gated_mlp_rows_bf16(
...
ds4rt_status_t ds4rt_cuda_silu_gated_mlp_rows_bf16_async(
    const uint16_t* x, const uint16_t* gate_weight, const uint16_t* up_weight,
    const uint16_t* down_weight, uint16_t* out, size_t rows, size_t hidden, size_t intermediate,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_silu_mul_bf16_async(const uint16_t* gate_up, uint16_t* out,
                                              size_t rows, size_t intermediate,
                                              void* cuda_stream);
ds4rt_status_t ds4rt_cuda_silu_gated_mlp_rows_bf16_down_stride(
    const uint16_t* x, const uint16_t* gate_weight, const uint16_t* up_weight,
    const uint16_t* down_weight, uint16_t* out, size_t rows, size_t hidden, size_t intermediate,
    size_t down_stride);
ds4rt_status_t ds4rt_cuda_silu_gated_mlp_rows_bf16_down_stride_async(
    const uint16_t* x, const uint16_t* gate_weight, const uint16_t* up_weight,
    const uint16_t* down_weight, uint16_t* out, size_t rows, size_t hidden, size_t intermediate,
    size_t down_stride, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_silu_gated_mlp_rows_bf16_down_stride_staged(
    const uint16_t* x, const uint16_t* gate_weight, const uint16_t* up_weight,
...
ds4rt_status_t ds4rt_cuda_silu_gated_mlp_rows_bf16_down_stride_staged_async(
    const uint16_t* x, const uint16_t* gate_weight, const uint16_t* up_weight,
    const uint16_t* down_weight, float* activation_workspace, uint16_t* out, size_t rows,
    size_t hidden, size_t intermediate, size_t down_stride, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_nvfp4_silu_gated_mlp_route_bf16_grouped_staged_accumulate_f32(
...
ds4rt_status_t ds4rt_cuda_nvfp4_silu_gated_mlp_route_bf16_grouped_staged_accumulate_f32_async(
    const uint16_t* hidden, const uint32_t* row_indices, const float* route_weights,
    const uint8_t* gate_weight, const uint8_t* gate_scale, const uint8_t* up_weight,
    const uint8_t* up_scale, const uint8_t* down_weight, const uint8_t* down_scale,
    float* activation_workspace, float* accumulator, size_t rows, size_t routes,
...
ds4rt_status_t ds4rt_cuda_nvfp4_silu_gated_mlp_route_bf16_batched_staged_accumulate_f32(
    const uint16_t* hidden, const uint32_t* row_indices, const float* route_weights,
...
ds4rt_status_t ds4rt_cuda_nvfp4_silu_gated_mlp_route_bf16_batched_staged_accumulate_f32_async(
    const uint16_t* hidden, const uint32_t* row_indices, const float* route_weights,
    const ds4rt_nvfp4_route_batched_metadata_t* route_metadata, float* activation_workspace,
    float* accumulator, size_t rows, size_t routes, size_t hidden_dim,
    size_t hidden_row_stride, size_t max_intermediate, size_t output_dim, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_nvfp4_silu_gated_mlp_route_bf16_batched_staged_single_row_bf16(
    const uint16_t* hidden, const uint32_t* row_indices, const float* route_weights,
    const ds4rt_nvfp4_route_batched_metadata_t* route_metadata, float* activation_workspace,
...
ds4rt_status_t ds4rt_cuda_nvfp4_silu_gated_mlp_route_bf16_batched_staged_single_row_bf16_async(
    const uint16_t* hidden, const uint32_t* row_indices, const float* route_weights,
    const ds4rt_nvfp4_route_batched_metadata_t* route_metadata, float* activation_workspace,
    uint16_t* out, size_t rows, size_t routes, size_t hidden_dim, size_t hidden_row_stride,
    size_t max_intermediate, size_t output_dim, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_spark_aot_available(int* out_available);
ds4rt_status_t ds4rt_cuda_b12x_spark_aot_init(void);
ds4rt_status_t ds4rt_cuda_b12x_quantize_bf16_nvfp4_row_payload_async(
    ds4rt_device_buffer_t input, ds4rt_device_buffer_t payload, size_t rows, size_t hidden_dim,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_w4a16_pack_weight_async(
...
ds4rt_status_t ds4rt_cuda_b12x_w4a16_pack_weight_strided_async(
    ds4rt_device_buffer_t source, ds4rt_device_buffer_t destination, size_t size_k,
    size_t source_size_k, size_t source_start_k, size_t size_n,
    size_t row_rotation, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_w4a16_pack_scale_async(
    ds4rt_device_buffer_t source, ds4rt_device_buffer_t destination, size_t size_k,
    size_t size_n, size_t row_rotation, float scale_factor, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_w4a16_pack_scale_strided_async(
    ds4rt_device_buffer_t source, ds4rt_device_buffer_t destination, size_t size_k,
    size_t source_size_k, size_t source_start_k, size_t size_n,
    size_t row_rotation, float scale_factor, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_flash_w4a16_pack_weight_async(
    ds4rt_device_buffer_t source, ds4rt_device_buffer_t destination, size_t size_k,
    size_t size_n, size_t row_rotation, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_flash_w4a16_pack_weight_strided_async(
    ds4rt_device_buffer_t source, ds4rt_device_buffer_t destination, size_t size_k,
    size_t source_size_k, size_t source_start_k, size_t size_n,
    size_t row_rotation, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_flash_w4a16_pack_e8m0_scale_async(
    ds4rt_device_buffer_t source, ds4rt_device_buffer_t destination, size_t size_k,
    size_t size_n, size_t row_rotation, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_flash_w4a16_pack_e8m0_scale_strided_async(
    ds4rt_device_buffer_t source, ds4rt_device_buffer_t destination, size_t size_k,
    size_t source_size_k, size_t source_start_k, size_t size_n,
    size_t row_rotation, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_flash_fp8_pack_block_scale_mma_async(
    ds4rt_device_buffer_t source, ds4rt_device_buffer_t destination,
    size_t size_n, size_t size_k, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_flash_spark_aot_available(int* out_available);
ds4rt_status_t ds4rt_cuda_ds4_flash_spark_aot_init(void);
ds4rt_status_t ds4rt_cuda_ds4_flash_shared_expert_fp8_bf16_async(
...
ds4rt_status_t ds4rt_cuda_ds4_flash_spark_w4a16_decode_m1_bf16_async(
...
ds4rt_status_t ds4rt_cuda_ds4_flash_spark_w4a16_prefill_topk6_bf16_async(
...
ds4rt_status_t ds4rt_cuda_ds4_flash_spark_exl3_k2_decode_m1_async(
    const ds4rt_ds4_flash_spark_exl3_k2_moe_buffers_t* buffers,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_flash_spark_exl3_k2_prefill_topk6_async(
    const ds4rt_ds4_flash_spark_exl3_k2_moe_buffers_t* buffers, size_t rows,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_flash_spark_exl3_k3_decode_m1_async(
    const ds4rt_ds4_flash_spark_exl3_k2_moe_buffers_t* buffers,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_flash_spark_exl3_k3_prefill_topk6_async(
    const ds4rt_ds4_flash_spark_exl3_k2_moe_buffers_t* buffers, size_t rows,
    void* cuda_stream);
...
ds4rt_status_t ds4rt_cuda_ds4_pro_shared_expert_fp8_bf16_async(
    const ds4rt_ds4_flash_shared_expert_fp8_buffers_t* buffers,
    size_t rows, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_pro_shared_expert_fp8_prefill_bf16_async(
...
ds4rt_status_t ds4rt_cuda_ds4_pro_spark_exl3_k2_decode_m1_async(
    const ds4rt_ds4_pro_spark_exl3_k2_moe_buffers_t* buffers,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_pro_spark_exl3_k2_prefill_topk6_async(
...
ds4rt_status_t ds4rt_cuda_ds4_flash_pack_topk6_routes_async(
    const ds4rt_ds4_flash_route_pack_buffers_t* buffers, size_t rows,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_quantize_bf16_weight_nvfp4_async(
...
ds4rt_status_t ds4rt_cuda_b12x_gather_nvfp4_rows_bf16_async(
...
ds4rt_status_t ds4rt_cuda_b12x_dequantize_nvfp4_rows_bf16_async(
    ds4rt_device_buffer_t payload, size_t row_stride_bytes,
    ds4rt_device_buffer_t output, size_t rows, size_t hidden_dim,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_spark_w4a16_decode_m1_nvfp4_async(
...
ds4rt_status_t ds4rt_cuda_b12x_spark_w4a16_decode_m1_fused_sum_nvfp4_async(
    const ds4rt_b12x_spark_w4a16_moe_buffers_t* buffers,
    ds4rt_device_buffer_t input_payload, size_t input_payload_stride_bytes,
    ds4rt_device_buffer_t topk_ids, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_spark_w4a16_m1_parity_m2_8_nvfp4_async(
    const ds4rt_b12x_spark_w4a16_moe_buffers_t* buffers,
    ds4rt_device_buffer_t input_payload, size_t input_payload_stride_bytes,
...
ds4rt_status_t ds4rt_cuda_b12x_spark_w4a16_decode_m1_nvfp4_grid_candidate_async(
    const ds4rt_b12x_spark_w4a16_moe_buffers_t* buffers,
    ds4rt_device_buffer_t input_payload, size_t input_payload_stride_bytes,
    ds4rt_device_buffer_t topk_ids, int grid_x, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_spark_w4a16_prefill_topk8_nvfp4_async(
    const ds4rt_b12x_spark_w4a16_moe_buffers_t* buffers,
    ds4rt_device_buffer_t input_payload, size_t input_payload_stride_bytes,
    size_t rows, void* cuda_stream);
...
ds4rt_status_t ds4rt_cuda_b12x_spark_w4a16_prefill_topk8_nvfp4_fp8_async(
    const ds4rt_b12x_spark_w4a16_moe_buffers_t* buffers,
    ds4rt_device_buffer_t input_payload, size_t input_payload_stride_bytes,
...
ds4rt_status_t ds4rt_cuda_b12x_spark_sum_topk8_bf16_async(
    ds4rt_device_buffer_t routed_bf16, ds4rt_device_buffer_t output_bf16,
    size_t rows, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_spark_sum_topk8_bf16_to_fp8_async(
...
ds4rt_status_t ds4rt_cuda_b12x_spark_w4a16_top1_async(
...
ds4rt_status_t ds4rt_cuda_b12x_spark_w4a16_top1_grid_candidate_async(
    const ds4rt_b12x_spark_w4a16_moe_buffers_t* buffers, size_t rows,
    size_t capacity_rows, uint32_t expert_id, int grid_x, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_coordinator_aot_available(int* out_available);
ds4rt_status_t ds4rt_cuda_b12x_coordinator_aot_init(void);
ds4rt_status_t ds4rt_cuda_b12x_coordinator_w4a16_quantize_pack_weight_async(
    ...
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_coordinator_w4a16_initialize_launch_buffers_async(
    const ds4rt_b12x_coordinator_w4a16_buffers_t* buffers, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_coordinator_w4a16_q_b_m8_async(
    const ds4rt_b12x_coordinator_w4a16_buffers_t* buffers, size_t active_rows,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_coordinator_w4a16_q_b_m1_async(
    const ds4rt_b12x_coordinator_w4a16_buffers_t* buffers, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_coordinator_w4a16_o_proj_m1_async(
    const ds4rt_b12x_coordinator_w4a16_buffers_t* buffers, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_b12x_coordinator_w4a16_o_proj_m1_tn64_candidate_async(
    const ds4rt_b12x_coordinator_w4a16_buffers_t* buffers, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_residual_add_f32(const float* residual, const float* delta,
                                           float* out, size_t count);
ds4rt_status_t ds4rt_cuda_residual_add_f32_async(const float* residual, const float* delta,
                                                 float* out, size_t count, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_residual_add_bf16(const uint16_t* residual, const uint16_t* delta,
                                            uint16_t* out, size_t count);
ds4rt_status_t ds4rt_cuda_residual_add_bf16_async(const uint16_t* residual, const uint16_t* delta,
                                                  uint16_t* out, size_t count,
                                                  void* cuda_stream);
ds4rt_status_t ds4rt_cuda_residual_add_f32_delta_bf16(
    const uint16_t* residual, const float* delta_f32, uint16_t* out, size_t count);
ds4rt_status_t ds4rt_cuda_residual_add_f32_delta_bf16_async(
    const uint16_t* residual, const float* delta_f32, uint16_t* out, size_t count,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_residual_add_shared_f32_delta_bf16(
    const uint16_t* residual, const uint16_t* shared_delta, const float* routed_delta_f32,
    uint16_t* out, size_t count);
ds4rt_status_t ds4rt_cuda_residual_add_shared_f32_delta_bf16_async(
    const uint16_t* residual, const uint16_t* shared_delta, const float* routed_delta_f32,
    uint16_t* out, size_t count, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_residual_add_shared_fp8_e4m3_row_scaled_bf16_async(
    const uint16_t* residual, const uint16_t* shared_delta, const uint8_t* routed_delta_fp8,
    uint16_t* out, size_t count, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_fp8_decode_combine_residual_async(
...
ds4rt_status_t ds4rt_cuda_scheduler_mlp_delta_bf16(
...
ds4rt_status_t ds4rt_cuda_scheduler_mlp_delta_bf16_async(
    const uint16_t* hidden, const uint16_t* gate_weight, const uint16_t* up_weight,
    const uint16_t* down_weight, uint16_t* out, size_t rows, size_t hidden_dim,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_summarize_bf16(const uint16_t* input, size_t count,
                                         ds4rt_bf16_summary_t* out);
ds4rt_status_t ds4rt_cuda_summarize_bf16_async(const uint16_t* input, size_t count,
                                               ds4rt_bf16_summary_t* out_device,
                                               void* cuda_stream);
ds4rt_status_t ds4rt_cuda_zero_f32(float* dst, size_t count);
ds4rt_status_t ds4rt_cuda_zero_f32_async(float* dst, size_t count, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_accumulate_bf16_to_f32(const uint16_t* src, float* dst,
                                                  size_t count);
ds4rt_status_t ds4rt_cuda_accumulate_bf16_to_f32_async(const uint16_t* src, float* dst,
                                                        size_t count, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_zero_bytes(void* dst, size_t bytes);
ds4rt_status_t ds4rt_cuda_zero_bytes_async(void* dst, size_t bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_f32_to_bf16(const float* src, uint16_t* dst, size_t count);
ds4rt_status_t ds4rt_cuda_f32_to_bf16_async(const float* src, uint16_t* dst, size_t count,
                                            void* cuda_stream);
ds4rt_status_t ds4rt_cuda_gather_rows_f32(const float* src, const uint32_t* row_indices,
                                          float* dst, size_t rows, size_t row_width);
ds4rt_status_t ds4rt_cuda_gather_rows_f32_async(const float* src, const uint32_t* row_indices,
                                                float* dst, size_t rows, size_t row_width,
                                                void* cuda_stream);
/* Benchmark-only fused BF16 response pack; serving does not reference it. */
ds4rt_status_t ds4rt_cuda_gather_rows_f32_to_bf16_candidate_async(
    const float* src, const uint32_t* row_indices, uint16_t* dst, size_t rows,
    size_t row_width, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_gather_rows_f32_to_fp8_e4m3_row_scaled(
    const float* src, const uint32_t* row_indices, uint8_t* dst, size_t rows,
    size_t row_width, size_t dst_row_stride_bytes);
ds4rt_status_t ds4rt_cuda_gather_rows_f32_to_fp8_e4m3_row_scaled_async(
    const float* src, const uint32_t* row_indices, uint8_t* dst, size_t rows,
...
ds4rt_status_t ds4rt_cuda_gather_rows_f32_to_fp8_e4m3_row_scaled_register_candidate_async(
    const float* src, const uint32_t* row_indices, uint8_t* dst, size_t rows,
    size_t row_width, size_t dst_row_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_bf16_rows_to_fp8_e4m3_row_scaled_async(
    const uint16_t* src, uint8_t* dst, size_t rows, size_t row_width,
    size_t dst_row_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_combine_fp8_e4m3_row_scaled_to_fp8_async(
    ...
    size_t row_width, size_t dst_row_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_combine_bf16_fp8_e4m3_row_scaled_to_fp8_async(
    const uint16_t* local, const uint8_t* peers, size_t peer_payload_stride_bytes,
    size_t peer_count, size_t peer_row_stride_bytes, uint8_t* dst, size_t rows,
    size_t row_width, size_t dst_row_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_gather_rows_f32_to_nvfp4_e2m1_fp8_e4m3(
    const float* src, const uint32_t* row_indices, uint8_t* dst, size_t rows,
    size_t row_width, size_t dst_row_stride_bytes);
ds4rt_status_t ds4rt_cuda_gather_rows_f32_to_nvfp4_e2m1_fp8_e4m3_async(
    const float* src, const uint32_t* row_indices, uint8_t* dst, size_t rows,
    size_t row_width, size_t dst_row_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_gather_rows_bf16(const uint16_t* src, const uint32_t* row_indices,
                                           uint16_t* dst, size_t rows, size_t row_width);
ds4rt_status_t ds4rt_cuda_gather_rows_bf16_async(const uint16_t* src,
                                                 const uint32_t* row_indices, uint16_t* dst,
                                                 size_t rows, size_t row_width,
                                                 void* cuda_stream);
ds4rt_status_t ds4rt_cuda_copy_row_prefix_bf16(
...
ds4rt_status_t ds4rt_cuda_copy_row_prefix_bf16_async(
    const uint16_t* src, uint16_t* dst, size_t rows, size_t src_row_width,
    size_t dst_row_width, size_t prefix_width, size_t src_row_offset, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_f32(const float* src, const uint32_t* row_indices,
                                               float* dst, size_t rows, size_t row_width);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_f32_async(const float* src,
                                                     const uint32_t* row_indices, float* dst,
                                                     size_t rows, size_t row_width,
                                                     void* cuda_stream);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_bf16_to_f32(const uint16_t* src,
                                                       const uint32_t* row_indices, float* dst,
                                                       size_t rows, size_t row_width);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_bf16_to_f32_async(
    const uint16_t* src, const uint32_t* row_indices, float* dst, size_t rows, size_t row_width,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_fp8_e4m3_row_scaled_to_f32(
    const uint8_t* src, size_t src_row_stride_bytes, const uint32_t* row_indices, float* dst,
    size_t rows, size_t row_width);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_fp8_e4m3_row_scaled_to_f32_async(
    const uint8_t* src, size_t src_row_stride_bytes, const uint32_t* row_indices, float* dst,
    size_t rows, size_t row_width, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_nvfp4_e2m1_fp8_e4m3_to_f32(
    const uint8_t* src, size_t src_row_stride_bytes, const uint32_t* row_indices, float* dst,
    size_t rows, size_t row_width);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_nvfp4_e2m1_fp8_e4m3_to_f32_async(
    const uint8_t* src, size_t src_row_stride_bytes, const uint32_t* row_indices, float* dst,
    size_t rows, size_t row_width, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_reduce_route_shards_to_f32(
    const ds4rt_route_shard_reduction_buffers_t* buffers, size_t rows, size_t row_width,
    size_t peer_row_stride_bytes, uint32_t local_dtype, uint32_t peer_dtype,
    uint32_t peer_count);
ds4rt_status_t ds4rt_cuda_reduce_route_shards_to_f32_async(
    const ds4rt_route_shard_reduction_buffers_t* buffers, size_t rows, size_t row_width,
    size_t peer_row_stride_bytes, uint32_t local_dtype, uint32_t peer_dtype,
    uint32_t peer_count, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_reduce_route_shards_bf16_fp8_to_fp8_rail_candidate_async(
    const ds4rt_route_shard_fp8_rail_reduction_buffers_t* buffers, size_t rows,
    size_t rail0_rows, size_t row_width, size_t peer_row_stride_bytes,
    size_t output_row_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_bf16_weighted_to_f32(
    const uint16_t* src, const uint32_t* row_indices, const float* row_weights, float* dst,
    size_t rows, size_t row_width);
ds4rt_status_t ds4rt_cuda_scatter_add_rows_bf16_weighted_to_f32_async(
    const uint16_t* src, const uint32_t* row_indices, const float* row_weights, float* dst,
    size_t rows, size_t row_width, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_kv_cache_write_bytes(const uint8_t* src, uint8_t* cache,
                                               size_t cache_offset_bytes, size_t bytes);
ds4rt_status_t ds4rt_cuda_kv_cache_write_bytes_async(const uint8_t* src, uint8_t* cache,
                                                     size_t cache_offset_bytes, size_t bytes,
                                                     void* cuda_stream);
ds4rt_status_t ds4rt_cuda_kv_cache_read_bytes(const uint8_t* cache, uint8_t* dst,
                                              size_t cache_offset_bytes, size_t bytes);
ds4rt_status_t ds4rt_cuda_kv_cache_read_bytes_async(const uint8_t* cache, uint8_t* dst,
                                                    size_t cache_offset_bytes, size_t bytes,
                                                    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_kv_cache_write_blocks(
...
ds4rt_status_t ds4rt_cuda_kv_cache_write_blocks_async(
    const uint8_t* src, uint8_t* cache, const uint64_t* src_offsets,
    const uint64_t* cache_offsets, const uint64_t* block_bytes, size_t block_count,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_kv_cache_read_blocks(
...
ds4rt_status_t ds4rt_cuda_kv_cache_read_blocks_async(
    const uint8_t* cache, uint8_t* dst, const uint64_t* cache_offsets,
    const uint64_t* dst_offsets, const uint64_t* block_bytes, size_t block_count,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_kv_cache_unpack_bf16(
...
ds4rt_status_t ds4rt_cuda_mla_kv_cache_unpack_bf16_async(
    const uint8_t* payload, uint16_t* kv_latent, uint16_t* k_rope, uint16_t* dsa_key,
    size_t rows, size_t kv_lora_rank, size_t rope_dim, size_t dsa_dim,
    size_t payload_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_kv_projected_split_bf16(
    const uint16_t* projected, uint16_t* k_nope, uint16_t* v, size_t rows, size_t heads,
    size_t nope_dim, size_t v_dim);
ds4rt_status_t ds4rt_cuda_mla_kv_projected_split_bf16_async(
    const uint16_t* projected, uint16_t* k_nope, uint16_t* v, size_t rows, size_t heads,
    size_t nope_dim, size_t v_dim, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_kv_prepare_bf16(
...
ds4rt_status_t ds4rt_cuda_mla_kv_prepare_bf16_async(
    const uint16_t* projected, const uint32_t* positions, const uint16_t* norm_weight,
    uint16_t* prepared, size_t rows, size_t projected_stride_bytes,
    size_t prepared_stride_bytes, float eps, float theta, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_transpose_rows_heads_bf16(
    const uint16_t* input, uint16_t* output, size_t rows, size_t heads,
    size_t width);
ds4rt_status_t ds4rt_cuda_transpose_rows_heads_bf16_async(
    const uint16_t* input, uint16_t* output, size_t rows, size_t heads,
    size_t width, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_transpose_heads_rows_bf16(
    const uint16_t* input, uint16_t* output, size_t rows, size_t heads,
    size_t width);
ds4rt_status_t ds4rt_cuda_transpose_heads_rows_bf16_async(
    const uint16_t* input, uint16_t* output, size_t rows, size_t heads,
    size_t width, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_compose_absorbed_query_bf16(
...
ds4rt_status_t ds4rt_cuda_mla_compose_absorbed_query_bf16_async(
    const uint16_t* latent_heads_rows, const uint16_t* rope_rows_heads,
    uint16_t* output_rows_heads, size_t rows, size_t heads,
...
ds4rt_status_t ds4rt_cuda_generic_kv_page_table_init(
    int32_t* page_table, size_t query_rows, size_t page_table_width);
ds4rt_status_t ds4rt_cuda_generic_kv_page_table_init_async(
    int32_t* page_table, size_t query_rows, size_t page_table_width,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_generic_kv_page_table_init_base(
    int32_t* page_table, size_t query_rows, size_t page_table_width,
    size_t base_offset);
ds4rt_status_t ds4rt_cuda_generic_kv_page_table_init_base_async(
    int32_t* page_table, size_t query_rows, size_t page_table_width,
    size_t base_offset, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_generic_kv_page_table_init_offsets(
    int32_t* page_table, const int32_t* row_offsets, size_t query_rows,
    size_t page_table_width);
ds4rt_status_t ds4rt_cuda_generic_kv_page_table_init_offsets_async(
    int32_t* page_table, const int32_t* row_offsets, size_t query_rows,
    size_t page_table_width, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_generic_kv_page_table_expand_indices(
    int32_t* output_indices, const uint32_t* physical_pages,
    size_t query_rows, size_t output_width, size_t active_tokens);
ds4rt_status_t ds4rt_cuda_generic_kv_page_table_expand_indices_async(
    int32_t* output_indices, const uint32_t* physical_pages,
    size_t query_rows, size_t output_width, size_t active_tokens,
    void* cuda_stream);
/* Benchmark-only cross-layer RoPE reuse candidate; serving does not reference it. */
ds4rt_status_t ds4rt_cuda_mla_rope_factors_f32_candidate_async(
    const uint32_t* positions, float* factors, size_t rows, float theta,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_kv_prepare_bf16_precomputed_rope_candidate_async(
    ...
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_kv_pack_fp8_ds_mla(
    const uint16_t* projected, uint8_t* packed, size_t rows, size_t projected_stride_bytes,
    size_t packed_stride_bytes);
ds4rt_status_t ds4rt_cuda_mla_kv_pack_fp8_ds_mla_async(
    const uint16_t* projected, uint8_t* packed, size_t rows, size_t projected_stride_bytes,
    size_t packed_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_kv_unpack_fp8_ds_mla(
    const uint8_t* packed, uint16_t* projected, size_t rows, size_t packed_stride_bytes,
    size_t projected_stride_bytes);
ds4rt_status_t ds4rt_cuda_mla_kv_unpack_fp8_ds_mla_async(
    const uint8_t* packed, uint16_t* projected, size_t rows, size_t packed_stride_bytes,
    size_t projected_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_kv_pack_mxfp4_ds_mla(
    const uint16_t* projected, uint8_t* packed, size_t rows, size_t projected_stride_bytes,
    size_t packed_stride_bytes);
ds4rt_status_t ds4rt_cuda_mla_kv_pack_mxfp4_ds_mla_async(
    const uint16_t* projected, uint8_t* packed, size_t rows, size_t projected_stride_bytes,
    size_t packed_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_kv_unpack_mxfp4_ds_mla(
    const uint8_t* packed, uint16_t* projected, size_t rows, size_t packed_stride_bytes,
    size_t projected_stride_bytes);
ds4rt_status_t ds4rt_cuda_mla_kv_unpack_mxfp4_ds_mla_async(
    const uint8_t* packed, uint16_t* projected, size_t rows, size_t packed_stride_bytes,
    size_t projected_stride_bytes, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_router_topk_f32(const float* hidden, const float* router_weight,
                                          const float* correction_bias, uint32_t* topk_indices,
                                          float* topk_scores, float* topk_weights, size_t rows,
                                          size_t hidden_dim, size_t experts, size_t top_k);
ds4rt_status_t ds4rt_cuda_router_topk_f32_async(
...
ds4rt_status_t ds4rt_cuda_router_topk_bf16(const uint16_t* hidden,
                                           const uint16_t* router_weight,
                                           const float* correction_bias, uint32_t* topk_indices,
                                           float* topk_scores, float* topk_weights, size_t rows,
                                           size_t hidden_dim, size_t experts, size_t top_k);
ds4rt_status_t ds4rt_cuda_router_topk_bf16_async(
...
ds4rt_status_t ds4rt_cuda_ds4_flash_router_topk_bf16(
...
ds4rt_status_t ds4rt_cuda_ds4_flash_router_topk_bf16_async(
    const uint16_t *hidden, const uint16_t *router_weight,
    const float *correction_bias, const int64_t *token_to_experts,
    const int64_t *token_ids, uint32_t *topk_indices, float *topk_scores,
...
ds4rt_status_t ds4rt_cuda_ds4_flash_router_refine_topk_bf16_async(
    const uint16_t *hidden, const uint16_t *router_weight,
    const float *correction_bias, float *approximate_logits,
    uint32_t *topk_indices, float *topk_scores, float *topk_weights, size_t rows,
    void *cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_pro_router_topk_bf16(
    const uint16_t *hidden, const uint16_t *router_weight,
    const float *correction_bias, const int64_t *token_to_experts,
    const int64_t *token_ids, uint32_t *topk_indices, float *topk_scores,
    float *topk_weights, size_t rows, int hash_routing);
ds4rt_status_t ds4rt_cuda_ds4_pro_router_topk_bf16_async(
    const uint16_t *hidden, const uint16_t *router_weight,
    const float *correction_bias, const int64_t *token_to_experts,
    const int64_t *token_ids, uint32_t *topk_indices, float *topk_scores,
    float *topk_weights, size_t rows, int hash_routing, void *cuda_stream);
ds4rt_status_t ds4rt_cuda_router_topk_bf16_cub(
    const uint16_t* hidden, const uint16_t* router_weight, const float* correction_bias,
    ...
    size_t hidden_dim, size_t experts, size_t top_k);
ds4rt_status_t ds4rt_cuda_router_topk_bf16_cub_async(
    const uint16_t* hidden, const uint16_t* router_weight, const float* correction_bias,
    float* corrected_scores, float* sorted_corrected_scores, uint32_t* unsorted_indices,
    uint32_t* sorted_indices, int* segment_offsets, uint32_t* topk_indices, float* topk_scores,
    float* topk_weights, void* cub_temp_storage, size_t cub_temp_storage_bytes, size_t rows,
    size_t hidden_dim, size_t experts, size_t top_k, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_linear_f32(const float* input, const float* weight, const float* bias,
                                     float* output, size_t rows, size_t input_dim,
                                     size_t output_dim);
ds4rt_status_t ds4rt_cuda_linear_f32_async(const float* input, const float* weight,
                                           const float* bias, float* output, size_t rows,
                                           size_t input_dim, size_t output_dim,
                                           void* cuda_stream);
ds4rt_status_t ds4rt_cuda_linear_bf16(const uint16_t* input, const uint16_t* weight,
                                      const uint16_t* bias, uint16_t* output, size_t rows,
                                      size_t input_dim, size_t output_dim);
ds4rt_status_t ds4rt_cuda_linear_bf16_async(const uint16_t* input, const uint16_t* weight,
                                            const uint16_t* bias, uint16_t* output, size_t rows,
                                            size_t input_dim, size_t output_dim,
                                            void* cuda_stream);
ds4rt_status_t ds4rt_cuda_linear_bf16_cublas(const uint16_t* input, const uint16_t* weight,
                                             const uint16_t* bias, uint16_t* output, size_t rows,
                                             size_t input_dim, size_t output_dim);
ds4rt_status_t ds4rt_cuda_linear_bf16_cublas_async(
...
ds4rt_status_t ds4rt_cuda_linear_bf16_f32_cublas_async(
...
ds4rt_status_t ds4rt_cuda_linear_bf16_m1_parity_batched_cublaslt_async(
...
ds4rt_status_t ds4rt_cuda_linear_lossless_bf16_m1_async(
...
ds4rt_status_t ds4rt_cuda_quantize_bf16_w8a16_group256_async(
...
ds4rt_status_t ds4rt_cuda_quantize_bf16_w8a16_group256_packed_async(
    const uint16_t* source, int8_t* weight, float* scales, size_t input_dim,
...
ds4rt_status_t ds4rt_cuda_dequantize_w8a16_group256_bf16_async(
...
ds4rt_status_t ds4rt_cuda_linear_w8a16_group256_m1_simt_async(
...
ds4rt_status_t ds4rt_cuda_linear_w8a16_group256_m1_parity_batched_async(
    const uint16_t* input, const int8_t* weight, const float* scales,
...
ds4rt_status_t ds4rt_cuda_linear_w8a16_group256_m1_warp_packed_async(
    const uint16_t* input, const int8_t* weight, const float* scales,
...
ds4rt_status_t ds4rt_cuda_linear_w8a8_group256_wmma_async(
...
ds4rt_status_t ds4rt_cuda_linear_w8a16_group256_triton_file_async(
    const uint16_t* input, const int8_t* weight, const float* scales,
    uint16_t* output, size_t rows, size_t input_dim, size_t output_dim,
...
ds4rt_status_t ds4rt_cuda_preload_w8a16_group256_aot(
    size_t input_dim, size_t output_dim);
// Bucketed M=2..256 row-major W8A16 projection from embedded Triton cubins.
ds4rt_status_t ds4rt_cuda_linear_w8a16_group256_aot_async(
    const uint16_t* input, const int8_t* weight, const float* scales,
    uint16_t* output, size_t rows, size_t input_dim, size_t output_dim,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_w8a16_packed_o_aot_init(void);
ds4rt_status_t ds4rt_cuda_w8a16_packed_o_initialize_launch_buffers_async(
    const ds4rt_b12x_coordinator_w4a16_buffers_t* buffers, size_t rows,
    size_t block_m, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_w8a16_packed_o_async(
    const ds4rt_b12x_coordinator_w4a16_buffers_t* buffers, size_t rows,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_linear_bf16_strided_batched_cublas(
    const uint16_t* input, const uint16_t* weight, uint16_t* output,
...
ds4rt_status_t ds4rt_cuda_linear_bf16_strided_batched_cublas_async(
    const uint16_t* input, const uint16_t* weight, uint16_t* output,
    size_t batch_count, size_t rows, size_t input_dim, size_t output_dim,
    size_t input_batch_stride, size_t weight_batch_stride,
    size_t output_batch_stride, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_matmul_bf16_strided_batched_cublas_async(
    const uint16_t* input, const uint16_t* right, uint16_t* output,
    size_t batch_count, size_t rows, size_t input_dim, size_t output_dim,
    size_t input_batch_stride, size_t right_batch_stride,
    size_t output_batch_stride, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_causal_attention_f32(const float* q, const float* k, const float* v,
                                               float* out, size_t rows, size_t heads,
                                               size_t qk_dim, size_t v_dim, float scale);
ds4rt_status_t ds4rt_cuda_causal_attention_f32_async(
...
ds4rt_status_t ds4rt_cuda_causal_attention_bf16(const uint16_t* q, const uint16_t* k,
                                                const uint16_t* v, uint16_t* out, size_t rows,
                                                size_t heads, size_t qk_dim, size_t v_dim,
                                                float scale);
ds4rt_status_t ds4rt_cuda_causal_attention_bf16_async(
...
ds4rt_status_t ds4rt_cuda_rope_f32(const float* input, const uint32_t* positions, float* out,
                                   size_t rows, size_t heads, size_t rotary_dim, float theta);
ds4rt_status_t ds4rt_cuda_rope_f32_async(const float* input, const uint32_t* positions,
                                         float* out, size_t rows, size_t heads,
                                         size_t rotary_dim, float theta, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_rope_bf16(const uint16_t* input, const uint32_t* positions,
                                    uint16_t* out, size_t rows, size_t heads,
                                    size_t rotary_dim, float theta);
ds4rt_status_t ds4rt_cuda_rope_bf16_async(const uint16_t* input, const uint32_t* positions,
                                          uint16_t* out, size_t rows, size_t heads,
                                          size_t rotary_dim, float theta, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_ds4_dspark_prompt_metadata_async(
...
ds4rt_status_t ds4rt_cuda_ds4_dspark_prepare_proposal_async(
    ...
    float theta, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_rope_attention_bf16(
...
ds4rt_status_t ds4rt_cuda_mla_rope_attention_bf16_async(
    const uint16_t* q_nope, const uint16_t* q_rope, const uint16_t* k_nope,
    const uint16_t* k_rope, const uint16_t* v, uint16_t* out, size_t rows, size_t heads,
    size_t nope_dim, size_t rope_dim, size_t v_dim, float scale, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_rope_attention_bf16_suffix(
    const uint16_t* q_nope, const uint16_t* q_rope, const uint16_t* k_nope,
...
ds4rt_status_t ds4rt_cuda_mla_rope_attention_bf16_suffix_async(
    const uint16_t* q_nope, const uint16_t* q_rope, const uint16_t* k_nope,
    const uint16_t* k_rope, const uint16_t* v, uint16_t* out, size_t rows,
    size_t query_row_offset, size_t query_rows, size_t heads, size_t nope_dim,
    size_t rope_dim, size_t v_dim, float scale, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_compressed_attention_bf16(
...
ds4rt_status_t ds4rt_cuda_mla_compressed_attention_bf16_async(
    const uint16_t* q_absorbed, const uint16_t* q_rope,
    const uint16_t* kv_latent, const uint16_t* k_rope, uint16_t* out_latent,
    size_t rows, size_t heads, size_t rope_dim, size_t kv_lora_rank, float scale,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_compressed_attention_interleaved_bf16(
    const uint16_t* q_absorbed, const uint16_t* q_rope,
...
ds4rt_status_t ds4rt_cuda_mla_compressed_attention_interleaved_bf16_async(
    const uint16_t* q_absorbed, const uint16_t* q_rope,
    const uint16_t* kv_payload, uint16_t* out_latent, size_t rows,
    size_t heads, size_t rope_dim, size_t kv_lora_rank,
    size_t kv_row_stride_bytes, size_t rope_offset_bytes, float scale,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_compressed_attention_interleaved_fp8(
    const uint16_t* q_absorbed, const uint16_t* q_rope,
    const uint8_t* kv_payload, uint16_t* out_latent, size_t rows,
    size_t heads, size_t rope_dim, size_t kv_lora_rank,
    size_t kv_row_stride_bytes, float scale);
ds4rt_status_t ds4rt_cuda_mla_compressed_attention_interleaved_fp8_async(
    const uint16_t* q_absorbed, const uint16_t* q_rope,
    const uint8_t* kv_payload, uint16_t* out_latent, size_t rows,
    size_t heads, size_t rope_dim, size_t kv_lora_rank,
    size_t kv_row_stride_bytes, float scale, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_mla_compressed_attention_interleaved_mxfp4(
    const uint16_t* q_absorbed, const uint16_t* q_rope,
    const uint8_t* kv_payload, uint16_t* out_latent, size_t rows,
    size_t heads, size_t rope_dim, size_t kv_lora_rank,
    size_t kv_row_stride_bytes, float scale);
ds4rt_status_t ds4rt_cuda_mla_compressed_attention_interleaved_mxfp4_async(
    const uint16_t* q_absorbed, const uint16_t* q_rope,
    const uint8_t* kv_payload, uint16_t* out_latent, size_t rows,
    size_t heads, size_t rope_dim, size_t kv_lora_rank,
    size_t kv_row_stride_bytes, float scale, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_sparse_mla_nvfp4_async(
    ...
    size_t kv_row_stride_bytes, float scale, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_sparse_mla_bf16_async(
    const uint16_t* query, const uint8_t* kv_payload,
    const int32_t* selected_indices, const int32_t* topk_lengths,
    uint16_t* partial, float* partial_lse, uint16_t* output,
    float* output_lse, size_t query_rows, size_t heads, size_t topk,
    size_t kv_row_stride_bytes, float scale, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_sparse_mla_bf16_gather_kv_async(
...
ds4rt_status_t ds4rt_cuda_sparse_mla_bf16_softmax_async(
    uint16_t* scores, const int32_t* topk_lengths, float* output_lse,
    size_t query_rows, size_t heads, size_t topk, float scale,
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_sparse_mla_nvfp4_gather_fp8_async(
...
ds4rt_status_t ds4rt_cuda_mla_nvfp4_expand_fp8_paged_async(
...
ds4rt_status_t ds4rt_cuda_mla_merge_state_bf16(
...
ds4rt_status_t ds4rt_cuda_mla_merge_state_bf16_async(
    uint16_t* accumulator, float* accumulator_lse, const uint16_t* partial,
    const float* partial_lse, size_t heads, size_t kv_lora_rank, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_packed_fp8_mla_exact_grouped_async(
    ...
    void* cuda_stream);
ds4rt_status_t ds4rt_cuda_embedding_lookup_f32(const float* embedding, const uint32_t* token_ids,
                                               float* out, size_t rows, size_t vocab,
                                               size_t hidden);
ds4rt_status_t ds4rt_cuda_embedding_lookup_f32_async(const float* embedding,
                                                     const uint32_t* token_ids, float* out,
                                                     size_t rows, size_t vocab, size_t hidden,
                                                     void* cuda_stream);
ds4rt_status_t ds4rt_cuda_embedding_lookup_bf16(const uint16_t* embedding,
                                                const uint32_t* token_ids, uint16_t* out,
                                                size_t rows, size_t vocab, size_t hidden);
ds4rt_status_t ds4rt_cuda_embedding_lookup_bf16_async(
    const uint16_t* embedding, const uint32_t* token_ids, uint16_t* out, size_t rows,
    size_t vocab, size_t hidden, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_lm_head_argmax_bf16(const uint16_t* hidden, const uint16_t* lm_head,
                                              uint32_t* out_indices, float* out_scores,
                                              size_t rows, size_t hidden_dim, size_t vocab);
ds4rt_status_t ds4rt_cuda_lm_head_argmax_bf16_async(
...
ds4rt_status_t ds4rt_cuda_lm_head_sample_topk_topp_bf16(
    ...
    float temperature, size_t top_k, float top_p);
ds4rt_status_t ds4rt_cuda_lm_head_sample_topk_topp_bf16_async(
    const uint16_t* hidden, const uint16_t* lm_head, const float* random_uniforms,
    uint32_t* out_indices, float* out_scores, size_t rows, size_t hidden_dim, size_t vocab,
    float temperature, size_t top_k, float top_p, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_lm_head_argmax_sample_topk_topp_bf16_staged(
    const uint16_t* hidden, const uint16_t* lm_head, const float* random_uniforms,
...
ds4rt_status_t ds4rt_cuda_lm_head_argmax_sample_topk_topp_bf16_staged_async(
    const uint16_t* hidden, const uint16_t* lm_head, const float* random_uniforms,
    uint32_t* out_argmax_indices, float* out_argmax_scores, uint32_t* out_sample_indices,
    float* out_sample_scores, float* logits_workspace, size_t rows, size_t hidden_dim,
    size_t vocab, float temperature, size_t top_k, float top_p, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_lm_head_sample_topk_topp_bf16_cub(
    const uint16_t* hidden, const uint16_t* lm_head, const float* random_uniforms,
    ...
    size_t vocab, float temperature, size_t top_k, float top_p);
ds4rt_status_t ds4rt_cuda_lm_head_sample_topk_topp_bf16_cub_async(
    const uint16_t* hidden, const uint16_t* lm_head, const float* random_uniforms,
    float* logits_workspace, float* sorted_logits, uint32_t* unsorted_indices,
    uint32_t* sorted_indices, int* segment_offsets, uint32_t* out_indices, float* out_scores,
    void* cub_temp_storage, size_t cub_temp_storage_bytes, size_t rows, size_t hidden_dim,
    size_t vocab, float temperature, size_t top_k, float top_p, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_logits_argmax_f32(const float* logits, uint32_t* out_indices,
                                            float* out_scores, size_t rows, size_t vocab);
ds4rt_status_t ds4rt_cuda_logits_argmax_f32_async(const float* logits, uint32_t* out_indices,
                                                  float* out_scores, size_t rows, size_t vocab,
                                                  void* cuda_stream);
...
ds4rt_status_t ds4rt_cuda_dspark_terminal_greedy_bf16_async(
...
ds4rt_status_t ds4rt_cuda_logits_sample_topk_topp_f32(
...
ds4rt_status_t ds4rt_cuda_logits_sample_topk_topp_f32_async(
    const float* logits, const float* random_uniforms, uint32_t* out_indices, float* out_scores,
    size_t rows, size_t vocab, float temperature, size_t top_k, float top_p, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_logits_sample_topk_topp_f32_cub(
    ...
    float top_p);
ds4rt_status_t ds4rt_cuda_logits_sample_topk_topp_f32_cub_async(
    const float* logits, const float* random_uniforms, float* sorted_logits,
    uint32_t* unsorted_indices, uint32_t* sorted_indices, int* segment_offsets,
    uint32_t* out_indices, float* out_scores, void* cub_temp_storage,
    size_t cub_temp_storage_bytes, size_t rows, size_t vocab, float temperature, size_t top_k,
    float top_p, void* cuda_stream);
ds4rt_status_t ds4rt_cuda_pack_nibbles(const uint8_t* codes, uint8_t* packed, size_t count);
ds4rt_status_t ds4rt_cuda_unpack_nibbles(const uint8_t* packed, uint8_t* codes, size_t count);
