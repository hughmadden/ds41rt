...
BenchmarkExtractionDensity(int vocab_size, int warmup, int iterations) {
  const std::vector<int> candidate_counts = {
      8, 256, 1024, 4096, 8192, 16384, 24576, 38720, 77440, 116160};
  std::vector<int> counts;
  for (const int count : candidate_counts) {
    if (count <= vocab_size) {
      counts.push_back(count);
    }
  }
  if (counts.empty() || counts.back() != vocab_size) {
    counts.push_back(vocab_size);
  }
  ...
  for (size_t row = 0; row < counts.size(); ++row) {
    for (int64_t index = 0; index < counts[row]; ++index) {
      const int token = static_cast<int>(
          (index + static_cast<int64_t>(row) * 1009 + 17) % vocab_size);
...
int Run(const Options &options) {
  ...
  output["benchmark"] = picojson::value(std::string("xgrammar_ds4_native"));
...
int main(int argc, char **argv) {
  try {
    ...
    std::cerr << "xgrammar_ds4_bench: " << error.what() << '\n';
    return 1;
  }
}
