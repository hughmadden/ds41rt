...
void test_cuda_copy_h2d_2d_async_copies_active_row_prefixes() {
  const std::vector<uint8_t> source = {
      1, 2, 3, 90, 91,
      4, 5, 6, 92, 93,
      7, 8, 9, 94, 95,
...
void test_cuda_accumulate_bf16_to_f32_preserves_launch_order() {
  ...
  for (size_t index = 0; index < first.size(); ++index) {
    expected[index] = bf16_to_f32(first[index]) + bf16_to_f32(second[index]);
  }

  auto dfirst = device_buffer(first.size() * sizeof(uint16_t));
  auto dsecond = device_buffer(second.size() * sizeof(uint16_t));
  ...
  assert_close(copy_d2h<float>(daccumulator, expected.size()), expected);
  free_buffer(&dfirst);
  free_buffer(&dsecond);
  free_buffer(&daccumulator);
}
...
void test_cuda_generic_kv_page_table_init_base_is_exported_and_correct() {
  constexpr size_t rows = 2;
...
int main() {
  ...
  test_cuda_generic_kv_page_table_init_base_is_exported_and_correct();
  ...
  test_cuda_accumulate_bf16_to_f32_preserves_launch_order();
