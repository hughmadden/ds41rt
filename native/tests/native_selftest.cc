...
void test_version() {
  char version[128] = {};
  assert(ds4rt_native_version(version, sizeof(version)) == DS4RT_STATUS_OK);
  assert(std::strstr(version, "ds4rt_native") != nullptr);
}

void test_cuda_device_info() {
  ds4rt_cuda_device_info_t info = {};
  assert(ds4rt_cuda_device_info(0, &info) == DS4RT_STATUS_OK);
...
void test_allocate_copy_free_roundtrip() {
  ...
  assert(ds4rt_alloc_device_buffer(input.size(), &buffer) == DS4RT_STATUS_OK);
  assert(buffer.ptr != nullptr);
  assert(buffer.bytes == input.size());

  assert(ds4rt_copy_h2d(buffer, input.data(), input.size()) == DS4RT_STATUS_OK);
  assert(ds4rt_copy_d2h(output.data(), buffer, output.size()) == DS4RT_STATUS_OK);
  assert(output == input);

  assert(ds4rt_free_device_buffer(&buffer) == DS4RT_STATUS_OK);
  assert(buffer.ptr == nullptr);
...
void test_device_to_device_copy_roundtrip() {
  ...
  ds4rt_device_buffer_t src = {};
  ds4rt_device_buffer_t dst = {};
  assert(ds4rt_alloc_device_buffer(input.size(), &src) == DS4RT_STATUS_OK);
  assert(ds4rt_alloc_device_buffer(input.size(), &dst) == DS4RT_STATUS_OK);

  assert(ds4rt_copy_h2d(src, input.data(), input.size()) == DS4RT_STATUS_OK);
  assert(ds4rt_copy_d2d(dst, src, input.size()) == DS4RT_STATUS_OK);
  assert(ds4rt_copy_d2h(output.data(), dst, output.size()) == DS4RT_STATUS_OK);
  assert(output == input);

  assert(ds4rt_free_device_buffer(&src) == DS4RT_STATUS_OK);
  assert(ds4rt_free_device_buffer(&dst) == DS4RT_STATUS_OK);
}
...
void test_host_buffer_copy_roundtrip() {
  ...
  ds4rt_host_buffer_t host = {};
  assert(ds4rt_alloc_host_buffer(input.size(), &host) == DS4RT_STATUS_OK);
  assert(host.ptr != nullptr);
  assert(host.bytes == input.size());
  assert(host.flags != DS4RT_HOST_BUFFER_FLAG_NONE);
  std::memcpy(host.ptr, input.data(), input.size());

  ds4rt_device_buffer_t device = {};
  assert(ds4rt_alloc_device_buffer(input.size(), &device) == DS4RT_STATUS_OK);
  assert(ds4rt_copy_h2d(device, host.ptr, input.size()) == DS4RT_STATUS_OK);
  assert(ds4rt_copy_d2h(output.data(), device, output.size()) == DS4RT_STATUS_OK);
  assert(output == input);

  assert(ds4rt_free_device_buffer(&device) == DS4RT_STATUS_OK);
  assert(ds4rt_free_host_buffer(&host) == DS4RT_STATUS_OK);
  assert(host.ptr == nullptr);
  assert(host.bytes == 0);
  assert(host.flags == DS4RT_HOST_BUFFER_FLAG_NONE);
}
...
void test_error_propagation() {
  char tiny[4] = {};
  assert(ds4rt_native_version(tiny, sizeof(tiny)) == DS4RT_STATUS_BUFFER_TOO_SMALL);

  char error[128] = {};
  assert(ds4rt_last_error(error, sizeof(error)) == DS4RT_STATUS_OK);
  assert(std::strstr(error, "too small") != nullptr);

  assert(ds4rt_alloc_device_buffer(0, nullptr) == DS4RT_STATUS_INVALID_ARGUMENT);
  assert(ds4rt_last_error(error, sizeof(error)) == DS4RT_STATUS_OK);
...
void test_nccl_abi() {
  ...
  const ds4rt_status_t size_status = ds4rt_nccl_unique_id_bytes(&unique_id_bytes);
  assert(size_status == DS4RT_STATUS_OK || size_status == DS4RT_STATUS_NCCL_UNAVAILABLE);
  if (size_status == DS4RT_STATUS_NCCL_UNAVAILABLE) {
    assert(unique_id_bytes == 0);
    return;
  }
  assert(unique_id_bytes > 0);
  std::vector<unsigned char> unique_id(unique_id_bytes, 0);
  assert(ds4rt_nccl_get_unique_id(unique_id.data(), unique_id.size()) == DS4RT_STATUS_OK);
}

void test_ds4_flash_aot_availability_abi() {
  assert(ds4rt_cuda_ds4_flash_spark_aot_available(nullptr) ==
         DS4RT_STATUS_INVALID_ARGUMENT);
  int available = -1;
  assert(ds4rt_cuda_ds4_flash_spark_aot_available(&available) ==
         DS4RT_STATUS_OK);
...
void test_rdma_abi() {
  ...
  assert(ds4rt_rdma_rc_endpoint_post_recv_at(nullptr, 0, 1, 1) ==
         DS4RT_STATUS_INVALID_ARGUMENT);
  assert(ds4rt_rdma_rc_endpoint_send_at(nullptr, &endpoint_byte, 0, 1, 1) ==
         DS4RT_STATUS_INVALID_ARGUMENT);
  assert(ds4rt_rdma_rc_endpoint_send_parts_at(nullptr, &endpoint_byte, 1,
                                               &endpoint_byte, 1, 0, 1) ==
         DS4RT_STATUS_INVALID_ARGUMENT);
  assert(ds4rt_rdma_rc_endpoint_copy_recv_at(nullptr, &endpoint_byte, 1, 0, 1) ==
         DS4RT_STATUS_INVALID_ARGUMENT);

  ds4rt_rdma_device_info_t info = {};
  assert(ds4rt_rdma_device_info(&info) == DS4RT_STATUS_OK);
  assert(info.first_device_name[0] != '\0');
  assert(info.status[0] != '\0');

  std::vector<unsigned char> buffer(12288, 0);
  ds4rt_rdma_host_buffer_plan_t plan = {};
  assert(ds4rt_rdma_plan_host_buffer_registration(buffer.data(), buffer.size(), 4096, &plan) ==
         DS4RT_STATUS_OK);
  ...
  ds4rt_rdma_register_probe_t probe = {};
  const ds4rt_status_t status =
      ds4rt_rdma_register_host_buffer_probe(buffer.data(), buffer.size(), &probe);
  assert(status == DS4RT_STATUS_OK || status == DS4RT_STATUS_RDMA_UNAVAILABLE);
  ...
  ds4rt_rdma_rc_qp_probe_t qp = {};
  const ds4rt_status_t qp_status = ds4rt_rdma_create_rc_qp_probe(1, 16, 16, 1, &qp);
  assert(qp_status == DS4RT_STATUS_OK || qp_status == DS4RT_STATUS_RDMA_UNAVAILABLE);
  assert(qp.port_num == 1);
  assert(qp.requested_send_wr == 16);
  assert(qp.requested_recv_wr == 16);
  assert(qp.requested_max_sge == 1);
  if (qp_status == DS4RT_STATUS_OK) {
    assert(qp.created == 1);
    assert(qp.qp_num != 0);
  ...
  ds4rt_rdma_rc_send_recv_probe_t loopback = {};
  const ds4rt_status_t loopback_status =
      ds4rt_rdma_rc_send_recv_loopback_probe(1, 12288, &loopback);
  assert(loopback_status == DS4RT_STATUS_OK ||
         loopback_status == DS4RT_STATUS_RDMA_UNAVAILABLE);
  assert(loopback.port_num == 1);
  assert(loopback.bytes == 12288);
  if (loopback_status == DS4RT_STATUS_OK) {
  ...
  ds4rt_rdma_rc_protocol_v2_loopback_probe_t protocol_loopback = {};
  const ds4rt_status_t protocol_loopback_status =
      ds4rt_rdma_rc_protocol_v2_loopback_probe(1, request_frame.data(), request_frame.size(),
                                               response_frame.data(), response_frame.size(),
                                               &protocol_loopback);
  assert(protocol_loopback_status == DS4RT_STATUS_OK ||
         protocol_loopback_status == DS4RT_STATUS_RDMA_UNAVAILABLE);
  ...
  if (protocol_loopback_status == DS4RT_STATUS_OK) {
...
int main() {
  ...
  test_rdma_abi();
  std::cout << "ds4rt_native_selftest passed\n";
  return 0;
}
