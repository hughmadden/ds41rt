...
case "$role" in
  coordinator)
    image="${DS4RT_COORDINATOR_DOCKER_DEV:-ds4rt-coordinator-dev}"
    ;;
  expert|spark)
    image="${DS4RT_SPARK_EXPERT_DOCKER_DEV:-ds4rt-spark-expert-dev}"
    ;;
  dev)
    image="${DS4RT_COORDINATOR_DOCKER_DEV:-ds4rt-coordinator-dev}"
    ;;
...
docker_args=(
  ...
  -e DS4RT_MODEL_ID="${DS4RT_MODEL_ID:-wrldsuksgo2mars/DeepSeek-V4-Pro-0813-EXL3-K2-calibrated-v1}"
  -e DS4RT_MODEL_REVISION="${DS4RT_MODEL_REVISION:-}"
)
...
if [[ "$role" == "expert" || "$role" == "spark" ]]; then
  ...
  release_load_config "${DS4RT_CONFIG:-$repo_root/ds4rt.config}"
...
if [[ "$repo_root" != "/workspace/ds4rt" ]]; then
...
if [[ "$role" == "coordinator" || "$role" == "dev" ]]; then
  if [[ "${DS4RT_DOCKER_HOST_NETWORK:-0}" == "1" ]]; then
    docker_args+=(--net=host --ipc=host --ulimit memlock=-1:-1 --cap-add IPC_LOCK)
    if [[ -e /dev/infiniband ]]; then
      docker_args+=(--device=/dev/infiniband)
    fi
