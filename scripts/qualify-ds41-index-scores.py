#!/usr/bin/env python3
"""Paged FP4 index scores against the pinned reference's BF16 scoring expressions."""
import argparse
import ast
import ctypes as C
import hashlib
...
def main():
    ...
    score = lib.ds41rt_v41_index_scores
    score.argtypes = [C.c_void_p] * 10 + [C.c_int32] * 4 + [C.c_uint64, C.c_void_p]
    ...
    with torch.cuda.stream(stream), torch.no_grad():
        ...
        for queries, count, slots, stride in ((1,3,1,1), (16,255,16,4), (80,511,16,8),
                                              (256,64,16,8), (4096,3,16,1), (1,4096,16,4096), (1,16384,1,64)):
            ...
            meta = torch.zeros(queries,2, device='cuda', dtype=torch.int64)
            ...
            if queries > 1:
                ...
                meta[1] = torch.tensor([0,stride*256],device='cuda')
                positions[1,0] = 0
                meta[-1,0] = slots  # invalid request slot
                lengths[1] = 0
            y = torch.empty(queries,count,device='cuda')
            tensors = [q,qs,w,keys,ks,pages,lengths,meta,positions,y]
            def launch():
                assert score(*(t.data_ptr() for t in tensors),queries,count,slots,stride,capacity,stream.cuda_stream) == 0
            def compare():
                ...
                expected = reference(decode(q,qs)[:,None],decode(keys[physical],ks[physical]),w[:,None])[:,0].float()
            ...
            for i in range(10):
                bad = ptrs.copy()
                bad[i] = 0
                assert score(*bad,queries,count,slots,stride,capacity,stream.cuda_stream) != 0
                bad[i] = (1<<64)-1
                assert score(*bad,queries,count,slots,stride,capacity,stream.cuda_stream) != 0
                if i < 9:
                    bad = ptrs.copy()
            ...
            for shape in ((0,count,slots,stride,capacity),(4097,count,slots,stride,capacity),
                ...
                assert score(*ptrs,*shape,stream.cuda_stream) != 0
            results.append({'queries':queries,'candidates':count,'slots':slots,'stride':stride,
                            'capacity':capacity,'valid_scores':valid,'changed_valid_scores':changed,
        ...
        stream.synchronize()
    args.output.write_text(json.dumps({'scope':'Committed-history index scores only; no selection or proposal overlay',
