...
Environment:
  ADDR or DS4RT_REAL_FULL_SERVE_ADDR
      API listen address; default: 0.0.0.0:8000
  DS4RT_REAL_FULL_SERVE_TRANSPORT
  ...
  DS4RT_REAL_FULL_KV_POOL_TOKENS
      ...
      physical.
  DS4RT_REAL_FULL_MAX_EXECUTION_LANES
      ...
      remains diagnostic.
  DS4RT_REAL_FULL_MOE_RESPONSE_DTYPE
  ...
  DS4RT_EXPERT_INTERMEDIATE_SHARDS
      expert MLP intermediate shards. Strict DS4 serving requires 4.
  DS4RT_EXPERT_INTERMEDIATE_REDUCTION
  ...
  DS4RT_EXPERT_INTERMEDIATE_REDUCTION_DTYPE
  ...
  DS4RT_PYTHON
      ...
      otherwise python3.
  DS4RT_REAL_FULL_SERVE_WARMUP_EXPERTS
      set 0 to skip binary ProtocolV2 expert precompile warmup; default: 1
  DS4RT_REAL_FULL_SERVE_WARMUP_TIMEOUT_MS
      timeout for precompile warmup frames; default: 120000
  DS4RT_REAL_FULL_SERVE_WARMUP_HIDDEN_DIM
  DS4RT_REAL_FULL_SERVE_WARMUP_ROUTES_PER_ROW
  ...
  DS4RT_REAL_FULL_PROTOCOL_V2_TIMEOUT_MS
      ...
      real request.
  DS4RT_REAL_FULL_SERVE_PREWARM_REQUEST
  ...
  DS4RT_REAL_FULL_SERVE_EXPERT_LINK_SUFFIX
  ...
  DS4RT_REAL_FULL_REQUEST_THREAD_PINNED
      keep each sequence's graph capture and replay on one CUDA-owning host thread;
      defaults to 1 when DS4RT_B12X=1 and otherwise defaults to 0.
  ...
  DS4RT_REAL_FULL_SERVE_SHARED_CPU_LIST
  ...
  DS4RT_REAL_FULL_REQUEST_WORKER_CPUS
      optional comma-separated physical CPU assignment, one entry per pinned
      request worker.
  DS4RT_REAL_FULL_SCHEDULER_WORKER_CPU
  ...
  DS4RT_REAL_FULL_REQUEST_LONG_PREFIX_SMALL_PREFILL_CHUNK_TOKENS
      ...
      1024-query limit.
  DS4RT_REAL_FULL_ATTENTION_READY_FRONTIER_MAX_TOKENS
  ...
  DS4RT_REAL_FULL_KV_SNAPSHOT_SAVE
  ...
  DS4RT_REAL_FULL_KV_SNAPSHOT_SAVE_TOKENS
  ...
  DS4RT_REAL_FULL_KV_SNAPSHOT_SAVE_POINTS
  ...
  DS4RT_REAL_FULL_KV_SNAPSHOT_LOAD
  ...
  DS4RT_REAL_FULL_PACKED_FP8_MLA_BATCHED_SUFFIX
  ...
  DS4RT_COORDINATOR_W8A16_Q_A
  DS4RT_COORDINATOR_W8A16_Q_B
  DS4RT_COORDINATOR_W8A16_O_PROJ
  ...
  DS4RT_COORDINATOR_W8A16_PACKED_O
  ...
  DS4RT_REAL_FULL_RETAIN_SPECULATIVE_QUERY_PROJECTION_GRAPHS
  ...
  DS4RT_FLASHINFER_WORKSPACE_BASE
  ...
  DS4RT_REAL_FULL_SERVE_EXPERT_HOSTS or DS4RT_REAL_FULL_TCP_EXPERT_HOSTS
      explicit owner=host:port list; overrides DS4RT_SPARK_HOSTS/suffix mapping
  DS4RT_REAL_FULL_SERVE_EXPERT_ADDITIONAL_RAILS
  ...
  DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_MIN_ROWS
      minimum logical request rows before striping across rails; default: 512
  DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_SPARK_REDUCTION
  ...
  DS4RT_SPARK_EXPERT_PORT
      expert TCP port; default: 9100
EOF
  exit 0
fi

addr="${ADDR:-${DS4RT_REAL_FULL_SERVE_ADDR:-0.0.0.0:8000}}"
...
expert_link_suffix="${DS4RT_REAL_FULL_SERVE_EXPERT_LINK_SUFFIX:-}"
expert_hosts="${DS4RT_REAL_FULL_SERVE_EXPERT_HOSTS:-${DS4RT_REAL_FULL_TCP_EXPERT_HOSTS:-}}"
expert_additional_rails="${DS4RT_REAL_FULL_SERVE_EXPERT_ADDITIONAL_RAILS:-}"
catalog="${CATALOG:-}"
coordinator_transport="${DS4RT_REAL_FULL_SERVE_TRANSPORT:-tcp}"
protocol_v2_verbs_host_execution_lanes="${DS4RT_PROTOCOL_V2_VERBS_HOST_EXECUTION_LANES:-4}"
protocol_v2_verbs_host_shared_cq_harvester="${DS4RT_PROTOCOL_V2_VERBS_HOST_SHARED_CQ_HARVESTER:-1}"
protocol_v2_verbs_host_stripe_min_rows="${DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_MIN_ROWS:-512}"
...
intermediate_reduction_min_rows="${DS4RT_EXPERT_INTERMEDIATE_REDUCTION_MIN_ROWS:-16}"
...
if [ "${DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_SPARK_REDUCTION+x}" ]; then
  protocol_v2_verbs_host_stripe_spark_reduction="$DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_SPARK_REDUCTION"
else
  case "$intermediate_reduction" in
...
kv_cache_dtype="${DS4RT_REAL_FULL_SERVE_KV_CACHE_DTYPE:-bf16}"
max_context_tokens="${DS4RT_REAL_FULL_SERVE_MAX_CONTEXT_TOKENS:-131072}"
native_lib_explicit=0
if [ -n "${DS4RT_NATIVE_LIB:-}" ]; then
  native_lib="$DS4RT_NATIVE_LIB"
  native_lib_explicit=1
elif [ "$coordinator_transport" = "verbs-host" ]; then
  native_lib="$repo_root/native/build-cuda-rdma-coordinator-aot/libds4rt_native.so"
else
  native_lib="$repo_root/native/build-cuda/libds4rt_native.so"
fi
build_profile="${DS4RT_REAL_FULL_SERVE_BUILD_PROFILE:-release}"
bin="${DS4RT_BIN:-$repo_root/rust/target/${build_profile}/ds4rt}"
start_experts="${DS4RT_REAL_FULL_SERVE_START_EXPERTS:-1}"
check_experts="${DS4RT_REAL_FULL_SERVE_CHECK_EXPERTS:-1}"
build_daemon="${DS4RT_REAL_FULL_SERVE_BUILD_DAEMON:-1}"
build_native="${DS4RT_REAL_FULL_SERVE_BUILD_NATIVE:-1}"
require_cuda="${DS4RT_REAL_FULL_SERVE_REQUIRE_CUDA:-1}"
shared_cpu_list="${DS4RT_REAL_FULL_SERVE_SHARED_CPU_LIST:-}"
w8a16_q_a="${DS4RT_COORDINATOR_W8A16_Q_A:-1}"
w8a16_q_b="${DS4RT_COORDINATOR_W8A16_Q_B:-1}"
w8a16_o_proj="${DS4RT_COORDINATOR_W8A16_O_PROJ:-1}"
w8a16_packed_o="${DS4RT_COORDINATOR_W8A16_PACKED_O:-1}"
w8a16_async_attention="${DS4RT_COORDINATOR_W8A16_ASYNC_ATTENTION:-1}"
w8a16_aot="${DS4RT_W8A16_AOT:-OFF}"
for projection_flag in "$w8a16_q_a" "$w8a16_q_b" "$w8a16_o_proj"; do
  case "${projection_flag,,}" in
    1|true|yes|on|w8a16) w8a16_aot=ON ;;
  esac
done
warmup_experts="${DS4RT_REAL_FULL_SERVE_WARMUP_EXPERTS:-1}"
warmup_layer_id="${DS4RT_REAL_FULL_SERVE_WARMUP_LAYER_ID:-${DS4RT_PHASE0_TCP_LAYER_ID:-3}}"
warmup_iterations="${DS4RT_REAL_FULL_SERVE_WARMUP_ITERATIONS:-1}"
warmup_rows="${DS4RT_REAL_FULL_SERVE_WARMUP_ROWS:-1}"
warmup_timeout_ms="${DS4RT_REAL_FULL_SERVE_WARMUP_TIMEOUT_MS:-120000}"
warmup_measured_timeout_ms="${DS4RT_REAL_FULL_SERVE_WARMUP_MEASURED_TIMEOUT_MS:-5000}"
warmup_roundtrip_rows="${DS4RT_REAL_FULL_SERVE_WARMUP_ROUNDTRIP_ROWS:-1}"
warmup_prefill_roundtrip_rows="${DS4RT_REAL_FULL_SERVE_WARMUP_PREFILL_ROUNDTRIP_ROWS:-16,256,512}"
warmup_prefill_chain_rows="${DS4RT_REAL_FULL_SERVE_WARMUP_PREFILL_CHAIN_ROWS:-16,256,512}"
warmup_expected_executor="${DS4RT_REAL_FULL_SERVE_WARMUP_EXPECTED_EXECUTOR:-protocol-v2-real-nvfp4-checkpoint-executor}"
warmup_hidden_dim="${DS4RT_REAL_FULL_SERVE_WARMUP_HIDDEN_DIM:-}"
warmup_routes_per_row="${DS4RT_REAL_FULL_SERVE_WARMUP_ROUTES_PER_ROW:-}"
warmup_routed_experts=""
expert_ready_timeout_secs="${DS4RT_REAL_FULL_SERVE_EXPERT_READY_TIMEOUT_SECS:-900}"
expert_warmup_status_file="${DS4RT_REAL_FULL_SERVE_EXPERT_WARMUP_STATUS_FILE:-}"
prewarm_request="${DS4RT_REAL_FULL_SERVE_PREWARM_REQUEST:-1}"
protocol_v2_timeout_ms="${DS4RT_REAL_FULL_PROTOCOL_V2_TIMEOUT_MS:-120000}"
dspark="${DS4RT_REAL_FULL_DSPARK:-1}"
dspark_shadow="${DS4RT_REAL_FULL_DSPARK_SHADOW:-0}"
ds4_flash_spark_aot="${DS4RT_DS4_FLASH_SPARK_AOT:-1}"
...
for affinity_list in \
  "$shared_cpu_list" \
  "${DS4RT_REAL_FULL_REQUEST_WORKER_CPUS:-}" \
  "${DS4RT_REAL_FULL_SCHEDULER_WORKER_CPU:-}"; do
...
configure_host_python() {
  local configured_python="${DS4RT_PYTHON:-}"
...
PY
  ...
  host_nccl_include_dir="${DS4RT_NCCL_INCLUDE_DIR:-}"
  host_nccl_library="${DS4RT_NCCL_LIBRARY:-}"
  ...
  if [ -n "${DS4RT_CUTE_DSL_LIBDIR:-}" ]; then
    python_libs+=("$DS4RT_CUTE_DSL_LIBDIR")
...
configure_pinned_sparkinfer() {
  ...
  "$DS4RT_PYTHON" "$repo_root/scripts/verify-sparkinfer-source.py" \
...
resolve_warmup_model_geometry() {
  local geometry
  geometry="$("$DS4RT_PYTHON" - "$model_id" "${DS4RT_MODEL_REVISION:-}" <<'PY'
import json
import os
import sys
from pathlib import Path
...
PY
  ...
  if ! [[ "$warmup_hidden_dim" =~ ^[0-9]+$ ]] || [ "$warmup_hidden_dim" -lt 1 ]; then
    echo "DS4RT_REAL_FULL_SERVE_WARMUP_HIDDEN_DIM must be a positive integer" >&2
    exit 2
  fi
  if ! [[ "$warmup_routes_per_row" =~ ^[0-9]+$ ]] || [ "$warmup_routes_per_row" -lt 1 ]; then
    echo "DS4RT_REAL_FULL_SERVE_WARMUP_ROUTES_PER_ROW must be a positive integer" >&2
    exit 2
  fi
...
require_bool_flag DS4RT_REAL_FULL_SERVE_START_EXPERTS "$start_experts"
require_bool_flag DS4RT_REAL_FULL_SERVE_CHECK_EXPERTS "$check_experts"
require_bool_flag DS4RT_REAL_FULL_SERVE_BUILD_DAEMON "$build_daemon"
require_bool_flag DS4RT_REAL_FULL_SERVE_BUILD_NATIVE "$build_native"
require_bool_flag DS4RT_REAL_FULL_SERVE_REQUIRE_CUDA "$require_cuda"
require_bool_flag DS4RT_REAL_FULL_SERVE_WARMUP_EXPERTS "$warmup_experts"
require_bool_flag DS4RT_REAL_FULL_SERVE_PREWARM_REQUEST "$prewarm_request"
require_bool_flag DS4RT_REAL_FULL_DSPARK "$dspark"
require_bool_flag DS4RT_REAL_FULL_DSPARK_SHADOW "$dspark_shadow"
require_bool_flag DS4RT_DS4_FLASH_SPARK_AOT "$ds4_flash_spark_aot"
if [ "$dspark" = "1" ] && [ "$dspark_shadow" = "1" ]; then
  echo "DS4RT_REAL_FULL_DSPARK and DS4RT_REAL_FULL_DSPARK_SHADOW cannot both be enabled" >&2
  exit 2
fi
case "$coordinator_transport" in
  tcp|verbs-host) ;;
  *)
    echo "DS4RT_REAL_FULL_SERVE_TRANSPORT must be tcp or verbs-host" >&2
    exit 2
    ;;
esac
case "$kv_cache_dtype" in
  bf16|fp8|nvfp4) ;;
  *)
    echo "DS4RT_REAL_FULL_SERVE_KV_CACHE_DTYPE must be bf16, fp8, or nvfp4" >&2
    exit 2
    ;;
esac
case "$intermediate_shards" in
  4) ;;
  *)
    echo "strict DS4 serving requires DS4RT_EXPERT_INTERMEDIATE_SHARDS=4" >&2
    exit 2
    ;;
esac
case "$intermediate_reduction" in
  ...
  *)
    echo "DS4RT_EXPERT_INTERMEDIATE_REDUCTION must be coordinator, spark, spark-owner, spark-hybrid, spark-rdma, or spark-rdma-hybrid" >&2
    exit 2
    ;;
esac
if [ "$intermediate_reduction" != "coordinator" ] && [ "$intermediate_shards" != "4" ]; then
  echo "DS4RT_EXPERT_INTERMEDIATE_REDUCTION=$intermediate_reduction requires DS4RT_EXPERT_INTERMEDIATE_SHARDS=4" >&2
  exit 2
fi
case "$intermediate_reduction" in
  spark-owner|spark-hybrid|spark-rdma|spark-rdma-hybrid)
    if [ "$coordinator_transport" != "verbs-host" ]; then
      echo "DS4RT_EXPERT_INTERMEDIATE_REDUCTION=$intermediate_reduction requires verbs-host serving" >&2
      exit 2
    fi
    ;;
esac
case "$intermediate_reduction_dtype" in
  bf16|fp8|nvfp4) ;;
  *)
    echo "DS4RT_EXPERT_INTERMEDIATE_REDUCTION_DTYPE must be bf16, fp8, or nvfp4" >&2
    exit 2
    ;;
esac
case "$intermediate_owner_reduction_dtype" in
  bf16|fp8|nvfp4) ;;
  *)
    echo "DS4RT_EXPERT_INTERMEDIATE_OWNER_REDUCTION_DTYPE must be bf16, fp8, or nvfp4" >&2
    exit 2
    ;;
esac
...
case "$intermediate_reduction" in
  spark-rdma|spark-rdma-hybrid)
    if [ "$intermediate_row_sharded_reduction" != "1" ]; then
      echo "DS4RT_EXPERT_INTERMEDIATE_REDUCTION=$intermediate_reduction requires row-sharded reduction" >&2
      exit 2
    fi
    ;;
esac
if ! [[ "$intermediate_reduction_min_rows" =~ ^[1-9][0-9]*$ ]]; then
  echo "DS4RT_EXPERT_INTERMEDIATE_REDUCTION_MIN_ROWS must be a positive integer" >&2
  exit 2
fi
if ! [[ "$intermediate_owner_max_rows" =~ ^[1-9][0-9]*$ ]]; then
  echo "DS4RT_EXPERT_INTERMEDIATE_OWNER_MAX_ROWS must be a positive integer" >&2
  exit 2
fi
export DS4RT_EXPERT_INTERMEDIATE_SHARDS="$intermediate_shards"
...
export DS4RT_EXPERT_INTERMEDIATE_OWNER_REDUCTION_DTYPE="$intermediate_owner_reduction_dtype"
export DS4RT_EXPERT_INTERMEDIATE_REDUCTION_MIN_ROWS="$intermediate_reduction_min_rows"
export DS4RT_EXPERT_INTERMEDIATE_OWNER_MAX_ROWS="$intermediate_owner_max_rows"
...
if ! [[ "$protocol_v2_verbs_host_execution_lanes" =~ ^[1-8]$ ]]; then
  echo "DS4RT_PROTOCOL_V2_VERBS_HOST_EXECUTION_LANES must be an integer in 1..8" >&2
  exit 2
fi
export DS4RT_PROTOCOL_V2_VERBS_HOST_EXECUTION_LANES="$protocol_v2_verbs_host_execution_lanes"
case "$protocol_v2_verbs_host_shared_cq_harvester" in
  0|1) ;;
  *)
    echo "DS4RT_PROTOCOL_V2_VERBS_HOST_SHARED_CQ_HARVESTER must be 0 or 1" >&2
    exit 2
    ;;
esac
export DS4RT_PROTOCOL_V2_VERBS_HOST_SHARED_CQ_HARVESTER="$protocol_v2_verbs_host_shared_cq_harvester"
if ! [[ "$protocol_v2_verbs_host_stripe_min_rows" =~ ^[1-9][0-9]*$ ]]; then
  echo "DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_MIN_ROWS must be a positive integer" >&2
  exit 2
fi
export DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_MIN_ROWS="$protocol_v2_verbs_host_stripe_min_rows"
case "$protocol_v2_verbs_host_stripe_spark_reduction" in
  0|1) ;;
  *)
    echo "DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_SPARK_REDUCTION must be 0 or 1" >&2
    exit 2
    ;;
esac
export DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_SPARK_REDUCTION="$protocol_v2_verbs_host_stripe_spark_reduction"
case "$build_profile" in
  debug|release) ;;
  *)
    echo "DS4RT_REAL_FULL_SERVE_BUILD_PROFILE must be debug or release" >&2
    exit 2
    ;;
esac
need xargs
...
if ! [[ "$max_context_tokens" =~ ^[0-9]+$ ]] || [ "$max_context_tokens" -lt 1 ]; then
  echo "DS4RT_REAL_FULL_SERVE_MAX_CONTEXT_TOKENS must be a positive integer" >&2
  exit 2
fi
if ! [[ "$warmup_layer_id" =~ ^[0-9]+$ ]]; then
  echo "DS4RT_REAL_FULL_SERVE_WARMUP_LAYER_ID must be a non-negative integer" >&2
  exit 2
fi
if ! [[ "$warmup_iterations" =~ ^[0-9]+$ ]] || [ "$warmup_iterations" -lt 1 ]; then
  echo "DS4RT_REAL_FULL_SERVE_WARMUP_ITERATIONS must be a positive integer" >&2
  exit 2
fi
if ! [[ "$warmup_rows" =~ ^[0-9]+$ ]] || [ "$warmup_rows" -lt 1 ]; then
  echo "DS4RT_REAL_FULL_SERVE_WARMUP_ROWS must be a positive integer" >&2
  exit 2
fi
if ! [[ "$warmup_timeout_ms" =~ ^[0-9]+$ ]] || [ "$warmup_timeout_ms" -lt 1 ]; then
  echo "DS4RT_REAL_FULL_SERVE_WARMUP_TIMEOUT_MS must be a positive integer" >&2
  exit 2
fi
if ! [[ "$expert_ready_timeout_secs" =~ ^[0-9]+$ ]] || [ "$expert_ready_timeout_secs" -lt 1 ]; then
  echo "DS4RT_REAL_FULL_SERVE_EXPERT_READY_TIMEOUT_SECS must be a positive integer" >&2
  exit 2
fi
if ! [[ "$warmup_measured_timeout_ms" =~ ^[0-9]+$ ]] || [ "$warmup_measured_timeout_ms" -lt 1 ]; then
  echo "DS4RT_REAL_FULL_SERVE_WARMUP_MEASURED_TIMEOUT_MS must be a positive integer" >&2
  exit 2
fi
...
if [ "$require_cuda" = "1" ]; then
  if [ "$build_native" = "1" ] && [ "$native_lib_explicit" = "0" ]; then
    ...
    if [ -n "$host_nccl_include_dir" ] && [ -n "$host_nccl_library" ]; then
      nccl_cmake_args+=(
        "-DDS4RT_NCCL_INCLUDE_DIR=$host_nccl_include_dir"
        "-DDS4RT_NCCL_LIBRARY=$host_nccl_library"
      )
    fi
    native_rdma=OFF
    if [ "$coordinator_transport" = "verbs-host" ]; then
    ...
    native_cmake_args=(
      ...
      -DDS4RT_ENABLE_RDMA="$native_rdma"
      -DDS4RT_ENABLE_SPARKINFER_AOT=OFF
      -DDS4RT_ENABLE_DS4_FLASH_AOT="$ds4_flash_spark_aot"
      -DDS4RT_ENABLE_SPARKINFER_COORDINATOR_AOT="${DS4RT_SPARKINFER_COORDINATOR_AOT:-${DS4RT_B12X_COORDINATOR_AOT:-ON}}"
      ...
      -DDS4RT_ENABLE_NCCL=ON
      "${cuda_compiler_args[@]}"
      "${nccl_cmake_args[@]}"
      -DPython3_EXECUTABLE="$DS4RT_PYTHON"
      -DDS4RT_CUDA_ARCHITECTURES="${DS4RT_CUDA_ARCH:-120}"
    )
    cmake "${native_cmake_args[@]}"
    native_cmake_cache="$native_build_dir/CMakeCache.txt"
    if ! grep -Fqx 'DS4RT_ENABLE_CUDA:BOOL=ON' "$native_cmake_cache" \
      || ! grep -Fqx "DS4RT_ENABLE_RDMA:BOOL=$native_rdma" "$native_cmake_cache" \
      || ! grep -Fqx 'DS4RT_ENABLE_NCCL:BOOL=ON' "$native_cmake_cache"; then
...
or set DS4RT_REAL_FULL_SERVE_REQUIRE_CUDA=0 for a diagnostic-only start.
EOF
    exit 2
  }
...
export DS4RT_REAL_FULL_SERVE_PREWARM_REQUEST="$prewarm_request"
export DS4RT_REAL_FULL_DSPARK="$dspark"
export DS4RT_REAL_FULL_DSPARK_SHADOW="$dspark_shadow"
export DS4RT_DS4_FLASH_SPARK_AOT="$ds4_flash_spark_aot"
export DS4RT_REAL_FULL_PROTOCOL_V2_TIMEOUT_MS="$protocol_v2_timeout_ms"
export DS4RT_REAL_FULL_NVFP4_ROUTE_CUDA_GRAPHS="${DS4RT_REAL_FULL_NVFP4_ROUTE_CUDA_GRAPHS:-1}"
export DS4RT_B12X="${DS4RT_B12X:-1}"
export DS4RT_COORDINATOR_W8A16_Q_A="$w8a16_q_a"
export DS4RT_COORDINATOR_W8A16_Q_B="$w8a16_q_b"
export DS4RT_COORDINATOR_W8A16_O_PROJ="$w8a16_o_proj"
export DS4RT_COORDINATOR_W8A16_PACKED_O="$w8a16_packed_o"
export DS4RT_COORDINATOR_W8A16_ASYNC_ATTENTION="$w8a16_async_attention"
export DS4RT_REAL_FULL_REQUEST_THREAD_PINNED="${DS4RT_REAL_FULL_REQUEST_THREAD_PINNED:-$DS4RT_B12X}"
export DS4RT_REAL_FULL_REQUEST_THREAD_PINNED_WORKERS="${DS4RT_REAL_FULL_REQUEST_THREAD_PINNED_WORKERS:-1}"
export DS4RT_ENGINE_COMMIT="${DS4RT_ENGINE_COMMIT:-$(git rev-parse HEAD 2>/dev/null || echo unknown)}"
kernel_cache_base="${DS4RT_KERNEL_CACHE_BASE:-${DS4RT_FLASHINFER_WORKSPACE_BASE:-${FLASHINFER_WORKSPACE_BASE:-$HOME/.cache/ds4rt/kernel-cache-${UID}}}}"
kernel_cache_environment_id="${DS4RT_KERNEL_CACHE_ENVIRONMENT_ID:-}"
...
kernel_cache_identity="$("$DS4RT_PYTHON" "$repo_root/scripts/kernel-cache-identity.py" \
...
export SPARKINFER_COMPILE_CACHE_DIR="${DS4RT_SPARKINFER_COMPILE_CACHE_DIR:-$kernel_cache_identity_root/sparkinfer/compile}"
...
if [ -n "$expert_additional_rails" ]; then
  export DS4RT_PROTOCOL_V2_VERBS_HOST_ADDITIONAL_RAILS="$expert_additional_rails"
  ...
  unset DS4RT_PROTOCOL_V2_VERBS_HOST_ADDITIONAL_RAILS || true
fi
...
if [ "$start_experts" = "1" ]; then
  ...
  DS4RT_SPARK_HOSTS="$hosts_csv" \
    DS4RT_SPARK_KEEP_EXPERTS=1 \
    DS4RT_SPARK_EXPERT_REAL_LAYER="${DS4RT_SPARK_EXPERT_REAL_LAYER:-all}" \
    DS4RT_SPARK_EXPERT_TRANSPORT="$coordinator_transport" \
    DS4RT_PHASE0_SPARK_SKIP_BENCH=1 \
    DS4RT_PHASE0_SPARK_EXPERT_MODE=real \
...
if [ "$build_daemon" = "1" ]; then
  echo "== building ds4rt daemon (${build_profile}) ==" >&2
  need cargo
  cargo_profile_args=()
  if [ "$build_profile" = "release" ]; then
    cargo_profile_args+=(--release)
  fi
  cargo build --manifest-path rust/Cargo.toml -p ds4rt-daemon "${cargo_profile_args[@]}"
fi
report_shell_startup_phase daemon-build

if [ ! -x "$bin" ]; then
  bin="$repo_root/scripts/ds4rt"
fi
...
if [ -n "$shared_cpu_list" ] \
  || [ -n "${DS4RT_REAL_FULL_REQUEST_WORKER_CPUS:-}" ] \
  || [ -n "${DS4RT_REAL_FULL_SCHEDULER_WORKER_CPU:-}" ]; then
  echo "shared_cpu_list=${shared_cpu_list:-none} request_worker_cpus=${DS4RT_REAL_FULL_REQUEST_WORKER_CPUS:-none} scheduler_worker_cpu=${DS4RT_REAL_FULL_SCHEDULER_WORKER_CPU:-none}" >&2
fi
