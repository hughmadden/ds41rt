        ...
        if path.is_symlink():
            ...
            if not (path.parent / target).resolve(strict=False).is_relative_to(source):
                raise VerificationError(
                    f"GPTQModel source symlink escapes the source tree: {relative} -> {target}"
                )
...
def _verify_source_identity(source: Path) -> None:
    required = (
        ...
        source / "gptqmodel" / "models" / "definitions" / "deepseek_v4.py",
        source / "gptqmodel" / "looper" / "exllamav3_processor.py",
        source / "gptqmodel" / "exllamav3" / "modules" / "quant" / "exl3_lib" / "quantize.py",
    )
    missing = [os.fspath(path) for path in required if not path.is_file()]
    if missing:
        raise VerificationError("GPTQModel source is incomplete; missing " + ", ".join(missing))
...
def _load_lock(path: Path) -> dict[str, object]:
    ...
    if not isinstance(value, dict):
        raise VerificationError(f"GPTQModel lock must contain one JSON object: {path}")
    unexpected = set(value) - LOCK_FIELDS
    missing = LOCK_FIELDS - set(value)
    if unexpected or missing:
        raise VerificationError(
            f"GPTQModel lock fields mismatch: missing={sorted(missing)} "
    ...
    if not isinstance(value["source_tree_sha256"], str) or not DIGEST_RE.fullmatch(
        ...
        raise VerificationError("GPTQModel source_tree_sha256 must be lowercase 64-hex")
    return value
