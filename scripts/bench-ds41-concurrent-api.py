...
parser.add_argument('--case', choices=['counting','code','topic'], default='counting')
...
args.output.write_text(json.dumps(dict(scope=__doc__,base_url=args.base_url,label=args.label,case=args.case,corpus_sha256=hashlib.sha256(corpus_path.read_bytes()).hexdigest(),prompt=prompt,max_tokens=definition['max_tokens'],warmup=warmup,warmup_checks=warmup_checks,concurrency=args.concurrency,repeats=args.repeats,records=records,summaries=summaries,passed=True),indent=2)+'\n')
