#!/usr/bin/env python3
"""Run the preserved eight-type and Orchid release decode workloads."""
import argparse
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--include-orchid', action='store_true')
    ...
    selected = args.case or corpus['weighted_case_ids']
    unknown = set(selected) - set(corpus['weighted_case_ids'])
    if unknown:
        parser.error(f"unknown or non-weighted cases: {sorted(unknown)}")
    tokenizer = Tokenizer.from_file(str(args.tokenizer))
    nonces = token_zero_nonces(args.repeats * (len(selected) + int(args.include_orchid)),
    ...
    report = dict(
        ...
        include_orchid=args.include_orchid, controls=dict(temperature=0, thinking='disabled'),
    ...
    for repeat in range(args.repeats):
        repeat_samples = []
        for case_id in selected + (['orchid'] if args.include_orchid else []):
            ...
            sample = dict(repeat=repeat + 1, case=case_id, category=definition.get('category', 'low-entropy'),
                          weight=weight, nonce=nonce, request=body, started_ns=time.time_ns())
            ...
            try:
                ...
                sample['bounded_static_prefix_hit'] = 0 <= sample['cached_tokens'] <= 32
                sample['passed'] = sample['quality_contract_passed'] and sample['bounded_static_prefix_hit']
