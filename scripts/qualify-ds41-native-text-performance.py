...
report=dict(scope=__doc__,urls=urls,workloads=[],summaries=[],passed=False)
...
for name,prompt in [
    ...
    for index in range(a.pairs):
        ...
        save()
        assert run['text_equal'] and run['token_counts_equal'],run
...
# Passed denotes complete measurements and output parity, not a performance threshold.
