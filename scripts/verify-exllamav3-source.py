...
class VerificationError(RuntimeError):
    """The quantizer checkout does not match the committed source lock."""
        ...
        if path.is_symlink():
            ...
            if not (path.parent / target).resolve(strict=False).is_relative_to(source):
                raise VerificationError(
                    f"exllamav3 source symlink escapes the source tree: {relative} -> {target}"
                )
...
def _load_lock(path: Path) -> dict[str, object]:
    try:
        ...
        raise VerificationError(
            f"exllamav3 lock not found: {path}; initialize the pinned submodule"
        ) from exc
    except (OSError, json.JSONDecodeError) as exc:
        raise VerificationError(f"cannot read exllamav3 lock {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise VerificationError(f"exllamav3 lock must contain one JSON object: {path}")
    unexpected = set(value) - LOCK_FIELDS
    missing = LOCK_FIELDS - set(value)
    if unexpected or missing:
        raise VerificationError(
            f"exllamav3 lock fields mismatch: missing={sorted(missing)} "
            f"unexpected={sorted(unexpected)}"
        )
    if value["schema"] != 1:
        raise VerificationError(f"unsupported exllamav3 lock schema: {value['schema']!r}")
    if not isinstance(value["repository"], str) or not value["repository"].startswith("https://"):
        raise VerificationError("exllamav3 repository must be an HTTPS URL")
    if not isinstance(value["revision"], str) or not REVISION_RE.fullmatch(value["revision"]):
        raise VerificationError("exllamav3 revision must be a lowercase 40-hex commit")
    if not isinstance(value["source_tree_sha256"], str) or not DIGEST_RE.fullmatch(
        value["source_tree_sha256"]
    ):
        raise VerificationError("exllamav3 source_tree_sha256 must be lowercase 64-hex")
    return value
...
def verify(source: Path, lock_path: Path) -> dict[str, object]:
    ...
    if actual_digest != lock["source_tree_sha256"]:
        raise VerificationError(
            "exllamav3 source content does not match the lock: "
    ...
    if (source / ".git").exists():
        ...
        if actual_revision != lock["revision"]:
            raise VerificationError(
                "exllamav3 checkout revision does not match the lock: "
                f"expected {lock['revision']}, found {actual_revision}"
            )
        status = _run_git(source, "status", "--porcelain", "--untracked-files=all")
        if status:
            raise VerificationError("exllamav3 checkout has source changes:\n" + status)
        ...
        if _normalized_repository(actual_repository) != _normalized_repository(
            ...
            raise VerificationError(
                "exllamav3 origin does not match the lock: "
...
if __name__ == "__main__":
    try:
        ...
        print(f"verify-exllamav3-source: {exc}", file=sys.stderr)
