...
for i,lib in enumerate(libs):
    ...
    fn = lib.ds41rt_v41_sparse_attention if args.parts==0 else lib.ds41rt_v41_sparse_attention_split
    fn.argtypes = [C.c_void_p]*5+[C.c_int32,C.c_int32,C.POINTER(View),C.c_void_p]+([] if args.parts==0 else [C.c_void_p,C.c_uint64,C.c_int32])
...
for rows in args.rows:
    ...
    def launch(i):
        fn=libs[i].ds41rt_v41_sparse_attention if args.parts==0 else libs[i].ds41rt_v41_sparse_attention_split
        status = fn(
            q.data_ptr(),
            sink.data_ptr(),
            meta.data_ptr(),
            ...
            *([] if args.parts==0 else [scratch.data_ptr(),scratch.numel()*4,args.parts]),
        )
    ...
    for pattern in [
        ...
        results.append(
            dict(
                rows=rows,parts=args.parts,scope="Byte-exact regression against baseline; graph microbenchmark, not independent reference math",
                pattern=pattern,
                bit_exact=bit_exact,
                **metrics,
