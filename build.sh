...
Dirty checkouts get an automatic source manifest under .ds4rt-release/.
Keep source files unchanged during the build; local and remote inventories
are verified against that manifest. DS4RT_RELEASE_SOURCE_MANIFEST can supply
an existing manifest. Source archives without .git must also provide
DS4RT_RELEASE_ENGINE_REVISION (REVISION or REVISION-dirty-MANIFEST12).
EOF
}
...
if [[ -z "$source_manifest" ]] && ((engine_source_dirty)); then
  ...
  mkdir -p "$repo_root/.ds4rt-release/source-manifests"
  source_manifest="$(mktemp "$repo_root/.ds4rt-release/source-manifests/source.XXXXXXXX.sha256")"
