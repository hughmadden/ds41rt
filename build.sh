...
Exact dirty-tree releases must set DS4RT_RELEASE_SOURCE_MANIFEST to a
SOURCE_SHA256SUMS generated from a frozen source tree. Builds from a source
snapshot without .git must also set DS4RT_RELEASE_ENGINE_REVISION to the exact
...
source_manifest="${DS4RT_RELEASE_SOURCE_MANIFEST:-}"
...
engine_revision_override="${DS4RT_RELEASE_ENGINE_REVISION:-}"
...
if [[ -n "$engine_revision_override" ]]; then
  [[ "$engine_revision_override" =~ ^[0-9a-f]{40}(-dirty-[0-9a-f]{12})?$ ]] ||
    release_die "DS4RT_RELEASE_ENGINE_REVISION must be REVISION or REVISION-dirty-MANIFEST12"
  [[ -n "$source_manifest_sha256" ]] ||
    release_die "DS4RT_RELEASE_ENGINE_REVISION requires DS4RT_RELEASE_SOURCE_MANIFEST"
  engine_commit="$engine_revision_override"
elif [[ -z "$detected_engine_commit" ]]; then
  release_die "source snapshot has no Git metadata; set DS4RT_RELEASE_ENGINE_REVISION and DS4RT_RELEASE_SOURCE_MANIFEST"
elif ((engine_source_dirty)); then
  [[ -n "$source_manifest_sha256" ]] ||
    release_die "dirty release source requires DS4RT_RELEASE_SOURCE_MANIFEST"
...
if [[ "$engine_commit" == *-dirty-* ]]; then
  [[ -n "$source_manifest_sha256" ]] ||
    release_die "dirty engine revision requires DS4RT_RELEASE_SOURCE_MANIFEST"
...
if [[ -n "$source_manifest_sha256" ]]; then
  release_source_label_args+=(
    --label "io.ds4rt.source-manifest.sha256=$source_manifest_sha256"
  )
fi

hosts_csv="$(release_hosts_csv)"
seed_host="$SPARK_0_HOST"
remote_dir="${DS4RT_RELEASE_REMOTE_BUILD_DIR:-}"
artifact_dir="$repo_root/.ds4rt-release-image"
...
if [[ -z "$remote_dir" ]]; then
  remote_dir="$(
    ssh -o BatchMode=yes "$seed_host" \
      'printf "%s/ds4rt-release-build" "$HOME"'
  )"
fi
...
docker build \
  ...
  --build-arg DS4RT_SPARKINFER_COMMIT="$sparkinfer_commit" \
...
docker build \
  ...
  --build-arg DS4RT_ROLE=coordinator \
  --build-arg CUDA_ARCH=120 \
  --build-arg DS4RT_ENGINE_COMMIT="$engine_commit" \
  --build-arg DS4RT_SPARKINFER_COMMIT="$sparkinfer_commit" \
  --build-arg DS4RT_RELEASE_VERSION="$release_version" \
...
docker build \
  ...
  -t "$dev_image" .
mkdir -p .ds4rt-release-image
docker run --rm \
  --gpus all \
  --ipc=host \
  --ulimit memlock=-1:-1 \
  -v "$remote_dir:/source:ro" \
  -v "$remote_dir/.ds4rt-release-image:/output" \
  "$dev_image" \
...
docker cp "$coordinator_container:/opt/ds4rt/bin/ds4rt" "$repo_root/dist/coordinator/ds4rt"
docker cp "$coordinator_container:/opt/ds4rt/lib/libds4rt_native.so" "$repo_root/dist/coordinator/libds4rt_native.so"
docker cp \
  "$coordinator_container:/opt/ds4rt/share/THIRD_PARTY_NOTICES.md" \
  "$repo_root/dist/coordinator/THIRD_PARTY_NOTICES.md"
docker cp \
  "$coordinator_container:/opt/ds4rt/share/SPARKINFER_PROVENANCE.json" \
  "$repo_root/dist/coordinator/SPARKINFER_PROVENANCE.json"
docker cp \
  "$coordinator_container:/opt/ds4rt/share/licenses/sparkinfer/LICENSE" \
  "$repo_root/dist/coordinator/SPARKINFER_LICENSE"
docker cp \
  "$coordinator_container:/opt/ds4rt/share/SPARKINFER_SHA256SUMS" \
  "$repo_root/dist/coordinator/SPARKINFER_SHA256SUMS"
docker cp \
  "$coordinator_container:/opt/ds4rt/share/XGRAMMAR_PROVENANCE.json" \
  "$repo_root/dist/coordinator/XGRAMMAR_PROVENANCE.json"
docker cp \
  "$coordinator_container:/opt/ds4rt/share/licenses/xgrammar/LICENSE" \
  "$repo_root/dist/coordinator/XGRAMMAR_LICENSE"
docker cp \
  "$coordinator_container:/opt/ds4rt/share/XGRAMMAR_SHA256SUMS" \
...
docker cp "$container:/opt/ds4rt/bin/ds4rt" "$destination/ds4rt"
docker cp "$container:/opt/ds4rt/lib/libds4rt_native.so" "$destination/libds4rt_native.so"
docker cp \
  "$container:/opt/ds4rt/share/THIRD_PARTY_NOTICES.md" \
  "$destination/THIRD_PARTY_NOTICES.md"
docker cp \
  "$container:/opt/ds4rt/share/SPARKINFER_PROVENANCE.json" \
  "$destination/SPARKINFER_PROVENANCE.json"
docker cp \
  "$container:/opt/ds4rt/share/licenses/sparkinfer/LICENSE" \
  "$destination/SPARKINFER_LICENSE"
docker cp \
  "$container:/opt/ds4rt/share/SPARKINFER_SHA256SUMS" \
  "$destination/SPARKINFER_SHA256SUMS"
docker cp \
  "$container:/opt/ds4rt/share/XGRAMMAR_PROVENANCE.json" \
  "$destination/XGRAMMAR_PROVENANCE.json"
docker cp \
  "$container:/opt/ds4rt/share/licenses/xgrammar/LICENSE" \
  "$destination/XGRAMMAR_LICENSE"
docker cp \
  "$container:/opt/ds4rt/share/XGRAMMAR_SHA256SUMS" \
...
(
  cd "$repo_root/dist"
  sha256sum \
    coordinator/ds4rt coordinator/libds4rt_native.so \
    ...
    spark-expert/ds4rt spark-expert/libds4rt_native.so \
...
if ((rdmapipe_ready)); then
  ...
  DS4RT_SPARK_IMAGE="$SPARK_EXPERT_DOCKER_INFERENCE" \
  DS4RT_SPARK_IMAGE_SEED_HOST="$seed_host" \
  DS4RT_SPARK_IMAGE_COPY_METHOD=spark-netcat \
  DS4RT_SPARK_IMAGE_ONLY=1 \
  DS4RT_SPARK_SKIP_STAGE=1 \
...
if [[ -n "$source_manifest_sha256" ]]; then
  coordinator_source_manifest="$(
    docker image inspect -f '{{index .Config.Labels "io.ds4rt.source-manifest.sha256"}}' \
      "$COORDINATOR_DOCKER_INFERENCE"
  )"
...
for host in "$SPARK_0_HOST" "$SPARK_1_HOST" "$SPARK_2_HOST" "$SPARK_3_HOST"; do
  ...
  if [[ -n "$source_manifest_sha256" ]]; then
    spark_source_manifest="$(
      ssh -o BatchMode=yes "$host" \
        "docker image inspect -f '{{index .Config.Labels \"io.ds4rt.source-manifest.sha256\"}}' '$SPARK_EXPERT_DOCKER_INFERENCE'"
    )"
