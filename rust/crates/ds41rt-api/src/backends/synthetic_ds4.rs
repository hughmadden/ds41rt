use ds41rt_core::{
    ...
    KvCacheAllocator, KvCacheConfig, LayerWave, ModelFacts, PrefillChunkPolicy, Priority,
    RouteEntry, DS4_FLASH_HIDDEN_SIZE, DS4_FLASH_TOP_K,
};
...
pub(crate) async fn synthetic_ds4_layer_completion(
    state: &ApiState,
    prompt: &str,
    prompt_tokens: usize,
    ...
    let mut kv_allocator = KvCacheAllocator::new(KvCacheConfig::deepseek_v4_hybrid_bf16(
        prefill_rows + 1 + DS4_FLASH_TOP_K,
        &ModelFacts::default(),
    ));
    let kv_reservation_id = kv_allocator
        .reserve("synthetic-ds4-layer", prefill_rows + 1)
    ...
    for chunk in prefill_chunks.iter().cloned() {
        ...
        for (target, routes) in &route_groups {
            ...
            let request = ExpertRequest {
                ...
                placement_version: "phase0-synthetic-ds4-layer".to_owned(),
                layer_id: wave.layer_id.0,
                hidden_dim: DS4_FLASH_HIDDEN_SIZE as u32,
    ...
    let summed = sum_partials(&partials, DS4_FLASH_HIDDEN_SIZE)?;
    ...
    Ok(BackendCompletion {
        content: format!(
            "synthetic ds4 layer ok hidden={} top_k={} expert_kernel={} prefill_chunks={} prefill_rows={} prefill_partials={} prefill_checksum={:.6} decode_rows={} decode_partials={} checksum={:.6} first={:.6} last={:.6}",
            DS4_FLASH_HIDDEN_SIZE,
            DS4_FLASH_TOP_K,
            ds41rt_transport::SYNTHETIC_EXPERT_KERNEL,
            prefill_chunks.len(),
            prefill_rows,
