use ds41rt_core::{LayerWave, RowSourceKind};
...
impl RealFullRequestNumericProgression {
    ...
    pub(super) fn finish(self) -> RealFullRequestNumericProgressionSummary {
        ...
        let expected_visible_checksum = expected_source_checksum(
            self.prefill_tokens,
        ...
        let expected_selected_rows = source_rows * self.num_hidden_layers;
        ...
        let passed = selected_rows == expected_selected_rows
            && self.selected_prefill_rows == self.prefill_tokens * self.num_hidden_layers
            && self.selected_decode_rows == self.decode_rows * self.num_hidden_layers
            && self.selected_mtp_rows == self.mtp_rows * self.num_hidden_layers
...
fn expected_source_checksum(rows: usize, kind: RowSourceKind, num_hidden_layers: usize) -> f32 {
    let (attention_delta, mlp_delta) = source_deltas(kind);
    rows as f32
        * REQUEST_PROGRESS_HIDDEN_DIM as f32
        * num_hidden_layers as f32
