...
use crate::encoding::{
    render_ds4_tool_calls, DS4_ASSISTANT, DS4_BOS, DS4_EOS, DS4_LATEST_REMINDER, DS4_THINK_CLOSE,
    DS4_THINK_OPEN, DS4_USER,
};
use crate::metrics::{BackendMetrics, CompletionMetrics, RealFullDiagnosticMetrics};
use crate::request::{
    real_ds4_full_request_prompt_text, request_image_sources, request_max_tokens,
...
use crate::tooling::{Ds4ToolCallStreamParser, Ds4ToolStreamDelta, DS4_TOOL_CALLS_END};
use crate::{
...
struct RealFullMtpAcceptanceMetrics {
    ...
    dspark_joint_cycles: usize,
    dspark_max_joint_batch_width: usize,
    dspark_max_joint_cohort_width: usize,
    dspark_max_joint_cohort_count: usize,
    dspark_2x2_wavefront_cycles: usize,
}

impl RealFullMtpAcceptanceMetrics {
    fn record(&mut self, full: &crate::RealFullInfo, emitted_tokens: usize, cycle_ms: f64) {
        if full.request_dspark_joint_batch_width > 0 {
            self.dspark_joint_cycles += 1;
            self.dspark_max_joint_batch_width = self
                .dspark_max_joint_batch_width
                .max(full.request_dspark_joint_batch_width);
            self.dspark_max_joint_cohort_width = self
                .dspark_max_joint_cohort_width
                .max(full.request_dspark_joint_cohort_width);
            self.dspark_max_joint_cohort_count = self
                .dspark_max_joint_cohort_count
                .max(full.request_dspark_joint_cohort_count);
            self.dspark_2x2_wavefront_cycles += usize::from(full.request_dspark_2x2_wavefront);
        }
    ...
    fn apply(&self, metrics: &mut RealFullDiagnosticMetrics) {
        ...
        metrics.dspark_joint_cycles = self.dspark_joint_cycles;
        metrics.dspark_max_joint_batch_width = self.dspark_max_joint_batch_width;
        metrics.dspark_max_joint_cohort_width = self.dspark_max_joint_cohort_width;
        metrics.dspark_max_joint_cohort_count = self.dspark_max_joint_cohort_count;
        metrics.dspark_2x2_wavefront_cycles = self.dspark_2x2_wavefront_cycles;
    }
}

const DS4_THINK_OPEN_TOKEN_ID: usize = 128_821;
const DS4_THINK_CLOSE_TOKEN_ID: usize = 128_822;
const REAL_FULL_REQUEST_TIMING_ENV: &str = "DS41RT_REAL_FULL_REQUEST_TIMING";
const REAL_FULL_MAX_CONTEXT_TOKENS_ENV: &str = "DS41RT_REAL_FULL_SERVE_MAX_CONTEXT_TOKENS";
const REAL_FULL_MAX_OUTPUT_TOKENS_ENV: &str = "DS41RT_REAL_FULL_SERVE_MAX_OUTPUT_TOKENS";
const DS4_CHAT_STOP_TEXTS: &[&str] = &[
    DS4_EOS,
    DS4_BOS,
    DS4_USER,
    DS4_ASSISTANT,
...
fn persistent_real_full_sequence_request(
    ...
    RealFullSequenceRequest {
        ...
        min_output_tokens,
        ignore_eos,
        stop_token_ids: DS4_CHAT_STOP_TOKEN_IDS.to_vec(),
        stop_texts: DS4_CHAT_STOP_TEXTS
            .iter()
            .copied()
            .map(str::to_owned)
            .collect(),
...
pub(crate) fn resolve_real_full_prompt_token_ids(
    state: &ApiState,
...
pub(crate) fn try_real_ds4_full_streaming_response(
    state: Arc<ApiState>,
    request: &ChatCompletionRequest,
    ...
    state
        ...
        .real_full
        .as_ref()
        .ok_or_else(|| runtime_error("real-ds4-full backend has no preflight status"))?;
    ...
    Ok(Some(real_ds4_full_decode_stream_response(
        state,
        executor,
        prompt,
        prompt_tokens,
...
fn real_ds4_full_decode_stream_response(
    state: Arc<ApiState>,
    executor: Arc<dyn RealFullRequestExecutor>,
    prompt: String,
    prompt_tokens: usize,
    ...
    let stream = stream! {
        ...
        let mut output_filter = Ds4AssistantOutputFilter::new(
            allow_tool_calls,
            prompt.ends_with(DS4_THINK_OPEN),
        );
        let mut token_decoder = real_full_streaming_token_decoder(&state);
        let mut tool_stream = tools.map(|_| Ds4ToolCallStreamParser::new());
        ...
        while generated_token_ids.len() < max_tokens {
            ...
            let Some(samples) = terminal_cycle_samples(
                ...
                let step_trace = real_full_request_trace(
                    prompt_tokens + generated_token_ids.len(),
                    1,
                    &full.model_facts,
                )
                .unwrap_or_else(|_| {
                    real_full_request_trace(prompt_tokens, 1, &full.model_facts)
                        .expect("single-token real-full diagnostic trace should be valid")
                });
                let mut completion = diagnostic_real_full_completion(
                    &full,
                    &step_trace,
                ...
                if !sample.content.is_empty() {
                    if let Some(parser) = tool_stream.as_mut() {
                        for delta in parser.push(&sample.content) {
                            match delta {
                                Ds4ToolStreamDelta::Content(content) => {
                                    if !content.is_empty() {
        ...
        let completed_tool_calls = tool_stream
            .as_ref()
            .map_or(0, Ds4ToolCallStreamParser::completed_tool_calls);
        continuation_cacheable &= raw_generated_text.ends_with(DS4_TOOL_CALLS_END);
        continuation_cacheable &= prompt.ends_with(DS4_THINK_OPEN)
        ...
        if completed_tool_calls > 0 && continuation_cacheable {
                ...
                (prompt_token_ids.as_ref(), tool_stream.as_ref())
            {
                ...
                if prompt.ends_with(DS4_THINK_OPEN) {
                    prefix_text.push_str(&continuation_reasoning_content);
                    prefix_text.push_str(DS4_THINK_CLOSE);
                }
                prefix_text.push_str(&continuation_visible_content);
                prefix_text.push_str("\n\n");
                prefix_text.push_str(&render_ds4_tool_calls(parser.completed_tool_call_values()));
            ...
            if let Some(completion) = diagnostic_completion {
                ...
                let mut real_full_metrics =
                    if RequestDiagnosticEvidence::runtime_reported(final_full) {
                        real_full_runtime_diagnostic_metrics(
                            final_full,
                            &state.service_instance_id,
                        )
                    } else {
                        let final_trace = real_full_request_trace(
                            ...
                            1,
                            &final_full.model_facts,
                        )
                        .unwrap_or_else(|_| {
                            real_full_request_trace(prompt_tokens, 1, &final_full.model_facts)
                                .expect("single-token real-full diagnostic trace should be valid")
                        });
                        real_full_diagnostic_metrics(
                            final_full,
                            &final_trace,
                            &state.service_instance_id,
                        )
        ...
        let metrics = CompletionMetrics::from_backend(
            prompt_tokens,
            completion_tokens,
            backend_name(ApiBackend::RealDs4Full),
            transport_backend,
            backend_metrics,
        );
...
pub(crate) async fn real_ds4_full_completion(
    state: &ApiState,
    prompt: &str,
    prompt_tokens: usize,
    prompt_token_ids: Option<Arc<Vec<usize>>>,
    ...
    let trace = real_full_request_trace(prompt_tokens, max_tokens, &full.model_facts)?;
    ...
    let generated_sample = if blocked_multi_token.is_none() {
        terminal_sample.map(|sample| {
            Ds4AssistantOutputFilter::new(allow_tool_calls, prompt.ends_with(DS4_THINK_OPEN))
                .apply(sample)
        })
    } else {
        None
    ...
    let generated_runtime_metrics = || {
        (has_live_request_execution && request.runtime_reported).then(|| {
            real_full_diagnostic_metrics(diagnostic_full, &trace, &state.service_instance_id)
        })
    };
    let (content, completion_tokens, stream_chunks, real_full_metrics) = match generated_sample {
        ...
        None => (
            ...
            None,
            None,
            Some(real_full_diagnostic_metrics(
                diagnostic_full,
                &trace,
                &state.service_instance_id,
            )),
...
async fn execute_real_full_decode_loop(
    ...
    let mut mtp_acceptance_metrics = RealFullMtpAcceptanceMetrics::default();
    let mut stopped_by_model = false;
    let sequence_id = format!("real-ds4-full-api-decode-loop-{}", Uuid::new_v4());
    let reservation = u64::try_from(max_tokens.saturating_add(1)).unwrap_or(u64::MAX);
    let initial_request_index = state
        .next_request_id
        .fetch_add(reservation, Ordering::Relaxed);
    ...
    while generated_token_ids.len() < max_tokens {
        ...
        let Some(samples) = terminal_cycle_samples(
            ...
            let mut completion =
                diagnostic_real_full_completion(&full, &step_trace, &state.service_instance_id);
            completion.metrics.cache_load_ms = cache_load_ms;
    ...
    let mut real_full_metrics = RequestDiagnosticEvidence::runtime_reported(&final_full)
        .then(|| real_full_runtime_diagnostic_metrics(&final_full, &state.service_instance_id));
...
fn diagnostic_real_full_completion(
    ...
    BackendCompletion {
        ...
        metrics: BackendMetrics {
            ...
            layerwave_prefill_rows: request.prefill_tokens,
            layerwave_decode_rows: request.decode_budget,
            real_full: Some(real_full_diagnostic_metrics(
                full,
                trace,
                service_instance_id,
            )),
...
#[derive(Debug, Default)]
struct Ds4AssistantOutputFilter {
    inside_thinking: bool,
    allow_tool_calls: bool,
}

impl Ds4AssistantOutputFilter {
    ...
    fn apply(&mut self, sample: TerminalSample) -> FilteredTerminalSample {
        if sample.token_id == DS4_THINK_OPEN_TOKEN_ID {
        ...
        if sample.token_id == DS4_THINK_CLOSE_TOKEN_ID {
    ...
    fn filter_text_markers(&mut self, text: &str) -> (String, String, bool, bool) {
        ...
        while offset < text.len() {
            ...
            let Some(marker) = next_ds4_output_marker(rest, self.allow_tool_calls) else {
            ...
            match marker.kind {
                Ds4OutputMarkerKind::Stop => {
                    stop = true;
                    break;
                }
                Ds4OutputMarkerKind::ThinkOpen => {
                    self.inside_thinking = true;
                }
                Ds4OutputMarkerKind::ThinkClose => {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct Ds4OutputMarker {
    index: usize,
    text: &'static str,
    kind: Ds4OutputMarkerKind,
}

fn next_ds4_output_marker(text: &str, _allow_tool_calls: bool) -> Option<Ds4OutputMarker> {
    DS4_CHAT_STOP_TEXTS
        .iter()
        .map(|marker| (*marker, Ds4OutputMarkerKind::Stop))
        .chain([
            (DS4_THINK_OPEN, Ds4OutputMarkerKind::ThinkOpen),
            (DS4_THINK_CLOSE, Ds4OutputMarkerKind::ThinkClose),
        ])
        .filter_map(|(marker, kind)| {
            text.find(marker).map(|index| Ds4OutputMarker {
                index,
                text: marker,
                kind,
            })
...
fn terminal_sample(full: &crate::RealFullInfo, allow_tool_calls: bool) -> Option<TerminalSample> {
    ...
    let stop_by_token = ds4_chat_stop_token_id(token_id, allow_tool_calls);
    let content = if let Some(decoded) = terminal_output_token_text(full, token_id) {
        decoded
    } else if stop_by_token {
        String::new()
    } else {
        format!("ds41rt-token:{token_id}")
    };
    let (content, stop_by_text) = trim_ds4_chat_stop_text(&content, allow_tool_calls);
    Some(TerminalSample {
        token_id,
        content,
...
fn terminal_cycle_samples(
    ...
    allow_tool_calls: bool,
    mut token_decoder: Option<&mut ds41rt_loader::StreamingTokenDecoder>,
    ...
    Some(
        generated_tokens
            .iter()
            .map(|generated| {
                let stop_by_token = ds4_chat_stop_token_id(generated.token_id, allow_tool_calls);
                let fallback_content = generated
                    ...
                    .unwrap_or_else(|| {
                        if stop_by_token {
                            ...
                            format!("ds41rt-token:{}", generated.token_id)
                        }
                    });
                ...
                let (content, stop_by_text) =
                    trim_ds4_chat_stop_text(&sample.content, allow_tool_calls);
...
fn real_full_streaming_token_decoder(
    state: &ApiState,
) -> Option<ds41rt_loader::StreamingTokenDecoder> {
    let snapshot_path = state.config.real_full.as_ref()?.snapshot_path.as_deref()?;
    match ds41rt_loader::streaming_token_decoder(Path::new(snapshot_path), false) {
...
fn decode_terminal_sample_streaming(
    ...
    token_decoder: Option<&mut ds41rt_loader::StreamingTokenDecoder>,
) {
...
fn terminal_output_token_text(full: &crate::RealFullInfo, token_id: usize) -> Option<String> {
    ...
    ds41rt_loader::decode_tokenizer_ids(Path::new(snapshot_path), &[token_id], false)
        .ok()
        .map(|summary| summary.text)
        .filter(|text| !text.is_empty())
}

fn ds4_chat_stop_token_id(token_id: usize, _allow_tool_calls: bool) -> bool {
    DS4_CHAT_STOP_TOKEN_IDS.contains(&token_id)
}

fn trim_ds4_chat_stop_text(content: &str, _allow_tool_calls: bool) -> (String, bool) {
    let Some((stop_index, _)) = DS4_CHAT_STOP_TEXTS
        .iter()
...
fn real_full_diagnostic_metrics(
    ...
    let request = RequestDiagnosticEvidence::new(full, trace);
    real_full_diagnostic_metrics_from_request(full, request, service_instance_id)
}

fn real_full_runtime_diagnostic_metrics(
    full: &crate::RealFullInfo,
    service_instance_id: &str,
) -> RealFullDiagnosticMetrics {
...
fn real_full_diagnostic_metrics_from_request(
    ...
    RealFullDiagnosticMetrics {
        ...
        model_id: full.model_id.clone(),
        service_instance_id: service_instance_id.to_owned(),
        snapshot_path: full.snapshot_path.clone(),
        catalog_hash: full.catalog_hash.clone(),
        quantization_recipe: full.quantization_recipe.clone(),
        layer_count: full.layer_count,
        dense_layer_count: full.dense_layer_count,
        sparse_layer_count: full.sparse_layer_count,
        ...
        scheduler_sparse_tcp_dispatch_all_responses_real_checkpoint_experts: full
        ...
        dspark_max_joint_batch_width: 0,
        dspark_max_joint_cohort_width: 0,
        dspark_max_joint_cohort_count: 0,
