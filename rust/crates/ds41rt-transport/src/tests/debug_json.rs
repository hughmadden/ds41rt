use anyhow::Result;
use ds41rt_core::{LayerWaveMode, DS4_FLASH_HIDDEN_BF16_BYTES, DS4_FLASH_HIDDEN_SIZE};
...
#[tokio::test]
async fn decode_shaped_flash_hidden_row_roundtrips() -> Result<()> {
    let (addr, shutdown) = spawn_server().await?;
    let request = request_with_rows(50, 1, DS4_FLASH_HIDDEN_SIZE, LayerWaveMode::Decode);
    assert_eq!(request.header().row_count, 1);
    assert_eq!(request.header().wave_mode, Some(LayerWaveMode::Decode));
    assert_eq!(DS4_FLASH_HIDDEN_BF16_BYTES, 8_192);

    let response = tcp_roundtrip(addr, &request, TcpTransportConfig::default()).await?;

    assert_eq!(response.partial_outputs.len(), 1);
    assert_eq!(response.partial_outputs[0].len(), DS4_FLASH_HIDDEN_SIZE);
    let _ = shutdown.send(());
    Ok(())
}
...
async fn mtp_shaped_layerwave_rows_roundtrip() -> Result<()> {
    ...
    assert_eq!(
        request.rows.len() * DS4_FLASH_HIDDEN_BF16_BYTES,
        draft_rows * 8_192
    );
...
async fn prefill_shaped_layerwave_rows_roundtrip() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(prefill_rows * DS4_FLASH_HIDDEN_BF16_BYTES)
    );
    ...
    assert_eq!(tcp.partial_outputs[0].len(), DS4_FLASH_HIDDEN_SIZE);
    assert_eq!(serde_json::to_vec(&tcp)?, serde_json::to_vec(&inproc)?);
    let _ = shutdown.send(());
    Ok(())
}
...
fn malformed_wave_metadata_is_rejected() {
    let mut request = request_with_rows(53, 4, DS4_FLASH_HIDDEN_SIZE, LayerWaveMode::Prefill);
    request
        .wave
        .as_mut()
        .expect("test request has wave metadata")
        .logical_bf16_payload_bytes = DS4_FLASH_HIDDEN_BF16_BYTES;
