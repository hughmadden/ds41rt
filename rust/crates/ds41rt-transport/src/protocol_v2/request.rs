...
impl ExpertProtocolV2Request {
    ...
    pub(crate) fn new_bytes(
        ...
        Self::new_with_hidden_stride_bytes(
            request_id,
            placement_version,
            layer_id,
            hidden_dim,
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn new_with_hidden_stride_bytes(
        request_id: u64,
        placement_version: u64,
        layer_id: u32,
        hidden_dim: u32,
    ...
    pub fn new_stream_plan_with_hidden_stride(
        ...
        let request = Self {
            ...
            rows,
            routes,
            hidden_payload: plan_payload.into(),
        };
        request.validate()?;
        Ok(request)
    }
    ...
    pub fn new_stream_data(
        ...
        let request = Self {
            ...
            rows: Vec::new(),
            routes: Vec::new(),
            hidden_payload: hidden_payload.into(),
        };
        request.validate()?;
        Ok(request)
    }
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        let routes = (0..routes_len)
            ...
            .collect::<Result<Vec<_>>>()?;
        let hidden_payload = Bytes::copy_from_slice(&bytes[payload_start..frame_end]);
