...
use ds41rt_core::{
    ...
    ExpertBatchRoute, ExpertHostBatch, GraphBucket, LayerId, LayerWave, ModelFacts, MtpVerifyBlock,
    PlacementPolicy, PositionId, PrefillChunk, Priority, DS4_FLASH_HIDDEN_BF16_BYTES,
    DS4_FLASH_HIDDEN_SIZE, EXPERT_HOSTS,
};

fn request(row_count: usize, source_kind: ExpertV2SourceKind) -> Result<ExpertProtocolV2Request> {
    let hidden_dim = DS4_FLASH_HIDDEN_SIZE as u32;
    ...
    let routes = (0..row_count)
        ...
        .collect::<Vec<_>>();
    let hidden_payload = (0..row_count * DS4_FLASH_HIDDEN_BF16_BYTES)
...
fn encode_decode_one_row_flash_request_has_8192_payload_bytes() -> Result<()> {
    ...
    assert_eq!(
        ...
        DS4_FLASH_HIDDEN_BF16_BYTES as u64
    );
    assert_eq!(
        decoded.header.hidden_row_stride_bytes,
        DS4_FLASH_HIDDEN_BF16_BYTES as u32
    );
    assert_eq!(
...
fn row_scaled_fp8_response_negotiation_and_payload_roundtrip() -> Result<()> {
    ...
    let row_bytes = ExpertV2Dtype::Fp8E4m3RowScaled.row_bytes(DS4_FLASH_HIDDEN_SIZE)?;
    assert_eq!(row_bytes, 4_100);
...
fn nvfp4_response_negotiation_and_payload_roundtrip() -> Result<()> {
    ...
    let row_bytes = ExpertV2Dtype::Nvfp4E2m1Fp8E4m3.row_bytes(DS4_FLASH_HIDDEN_SIZE)?;
    assert_eq!(row_bytes, 2_304);
...
fn streamed_ingress_data_roundtrips_offset_rows_and_final_marker() -> Result<()> {
    let stride = ExpertV2Dtype::Nvfp4E2m1Fp8E4m3.row_bytes(DS4_FLASH_HIDDEN_SIZE)?;
...
fn response_encode_decode_reports_wire_and_logical_bytes() -> Result<()> {
    let row_count = 2;
    let payload = vec![7_u8; row_count * DS4_FLASH_HIDDEN_BF16_BYTES];
    let response = ExpertProtocolV2Response::new(
        42,
        7,
        13,
    ...
    assert_eq!(decoded.header.output_dim, DS4_FLASH_HIDDEN_SIZE as u32);
    assert_eq!(
...
fn default_frames_omit_debug_checksum_from_hot_path() -> Result<()> {
    ...
    let response = ExpertProtocolV2Response::new(
        ...
        ExpertV2Dtype::Bf16,
        ExpertProtocolV2Status::Ok,
        vec![7_u8; DS4_FLASH_HIDDEN_BF16_BYTES],
    )?;
...
fn request_frame_view_borrows_sections_without_decoding_payload() -> Result<()> {
    ...
    assert_eq!(view.hidden_payload(), request.hidden_payload.as_ref());
    assert_eq!(view.hidden_row_payload(1)?.as_ptr(), unsafe {
        request_payload_ptr(&encoded, &request).add(DS4_FLASH_HIDDEN_BF16_BYTES)
    });
    assert_eq!(
        view.hidden_row_payload(1)?,
        &request.hidden_payload[DS4_FLASH_HIDDEN_BF16_BYTES..2 * DS4_FLASH_HIDDEN_BF16_BYTES]
    );
    assert!(view
...
fn request_frame_view_checksum_is_explicit_debug_validation() -> Result<()> {
    ...
    let view = ExpertProtocolV2RequestView::parse(&encoded)?;
    assert_ne!(view.hidden_payload(), request.hidden_payload.as_ref());
    assert!(view
        .verify_checksum()
        .unwrap_err()
        .to_string()
...
fn response_frame_view_borrows_payload_without_decoding_owned_vec() -> Result<()> {
    ...
    assert_eq!(view.partial_output_row_payload(1)?.as_ptr(), unsafe {
        encoded[response.header_len()..]
            .as_ptr()
            .add(DS4_FLASH_HIDDEN_BF16_BYTES)
    });
    assert_eq!(
...
fn reusable_frame_buffer_preserves_response_allocation() -> Result<()> {
    ...
    let response = ExpertProtocolV2Response::new(
        ...
        ExpertV2Dtype::Bf16,
        ExpertProtocolV2Status::Ok,
        vec![9_u8; row_count as usize * DS4_FLASH_HIDDEN_BF16_BYTES],
    )?;
...
fn frame_prefix_encoders_match_full_wire_frames() -> Result<()> {
    ...
    let response = ExpertProtocolV2Response::new(
        ...
        ExpertV2Dtype::Bf16,
        ExpertProtocolV2Status::Ok,
        vec![9_u8; 4 * DS4_FLASH_HIDDEN_BF16_BYTES],
    )?
    .with_debug_checksum()
...
fn device_response_prefix_matches_host_response_metadata() -> Result<()> {
    ...
    let payload_bytes = 2 * DS4_FLASH_HIDDEN_BF16_BYTES;
...
fn frame_arena_preserves_request_response_allocations_and_returns_views() -> Result<()> {
    ...
    let response = ExpertProtocolV2Response::new(
        ...
        ExpertV2Dtype::Bf16,
        ExpertProtocolV2Status::Ok,
        vec![13_u8; 8 * DS4_FLASH_HIDDEN_BF16_BYTES],
    )?;
...
fn expert_batch_mixed_rows_build_protocol_v2_request() -> Result<()> {
    ...
    assert_eq!(decoded.header.hidden_dim, DS4_FLASH_HIDDEN_SIZE as u32);
    assert_eq!(
        decoded.header.hidden_row_stride_bytes,
        DS4_FLASH_HIDDEN_BF16_BYTES as u32
    );
    ...
    assert_eq!(decoded.rows[4].route_offset, 24);
    assert_eq!(decoded.rows[6].route_offset, 36);
...
fn mixed_expert_batch() -> Result<ExpertBatch> {
    ...
    let mut batch =
        ExpertBatch::from_wave_with_envelope(&prefill, DType::Bf16, recipe.clone(), graph_bucket)?;
    batch.try_append_wave(&mtp, DType::Bf16, recipe.clone())?;
    batch.try_append_wave(&decode, DType::Bf16, recipe)?;
    Ok(batch)
}
