...
pub const DEEPSEEK_V4_EXL3_SCHEMA: &str = "ds41rt.exl3.expert-trellis";
...
struct Exl3QuantizationConfig {
    ...
    bits: f64,
    codebook: String,
    ds41rt: Ds41rtExl3Config,
}

#[derive(Debug, Deserialize)]
struct Ds41rtExl3Config {
    schema: String,
...
fn validate_gptqmodel_serving_quantization_config(value: &Value) -> Result<Exl3RecipeContract> {
    ...
    let legacy_mixed_plan = value.pointer("/meta/ds41rt_expert_bit_plan").filter(|plan| {
        plan.get("recipe").and_then(Value::as_str) == Some(DEEPSEEK_V4_EXL3_RECIPE_MIXED_K2_K3_V1)
            && plan.get("schema").and_then(Value::as_str) == Some("ds41rt.exl3-mixed-k2-k3-v1")
    });
    let inline_mixed = value.pointer("/meta/ds41rt_inline_mixed");
    ...
    anyhow::ensure!(
        ...
        "fractional EXL3 bits require a bound DS41RT K2/K3 expert plan"
    );
    anyhow::ensure!(
...
pub(crate) fn exl3_contract_from_quantization_config(
    ...
    anyhow::ensure!(
        ...
        "legacy quant_method={quant_method:?} is not a DS41RT EXL3 artifact; expected exl3"
    );
    if value.get("tensor_storage").is_some() {
        return validate_gptqmodel_serving_quantization_config(value).map(Some);
    }
    validate_legacy_ds41rt_quantization_config(value).map(Some)
}
...
fn validate_legacy_ds41rt_quantization_config(value: &Value) -> Result<Exl3RecipeContract> {
    let config: Exl3QuantizationConfig =
        serde_json::from_value(value.clone()).context("parsing DS41RT EXL3 quantization_config")?;
    anyhow::ensure!(config.quant_method.eq_ignore_ascii_case("exl3"));
    anyhow::ensure!(
        config.version == "1.3.0",
        "DS41RT EXL3 requires exllamav3 version 1.3.0, got {:?}",
        config.version
    );
    anyhow::ensure!(
        (config.bits - DEEPSEEK_V4_EXL3_TRELLIS_BITS as f64).abs() < f64::EPSILON,
        "DS41RT EXL3 requires a native 2-bpw trellis payload, got {}",
        config.bits
    );
    anyhow::ensure!(
        config.codebook == DEEPSEEK_V4_EXL3_CODEBOOK,
        "DS41RT EXL3 requires the MCG codebook, got {:?}",
        config.codebook
    );
    let recipe = config.ds41rt.recipe.as_str();
    ...
    let ds41rt = config.ds41rt;
    anyhow::ensure!(
        ds41rt.schema == DEEPSEEK_V4_EXL3_SCHEMA
            && ds41rt.schema_version == DEEPSEEK_V4_EXL3_SCHEMA_VERSION,
        "unsupported DS41RT EXL3 schema {:?} version {}",
        ds41rt.schema,
        ds41rt.schema_version
    );
    anyhow::ensure!(
        matches!(
            ds41rt.recipe.as_str(),
            DEEPSEEK_V4_EXL3_RECIPE_V2 | DEEPSEEK_V4_EXL3_RECIPE_V3 | DEEPSEEK_V4_EXL3_RECIPE_V4
        ),
        "unsupported DS41RT EXL3 recipe {:?}",
        ds41rt.recipe
    );
    anyhow::ensure!(
        ds41rt.scope == "routed_experts",
        "DS41RT EXL3 quantizes only routed_experts, got {:?}",
        ds41rt.scope
    );
    anyhow::ensure!(
        ds41rt.source_format == DEEPSEEK_V4_EXL3_SOURCE_FORMAT
            && ds41rt.tensor_format == DEEPSEEK_V4_EXL3_TENSOR_FORMAT,
        "unsupported DS41RT EXL3 source/tensor format {:?}/{:?}",
        ds41rt.source_format,
        ds41rt.tensor_format
    );
    anyhow::ensure!(
        ds41rt.expert_tp_world_size == DS4_EXPERT_TP_WORLD_SIZE,
        "DS41RT EXL3 requires strict expert TP{}, got TP{}",
        DS4_EXPERT_TP_WORLD_SIZE,
        ds41rt.expert_tp_world_size
    );
    Ok(Exl3RecipeContract {
    ...
    let projection = |kind: Exl3ProjectionKind| -> Result<Exl3Projection<'_>> {
        ...
        let find = |suffix: &str| -> Result<&TensorInfo> {
            ...
            catalog
                ...
                .with_context(|| format!("missing DS41RT EXL3 tensor {name}"))
        };
...
mod tests {
    use super::*;
    use ds41rt_core::{ModelFacts, ModelVariant};
    ...
    fn valid_quantization_config() -> Value {
        serde_json::json!({
            ...
            "codebook": "mcg",
            "ds41rt": {
                "schema": "ds41rt.exl3.expert-trellis",
                "schema_version": 1,
    ...
    fn valid_activation_quantization_config() -> Value {
        ...
        *config.pointer_mut("/ds41rt/recipe").unwrap() =
    ...
    fn legacy_schema_checks_only_the_k2_serving_layout() {
        ...
        extra_quantization_provenance["ds41rt"]["quantizer_source"] =
    ...
    fn gptqmodel_schema_accepts_uniform_k2_and_k3_serving_layouts() {
        ...
        mixed["meta"] = serde_json::json!({
            "ds41rt_expert_bit_plan": {
                "schema": "ds41rt.exl3-mixed-k2-k3-v1",
                "recipe": DEEPSEEK_V4_EXL3_RECIPE_MIXED_K2_K3_V1
            }
        });
        assert_eq!(
        ...
        leaked_path["meta"]["ds41rt_inline_mixed"]["tier_plan_root"] =
        ...
        mismatched_target["meta"]["ds41rt_inline_mixed"]["target_bpw"] = serde_json::json!("22/10");
        assert!(
        ...
        noncanonical_bitrate["meta"]["ds41rt_inline_mixed"]["extra_bits"] =
        ...
        for (pointer, value, expected) in [
            (
                "/bits",
                serde_json::json!(2.5),
                "bound DS41RT K2/K3 expert plan",
            ),
            (
                "/codebook",
                serde_json::json!("different"),
