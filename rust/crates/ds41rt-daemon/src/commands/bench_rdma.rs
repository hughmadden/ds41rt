...
use app::{
    ds41rt_verbs_app_benchmark_attempt, run_ds41rt_verbs_app_benchmark, Ds41rtVerbsAppBenchmarkAttempt,
};
...
const DS41RT_VERBS_APP_BENCHMARK_KIND: &str = "ds41rt-verbs-app-protocol-v2";
const DEFAULT_VENDOR_PAYLOAD_BYTES: &str = "4096,8192,12288,16384,32768,65536";
const DEFAULT_APP_PROTOCOL_V2_PAYLOAD_BYTES: &str =
    "8192,16384,32768,65536,131072,524288,2097152,4194304";
...
struct RdmaBenchReport {
    ...
    ds41rt_app_benchmark: Option<Ds41rtVerbsAppBenchmarkAttempt>,
}
...
pub(crate) fn run_bench_rdma(args: BenchRdmaArgs) -> Result<()> {
    ...
    let app_mode = is_ds41rt_app_mode(&resolved_mode);
    ...
    let benchmark_kind = if app_mode {
        DS41RT_VERBS_APP_BENCHMARK_KIND
    } else {
    ...
    let mut report = RdmaBenchReport {
        ...
        skip_reason: None,
        runs: Vec::new(),
        ds41rt_app_benchmark: None,
    };

    if app_mode {
        let mut attempt = ds41rt_verbs_app_benchmark_attempt(
            &resolved_mode,
            args.peer.clone(),
        ...
        if resolved_mode == "app-capability" {
            report.skipped = true;
            report.skip_reason = Some("DS41RT verbs app capability mode only".to_owned());
        } else if !attempt.preflight_ok {
            report.skipped = true;
            report.skip_reason = Some(match attempt.preflight_error.as_deref() {
                Some(err) => format!("DS41RT verbs app benchmark preflight failed: {err}"),
                None => "DS41RT verbs app benchmark preflight failed".to_owned(),
            });
        } else {
            match run_ds41rt_verbs_app_benchmark(
                &resolved_mode,
                args.peer.as_deref(),
                args.port,
                &attempt.payloads,
                ...
                Err(err) => {
                    ...
                    report.skipped = true;
                    report.skip_reason = Some(format!("DS41RT verbs app benchmark failed: {err}"));
                }
            }
        }
        report.ds41rt_app_benchmark = Some(attempt);
...
fn is_ds41rt_app_mode(resolved_mode: &str) -> bool {
    matches!(
        resolved_mode,
