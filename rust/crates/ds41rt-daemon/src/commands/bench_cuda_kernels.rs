...
use ds41rt_core::GLM52_ROUTED_SCALING_FACTOR;
use ds41rt_ffi::{
...
fn router_top_k(args: &BenchCudaKernelsArgs, experts: usize, label: &str) -> Result<usize> {
    if args.top_k > experts || args.top_k > DS41RT_CUDA_ROUTER_TOPK_MAX_K {
        anyhow::bail!(
            "{label} invalid top_k={} for experts={experts}; max supported top_k={DS41RT_CUDA_ROUTER_TOPK_MAX_K}",
            args.top_k
        );
    }
    Ok(args.top_k)
...
fn sample_top_k(args: &BenchCudaKernelsArgs, label: &str) -> Result<usize> {
    if args.top_k > args.vocab || args.top_k > DS41RT_CUDA_SAMPLE_TOPK_MAX_K {
        anyhow::bail!(
            "{label} invalid top_k={} for vocab={}; max supported top_k={DS41RT_CUDA_SAMPLE_TOPK_MAX_K}",
            args.top_k,
            args.vocab
        );
    }
...
fn native_library_path(args: &BenchCudaKernelsArgs) -> PathBuf {
    args.native_lib
        ...
        .unwrap_or_else(|| PathBuf::from("native/build-cuda/libds41rt_native.so"))
}
...
impl<'a> BenchCudaGraph<'a> {
    fn new(library: &'a NativeLibrary, capture: Ds41rtCudaGraphCaptureInfo) -> Result<Self> {
...
fn python_buffer(name: &'static str, buffer: Ds41rtDeviceBuffer) -> PythonDeviceBufferArg<'static> {
    PythonDeviceBufferArg {
        name,
        ptr: buffer.ptr,
        bytes: buffer.bytes,
