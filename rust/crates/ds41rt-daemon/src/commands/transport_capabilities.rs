...
pub(crate) fn run_transport_capabilities(args: TransportCapabilitiesArgs) -> Result<()> {
    let mut transports = vec![
        ds41rt_transport::inproc_capabilities(),
        ds41rt_transport::tcp_capabilities(),
        ds41rt_transport::verbs_host_capabilities(),
    ];
...
fn verbs_app_payload_bytes(row: &serde_json::Value) -> Result<usize> {
    ...
    if let Some(row_count) = json_usize_opt(row, "row_count") {
        if row_count > 0 {
            return row_count
                .checked_mul(DS4_FLASH_HIDDEN_SIZE)
                .and_then(|value| value.checked_mul(2))
                .context("DeepSeek V4 verbs app ProtocolV2 logical payload byte count overflow");
        }
    }
...
mod tests {
    ...
    fn transport_capabilities_parse_prefill_roundtrip_measurements() {
        let path = std::env::temp_dir().join(format!(
            "ds41rt-prefill-roundtrip-{}-{}.jsonl",
            std::process::id(),
            "test"
        ));
        fs::write(
    ...
    fn transport_capabilities_annotate_verbs_app_client_rtt() {
        ...
        assert_eq!(measurement.payload_bytes, 8192);
    ...
    fn transport_capabilities_annotate_verbs_app_prefill_bandwidth() {
        let path = std::env::temp_dir().join(format!(
            "ds41rt-verbs-app-prefill-{}-{}.jsonl",
            std::process::id(),
            "test"
        ));
        fs::write(
            &path,
            r#"{"benchmark":"spark_verbs_app_protocol_v2","artifact":"verbs_app_ostrich_dodo_client.json","pair":"ostrich->dodo","role":"client","run_kind":"chain_model_depth","source_kind":"prefill","ok":true,"row_count":16,"hops_per_iteration":43,"roundtrips":43,"roundtrip_latency_micros_avg":1000.0,"request_payload_matches":true,"response_payload_matches":true,"send_completions":43,"recv_completions":43}"#,
        )
        .unwrap();
        let mut transports = vec![
            ds41rt_transport::inproc_capabilities(),
        ...
        assert_eq!(measurement.logical_payload_bytes, 131_072);
        assert_eq!(measurement.hops, 43);
        assert_eq!(measurement.total_ms, 43.0);
        assert_eq!(measurement.avg_ms, 1.0);
        assert!((measurement.effective_prefill_tokens_per_sec - 372.09302325581393).abs() < 1.0e-9);
        assert!((measurement.aggregate_logical_gbps - 2.097152).abs() < 1.0e-12);
        assert_eq!(
            measurement.source,
            "spark_verbs_app_protocol_v2:ostrich->dodo:chain_model_depth:prefill:verbs_app_ostrich_dodo_client.json"
        );
        assert!(rdma_component_perftest_rtt_by_size.is_empty());
    }
}
