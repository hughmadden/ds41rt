use anyhow::Result;
use ds41rt_core::{NodeRole, EXPERT_HOSTS};
use ds41rt_loader::resolve_snapshot;
use serde::Serialize;
...
fn collect_doctor(role: NodeRole, model_id: &str, hf_home: Option<&Path>) -> Result<DoctorReport> {
    ...
    let host_list = std::env::var("DS41RT_EXPERT_HOSTS")
    ...
    Ok(DoctorReport {
        ...
        docker_image: DockerImageInfo {
            role_env: std::env::var("DS41RT_ROLE").ok(),
            cuda_arch_env: std::env::var("DS41RT_CUDA_ARCH").ok(),
            target_platform_env: std::env::var("DS41RT_TARGET_PLATFORM").ok(),
