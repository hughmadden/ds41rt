...
use ds41rt_core::{KvCacheConfig, LayerId, TensorCatalog};
...
const REAL_FULL_KV_SNAPSHOT_FORMAT: &str = "ds41rt-kv-v3";
const LEGACY_REAL_FULL_KV_SNAPSHOT_FORMATS: &[&str] = &["ds41rt-kv-dsa-v1", "ds41rt-kv-dsa-v2"];
...
fn current_snapshot_producer_profile() -> RealFullKvSnapshotProducerProfile {
    let moe_response_dtype = env_or_default("DS41RT_REAL_FULL_MOE_RESPONSE_DTYPE", "bf16");
    let intermediate_reduction =
        env_or_default("DS41RT_EXPERT_INTERMEDIATE_REDUCTION", "coordinator");
    let intermediate_reduction_dtype =
        env_or_default("DS41RT_EXPERT_INTERMEDIATE_REDUCTION_DTYPE", "fp8");
    RealFullKvSnapshotProducerProfile {
        ...
        b12x_direct_route: default_true_env("DS41RT_B12X_SPARK_DIRECT_ROUTE"),
        b12x_grouped_decode: default_true_env("DS41RT_B12X_SPARK_GROUPED_DECODE"),
        route_grouped_multirow: default_true_env("DS41RT_REAL_FULL_NVFP4_ROUTE_GROUPED_MULTIROW"),
        fused_fp8_reduction: default_true_env("DS41RT_EXPERT_FUSED_FP8_REDUCTION"),
        nccl_bf16_reduce: explicit_truthy_env("DS41RT_EXPERT_NCCL_BF16_REDUCE", false),
        cuda_reference_kernels: env_or_default("DS41RT_REAL_FULL_CUDA_REFERENCE_KERNELS", "false"),
        moe_owner_response_dtype: env_or_default(
            "DS41RT_REAL_FULL_MOE_OWNER_RESPONSE_DTYPE",
            &moe_response_dtype,
        ),
        moe_response_dtype,
        intermediate_shards: positive_env_usize("DS41RT_EXPERT_INTERMEDIATE_SHARDS", 4),
