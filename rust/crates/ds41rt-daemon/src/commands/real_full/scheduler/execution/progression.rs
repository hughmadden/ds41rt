...
use ds41rt_loader::{is_deepseek_v4_exl3_recipe, read_tensor_bytes_into, DEEPSEEK_V4_EXL3_RECIPE};
...
const SPARSE_TCP_STAGE_TIMING_ENV: &str = "DS41RT_REAL_FULL_SPARSE_TCP_STAGE_TIMING";
const BF16_HIDDEN_READBACK_OVERLAP_ENV: &str = "DS41RT_REAL_FULL_BF16_HIDDEN_READBACK_OVERLAP";
const MOE_PAYLOAD_HASH_DIAGNOSTIC_ENV: &str = "DS41RT_REAL_FULL_DIAGNOSTIC_MOE_PAYLOAD_HASH";
const LAYER_BOUNDARY_DUMP_DIR_ENV: &str = "DS41RT_REAL_FULL_DIAGNOSTIC_LAYER_DUMP_DIR";
const LAYER_BOUNDARY_DUMP_LAYER_ENV: &str = "DS41RT_REAL_FULL_DIAGNOSTIC_LAYER_DUMP_LAYER";
const PHASE0_SPARK_EXPERT_MODE_ENV: &str = "DS41RT_PHASE0_SPARK_EXPERT_MODE";
const B12X_PACKED_HIDDEN_EXCHANGE_ENV: &str = "DS41RT_REAL_FULL_B12X_PACKED_HIDDEN_EXCHANGE";
const B12X_COORDINATOR_ENV: &str = "DS41RT_B12X";
...
const ROLLING_SPARSE_PACKS_ENV: &str = "DS41RT_REAL_FULL_ROLLING_SPARSE_PACKS";
...
pub(super) struct RealFullSchedulerDeviceHiddenSource {
    pub(super) buffer: Ds41rtDeviceBuffer,
...
impl RealFullSchedulerSparseDispatchTransport {
    ...
    fn thread_name(self) -> &'static str {
        match self {
            Self::Tcp => "ds41rt-scheduler-sparse-tcp-persistent",
            Self::VerbsHost => "ds41rt-scheduler-sparse-verbs-host",
        }
    }
...
impl RealFullSchedulerSparseTcpDispatchWorker {
        ...
        let join = thread::Builder::new()
            ...
            .spawn(move || {
                let startup_result = worker_cpu
                    .map(ds41rt_core::pin_current_thread_to_cpu)
                    .unwrap_or(Ok(()));
    ...
    fn try_start_direct_ds4_tp4_raw_payload(
        ...
        match real_full_scheduler_ds4_tp4_try_start_verbs_raw(
            ...
            ds41rt_transport::VerbsHostProtocolV2HostBatchSetPayloadStart::Started(pending) => {
                Ok(DirectOwnerPayloadDispatchStart::Started(pending))
            }
            ds41rt_transport::VerbsHostProtocolV2HostBatchSetPayloadStart::Busy(payload) => {
...
fn scheduler_tcp_max_global_rows_per_dispatch() -> usize {
    match env::var("DS41RT_REAL_FULL_SCHEDULER_TCP_MAX_GLOBAL_ROWS") {
...
const ROUTED_ACTIVATION_RETENTION_CONTROL_FILE: &str = ".ds41rt_route_retention_control_v1.bin";
const ROUTED_ACTIVATION_RETENTION_CONTROL_MAGIC: &[u8; 8] = b"DS41RRC1";
...
fn scheduler_sparse_tcp_batch_slice(
    ...
    for (local_row_index, row) in batch.rows[row_start..row_end].iter().enumerate() {
        ...
        slice_rows.push(ds41rt_core::ExpertBatchRow {
...
impl RealFullSchedulerNumericProgression {
    ...
    fn apply_source_delta(
        ...
        placement_version: &ds41rt_core::PlacementVersion,
...
fn scheduler_sparse_source_batch(
    ...
    let rows = (0..row_count)
        .map(|row_offset| ds41rt_core::ExpertBatchRow {
            row_id: row_offset as u64,
            source_kind: source.kind,
            request_id: source.request_id.clone(),
            sequence_id: source.sequence_id.clone(),
            token_position: ds41rt_core::PositionId(source.token_start.0 + row_offset as u64),
...
mod tests {
    ...
    fn pro_progression_preserves_flash_execution_shape_at_pro_geometry() -> Result<()> {
        ...
        assert_eq!(
            ...
            ds41rt_core::DS4_PRO_NUM_HIDDEN_LAYERS
        );
    ...
    fn native_flash_checkpoint_shared_expert_executes_scheduler_resident_path() {
        ...
        let catalog = ds41rt_loader::build_catalog_for_snapshot(
    ...
    fn scheduler_sparse_rolling_response_rows_remap_to_layer_coordinates() -> Result<()> {
        let mut chunk = VerbsHostProtocolV2HostBatchSetBf16PayloadChunk {
            host_index: 2,
            partial_output: ds41rt_transport::VerbsHostProtocolV2ResponsePayload::from_owned(
                vec![0_u8; 12],
            ),
