...
use ds41rt_core::{
    ...
    GLM52_DSA_INDEX_HEAD_DIM, GLM52_HIDDEN_SIZE, GLM52_MLA_KV_LORA_RANK,
    GLM52_MLA_QK_ROPE_HEAD_DIM, GLM52_MLA_ROPE_THETA, GLM52_TOP_K,
};
...
const SPECULATIVE_TARGET_ATTENTION_FUSION_ENV: &str =
    "DS41RT_REAL_FULL_SPECULATIVE_TARGET_ATTENTION_FUSION";
// One authoritative decode row plus up to fifteen integrated proposal rows.
pub(super) const SPECULATIVE_TARGET_ATTENTION_FUSION_MAX_ROWS: usize = 16;
const SPECULATIVE_TARGET_SCALAR_Q_A_BATCHED_Q_B_MAX_ROWS: usize = 16;
...
pub(super) fn real_full_apply_admitted_scheduler_iteration(
    ...
    if admission
        .selected
        .first()
        .is_some_and(|wave| (wave.layer_id.0 as usize) >= catalog.facts.first_k_dense_replace)
    {
        if record_sparse_host_partition {
            ...
            counters.sparse_expert_batches += 1;
            accumulate_sparse_expert_row_sources_from_waves(
                &admission.selected,
                catalog.facts.top_k,
                counters,
            )?;
    ...
    let native_target = numeric_progression.native_target_enabled();
    anyhow::ensure!(
        !native_target || catalog.facts.model_type == "deepseek_v4",
        "native target runtime was attached to non-DeepSeek model {}",
        catalog.facts.model_type
    );
    let fuse_mtp_target_attention = should_fuse_speculative_target_attention(
        store.config(),
        &admission.selected,
        native_target,
    )?;
    let fuse_native_mtp_target_attention = native_target && fuse_mtp_target_attention;
    let fuse_legacy_mtp_target_attention = !native_target && fuse_mtp_target_attention;
    let mut prepared_attention =
        Vec::with_capacity(usize::from(fuse_legacy_mtp_target_attention) * 2);
    let mut fused_native_accounting = Vec::with_capacity(2);
    for (wave_index, wave) in admission.selected.iter().enumerate() {
        ...
        if native_target {
            let native_attention_start = stage_timing.then(Instant::now);
            let accounting = apply_native_target_wave(
                store,
                wave,
                &visible_blocks,
                counters,
                ...
                numeric_progression,
                catalog,
                !fuse_native_mtp_target_attention,
            )?;
            fused_native_accounting.extend(accounting);
            attention_ms += elapsed_ms_optional(native_attention_start);
            if !fuse_native_mtp_target_attention && !wave.tentative_kv_writes.is_empty() {
        ...
        if !wave.kv_writes.is_empty() {
            ...
            if fuse_legacy_mtp_target_attention {
    ...
    if fuse_native_mtp_target_attention {
        let attention_start = stage_timing.then(Instant::now);
        launch_fused_native_mtp_target_attention(
            &admission.selected,
            &fused_native_accounting,
            counters,
            device_kv,
            numeric_progression,
            catalog,
...
#[derive(Clone, Copy)]
struct NativeTargetAttentionAccounting {
    layer_id: usize,
    kind: RowSourceKind,
    token_start: usize,
    token_end: usize,
    row_count: usize,
    kv_descriptors: usize,
}

fn apply_native_target_wave(
    store: &mut KvCacheBackingStore,
    wave: &LayerWave,
    ...
    numeric_progression: &mut RealFullSchedulerNumericProgression,
    catalog: &TensorCatalog,
    launch_attention: bool,
) -> Result<Vec<NativeTargetAttentionAccounting>> {
    anyhow::ensure!(
        wave.kv_writes.is_empty() || wave.tentative_kv_writes.is_empty(),
        "native target wave for layer {} mixes committed and tentative KV writes",
        wave.layer_id.0
    );
    let (descriptors, tentative) = if !wave.kv_writes.is_empty() {
        (wave.kv_writes.as_slice(), false)
    } else if !wave.tentative_kv_writes.is_empty() {
        (wave.tentative_kv_writes.as_slice(), true)
    } else {
        return Ok(Vec::new());
    };
    let written_tokens = descriptors.iter().try_fold(0_usize, |total, descriptor| {
        total
            .checked_add(descriptor.token_count)
            .context("native target KV write token count overflow")
    })?;
    let projected_bytes = written_tokens
        .checked_mul(store.config().bytes_per_token())
        .context("native target KV write byte count overflow")?;
    let write_ids = if tentative {
        store.write_tentative_block_metadata_for_wave(wave)
    } else {
        store.write_committed_block_metadata_for_wave(wave)
    }
    .with_context(|| {
        format!(
            "writing native target {} KV metadata for layer {}",
            if tentative { "tentative" } else { "committed" },
            wave.layer_id.0
        )
    })?;
    if tentative {
    ...
    counters.projected_device_kv_writes += descriptors.len();
    counters.projected_device_kv_write_bytes += projected_bytes;
        ...
        .filter(|source| source.row_count > 0)
    {
        anyhow::ensure!(
            ...
            "native target attention does not support benchmark rows"
        );
        let token_start = usize::try_from(source.token_start.0)
            .context("native target logical token start exceeds usize")?;
        let token_end = token_start
            .checked_add(source.row_count)
            .context("native target logical token end overflow")?;
        let source_accounting = NativeTargetAttentionAccounting {
            layer_id: wave.layer_id.0 as usize,
            kind: source.kind,
            token_start,
            token_end,
            row_count: source.row_count,
            kv_descriptors: visible_blocks.len() + descriptors.len(),
        };
        if launch_attention {
            let physical_slots = device_kv
                .physical_token_positions(0, token_end)
                .with_context(|| {
                    format!(
                        "resolving native target physical slots through logical token {token_end}"
                    )
                })?;
            numeric_progression.run_native_target_attention(
                source_accounting.layer_id,
                source.kind,
                token_start,
                source.row_count,
                &physical_slots,
            ...
            let output_values = source
                .row_count
                .checked_mul(catalog.facts.hidden_size)
                .context("native target attention output value count overflow")?;
            counters.device_attention_launches += 1;
            counters.device_attention_status = Some("native-deepseek-v4-graph-replay");
            counters.device_attention_rows += token_end;
            counters.device_attention_query_rows += source.row_count;
            counters.device_attention_kv_descriptors += source_accounting.kv_descriptors;
            counters.device_attention_output_bytes += output_values * std::mem::size_of::<u16>();
            counters.device_attention_output_values += output_values;
            counters.device_attention_hidden_projection_launches += 1;
        }
        accounting.push(source_accounting);
    }
    Ok(accounting)
}

fn launch_fused_native_mtp_target_attention(
    selected: &[LayerWave],
    accounting: &[NativeTargetAttentionAccounting],
    counters: &mut RealFullSchedulerExecutionCounters,
    device_kv: &mut RealFullDeviceKvExecutionMirror,
    numeric_progression: &mut RealFullSchedulerNumericProgression,
    catalog: &TensorCatalog,
    ...
    anyhow::ensure!(
        selected.len() == 2 && accounting.len() == 2,
        "fused native decode/MTP attention expected two waves and accounting records, got selected={} accounting={}",
        selected.len(),
        accounting.len()
    );
    let decode = accounting[0];
    let mtp = accounting[1];
    anyhow::ensure!(
        decode.layer_id == mtp.layer_id
            && decode.kind == RowSourceKind::DecodeStep
            && mtp.kind == RowSourceKind::MtpVerifyBlock
            && decode.row_count == 1
            && mtp.row_count > 0
            && decode.token_end == mtp.token_start,
        "fused native decode/MTP attention accounting is not one contiguous target suffix"
    );
    let total_rows = decode
        .row_count
        .checked_add(mtp.row_count)
        .context("fused native decode/MTP attention row count overflow")?;
    let decode_source = single_wave_source(&selected[0])?;
    let mtp_source = single_wave_source(&selected[1])?;
    anyhow::ensure!(
        decode_source.kind == decode.kind
            && decode_source.token_start.0 as usize == decode.token_start
            && decode_source.row_count == decode.row_count
            && mtp_source.kind == mtp.kind
            && mtp_source.token_start.0 as usize == mtp.token_start
            && mtp_source.row_count == mtp.row_count,
        "fused native decode/MTP sources differ from their admission accounting"
    );
    let physical_slots = device_kv
        .physical_token_positions(0, mtp.token_end)
        .with_context(|| {
            format!(
                "resolving fused native target physical slots through logical token {}",
                mtp.token_end
            )
        })?;
    numeric_progression.run_fused_native_target_attention(
        decode.layer_id,
        decode_source,
        mtp_source,
        &physical_slots,
    ...
    let output_values = total_rows
        .checked_mul(catalog.facts.hidden_size)
        .context("fused native target attention output value count overflow")?;
    counters.device_attention_launches += 1;
    counters.device_attention_status = Some("native-deepseek-v4-fused-target-graph-replay");
    counters.device_attention_rows += mtp.token_end;
    counters.device_attention_query_rows += total_rows;
    counters.device_attention_kv_descriptors += decode.kv_descriptors.max(mtp.kv_descriptors);
    counters.device_attention_output_bytes += output_values * std::mem::size_of::<u16>();
    counters.device_attention_output_values += output_values;
    counters.device_attention_hidden_projection_launches += 1;
    Ok(())
...
impl CachedResidentDeviceBuffer {
    fn device_buffer(self) -> Ds41rtDeviceBuffer {
        Ds41rtDeviceBuffer {
...
impl From<Ds41rtDeviceBuffer> for CachedResidentDeviceBuffer {
    fn from(buffer: Ds41rtDeviceBuffer) -> Self {
        Self {
...
fn scheduler_current_kv_resident_layer(
    ...
    let dsa = if ds41rt_core::GLM52_DSA_INDEXER_LAYER_IDS_WITH_MTP.contains(&layer_id) {
...
fn project_real_scheduler_attention_queries_scalar_q_a_batched_q_b(
    ...
    let max_rows = if coordinator_w8a16_q_b_decode_enabled() {
        SPECULATIVE_TARGET_SCALAR_Q_A_BATCHED_Q_B_MAX_ROWS
    } else {
        8
    };
    if hidden.rows <= 1
...
fn scheduler_device_kv_readback_validation_enabled() -> bool {
    match std::env::var("DS41RT_REAL_FULL_VALIDATE_DEVICE_KV_READBACK") {
        Ok(value) => matches!(
            value.as_str(),
...
fn speculative_target_attention_fusion_enabled() -> bool {
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        env::var(SPECULATIVE_TARGET_ATTENTION_FUSION_ENV)
            .map(|value| !matches!(value.as_str(), "0" | "false" | "FALSE" | "no" | "NO"))
            .unwrap_or(true)
    })
}

fn should_fuse_speculative_target_attention(
    config: &KvCacheConfig,
    selected: &[LayerWave],
    native_target: bool,
) -> Result<bool> {
    if !speculative_target_attention_fusion_enabled()
        || (!native_target && !matches!(config.dtype, KvCacheDType::Fp8 | KvCacheDType::Nvfp4))
        || selected.len() != 2
    {
        return Ok(false);
    }
    ...
    let rows = decode
        .row_count
        .checked_add(mtp.row_count)
        .context("fused speculative target attention row count overflow")?;
    ...
    Ok(decode_wave.layer_id == mtp_wave.layer_id
        ...
        && decode.row_count == 1
        && mtp.row_count > 0
        && rows <= SPECULATIVE_TARGET_ATTENTION_FUSION_MAX_ROWS
        && contiguous
...
fn real_full_launch_fused_mtp_target_attention(
    ...
    fused_wave.hidden_shape = ds41rt_core::HiddenShape::ds4_flash_bf16_rows(total_rows);
...
fn real_full_launch_sequential_mtp_target_attention(
    ...
    for (row, descriptor) in current_descriptors.iter().enumerate() {
        ...
        row_wave.hidden_shape = ds41rt_core::HiddenShape::ds4_flash_bf16_rows(1);
        row_wave.row_sources[0].token_start = ds41rt_core::PositionId(token_start);
        ...
        let row_hidden = RealFullSchedulerDeviceHiddenSource {
            ...
            ready_event: hidden.ready_event.clone(),
        };
...
fn real_full_launch_scheduler_attention_from_device_kv_with_query_override(
    ...
    if let Some(hidden) = query_hidden.as_ref() {
        if hidden.rows != source.row_count || hidden.values_per_row != GLM52_HIDDEN_SIZE {
            anyhow::bail!(
    ...
    let launch = if let Some(hidden) = query_hidden {
        ...
        if let Some(real_attention) = real_attention {
            if env::var_os("DS41RT_REAL_FULL_DIAGNOSTIC_LAYER_DUMP_DIR").is_some()
                && wave.layer_id.0 == 0
            {
                eprintln!(
...
fn single_wave_source(wave: &LayerWave) -> Result<&ds41rt_core::RowSource> {
...
mod tests {
    use super::*;
    use ds41rt_core::{DecodeStep, LayerId, MtpVerifyBlock, PositionId, Priority};

    #[test]
    fn native_target_fusion_uses_native_cache_abi_not_transactional_mirror_dtype() -> Result<()> {
        ...
        let selected = [decode, mtp];

        assert!(should_fuse_speculative_target_attention(
