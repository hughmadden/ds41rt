...
use ds41rt_core::{DType, TensorCatalog, TensorRole};
...
pub(super) const REAL_FULL_SCORE_LM_HEAD_ENV: &str = "DS41RT_REAL_FULL_SCORE_LM_HEAD";
...
pub(super) fn real_full_sampling_dry_run(
    ...
    if hidden_dim != catalog.facts.hidden_size {
        anyhow::bail!(
    ...
    let hidden_bytes_per_row = hidden_dim * std::mem::size_of::<u16>();
    let expected_lm_head_bytes = (vocab_size * hidden_bytes_per_row) as u64;
    ...
    let max_chunk_bytes = chunk_rows * hidden_bytes_per_row;
    let logical_lm_head_read_bytes = lm_head.byte_length * sampled_rows as u64;
    let logical_hidden_read_bytes = sampled_rows * hidden_bytes_per_row;
