...
use ds41rt_loader::{load_tensor_rows, read_tensor_rows_into};
...
fn real_full_embedding_tensor(catalog: &TensorCatalog) -> Result<&TensorInfo> {
    let mut embeddings = catalog
        .tensors
        .iter()
        .filter(|tensor| tensor.role == TensorRole::Embedding);
    let info = embeddings
        .next()
        .context("real full embedding tensor not found in catalog")?;
    anyhow::ensure!(
        embeddings.next().is_none(),
        "real full embedding lookup requires exactly one embedding tensor"
    );
    Ok(info)
}
...
pub(in crate::commands::real_full) fn real_full_embedding_device_hidden_for_tokens(
    ...
    let info = real_full_embedding_tensor(catalog)?;
    let tensor_name = info.name.as_str();
    ...
    if info.shape.len() != 2 || info.shape[1] != hidden_size {
        anyhow::bail!(
    ...
    if !coordinator_cuda_reference_kernels_enabled()
        || !resident_weight_is_preloaded(tensor_name, embedding_bytes)
    {
        return Ok(None);
    }
...
pub(in crate::commands::real_full) fn real_full_embedding_hidden_for_token(
    ...
    let embedding_key = format!("{tensor_name}[rows={token_id}..{window_end}]");
    ...
    if coordinator_cuda_reference_kernels_enabled() {
        ...
        preload_resident_weight_from_host_staging(
            ...
            |staging| {
                let summary =
                    read_tensor_rows_into(catalog, tensor_name, token_id, 1, staging)
                        .with_context(|| {
                            format!(
                ...
                if summary.row_width != hidden_size {
                    anyhow::bail!(
    ...
    let row = load_tensor_rows(catalog, tensor_name, token_id, 1)?;
    if row.row_width != hidden_size {
        anyhow::bail!(
            "real full embedding lookup hidden width mismatch: expected {} got {}",
            hidden_size,
            row.row_width
...
mod tests {
    ...
    use ds41rt_core::{
        DType, ModelFacts, TensorCatalog, TensorInfo, TensorRole, DS4_FLASH_HIDDEN_SIZE,
    };
    ...
    fn embedding_lookup_loads_one_bf16_hidden_row() {
        ...
        let snapshot_path = unique_temp_dir("ds41rt-real-full-embedding-test");
        fs::create_dir_all(&snapshot_path).expect("creating temp snapshot dir");
        let file_path = snapshot_path.join("model.safetensors");
        let mut row0 = vec![0.0_f32; DS4_FLASH_HIDDEN_SIZE];
        let mut row1 = vec![0.0_f32; DS4_FLASH_HIDDEN_SIZE];
        row0[0] = 1.0;
        row0[DS4_FLASH_HIDDEN_SIZE - 1] = -2.0;
        row1[0] = 3.0;
        row1[1] = -4.0;
        row1[DS4_FLASH_HIDDEN_SIZE - 1] = 5.0;
        ...
        let catalog = TensorCatalog {
            ...
            tensors: vec![TensorInfo {
                name: "embed.weight".to_owned(),
                file: "model.safetensors".to_owned(),
                dtype: DType::Bf16,
                shape: vec![2, DS4_FLASH_HIDDEN_SIZE],
                byte_offset: 0,
                byte_length: (2 * DS4_FLASH_HIDDEN_SIZE * 2) as u64,
        ...
        assert_eq!(hidden.bytes_read, (DS4_FLASH_HIDDEN_SIZE * 2) as u64);
        assert_eq!(hidden.hidden.len(), DS4_FLASH_HIDDEN_SIZE);
        assert_eq!(hidden.hidden[0], 3.0);
        assert_eq!(hidden.hidden[1], -4.0);
        assert_eq!(hidden.hidden[DS4_FLASH_HIDDEN_SIZE - 1], 5.0);
