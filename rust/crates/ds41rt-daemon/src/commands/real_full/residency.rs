...
use ds41rt_core::{DType, TensorCatalog, TensorInfo, TensorRole};
use ds41rt_ffi::{Ds41rtHostBuffer, NativeLibrary};
use ds41rt_loader::{read_tensor_bytes_into, LoadedTensorSummary};
...
const COORDINATOR_RESIDENT_PRELOAD_SCOPE: &str =
    "select coordinator-owned immutable model tensors for named startup GPU residency";
...
const BASE_COORDINATOR_RESIDENT_ROLE_LABELS: [&str; 4] =
    ["Embedding", "LmHead", "Attention", "Norm"];

fn required_coordinator_resident_role_labels(catalog: &TensorCatalog) -> Vec<&'static str> {
    let mut roles = BASE_COORDINATOR_RESIDENT_ROLE_LABELS.to_vec();
    if catalog.facts.model_type == "deepseek_v4" {
        if catalog.facts.compress_ratios.iter().any(|ratio| *ratio > 0) {
            roles.push("AttentionCompressor");
        }
        if catalog
            .facts
            .compress_ratios
            ...
            .any(|ratio| *ratio == 4)
        {
            roles.push("AttentionIndexer");
        }
        if catalog.facts.hyper_connection_multiplier > 1 {
            roles.push("HyperConnection");
        }
    }
    if catalog.facts.first_k_dense_replace > 0 {
        roles.push("DenseMlp");
    }
    if catalog.facts.first_k_dense_replace < catalog.facts.num_hidden_layers {
        roles.extend(["Router", "SharedExpert"]);
    }
    roles
}
...
fn coordinator_resident_preload_plan_for_tensors(
    ...
    let required_roles = required_coordinator_resident_role_labels(catalog);
    let missing_required_roles = required_roles
        .iter()
    ...
    let skipped_quantization_tensors = catalog
        ...
        .iter()
        .filter(|tensor| {
            (tensor.role == TensorRole::Quantization || tensor.is_quantization_metadata)
                && !coordinator_resident_tensor(tensor)
        })
        .count();
    ...
    let skipped_mtp_bytes = catalog
        ...
        .map(|tensor| tensor.byte_length)
        .sum();
    let skipped_dspark_tensors = catalog
        .tensors
        .iter()
        .filter(|tensor| tensor.role == TensorRole::Dspark && !coordinator_resident_tensor(tensor))
        .count();
    let skipped_dspark_bytes = catalog
        .tensors
        .iter()
        .filter(|tensor| tensor.role == TensorRole::Dspark && !coordinator_resident_tensor(tensor))
        .map(|tensor| tensor.byte_length)
    ...
    RealFullCoordinatorResidentPreloadPlan {
        ...
        role_counts,
        role_bytes,
        required_role_count: required_roles.len(),
        required_roles_present: required_roles.len() - missing_required_roles.len(),
        ...
        skipped_dspark_tensors,
        skipped_dspark_bytes,
        skipped_mtp_tensors,
        skipped_mtp_bytes,
...
fn coordinator_resident_tensor(tensor: &TensorInfo) -> bool {
    coordinator_resident_role(&tensor.role)
        && (!tensor.is_quantization_metadata
            || coordinator_resident_quantization_metadata(&tensor.role))
        && (tensor.role != TensorRole::Dspark || coordinator_dspark_residency_enabled())
        && !(tensor.role == TensorRole::Dspark && tensor.expert_id.is_some())
}

fn coordinator_resident_quantization_metadata(role: &TensorRole) -> bool {
    matches!(
        role,
...
fn coordinator_dspark_residency_enabled() -> bool {
    let dspark = env::var(REAL_FULL_DSPARK_ENV)
        .ok()
        .and_then(|value| parse_bool_env_value(&value))
        .unwrap_or(true);
    let dspark_shadow = env::var(REAL_FULL_DSPARK_SHADOW_ENV)
        .ok()
        .and_then(|value| parse_bool_env_value(&value))
        .unwrap_or(false);
    dspark || dspark_shadow
}
...
fn coordinator_resident_role_label(role: &TensorRole) -> &'static str {
    match role {
        ...
        TensorRole::AttentionCompressor => "AttentionCompressor",
        TensorRole::AttentionIndexer => "AttentionIndexer",
        TensorRole::HyperConnection => "HyperConnection",
        TensorRole::Router => "Router",
        TensorRole::Norm => "Norm",
        TensorRole::DenseMlp => "DenseMlp",
        TensorRole::SharedExpert => "SharedExpert",
        TensorRole::Dspark => "Dspark",
        TensorRole::Mtp => "Mtp",
        _ => "Other",
    }
}
...
mod tests {
    ...
    use ds41rt_core::{DType, ModelFacts, TensorCatalog, TensorInfo, TensorRole, DEFAULT_MODEL_ID};
    use ds41rt_loader::LoadedTensorSummary;
    use std::fs::File;
    use std::io::Write;
    ...
    fn coordinator_resident_preload_plan_selects_only_coordinator_weights() {
        ...
        assert_eq!(plan.role_counts.get("DenseMlp"), None);
        assert!(plan.role_counts.get("SharedExpert").copied().unwrap_or(0) > 0);
        assert_eq!(
    ...
    #[test]
    fn coordinator_residency_selects_native_dspark_weights_and_scales_but_not_experts() {
        let dspark_layer = ModelFacts::default().num_hidden_layers as u32;
        let catalog = TensorCatalog {
            ...
            tensors: vec![
                TensorInfo {
                    name: "mtp.0.main_proj.weight".to_owned(),
                    ...
                    layer_id: Some(dspark_layer),
                    expert_id: None,
                    is_quantization_metadata: false,
                },
                TensorInfo {
                    name: "mtp.0.main_proj.scale".to_owned(),
                    file: "model.safetensors".to_owned(),
                    dtype: DType::F32,
                    shape: vec![1],
                    byte_offset: 2,
                ...
                TensorInfo {
                    name: "mtp.0.ffn.experts.0.w1.weight".to_owned(),
                    file: "model.safetensors".to_owned(),
                    dtype: DType::U8,
                    shape: vec![1],
                    byte_offset: 6,
                ...
                TensorInfo {
                    name: "mtp.0.ffn.experts.1.w1.weight".to_owned(),
                    file: "model.safetensors".to_owned(),
                    dtype: DType::U8,
                    shape: vec![1],
                    byte_offset: 7,
        ...
        assert_eq!(plan.selected_tensor_count, 2);
        assert_eq!(plan.selected_tensor_bytes, 6);
        assert_eq!(plan.role_counts.get("Dspark"), Some(&2));
        assert_eq!(plan.skipped_dspark_tensors, 1);
        assert_eq!(plan.skipped_dspark_bytes, 1);
        assert_eq!(plan.skipped_mtp_tensors, 0);
        assert_eq!(plan.skipped_mtp_bytes, 0);
