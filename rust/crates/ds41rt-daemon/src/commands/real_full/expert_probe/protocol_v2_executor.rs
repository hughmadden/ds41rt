...
use ds41rt_core::{
    owner_for_expert, plan_completion_first_routes, CompletionRoutePlanEntry, DType,
    PlacementPolicy, TensorCatalog, TensorInfo, TensorRole, EXPERT_HOSTS,
};
...
use crate::commands::real_full::sparse_mlp::route::{
    ...
    cuda_route_validation_enabled, execute_native_flash_route_bf16_cached,
    execute_native_flash_route_nvfp4_cached, execute_nvfp4_route_cached,
    ...
    preload_bf16_route_projection_group_cache, preload_native_flash_projection_cuda_cache,
    preload_routed_quant_projection_cuda_cache, preload_routed_quant_projection_host_cache,
    preload_routed_quant_projection_scalar_cache_parallel, reduce_mapped_route_shards_cached,
...
pub(crate) const REAL_NVFP4_CUDA_REFERENCE_KERNELS_ENV: &str =
    "DS41RT_REAL_FULL_CUDA_REFERENCE_KERNELS";
...
impl RealNvfp4ProtocolV2Executor {
    ...
    fn native_flash_tp4_shard(&self) -> Result<ExpertIntermediateShard> {
        anyhow::ensure!(
            native_flash_expert_catalog(&self.catalog),
            "native Flash TP=4 shard requested for a non-native catalog"
        );
        let shard = self.intermediate_shard.context(
            "DeepSeek V4 Flash native expert execution requires an intermediate TP=4 shard",
        )?;
        anyhow::ensure!(
            shard.count == 4,
            "DeepSeek V4 Flash native expert execution requires TP=4, got TP={}",
            shard.count
        );
        Ok(shard)
    }
    ...
    pub(crate) fn with_owner_reduction(
        ...
        anyhow::ensure!(
            !native_flash_expert_catalog(&self.catalog),
            "DeepSeek V4 Flash TP=4 experts return raw rank-local partials and cannot enable Spark-side owner reduction"
        );
    ...
    pub(crate) fn resident_preload_plan(&self) -> Result<RealNvfp4ResidentPreloadPlan> {
        if native_flash_expert_catalog(&self.catalog) {
            return native_flash_resident_preload_plan(
                &self.catalog,
                Some(self.native_flash_tp4_shard()?),
            );
        }
    ...
    fn preload_assigned_projections_with_cuda(
        ...
        if native_flash_expert_catalog(&self.catalog) {
            return self.preload_native_flash_assigned_projections(preload_cuda);
        }
    ...
    fn preload_native_flash_assigned_projections(
        &self,
        preload_cuda: bool,
    ) -> Result<RealNvfp4ResidentPreloadStats> {
        let preload_started = Instant::now();
        let shard = self.native_flash_tp4_shard()?;
        let plan = native_flash_resident_preload_plan(&self.catalog, Some(shard))?;
        let mut stats = RealNvfp4ResidentPreloadStats {
            projection_groups: plan.projection_groups,
            layers: plan.layers,
            experts: plan.experts,
            weight_bytes: plan.weight_bytes,
            quant_metadata_bytes: plan
                .weight_scale_bytes
                .checked_add(plan.scalar_metadata_bytes)
                .context("native Flash preload metadata byte count overflow")?,
            ..Default::default()
        };
        if !preload_cuda {
            return Ok(stats);
        ...
        let mut route_cache = self
            .route_caches
            .first()
            .context("native Flash executor is missing execution lane zero")?
            .lock()
            .map_err(|_| anyhow::anyhow!("native Flash route tensor cache is poisoned"))?;
        let cuda_preload =
            preload_native_flash_projection_cuda_cache(&self.catalog, &mut route_cache, shard)?;
        stats.cuda_reference_enabled = true;
        stats.cuda_projection_groups = cuda_preload.projection_groups;
        stats.cuda_weight_bytes = cuda_preload.weight_bytes;
        stats.cuda_weight_scale_bytes = cuda_preload.weight_scale_bytes;
        ...
        validate_native_flash_resident_preload(&plan, &stats)?;
        ...
        for execution_lane in 1..active_lanes {
            ...
            eprintln!(
                "ds4_flash_execution_lane_connect execution_lane={} elapsed_ms={:.3}",
                execution_lane,
                lane_started.elapsed().as_secs_f64() * 1_000.0,
            );
            let mut destination = self
                .route_caches
                .get(execution_lane)
                .context("execution lane exceeds native Flash route-cache capacity")?
                .lock()
                .map_err(|_| {
                    anyhow::anyhow!(
                        "native Flash route tensor cache lane {execution_lane} is poisoned"
                    )
                })?;
            *destination = auxiliary_cache;
        }
        eprintln!(
            "ds4_flash_preload_complete layers={} experts={} projections={} resident_weight_bytes={} resident_scale_bytes={} lanes={} lane_fork_ms={:.3} total_ms={:.3}",
            stats.layers,
            stats.experts,
            stats.cuda_projection_groups,
            stats.cuda_weight_bytes,
            stats.cuda_weight_scale_bytes,
            active_lanes,
            lane_fork_started.elapsed().as_secs_f64() * 1_000.0,
            preload_started.elapsed().as_secs_f64() * 1_000.0,
        );
...
fn validate_native_flash_resident_preload(
    plan: &RealNvfp4ResidentPreloadPlan,
    resident: &RealNvfp4ResidentPreloadStats,
) -> Result<()> {
    let expected_metadata_bytes = plan
        .weight_scale_bytes
        .checked_add(plan.scalar_metadata_bytes)
        .context("DeepSeek V4 resident metadata byte count overflow")?;
    anyhow::ensure!(
        plan.startup_required,
        "DeepSeek V4 resident preload cannot validate an optional startup plan"
    );
    anyhow::ensure!(
        resident.projection_groups == plan.projection_groups
            && resident.layers == plan.layers
            && resident.experts == plan.experts,
        "DeepSeek V4 resident preload topology differs from plan: projections={}/{} layers={}/{} experts={}/{}",
        resident.projection_groups,
        plan.projection_groups,
        resident.layers,
        plan.layers,
        resident.experts,
        plan.experts,
    ...
    anyhow::ensure!(
        resident.weight_bytes == plan.weight_bytes,
        "DeepSeek V4 preload weight accounting differs from strict-TP4 plan: actual={} planned={}",
        resident.weight_bytes,
        plan.weight_bytes,
    );
    anyhow::ensure!(
        resident.quant_metadata_bytes == expected_metadata_bytes,
        "DeepSeek V4 preload metadata accounting differs from strict-TP4 plan: actual={} planned={}",
        resident.quant_metadata_bytes,
        expected_metadata_bytes,
    );
    if resident.cuda_reference_enabled {
        anyhow::ensure!(
            resident.cuda_projection_groups == plan.projection_groups,
            "DeepSeek V4 CUDA resident projection count differs from strict-TP4 plan: actual={} planned={}",
            resident.cuda_projection_groups,
            plan.projection_groups,
        );
        anyhow::ensure!(
            resident.cuda_projection_entries == plan.projection_groups
                && resident.cuda_projection_uploads == plan.projection_groups,
            "DeepSeek V4 CUDA resident projection table differs from strict-TP4 plan: entries={} uploads={} planned={}",
            resident.cuda_projection_entries,
            resident.cuda_projection_uploads,
            plan.projection_groups,
        );
        anyhow::ensure!(
            resident.cuda_weight_bytes == plan.weight_bytes,
            "DeepSeek V4 CUDA resident weight bytes differ from strict-TP4 plan: actual={} planned={}",
            resident.cuda_weight_bytes,
            plan.weight_bytes,
        );
        anyhow::ensure!(
            resident.cuda_weight_scale_bytes == expected_metadata_bytes,
            "DeepSeek V4 CUDA resident metadata bytes differ from strict-TP4 plan: actual={} planned={}",
            resident.cuda_weight_scale_bytes,
            expected_metadata_bytes,
        );
        anyhow::ensure!(
            resident.cuda_managed_projection_entries == 0
                && !resident.cuda_managed_projection_allocations_enabled,
            "DeepSeek V4 CUDA resident preload created {} managed projections (managed allocations enabled={}); strict-TP4 serving requires direct final-format slabs only",
            resident.cuda_managed_projection_entries,
            resident.cuda_managed_projection_allocations_enabled,
        );
    }
    Ok(())
...
impl RealNvfp4ProtocolV2Executor {
    ...
    fn execute_protocol_v2_unordered(
        ...
        // Stream frames are a transport envelope, not an expert backend. Keep
        // them ahead of backend selection so native Flash can accept chunked
        // ingress too. The fallback state restores logical row order and only
        // invokes this function again after the complete batch is assembled;
        // therefore one native TP4 AOT launch still covers the whole batch.
        ...
        if native_flash_expert_catalog(&self.catalog) {
            ...
            let _shard = self.native_flash_tp4_shard()?;
            let prequantized_hidden =
                request.header.hidden_dtype == ExpertV2Dtype::Nvfp4E2m1Fp8E4m3;
            anyhow::ensure!(
                (request.header.hidden_dtype == ExpertV2Dtype::Bf16 || prequantized_hidden)
                    && requested_response_dtype(request) == ExpertV2Dtype::Bf16,
                "native Flash AOT requires BF16 or NVFP4 ingress and a BF16 response"
            );
            let row_count = request.header.row_count as usize;
            let expected_routes = row_count
                .checked_mul(self.catalog.facts.top_k)
                .context("native Flash Protocol V2 route count overflow")?;
            anyhow::ensure!(
                (1..=2048).contains(&row_count)
                    && request.header.hidden_dim as usize == self.catalog.facts.hidden_size
                    && request.header.route_count as usize == expected_routes,
                "native Flash AOT requires rows=1..=2048 hidden={} and {} routes per row, got rows={} hidden={} routes={}",
                self.catalog.facts.hidden_size,
                self.catalog.facts.top_k,
                request.header.row_count,
                request.header.hidden_dim,
                request.header.route_count,
            );
            anyhow::ensure!(
                !request.spark_row_sharded_reduction_enabled() || request.spark_reduction_enabled(),
                "native Flash row-sharded reduction requires Spark reduction"
            );
            anyhow::ensure!(
                !request.spark_reduction_enabled() || request.spark_row_sharded_reduction_enabled(),
                "native Flash distributed reduction requires row-sharded Spark exchange"
            );
            let output_row_bytes =
                self.catalog.facts.hidden_size * ExpertV2Dtype::Bf16.bytes_per_element();
            let input_row_bytes = request
                .header
                .hidden_dtype
                .row_bytes(self.catalog.facts.hidden_size)?;
            anyhow::ensure!(
                request.header.hidden_row_stride_bytes as usize == input_row_bytes,
                "native Flash AOT requires compact {:?} rows with stride {input_row_bytes}, got {}",
                request.header.hidden_dtype,
                request.header.hidden_row_stride_bytes
            );
            let layer_id = request.header.layer_id as usize;
            anyhow::ensure!(
                layer_id < self.catalog.facts.total_transformer_blocks(),
                "native Flash layer {layer_id} exceeds checkpoint depth {}",
                self.catalog.facts.total_transformer_blocks()
            );
            if let Some(expected_layer) = self.real_layer {
                anyhow::ensure!(
                    layer_id == expected_layer,
                    "native Flash executor pinned to layer {expected_layer}, got layer {layer_id}"
                );
            }
            let mut topk_ids = Vec::with_capacity(expected_routes);
            let mut topk_weights = Vec::with_capacity(expected_routes);
            for row_index in 0..row_count {
                let row = request.row(row_index)?;
                let expected_offset = row_index * self.catalog.facts.top_k;
                anyhow::ensure!(
                    row.route_offset as usize == expected_offset
                        && row.route_count as usize == self.catalog.facts.top_k,
                    "native Flash row {row_index} has route range {}..{}, expected {expected_offset}..{}",
                    row.route_offset,
                    row.route_offset + row.route_count,
                    expected_offset + self.catalog.facts.top_k,
                );
                for route_index in expected_offset..expected_offset + self.catalog.facts.top_k {
                    let route = request.route(route_index)?;
                    anyhow::ensure!(
                        route.row_index as usize == row_index,
                        "native Flash route {route_index} belongs to row {}, expected {row_index}",
                        route.row_index
                    );
                    anyhow::ensure!(
                        (route.expert_id as usize) < self.catalog.facts.routed_experts,
                        "native Flash route {route_index} expert {} exceeds routed expert count {}",
                        route.expert_id,
                        self.catalog.facts.routed_experts
                    );
                    anyhow::ensure!(
                        route.gate_weight.is_finite(),
                        "native Flash route {route_index} has non-finite gate weight {}",
                        route.gate_weight
                    );
                    topk_ids.push(route.expert_id);
                    topk_weights.push(route.gate_weight);
                }
            }
            let retain_device_output = request_device_payload.is_some()
                && !request.debug_checksum_enabled()
                && response_emit.is_some();
            let response_device_target = if retain_device_output {
                regular_response_device_target(request, request_device_payload, output_row_bytes)?
            } else {
                None
            };
            ...
            let execute = if prequantized_hidden {
                execute_native_flash_route_nvfp4_cached
            } else {
            ...
            let completed_rows = execution.completed_rows.len();
            let reduction_follower = execution.reduction_follower;
            let kernel_backend = execution.kernel_backend;
            let response_started = Instant::now();
            let response = emit_complete_route_stream_chunk(
                request,
                RouteNvfp4IngressStreamChunk {
                    completed_rows: execution.completed_rows,
                    output: execution.output_bf16,
                    device_output: execution.output_device,
                    reduction_follower: execution.reduction_follower,
                    complete: true,
                },
                ExpertV2Dtype::Bf16,
                output_row_bytes,
            ...
            let response_build_ms = elapsed_ms(response_started);
            if timing_enabled {
                eprintln!(
                    "real_nvfp4_protocol_v2_native_flash_timing execution_lane={} request_id={} layer_id={} rows={} routes={} completed_rows={} spark_reduction={} reduction_follower={} retain_device_output={} backend={} request_wire_bytes={} plan_ms={:.3} route_cache_lock_ms={:.3} execute_ms={:.3} response_build_ms={:.3} total_ms={:.3}",
                    execution_lane,
                    request.header.request_id,
                    request.header.layer_id,
                    request.header.row_count,
...
fn native_flash_expert_catalog(catalog: &TensorCatalog) -> bool {
    let native_fp4 = catalog.facts.quantization_recipe == "deepseek_v4_native_fp4_fp8_mixed_v1";
    let exl3 = is_deepseek_v4_exl3_recipe(&catalog.facts.quantization_recipe);
    let supported_variant = (native_fp4
        && catalog.facts.variant == ds41rt_core::ModelVariant::Flash)
        || (exl3
            && matches!(
                catalog.facts.variant,
                ds41rt_core::ModelVariant::Flash | ds41rt_core::ModelVariant::Pro
            ));
    catalog.facts.model_type == "deepseek_v4"
        && supported_variant
        && catalog.facts.expert_dtype == "fp4"
        && (native_fp4 || exl3)
        // ModelFacts::default() is also used by compact legacy fixtures. The
        // native backend must be selected from the checkpoint schema, not from
        // defaults alone. Native V4 and legacy EXL3 use
        // `layers.N.ffn.experts.E.*`; canonical GPTQModel EXL3 uses
        // `model.layers.N.mlp.experts.E.*`.
        && catalog.tensors.iter().any(|tensor| {
            if tensor.role != TensorRole::RoutedExpert {
                return false;
            }
            let native_layout = tensor.name.starts_with("layers.")
                && tensor.name.contains(".ffn.experts.");
            let gptqmodel_layout = tensor.name.starts_with("model.layers.")
                && tensor.name.contains(".mlp.experts.");
            (native_fp4 && native_layout && tensor.name.ends_with(".weight"))
                || (exl3
                    && (native_layout || gptqmodel_layout)
                    && tensor.name.ends_with(".trellis"))
        })
}

fn native_flash_resident_preload_plan(
    catalog: &TensorCatalog,
    intermediate_shard: Option<ExpertIntermediateShard>,
) -> Result<RealNvfp4ResidentPreloadPlan> {
    let shard = intermediate_shard
        .context("DeepSeek V4 Flash native expert residency requires an intermediate TP=4 shard")?;
    anyhow::ensure!(
        shard.count == 4,
        "DeepSeek V4 Flash native expert residency requires TP=4, got TP={}",
        shard.count
    );

    let layers = catalog.facts.total_transformer_blocks();
    let experts_per_layer = catalog.facts.routed_experts;
    anyhow::ensure!(layers > 0, "DeepSeek V4 Flash catalog has no expert layers");
    anyhow::ensure!(
        experts_per_layer > 0,
        "DeepSeek V4 Flash catalog has no routed experts"
    );

    let expert_count = layers
        .checked_mul(experts_per_layer)
        .context("DeepSeek V4 Flash resident expert count overflow")?;
    let projection_groups = expert_count
        .checked_mul(3)
        .context("DeepSeek V4 Flash resident projection count overflow")?;
    let mut plan = RealNvfp4ResidentPreloadPlan {
        startup_required: true,
        projection_groups,
        layers,
        experts: expert_count,
        complete_expert_projection_sets: expert_count,
        ..Default::default()
    };
    ...
    if is_deepseek_v4_mixed_exl3_recipe(&catalog.facts.quantization_recipe) {
        anyhow::ensure!(
            catalog.facts.variant == ds41rt_core::ModelVariant::Flash,
            "mixed K2/K3 EXL3 resident planning currently requires DeepSeek V4 Flash"
        );
        anyhow::ensure!(
            catalog.facts.moe_intermediate_size % shard.count == 0,
            "mixed EXL3 intermediate size {} is not divisible by TP{}",
        ...
        plan.weight_bytes = (layers as u64)
            .checked_mul(geometry.resident_weight_bytes())
            .context("DeepSeek V4 EXL3 resident trellis byte count overflow")?;
        plan.weight_scale_bytes = (layers as u64)
            .checked_mul(geometry.resident_rotation_bytes())
            .context("DeepSeek V4 EXL3 resident rotation byte count overflow")?;
        plan.scalar_metadata_bytes = (layers as u64)
            .checked_mul(geometry.scalar_metadata_bytes)
            .context("DeepSeek V4 EXL3 metadata byte count overflow")?;
    } else {
        for layer_id in 0..layers {
            for expert_id in 0..experts_per_layer {
                let expert = native_fp4_expert(catalog, layer_id, expert_id)
                    .with_context(|| {
                        format!(
                            "planning DeepSeek V4 Flash native layer {layer_id} expert {expert_id}"
                        )
                    })?
                    .tp_shard(shard.rank)?;
                for projection in [expert.up, expert.gate, expert.down] {
                    plan.weight_bytes = plan
                        .weight_bytes
                        .checked_add(projection.weight.source_bytes()?)
                        .context("DeepSeek V4 Flash resident weight byte count overflow")?;
                    plan.weight_scale_bytes = plan
                        .weight_scale_bytes
                        .checked_add(projection.scale.source_bytes()?)
                        .context("DeepSeek V4 Flash resident scale byte count overflow")?;
                }
            }
        }
        // SparkInfer consumes one synthetic global scale per expert for W13 and W2.
        plan.scalar_metadata_bytes = (expert_count as u64)
            .checked_mul(2 * std::mem::size_of::<f32>() as u64)
            .context("DeepSeek V4 Flash global-scale byte count overflow")?;
    }
    Ok(plan)
}
...
mod tests {
    use super::{
        completion_slices_for_all_rows, native_flash_expert_catalog,
        native_flash_resident_preload_plan, parse_real_nvfp4_protocol_v2_packed_direct_max_rows,
        real_nvfp4_cuda_reference_kernels_enabled, regular_response_device_target,
        validate_completion_slices, validate_native_flash_resident_preload,
        RealNvfp4ProtocolV2Executor, RealNvfp4ResidentPreloadStats, StreamedIngressState,
        REAL_NVFP4_PROTOCOL_V2_EXECUTOR,
    };
    ...
    use ds41rt_core::{
        ...
        TensorInfo, TensorRole, DS4_PRO_NUM_HIDDEN_LAYERS, DS4_PRO_ROUTED_EXPERTS, DS4_PRO_TOP_K,
        EXPERT_HOSTS,
    };
    use ds41rt_ffi::Ds41rtDeviceBuffer;
    use ds41rt_loader::{
        DEEPSEEK_V4_EXL3_RECIPE, DEEPSEEK_V4_EXL3_RECIPE_MIXED_K2_K3_V1, DEEPSEEK_V4_EXL3_RECIPE_V2,
    };
    use ds41rt_transport::{
    ...
    fn regular_device_response_target_follows_indexed_prefix() {
        ...
        let request = ds41rt_transport::ExpertProtocolV2RequestView::parse(&frame).unwrap();
        ...
        let payload = ProtocolV2RequestDevicePayload {
            ...
            response_slot: Some(Ds41rtDeviceBuffer {
    ...
    fn streamed_ingress_state_restores_logical_hidden_row_order() {
        ...
        let plan_view = ds41rt_transport::ExpertProtocolV2RequestView::parse(&plan_frame).unwrap();
        ...
        assert!(!state
            .accept_data(
                &ds41rt_transport::ExpertProtocolV2RequestView::parse(&first_frame).unwrap()
            )
            .unwrap());
        assert!(state
            .accept_data(
                &ds41rt_transport::ExpertProtocolV2RequestView::parse(&second_frame).unwrap()
            )
            .unwrap());
    ...
    fn real_nvfp4_protocol_v2_streaming_fallback_emits_one_complete_response() {
        ...
        let view = ds41rt_transport::ExpertProtocolV2RequestView::parse(&frame).unwrap();
    ...
    fn real_nvfp4_protocol_v2_streamed_ingress_matches_regular_execution() {
        ...
        let plan_view = ds41rt_transport::ExpertProtocolV2RequestView::parse(&plan_frame).unwrap();
        let data_view = ds41rt_transport::ExpertProtocolV2RequestView::parse(&data_frame).unwrap();
    ...
    fn real_nvfp4_protocol_v2_executor_preloads_cuda_projection_cache_when_available() {
        ...
        let preload = match executor.preload_assigned_projections_with_cuda(true) {
            ...
            Err(error) => {
                ...
                if message.contains("requires DS41RT_NATIVE_LIB")
    ...
    fn native_flash_plan_catalog() -> TensorCatalog {
        ...
        facts.moe_intermediate_size = 128;
        let mut tensors = Vec::new();
        for expert_id in 0..facts.routed_experts {
            for stem in ["w1", "w2", "w3"] {
                let (rows, columns) = if stem == "w2" {
                    (facts.hidden_size, facts.moe_intermediate_size)
                } else {
                    (facts.moe_intermediate_size, facts.hidden_size)
                };
                for (suffix, dtype, width, is_quantization_metadata) in [
                    ("weight", DType::I8, columns / 2, false),
                    ("scale", DType::F8E8M0, columns / 32, true),
                ] {
                    let name = format!("layers.0.ffn.experts.{expert_id}.{stem}.{suffix}");
        ...
        TensorCatalog {
            model_id: "deepseek-ai/test-flash-native-plan".to_owned(),
            snapshot_path: "/tmp/test-flash-native-plan".to_owned(),
            facts,
            tensors,
        }
    }
    ...
    fn exl3_flash_plan_catalog_with_experts(routed_experts: usize) -> TensorCatalog {
        ...
        for expert_id in 0..facts.routed_experts {
            for stem in ["w1", "w2", "w3"] {
                let (input_features, output_features) = if stem == "w2" {
                    (facts.moe_intermediate_size, facts.hidden_size)
                } else {
                    (facts.hidden_size, facts.moe_intermediate_size)
                };
                for (suffix, dtype, shape, bytes, metadata) in [
                    (
                        "trellis",
                        DType::I16,
                    ...
                    ("mcg", DType::I32, vec![], 4, true),
                ] {
                    tensors.push(TensorInfo {
                        name: format!("layers.0.ffn.experts.{expert_id}.{stem}.{suffix}"),
                        file: "model.safetensors".to_owned(),
                        dtype,
                        shape,
                        byte_offset: 0,
        ...
        tensors.sort_by(|left, right| left.name.cmp(&right.name));
        TensorCatalog {
            model_id: "deepseek-ai/test-flash-exl3-plan".to_owned(),
            snapshot_path: "/tmp/test-flash-exl3-plan".to_owned(),
            facts,
            tensors,
        }
    }

    fn exl3_pro_plan_catalog() -> TensorCatalog {
        let mut catalog = exl3_flash_plan_catalog();
        catalog.model_id = "deepseek-ai/test-pro-exl3-plan".to_owned();
        catalog.facts.variant = ds41rt_core::ModelVariant::Pro;
        catalog.facts.hidden_size = 7_168;
        catalog.facts.moe_intermediate_size = 3_072;
        catalog
    }

    fn gptqmodel_exl3_flash_plan_catalog() -> TensorCatalog {
        let mut catalog = exl3_flash_plan_catalog();
        catalog.model_id = "tpurtell/test-flash-gptqmodel-exl3-plan".to_owned();
        for tensor in &mut catalog.tensors {
            tensor.name = tensor
                .name
                .replace("layers.0.ffn.experts.", "model.layers.0.mlp.experts.")
    ...
    fn native_flash_request() -> ExpertProtocolV2Request {
        let catalog = native_flash_plan_catalog();
        let routes = (0..catalog.facts.top_k)
            .map(|route_index| ExpertProtocolV2RouteEntry {
                row_index: 0,
                expert_id: (route_index % catalog.facts.routed_experts) as u32,
                gate_weight: 1.0 / catalog.facts.top_k as f32,
            })
            .collect::<Vec<_>>();
        ExpertProtocolV2Request::new(
            ...
            0xD54F,
            0,
            catalog.facts.hidden_size as u32,
            ExpertV2Dtype::Bf16,
            vec![ExpertProtocolV2RowDescriptor {
                row_id: 0,
                source_kind: ExpertV2SourceKind::Decode,
                source_request_id: 73,
                token_position: 0,
                route_offset: 0,
                route_count: catalog.facts.top_k as u32,
            }],
            routes,
            vec![0; catalog.facts.hidden_size * ExpertV2Dtype::Bf16.bytes_per_element()],
        )
        .unwrap()
    }

    #[test]
    fn native_flash_resident_plan_is_rank_local_tp4_not_expert_parallel() {
        let catalog = native_flash_plan_catalog();
        for rank in 0..4 {
            let plan = native_flash_resident_preload_plan(
                &catalog,
                Some(ExpertIntermediateShard::new(4, rank).unwrap()),
            )
            .unwrap();
            ...
            assert_eq!(plan.complete_expert_projection_sets, 2);
            assert_eq!(plan.incomplete_expert_projection_sets, 0);
            assert_eq!(plan.weight_bytes, 6_144);
            assert_eq!(plan.weight_scale_bytes, 384);
            assert_eq!(plan.scalar_metadata_bytes, 16);
            assert_eq!(plan.missing_metadata_tensors, 0);
        }
    }

    #[test]
    fn exl3_flash_resident_plan_is_rank_local_k2_without_widening() {
        let catalog = exl3_flash_plan_catalog();
        assert!(native_flash_expert_catalog(&catalog));
        for rank in 0..4 {
            let plan = native_flash_resident_preload_plan(
                &catalog,
                Some(ExpertIntermediateShard::new(4, rank).unwrap()),
            ...
            assert_eq!(plan.projection_groups, 6);
            assert_eq!(plan.complete_expert_projection_sets, 2);
            assert_eq!(plan.weight_bytes, 6 * 524_288);
            assert_eq!(plan.weight_scale_bytes, 6 * 9_216);
            assert_eq!(plan.scalar_metadata_bytes, 4_132);
        }
    }

    #[test]
    fn mixed_exl3_flash_resident_plan_accounts_exact_k2_k3_tiers() {
        let mut catalog = exl3_flash_plan_catalog();
        catalog.facts.quantization_recipe = DEEPSEEK_V4_EXL3_RECIPE_MIXED_K2_K3_V1.to_owned();
        for tensor in &mut catalog.tensors {
            if tensor.expert_id == Some(1) && tensor.name.ends_with(".trellis") {
    ...
    #[test]
    fn mixed_recipe_uniform_layer_uses_uniform_slab_metadata_plan() {
        let mut catalog = exl3_flash_plan_catalog();
        catalog.facts.quantization_recipe = DEEPSEEK_V4_EXL3_RECIPE_MIXED_K2_K3_V1.to_owned();
    ...
    #[test]
    fn historical_exl3_flash_recipe_uses_the_shared_k2_tensor_backend() {
        let mut catalog = exl3_flash_plan_catalog();
        catalog.facts.quantization_recipe = DEEPSEEK_V4_EXL3_RECIPE_V2.to_owned();
        assert!(native_flash_expert_catalog(&catalog));
        let plan = native_flash_resident_preload_plan(
            &catalog,
            Some(ExpertIntermediateShard::new(4, 0).unwrap()),
    ...
    #[test]
    fn gptqmodel_exl3_flash_selects_rank_local_k2_backend() {
        let catalog = gptqmodel_exl3_flash_plan_catalog();
        assert!(native_flash_expert_catalog(&catalog));
        let expert = ds41rt_loader::exl3_expert(&catalog, 0, 0).unwrap();
        assert!(expert
            .gate
            .trellis
            .name
            .starts_with("model.layers.0.mlp.experts.0.gate_proj."));
        for rank in 0..4 {
            let plan = native_flash_resident_preload_plan(
                &catalog,
                Some(ExpertIntermediateShard::new(4, rank).unwrap()),
    ...
    #[test]
    fn exl3_pro_resident_plan_is_rank_local_k2_without_widening() {
        let catalog = exl3_pro_plan_catalog();
        assert!(native_flash_expert_catalog(&catalog));
        for rank in 0..4 {
            let plan = native_flash_resident_preload_plan(
                &catalog,
            ...
            assert_eq!(plan.experts, 2);
            assert_eq!(plan.projection_groups, 6);
            assert_eq!(plan.weight_bytes, 6 * 1_376_256);
            assert_eq!(plan.weight_scale_bytes, 95_232);
            assert_eq!(plan.scalar_metadata_bytes, 4_132);
        }

        let mut native_fp4_pro = native_flash_plan_catalog();
        native_fp4_pro.facts.variant = ds41rt_core::ModelVariant::Pro;
        assert!(!native_flash_expert_catalog(&native_fp4_pro));
    }

    #[test]
    fn production_pro_exl3_resident_plan_is_exactly_102639273216_bytes_per_rank() {
        let mut catalog = exl3_pro_plan_catalog();
        catalog.facts.num_hidden_layers = DS4_PRO_NUM_HIDDEN_LAYERS;
        catalog.facts.dspark_target_layer_ids = vec![58, 59, 60];
        catalog.facts.routed_experts = DS4_PRO_ROUTED_EXPERTS;
        catalog.facts.top_k = DS4_PRO_TOP_K;
        ...
        for rank in 0..4 {
            ...
            .unwrap();
            assert_eq!(plan.layers, 64);
            assert_eq!(plan.experts, 24_576);
            assert_eq!(plan.projection_groups, 73_728);
            assert_eq!(plan.weight_bytes, 101_468_602_368);
            assert_eq!(plan.weight_scale_bytes, 1_170_210_816);
            assert_eq!(plan.scalar_metadata_bytes, 460_032);
            assert_eq!(
                plan.weight_bytes + plan.weight_scale_bytes + plan.scalar_metadata_bytes,
                102_639_273_216
            );
        }
    }
    ...
    #[test]
    fn deepseek_startup_rejects_residency_that_differs_from_tp4_plan() {
        let plan = native_flash_resident_preload_plan(
            &exl3_pro_plan_catalog(),
            Some(ExpertIntermediateShard::new(4, 0).unwrap()),
        )
        .unwrap();
        let metadata_bytes = plan.weight_scale_bytes + plan.scalar_metadata_bytes;
        let resident = RealNvfp4ResidentPreloadStats {
            projection_groups: plan.projection_groups,
            layers: plan.layers,
            experts: plan.experts,
            weight_bytes: plan.weight_bytes,
            quant_metadata_bytes: metadata_bytes,
            cuda_reference_enabled: true,
            cuda_projection_groups: plan.projection_groups,
            cuda_projection_entries: plan.projection_groups,
            cuda_projection_uploads: plan.projection_groups,
            cuda_weight_bytes: plan.weight_bytes,
            cuda_weight_scale_bytes: metadata_bytes,
            ..Default::default()
        };
        validate_native_flash_resident_preload(&plan, &resident).unwrap();

        let mut widened = resident;
        widened.cuda_weight_bytes += 1;
        assert!(validate_native_flash_resident_preload(&plan, &widened)
            .unwrap_err()
            .to_string()
            .contains("CUDA resident weight bytes differ"));

        let mut incomplete = resident;
        incomplete.cuda_projection_groups -= 1;
        assert!(validate_native_flash_resident_preload(&plan, &incomplete)
            .unwrap_err()
            .to_string()
            .contains("CUDA resident projection count differs"));

        let mut managed = resident;
        managed.cuda_managed_projection_entries = 1;
        managed.cuda_managed_projection_allocations_enabled = true;
        assert!(validate_native_flash_resident_preload(&plan, &managed)
            .unwrap_err()
            .to_string()
            .contains("direct final-format slabs only"));
    }

    #[test]
    fn native_flash_resident_plan_rejects_missing_tp4_identity() {
        let error =
            native_flash_resident_preload_plan(&native_flash_plan_catalog(), None).unwrap_err();
        assert!(error
            .to_string()
            .contains("requires an intermediate TP=4 shard"));
    }

    #[test]
    fn native_flash_executor_requires_row_sharded_distributed_reduction() {
        let executor = RealNvfp4ProtocolV2Executor::new(native_flash_plan_catalog(), None, None)
            .with_intermediate_shard(ExpertIntermediateShard::new(4, 2).unwrap());
        let request = native_flash_request().with_spark_reduction();

        let error = execute_request(&executor, &request).unwrap_err();

        assert!(error
            .to_string()
            .contains("requires row-sharded Spark exchange"));
    }

    #[test]
    fn native_flash_executor_accepts_chunked_ingress_plan_before_aot_selection() {
        let executor = RealNvfp4ProtocolV2Executor::new(native_flash_plan_catalog(), None, None)
            .with_intermediate_shard(ExpertIntermediateShard::new(4, 2).unwrap());
        let regular = native_flash_request();
        let plan = ExpertProtocolV2StreamPlan::new(
            1,
            regular.routes.len() as u32,
            vec![0],
            (0..2)
                .map(|expert_id| ExpertProtocolV2StreamRouteGroup {
                    ready_after_rows: 1,
                    route_indices: regular
                        .routes
                        .iter()
                        .enumerate()
                        .filter_map(|(route_index, route)| {
                            (route.expert_id == expert_id).then_some(route_index as u32)
                        })
                        .collect(),
                    completed_rows: (expert_id == 1).then_some(0).into_iter().collect(),
                })
                .collect(),
        )
        .unwrap();
        let request = ExpertProtocolV2Request::new_stream_plan(
            regular.header.request_id,
            regular.header.placement_version,
            regular.header.layer_id,
            regular.header.hidden_dim,
        ...
        let acknowledgement = execute_request(&executor, &request).unwrap();

        assert_eq!(acknowledgement.header.row_count, 0);
        assert!(acknowledgement.partial_output_payload.is_empty());
        assert!(executor.streamed_ingress_by_lane[0]
            .lock()
            .unwrap()
            .is_some());
    }
    ...
    #[test]
    fn native_flash_executor_rejects_owner_reduction_configuration() {
        let executor = RealNvfp4ProtocolV2Executor::new(native_flash_plan_catalog(), None, None)
            .with_intermediate_shard(ExpertIntermediateShard::new(4, 1).unwrap());
        let config = SparkExpertOwnerReductionConfig {
            shard: ExpertIntermediateShard::new(4, 1).unwrap(),
            dtype: ExpertIntermediateReductionDtype::Bf16,
            max_rows: 2048,
            peers: vec![
                (0, "127.0.0.1:9700".parse().unwrap()),
                (2, "127.0.0.1:9702".parse().unwrap()),
                (3, "127.0.0.1:9703".parse().unwrap()),
            ],
        };

        let error = match executor.with_owner_reduction(config) {
            Ok(_) => panic!("native Flash owner reduction unexpectedly configured"),
            Err(error) => error,
        };

        assert!(error.to_string().contains("raw rank-local partials"));
    }
    ...
    fn real_checkpoint_host_batch_request(
        ...
        let batch = ExpertBatch {
            ...
            routed_experts: ModelFacts::default().routed_experts,
            quantization_recipe: ModelFacts::default().quantization_recipe,
            rows: vec![
                ds41rt_core::ExpertBatchRow {
                    row_id: 0,
    ...
    fn real_checkpoint_topk_host_batch_set_fixture(
        ...
        let batch = ExpertBatch {
            ...
            routed_experts: ModelFacts::default().routed_experts,
            quantization_recipe: ModelFacts::default().quantization_recipe,
            rows: vec![ds41rt_core::ExpertBatchRow {
                row_id: 0,
                source_kind: RowSourceKind::PrefillChunk,
    ...
    fn local_host_partial_bf16_accumulation(
        ...
        global_hidden: &[u8],
        owner_lookup: &ExpertOwnerLookup,
    ) -> anyhow::Result<ds41rt_core::ExpertHostBatchSetAccumulation> {
    ...
    fn execute_request(
        ...
        request: &ExpertProtocolV2Request,
    ) -> anyhow::Result<ds41rt_transport::ExpertProtocolV2Response> {
        let frame = request.encode().unwrap();
        let view = ds41rt_transport::ExpertProtocolV2RequestView::parse(&frame).unwrap();
    ...
    fn real_catalog_path() -> PathBuf {
        repo_root().join(".ds41rt-cache/model-artifacts/diagnostic/model_catalog.json")
    }
    ...
    fn load_real_owner_lookup_or_skip() -> Option<ExpertOwnerLookup> {
        ...
        let loadplan_path =
            repo_root().join(".ds41rt-cache/model-artifacts/diagnostic/loadplan.spark-0.json");
    ...
    fn load_full_owner_lookup_or_skip() -> Option<ExpertOwnerLookup> {
        ...
        let loadplan_path =
            repo_root().join(".ds41rt-cache/model-artifacts/diagnostic/loadplan.json");
        if !loadplan_path.exists() {
            eprintln!("skipped: missing {}", loadplan_path.display());
            return None;
        }
