use ds41rt_core::{
    DeepseekV4AttentionPlan, ModelFacts, ModelVariant, DS4_EXPERT_TP_WORLD_SIZE,
    DS4_FLASH_DSPARK_MARKOV_RANK, DS4_FLASH_HIDDEN_SIZE, DS4_FLASH_MOE_INTERMEDIATE_SIZE,
    DS4_FLASH_NUM_HIDDEN_LAYERS, DS4_FLASH_Q_LORA_RANK, DS4_FLASH_ROUTED_EXPERTS, DS4_FLASH_TOP_K,
    DS4_PRO_DSPARK_MARKOV_RANK, DS4_PRO_HIDDEN_SIZE, DS4_PRO_MOE_INTERMEDIATE_SIZE,
    DS4_PRO_NUM_HIDDEN_LAYERS, DS4_PRO_Q_LORA_RANK, DS4_PRO_ROUTED_EXPERTS, DS4_PRO_TOP_K,
};
...
use super::super::super::coverage::catalog_supports_full_model_execution_tensors;
...
pub(super) fn real_full_preflight_availability<'a>(
    ...
    let hidden_bf16_bytes = facts.hidden_size * std::mem::size_of::<u16>();
    let facts_match = deepseek_v4_runtime_facts_match(facts);
    let full_catalog_coverage = coverage.embedding_tensors > 0
        && coverage.lm_head_tensors > 0
        && coverage.hidden_layers_with_any_tensor == catalog.facts.num_hidden_layers
        && coverage.sparse_layers_with_routed_experts == sparse_layers
        && coverage.dense_layers_with_dense_mlp == catalog.facts.first_k_dense_replace;
    let execution_plan_available = !expert_hosts.is_empty()
        && execution_plan.layer_count == catalog.facts.num_hidden_layers
        && execution_plan.dense_layer_count == catalog.facts.first_k_dense_replace
    ...
    let residual_stream_dry_run_available = !expert_hosts.is_empty()
        && residual_stream_dry_run.layer_count == catalog.facts.num_hidden_layers
        ...
        && residual_stream_dry_run.dense_layers == catalog.facts.first_k_dense_replace
        && residual_stream_dry_run.sparse_layers == sparse_layers
        && residual_stream_dry_run.remote_sparse_layers == sparse_layers
        && residual_stream_dry_run.total_residual_adds == catalog.facts.num_hidden_layers * 2
    ...
    let sampling_dry_run_available = !expert_hosts.is_empty()
        ...
        && sampling_dry_run.hidden_dim == catalog.facts.hidden_size
    ...
    let sampling_lm_head_default_chunk_available = sampling_lm_head_default_chunk.passed
        ...
        && sampling_lm_head_default_chunk.hidden_dim == catalog.facts.hidden_size
    ...
    let scheduler_dry_run_available = !expert_hosts.is_empty()
        && scheduler_dry_run.total_layerwaves == catalog.facts.num_hidden_layers * 3
        ...
        && scheduler_dry_run.routes_per_sparse_expert_batch
            == scheduler_dry_run.rows_per_sparse_expert_batch * catalog.facts.top_k
        && scheduler_dry_run.protocol_v2_batch_probe.passed;
    let scheduler_real_tensor_catalog_available =
        catalog_supports_full_model_execution_tensors(catalog);
    let scheduler_prefill_tokens =
        REAL_FULL_PREFLIGHT_PREFILL_TOKEN_START as usize + REAL_FULL_PREFLIGHT_PREFILL_ROWS;
    let scheduler_prefill_chunks = scheduler_prefill_chunk_count_for_rows(
        scheduler_prefill_tokens,
        REAL_FULL_PREFLIGHT_PREFILL_ROWS,
        REAL_FULL_PREFLIGHT_DECODE_ROWS + REAL_FULL_PREFLIGHT_MTP_ROWS,
    );
    let scheduler_source_segments_per_layer = scheduler_prefill_chunks
        + REAL_FULL_PREFLIGHT_DECODE_ROWS
        + usize::from(REAL_FULL_PREFLIGHT_MTP_ROWS > 0);
    let scheduler_device_attention_launches =
        facts.num_hidden_layers * scheduler_source_segments_per_layer;
    ...
    // The physical prefill planner reserves capacity for the jointly admitted
    // decode/MTP terminal rows. Derive every availability count from that same
    // split so the preflight gate follows serving geometry exactly.
    let scheduler_execution_available = !expert_hosts.is_empty()
        && scheduler_execution_dry_run.iterations
            == facts.num_hidden_layers * scheduler_prefill_chunks
        && scheduler_execution_dry_run.candidate_layerwaves
            == facts.num_hidden_layers * scheduler_source_segments_per_layer
        ...
        && scheduler_execution_dry_run.sparse_expert_batches
            == sparse_layers * scheduler_prefill_chunks
        ...
        && scheduler_execution_dry_run.sparse_expert_host_batch_routes
            == scheduler_execution_dry_run
                ...
                .saturating_mul(DS4_EXPERT_TP_WORLD_SIZE)
        ...
        && scheduler_execution_dry_run.sparse_expert_host_request_payload_bytes
            == scheduler_execution_dry_run.sparse_expert_host_batch_rows * hidden_bf16_bytes
    ...
    let scheduler_numeric_unique_rows = REAL_FULL_PREFLIGHT_PREFILL_TOKEN_START as usize
        ...
        + REAL_FULL_PREFLIGHT_DECODE_ROWS
        + REAL_FULL_PREFLIGHT_MTP_ROWS;
    let scheduler_numeric_source_segments =
        facts.num_hidden_layers * scheduler_source_segments_per_layer;
    let scheduler_numeric_value_updates =
        scheduler_numeric_unique_rows * facts.num_hidden_layers * facts.hidden_size;
    let scheduler_numeric_resident_values = scheduler_numeric_unique_rows * facts.hidden_size;
    let scheduler_numeric_real_dense_mlp_rows =
        facts.first_k_dense_replace * scheduler_numeric_unique_rows;
    let scheduler_numeric_real_dense_mlp_values =
        scheduler_numeric_real_dense_mlp_rows * facts.hidden_size;
    let scheduler_numeric_real_dense_mlp_source_segments =
        facts.first_k_dense_replace * scheduler_source_segments_per_layer;
    ...
    let scheduler_numeric_real_sparse_shared_mlp_values =
        scheduler_numeric_real_sparse_shared_mlp_rows * facts.hidden_size;
    let scheduler_numeric_real_sparse_shared_mlp_source_segments =
        sparse_layers * scheduler_source_segments_per_layer;
    ...
    let scheduler_numeric_real_sparse_routed_mlp_routes =
        scheduler_numeric_real_sparse_routed_mlp_rows * facts.top_k;
        ...
        if coordinator_cuda_reference_kernels_enabled() {
            common
                ...
                && scheduler_numeric_progression.attention_device_output_delta_values
                    == scheduler_execution_dry_run.device_attention_query_rows * facts.hidden_size
                && scheduler_numeric_progression.attention_device_output_delta_values
                    == scheduler_execution_dry_run.device_attention_output_values
        } else {
            common
    ...
    let scheduler_numeric_progression_available = scheduler_execution_available
        ...
        && scheduler_numeric_progression.layers == facts.num_hidden_layers
        && scheduler_numeric_progression.unique_source_rows == scheduler_numeric_unique_rows
        && scheduler_numeric_progression.hidden_dim == facts.hidden_size
        ...
        && scheduler_numeric_progression.selected_decode_rows
            == REAL_FULL_PREFLIGHT_DECODE_ROWS * facts.num_hidden_layers
        && scheduler_numeric_progression.selected_mtp_rows
            == REAL_FULL_PREFLIGHT_MTP_ROWS * facts.num_hidden_layers
    ...
    let expert_execution_dry_run_available = !expert_hosts.is_empty()
        ...
        && expert_execution_dry_run.routed_experts_per_layer == facts.routed_experts
        && expert_execution_dry_run.expected_routed_experts == sparse_layers * facts.routed_experts
        ...
        && expert_execution_dry_run.planned_route_entries
            == expert_execution_dry_run.planned_expert_batch_rows * facts.top_k
    ...
    let kv_backing_store_available = kv_backing_store_dry_run.layer_count
        ...
        && kv_backing_store_dry_run.backed_prefill_writes == facts.num_hidden_layers
        && kv_backing_store_dry_run.backed_decode_writes == facts.num_hidden_layers
        && kv_backing_store_dry_run.committed_mtp_writes
            == facts.num_hidden_layers * REAL_FULL_PREFLIGHT_MTP_ACCEPTED_ROWS
        && kv_backing_store_dry_run.discarded_mtp_writes
            == facts.num_hidden_layers
                * (REAL_FULL_PREFLIGHT_MTP_ROWS - REAL_FULL_PREFLIGHT_MTP_ACCEPTED_ROWS);
    let attention_kv_io_available = attention_kv_io_dry_run.layer_count == facts.num_hidden_layers
        && attention_kv_io_dry_run.prefix_prefill_wave_writes == facts.num_hidden_layers
        && attention_kv_io_dry_run.later_prefill_prefix_read_blocks == facts.num_hidden_layers
        && attention_kv_io_dry_run.decode_prefix_read_blocks == facts.num_hidden_layers * 2
        && attention_kv_io_dry_run.mtp_tentative_wave_writes
            == facts.num_hidden_layers * REAL_FULL_PREFLIGHT_MTP_ROWS
    ...
    let attention_kv_binding_available = attention_kv_binding_dry_run.attention_layers
        ...
        && attention_kv_binding_dry_run.kv_io_layer_count == facts.num_hidden_layers
...
fn deepseek_v4_runtime_facts_match(facts: &ModelFacts) -> bool {
    let variant_shape_matches = match facts.variant {
        ModelVariant::Flash => {
            facts.hidden_size == DS4_FLASH_HIDDEN_SIZE
                && facts.num_hidden_layers == DS4_FLASH_NUM_HIDDEN_LAYERS
                ...
                && facts.q_lora_rank == DS4_FLASH_Q_LORA_RANK
                && facts.dspark_markov_rank == DS4_FLASH_DSPARK_MARKOV_RANK
        }
        ModelVariant::Pro => {
        ...
        ModelVariant::Custom => false,
    };
    facts.model_type == "deepseek_v4"
        && facts.first_k_dense_replace == 0
        && facts.dspark_block_size == 5
        && facts.dspark_target_layer_ids.len() == 3
        && facts.compress_ratios.len() == facts.total_transformer_blocks()
        && variant_shape_matches
        && DeepseekV4AttentionPlan::from_model_facts(facts)
            .and_then(|plan| plan.validate_sparkinfer_sm120_contract())
            .is_ok()
}
...
mod tests {
    use super::*;
    use ds41rt_core::{DS4_PRO_COMPRESS_RATIOS, DS4_PRO_TOP_K};

    fn pro_facts() -> ModelFacts {
        ModelFacts {
            model_id: "deepseek-ai/DeepSeek-V4-Pro-DSpark".to_owned(),
            variant: ModelVariant::Pro,
            hidden_size: DS4_PRO_HIDDEN_SIZE,
            num_hidden_layers: DS4_PRO_NUM_HIDDEN_LAYERS,
            routed_experts: DS4_PRO_ROUTED_EXPERTS,
            top_k: DS4_PRO_TOP_K,
            moe_intermediate_size: DS4_PRO_MOE_INTERMEDIATE_SIZE,
            q_lora_rank: DS4_PRO_Q_LORA_RANK,
            attention_heads: 128,
            index_top_k: 1_024,
            max_position_embeddings: 262_144,
            compress_ratios: DS4_PRO_COMPRESS_RATIOS.to_vec(),
            dspark_markov_rank: DS4_PRO_DSPARK_MARKOV_RANK,
            dspark_target_layer_ids: vec![58, 59, 60],
            ..ModelFacts::default()
        }
    }

    #[test]
    fn deepseek_runtime_facts_accept_flash_and_pro_profiles() {
        assert!(deepseek_v4_runtime_facts_match(&ModelFacts::default()));
        assert!(deepseek_v4_runtime_facts_match(&pro_facts()));
    }

    #[test]
    fn deepseek_runtime_facts_reject_custom_or_drifted_profiles() {
        let mut custom = ModelFacts::default();
        custom.variant = ModelVariant::Custom;
        assert!(!deepseek_v4_runtime_facts_match(&custom));

        let mut drifted = pro_facts();
        drifted.top_k += 1;
        assert!(!deepseek_v4_runtime_facts_match(&drifted));
    }
}
