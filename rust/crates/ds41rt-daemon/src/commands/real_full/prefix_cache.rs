...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(super) enum TargetKvExactLeafEviction {
    Evicted { pages: usize },
...
impl TargetKvRadixManager {
    pub(super) fn new(total_tokens: usize, max_active_requests: usize) -> Result<Self> {
        Self::new_with_page_tokens(total_tokens, max_active_requests, TARGET_KV_PAGE_TOKENS)
    }

    pub(super) fn new_with_page_tokens(
        total_tokens: usize,
        max_active_requests: usize,
        page_tokens: usize,
    ) -> Result<Self> {
        anyhow::ensure!(
            page_tokens > 0,
            "target KV physical page token count is zero"
        );
        anyhow::ensure!(
            total_tokens > 0 && total_tokens % page_tokens == 0,
            "target KV pool tokens {total_tokens} must be a positive multiple of {page_tokens}"
        );
        anyhow::ensure!(
    ...
    pub(super) fn reserve(
        ...
        let capacity_pages = pages_for_tokens(logical_capacity_tokens, self.page_tokens)?;
        anyhow::ensure!(
        ...
        let mut matched = match_prefix(&mut inner, &prompt_tokens, self.page_tokens)?;
        let (shared_direct_pages, private_credit_pages, proposed_reserved_private) = loop {
            ...
            let shared_direct_pages = if will_append && matched.tokens % self.page_tokens != 0 {
                matched.tokens / self.page_tokens
            } else {
                pages_for_tokens(matched.tokens, self.page_tokens)?
            };
            ...
            let guaranteed_pages = proposed_reserved_private
                .checked_add(pinned_cached_pages)
                .context("target KV guaranteed-capacity accounting overflow")?;
            if guaranteed_pages <= self.total_pages {
                break (
                    shared_direct_pages,
                    private_credit_pages,
                    proposed_reserved_private,
                );
            }

            // A partial-page hit needs both its pinned source page and a
            // private copy-on-write destination. If optional sequence
            // headroom fills the physical pool, retain only complete shared
            // pages and recompute the partial suffix instead of rejecting a
            // request whose required token budget still fits.
            let aligned_tokens = matched.tokens / self.page_tokens * self.page_tokens;
            let can_drop_partial_hit = will_append && aligned_tokens < matched.tokens;
            dec_lock_ref(&mut inner, matched.terminal);
            if !can_drop_partial_hit {
                anyhow::bail!(
        ...
        let allocation = (|| -> Result<()> {
            if shared_direct_pages < matched.page_table.len() {
                ...
                boundary_copy = Some(TargetKvBoundaryCopy {
                    source_page,
                    destination_page,
                    valid_tokens: matched.tokens % self.page_tokens,
                });
            }
            let prompt_pages = pages_for_tokens(prompt_tokens.len(), self.page_tokens)?;
    ...
    pub(super) fn evict_exact_inactive_leaf(&self, token_ids: &[usize]) -> Result<usize> {
        match self.evict_exact_inactive_leaf_if_present(token_ids)? {
            TargetKvExactLeafEviction::Evicted { pages } => Ok(pages),
            TargetKvExactLeafEviction::AlreadyAbsent { matched_tokens } => {
                anyhow::bail!(
                    "target KV radix exact-prefix eviction matched {matched_tokens} of {} tokens",
                    token_ids.len()
                )
            }
        }
    ...
    /// Removes an exact inactive leaf when it is still cached. Capacity
    /// pressure may have already evicted the requested suffix; in that case
    /// the surviving shorter prefix is left untouched.
    pub(super) fn evict_exact_inactive_leaf_if_present(
        &self,
        token_ids: &[usize],
    ) -> Result<TargetKvExactLeafEviction> {
        anyhow::ensure!(
            !token_ids.is_empty(),
        ...
        let mut inner = self
            ...
            .lock()
            .map_err(|error| anyhow::anyhow!("locking target KV radix manager failed: {error}"))?;
        let matched = match_prefix(&mut inner, &token_ids, self.page_tokens)?;
        if matched.tokens != token_ids.len() {
            debug_validate(&inner, self.total_pages);
            return Ok(TargetKvExactLeafEviction::AlreadyAbsent {
                matched_tokens: matched.tokens,
            });
        }
        let evicted_pages = evict_leaf(&mut inner, matched.terminal)?;
        debug_validate(&inner, self.total_pages);
        Ok(TargetKvExactLeafEviction::Evicted {
            pages: evicted_pages,
        })
    }
...
impl TargetKvRadixReservation {
    pub(super) fn page_tokens(&self) -> usize {
        self.manager.page_tokens
    }
    ...
    pub(super) fn ensure_materialized_through(&mut self, tokens: usize) -> Result<&[u32]> {
        ...
        let required_pages = pages_for_tokens(tokens, self.manager.page_tokens)?;
        let mut inner =
    ...
    pub(super) fn commit_prefix(
        ...
        let committed_pages = pages_for_tokens(committed_tokens, self.manager.page_tokens)?;
        anyhow::ensure!(
        ...
        let current = match_prefix(&mut inner, &committed_ids, self.manager.page_tokens)?;
        ...
        let canonical_pages = if current.tokens == committed_tokens {
            ...
            let reusable_full_pages = current.tokens / self.manager.page_tokens;
            anyhow::ensure!(
            ...
            let node_id = insert_radix_child(
                &mut inner,
                current.terminal,
                suffix_tokens,
                &pages,
...
fn pages_for_tokens(tokens: usize, page_tokens: usize) -> Result<usize> {
    if tokens == 0 {
        return Ok(0);
    }
    tokens
        ...
        .map(|rounded| rounded / page_tokens)
}
...
fn match_prefix(
    ...
    while matched_tokens < token_ids.len() {
        ...
        if common < edge.len() {
            let split_id = split_node(inner, child_id, common, page_tokens)?;
    ...
    let page_table = materialize_page_table(inner, node_id, page_tokens)?;
    debug_assert_eq!(
        page_table.len(),
        pages_for_tokens(matched_tokens, page_tokens)
            .expect("matched target KV token count is valid")
    );
    Ok(PrefixMatch {
        terminal: node_id,
...
fn split_node(
    ...
    let parent_prefix_tokens = inner.nodes[parent_id]
        ...
        .prefix_tokens;
    let parent_pages = materialize_page_table(inner, parent_id, page_tokens)?;
    let child_pages = materialize_page_table(inner, child_id, page_tokens)?;
    ...
    let split_page_count = pages_for_tokens(split_prefix_tokens, page_tokens)?;
...
fn insert_radix_child(
    ...
    anyhow::ensure!(
        page_table.len() == pages_for_tokens(prefix_tokens, page_tokens)?,
...
fn materialize_page_table(
    inner: &TargetKvRadixInner,
    terminal: NodeId,
    page_tokens: usize,
) -> Result<Vec<u32>> {
    ...
    let prefix_tokens = inner.nodes[terminal]
        ...
        .prefix_tokens;
    anyhow::ensure!(
        pages.len() == pages_for_tokens(prefix_tokens, page_tokens)?,
...
mod tests {
    use super::{
        TargetKvExactLeafEviction, TargetKvExactSubtreeEviction, TargetKvRadixManager,
        TARGET_KV_PAGE_TOKENS,
    };
    use ds41rt_core::DS4_KV_SOURCE_PAGE_TOKENS;
    use std::sync::Arc;
    ...
    #[test]
    fn native_ds4_page_geometry_preserves_copy_on_write_prefix_ownership() {
        let page_tokens = DS4_KV_SOURCE_PAGE_TOKENS;
        let manager = Arc::new(
            TargetKvRadixManager::new_with_page_tokens(4 * page_tokens, 16, page_tokens).unwrap(),
        );
        let original = tokens(300, 1);
        let original_pages = cache(&manager, &original);

        let mut branch = original[..270].to_vec();
        branch.extend(tokens(30, 9));
        let reservation = manager.reserve(&branch, branch.len()).unwrap();
        assert_eq!(reservation.matched_prefix_tokens(), 270);
        assert_eq!(reservation.physical_pages()[0], original_pages[0]);
        assert_ne!(reservation.physical_pages()[1], original_pages[1]);
        let copy = reservation.boundary_copy().unwrap();
        assert_eq!(copy.source_page, original_pages[1]);
        assert_eq!(copy.destination_page, reservation.physical_pages()[1]);
        assert_eq!(copy.valid_tokens, 14);
        reservation.commit_prefix(&branch, branch.len()).unwrap();

        let stats = manager.stats();
        assert_eq!(stats.page_tokens, page_tokens);
        assert_eq!(stats.cached_pages, 3);
        assert_eq!(stats.allocated_private_pages, 0);
    }
    ...
    #[test]
    fn full_capacity_partial_hit_recomputes_from_the_complete_page_boundary() {
        let manager = Arc::new(TargetKvRadixManager::new(4 * 64, 16).unwrap());
        let cached = tokens(70, 1);
        cache(&manager, &cached);
        let mut prompt = cached.clone();
        prompt.extend(tokens(10, 2));

        let mut reservation = manager.reserve(&prompt, 4 * 64).unwrap();
        assert_eq!(reservation.matched_prefix_tokens(), 64);
        assert_eq!(reservation.reserved_private_pages(), 3);
        assert_eq!(reservation.boundary_copy(), None);
        assert_eq!(
            reservation
                .ensure_materialized_through(4 * 64)
                .unwrap()
                .len(),
            4
        );
        ...
        let stats = manager.stats();
        assert_eq!(stats.pinned_cached_pages, 1);
        assert_eq!(stats.allocated_private_pages, 3);
        assert_eq!(stats.reserved_private_pages, 3);
    ...
    #[test]
    fn conditional_exact_leaf_eviction_accepts_an_lru_evicted_suffix() {
        let manager = Arc::new(TargetKvRadixManager::new(2 * 64, 16).unwrap());
        let long = tokens(2 * TARGET_KV_PAGE_TOKENS, 1);
        cache(&manager, &long);

        let short = &long[..TARGET_KV_PAGE_TOKENS];
        drop(manager.reserve(short, short.len()).unwrap());
        cache(&manager, &tokens(TARGET_KV_PAGE_TOKENS, 2));

        assert_eq!(
            manager.evict_exact_inactive_leaf_if_present(&long).unwrap(),
            TargetKvExactLeafEviction::AlreadyAbsent {
                matched_tokens: short.len(),
            }
        );
        let short_hit = manager.reserve(short, short.len()).unwrap();
        assert_eq!(short_hit.matched_prefix_tokens(), short.len());
    ...
    #[test]
    fn conditional_exact_leaf_eviction_preserves_exact_leaf_safety() {
        let manager = Arc::new(TargetKvRadixManager::new(2 * 64, 16).unwrap());
        let prompt = tokens(TARGET_KV_PAGE_TOKENS, 1);
        cache(&manager, &prompt);
        let active = manager.reserve(&prompt, prompt.len()).unwrap();
        assert!(manager
            .evict_exact_inactive_leaf_if_present(&prompt)
            .is_err());
        drop(active);
        assert_eq!(
            manager
                .evict_exact_inactive_leaf_if_present(&prompt)
                .unwrap(),
            TargetKvExactLeafEviction::Evicted { pages: 1 }
        );
    }
