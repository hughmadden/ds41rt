...
pub(in crate::commands::real_full) const CUDA_DS4_FLASH_RMSNORM_BF16_PRELOADED_RESIDENT_WEIGHT_BACKEND:
    &str = "deepseek-v4-flash-rmsnorm-bf16-rne";
pub(in crate::commands::real_full) const CUDA_DS4_PRO_RMSNORM_BF16_PRELOADED_RESIDENT_WEIGHT_BACKEND:
    &str = "deepseek-v4-pro-rmsnorm-bf16-rne";

pub(in crate::commands::real_full) fn ds4_rmsnorm_hidden_bf16_preloaded_resident_weight_device_input_output_for_geometry(
    weight_name: &str,
    hidden_buffer: Ds41rtDeviceBuffer,
    rows: usize,
    hidden_dim: usize,
    ...
    ds4_rmsnorm_hidden_bf16_preloaded_resident_weight_device_input_output_impl(
        weight_name,
        hidden_buffer,
        None,
        rows,
...
pub(in crate::commands::real_full) fn ds4_rmsnorm_hidden_bf16_preloaded_resident_weight_device_input_output_async_for_geometry(
    weight_name: &str,
    hidden_buffer: Ds41rtDeviceBuffer,
...
fn ds4_rmsnorm_hidden_bf16_preloaded_resident_weight_device_input_output_impl(
    weight_name: &str,
    hidden_buffer: Ds41rtDeviceBuffer,
    hidden_ready_event: Option<&CoordinatorCudaEvent>,
    rows: usize,
    ...
    anyhow::ensure!(
        ...
        "DeepSeek V4 RMSNorm requires {REAL_FULL_CUDA_REFERENCE_KERNELS_ENV}=1"
    );
    anyhow::ensure!(
        matches!(hidden_dim, 4_096 | 7_168) && eps > 0.0 && eps.is_finite(),
        "DeepSeek V4 RMSNorm requires hidden=4096 or 7168 and finite positive epsilon, got hidden={hidden_dim} eps={eps}"
    );
    let backend = match hidden_dim {
        4_096 => CUDA_DS4_FLASH_RMSNORM_BF16_PRELOADED_RESIDENT_WEIGHT_BACKEND,
        7_168 => CUDA_DS4_PRO_RMSNORM_BF16_PRELOADED_RESIDENT_WEIGHT_BACKEND,
        _ => unreachable!("validated DeepSeek V4 RMSNorm hidden width"),
    };
    let library = cuda_native_library()?;
    let weight = preloaded_resident_weight_device_buffer(weight_name, weight_bytes)?;
    anyhow::ensure!(
        hidden_buffer.device_id == weight.device_id,
        "DeepSeek V4 RMSNorm input and weight are on different devices: input={} weight={}",
    ...
    let output_bytes = rows
        ...
        .context("DeepSeek V4 RMSNorm output byte count overflow")?;
    let output_buffer =
        OwnedCoordinatorDeviceBuffer::new(library, output_bytes, "DeepSeek V4 RMSNorm output")?;
    let (stream, ready_event) = if asynchronous {
