...
pub(in crate::commands::real_full) fn validate_logits_sample_topk_topp_inputs(
    ...
    if top_k == 0 || top_k > vocab || top_k > DS41RT_CUDA_SAMPLE_TOPK_MAX_K {
        anyhow::bail!(
            "real full logits top-k/top-p sampler invalid top_k={top_k} for vocab={vocab}; max supported top_k={DS41RT_CUDA_SAMPLE_TOPK_MAX_K}"
        );
    }
...
pub(in crate::commands::real_full) fn validate_lm_head_sampler_options(
    ...
    if top_k == 0 || top_k > vocab || top_k > DS41RT_CUDA_SAMPLE_TOPK_MAX_K {
        anyhow::bail!(
            "real full BF16 lm_head sampler invalid top_k={top_k} for vocab={vocab}; max supported top_k={DS41RT_CUDA_SAMPLE_TOPK_MAX_K}"
        );
    }
    if !temperature.is_finite() || temperature <= 0.0 {
...
pub(in crate::commands::real_full) fn capture_or_update_terminal_lm_head_argmax_bf16_graph(
    ...
    lm_head_buffer: Ds41rtDeviceBuffer,
...
pub(in crate::commands::real_full) fn capture_or_update_terminal_lm_head_sample_topk_topp_bf16_graph(
    ...
    hidden_buffer: Ds41rtDeviceBuffer,
    lm_head_buffer: Ds41rtDeviceBuffer,
    random_buffer: Ds41rtDeviceBuffer,
    index_buffer: Ds41rtDeviceBuffer,
    score_buffer: Ds41rtDeviceBuffer,
    rows: usize,
    hidden_dim: usize,
...
pub(in crate::commands::real_full) fn capture_or_update_terminal_triton_lm_head_sample_topk_topp_bf16_graph(
    ...
    candidate_score_buffer: Ds41rtDeviceBuffer,
    candidate_index_buffer: Ds41rtDeviceBuffer,
    argmax_index_buffer: Ds41rtDeviceBuffer,
    argmax_score_buffer: Ds41rtDeviceBuffer,
    index_buffer: Ds41rtDeviceBuffer,
    score_buffer: Ds41rtDeviceBuffer,
    rows: usize,
    hidden_dim: usize,
...
fn capture_or_update_terminal_triton_constrained_lm_head_sample_topk_topp_bf16_graph(
    ...
    lm_head_buffer: Ds41rtDeviceBuffer,
    random_buffer: Ds41rtDeviceBuffer,
    token_bitmask_buffer: Ds41rtDeviceBuffer,
    logits_buffer: Ds41rtDeviceBuffer,
    candidate_score_buffer: Ds41rtDeviceBuffer,
    candidate_index_buffer: Ds41rtDeviceBuffer,
    argmax_index_buffer: Ds41rtDeviceBuffer,
...
fn triton_lm_head_sampler_supported(
    ...
    coordinator_python_capture_enabled()
        ...
        && top_k > 0
        && top_k <= vocab
        && top_k <= DS41RT_CUDA_SAMPLE_TOPK_MAX_K
}
