...
const B12X_COORDINATOR_W4A16_DECODE_ENV: &str = "DS41RT_B12X_COORDINATOR_W4A16_DECODE";
const B12X_COORDINATOR_W4A16_Q_B_DECODE_ENV: &str = "DS41RT_B12X_COORDINATOR_W4A16_Q_B";
const B12X_COORDINATOR_W4A16_O_PROJ_DECODE_ENV: &str = "DS41RT_B12X_COORDINATOR_W4A16_O_PROJ";
const COORDINATOR_W8A16_Q_A_DECODE_ENV: &str = "DS41RT_COORDINATOR_W8A16_Q_A";
const COORDINATOR_W8A16_Q_B_DECODE_ENV: &str = "DS41RT_COORDINATOR_W8A16_Q_B";
const COORDINATOR_W8A16_O_PROJ_DECODE_ENV: &str = "DS41RT_COORDINATOR_W8A16_O_PROJ";
const COORDINATOR_W8A16_PACKED_O_ENV: &str = "DS41RT_COORDINATOR_W8A16_PACKED_O";
...
thread_local! {
    static PRELOADED_RESIDENT_WEIGHT_CACHE: RefCell<HashMap<String, (usize, Ds41rtDeviceBuffer)>> =
...
pub(in crate::commands::real_full) struct CoordinatorW4a16ProjectionBuffers {
    pub(in crate::commands::real_full) weight: Ds41rtDeviceBuffer,
    pub(in crate::commands::real_full) scale: Ds41rtDeviceBuffer,
    pub(in crate::commands::real_full) global_scale: Ds41rtDeviceBuffer,
}

#[derive(Clone, Copy)]
pub(in crate::commands::real_full) struct CoordinatorW8a16ProjectionBuffers {
    pub(in crate::commands::real_full) weight: Ds41rtDeviceBuffer,
    pub(in crate::commands::real_full) scales: Ds41rtDeviceBuffer,
...
fn ds4_flash_fp8_scale_mma_resident_name(scale_name: &str) -> String {
    format!("{scale_name}#ds4-flash-fp8-block-scale-mma")
}
...
impl CoordinatorCudaResidentWeights {
    fn preload_ds4_flash_fp8_block_scale_mma(
        &mut self,
        library: &'static NativeLibrary,
        scale_name: &str,
        size_n: usize,
        ...
        anyhow::ensure!(
            size_n > 0 && size_n % 128 == 0 && size_k > 0 && size_k % 128 == 0,
            "DeepSeek V4 Flash FP8 scale {scale_name} requires positive N and K divisible by 128, got N={size_n} K={size_k}"
        );
        let compact_bytes = (size_n / 128)
            .checked_mul(size_k / 128)
            .context("DeepSeek V4 Flash compact FP8 scale byte count overflow")?;
        let mma_bytes = compact_bytes
            .checked_mul(32 * 4 * 4)
            .context("DeepSeek V4 Flash MMA FP8 scale byte count overflow")?;
        let source = self.preloaded_resident_weight_buffer(scale_name, compact_bytes)?;
        let mma_name = ds4_flash_fp8_scale_mma_resident_name(scale_name);
        let key = resident_weight_registry_key(&mma_name, mma_bytes);
        let destination = self
            ...
            .ensure_capacity(
                ...
                &mma_name,
                mma_bytes,
                "DeepSeek V4 Flash shared-expert MMA scales",
            )?;
        let already_packed = self
            .resident_weights
            .get(&key)
            .is_some_and(|resident| resident.uploaded);
        if !already_packed {
            unsafe {
                library
                    .cuda_ds4_flash_fp8_pack_block_scale_mma_async(
                        source,
                        destination,
                        size_n,
                        size_k,
                    ...
                    .with_context(|| {
                        format!("packing DeepSeek V4 Flash FP8 scale {scale_name} for MMA")
                    })?;
                library
                    .cuda_stream_synchronize(std::ptr::null_mut())
                    .with_context(|| {
                        format!(
                            "synchronizing DeepSeek V4 Flash FP8 scale packing for {scale_name}"
                        )
                    })?;
            }
            let resident = self.resident_weights.get_mut(&key).with_context(|| {
                format!("DeepSeek V4 Flash MMA scale buffer {mma_name} disappeared")
            })?;
            resident.uploaded = true;
            resident.upload_count += 1;
        }
    ...
    fn preloaded_ds4_flash_fp8_block_scale_mma(
        &self,
        scale_name: &str,
        size_n: usize,
        size_k: usize,
        ...
        let mma_bytes = compact_bytes
            .checked_mul(32 * 4 * 4)
            .context("DeepSeek V4 Flash MMA FP8 scale byte count overflow")?;
        self.preloaded_resident_weight_buffer(
            &ds4_flash_fp8_scale_mma_resident_name(scale_name),
            mma_bytes,
        )
    }
...
pub(in crate::commands::real_full) fn preload_ds4_flash_fp8_block_scale_mma(
    scale_name: &str,
    size_n: usize,
    size_k: usize,
) -> Result<()> {
    ...
    lock_coordinator_cuda_resident_weights()?
        .preload_ds4_flash_fp8_block_scale_mma(library, scale_name, size_n, size_k)
        .map(|_| ())
}

pub(in crate::commands::real_full) fn preloaded_ds4_flash_fp8_block_scale_mma(
    scale_name: &str,
    size_n: usize,
    size_k: usize,
) -> Result<Ds41rtDeviceBuffer> {
    lock_coordinator_cuda_resident_weights()?
        .preloaded_ds4_flash_fp8_block_scale_mma(scale_name, size_n, size_k)
}
    ...
    unsafe {
        ...
        while row_offset < rows {
            ...
            if chunk_rows == 1 {
                ...
                let buffers = Ds41rtB12xCoordinatorW4a16Buffers {
                    input: chunk_input,
...
) -> Result<Ds41rtB12xCoordinatorW4a16Buffers> {
...
pub(in crate::commands::real_full) fn coordinator_w8a16_packed_o_launch_buffers(
    ...
    Ok(Ds41rtB12xCoordinatorW4a16Buffers {
        input,
        weight: projection.weight,
        output,
        scale: projection.scales,
