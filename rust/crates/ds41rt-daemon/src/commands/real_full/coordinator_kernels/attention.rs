...
use ds41rt_ffi::{
    ...
    DS41RT_CUDA_GENERIC_KV_PAGE_SIZE, DS41RT_CUDA_ROUTER_TOPK_MAX_K, DS41RT_CUDA_SAMPLE_TOPK_MAX_K,
};
...
const REAL_FULL_ATTENTION_CUDA_TIMING_ENV: &str = "DS41RT_REAL_FULL_ATTENTION_CUDA_TIMING";
const W8A16_ASYNC_ATTENTION_ENV: &str = "DS41RT_COORDINATOR_W8A16_ASYNC_ATTENTION";
const PACKED_FP8_MLA_DIRECT_HIDDEN_OUTPUT_ENV: &str =
    "DS41RT_REAL_FULL_PACKED_FP8_MLA_DIRECT_HIDDEN_OUTPUT";

const DS41RT_B12X_MLA_MODULE_ENV: &str = "DS41RT_B12X_MLA_MODULE";
const DS41RT_B12X_MLA_FUNCTION_ENV: &str = "DS41RT_B12X_MLA_FUNCTION";
...
pub(in crate::commands::real_full) fn rope_bf16_device_buffers_for_layer(
    layer_id: usize,
    input_buffer: Ds41rtDeviceBuffer,
    position_buffer: Ds41rtDeviceBuffer,
...
pub(in crate::commands::real_full) fn mla_rope_attention_device_buffers_bf16_for_layer(
    layer_id: usize,
    q_nope_buffer: Ds41rtDeviceBuffer,
    q_rope_buffer: Ds41rtDeviceBuffer,
    k_nope_buffer: Ds41rtDeviceBuffer,
    k_rope_buffer: Ds41rtDeviceBuffer,
    value_buffer: Ds41rtDeviceBuffer,
    output_buffer: Ds41rtDeviceBuffer,
    rows: usize,
    heads: usize,
    nope_dim: usize,
...
pub(in crate::commands::real_full) fn mla_kv_cache_unpack_bf16_device_buffers_for_layer(
    layer_id: usize,
    payload_buffer: Ds41rtDeviceBuffer,
    kv_latent_buffer: Ds41rtDeviceBuffer,
    k_rope_buffer: Ds41rtDeviceBuffer,
    dsa_key_buffer: Option<Ds41rtDeviceBuffer>,
    rows: usize,
    kv_lora_rank: usize,
    rope_dim: usize,
    dsa_dim: usize,
...
pub(in crate::commands::real_full) fn mla_kv_prepare_bf16_device_buffers_for_layer(
    layer_id: usize,
    projected_buffer: Ds41rtDeviceBuffer,
    positions_buffer: Ds41rtDeviceBuffer,
    norm_weight_buffer: Ds41rtDeviceBuffer,
    prepared_buffer: Ds41rtDeviceBuffer,
    rows: usize,
...
pub(in crate::commands::real_full) fn mla_query_split_rope_bf16_device_buffers_for_layer(
    ...
    projected_buffer: Ds41rtDeviceBuffer,
    q_nope_buffer: Ds41rtDeviceBuffer,
    q_rope_unrotated_buffer: Ds41rtDeviceBuffer,
    position: u32,
    q_rope_rotated_buffer: Ds41rtDeviceBuffer,
    rows: usize,
    heads: usize,
    nope_dim: usize,
    rope_dim: usize,
...
pub(in crate::commands::real_full) struct FlashinferTargetKvPageTable {
    pub(in crate::commands::real_full) physical_pages: Ds41rtDeviceBuffer,
...
pub(in crate::commands::real_full) enum FlashinferCompressedMlaKvInput {
    SplitBf16 {
        latent: Ds41rtDeviceBuffer,
        rope: Ds41rtDeviceBuffer,
    },
    Interleaved {
...
struct FlashinferPackedFp8MlaDecodeBuffers {
    ...
    topk_length: Ds41rtDeviceBuffer,
    index_base: Ds41rtDeviceBuffer,
    output: Ds41rtDeviceBuffer,
    out_lse: Ds41rtDeviceBuffer,
    mid_out: Ds41rtDeviceBuffer,
    mid_lse: Ds41rtDeviceBuffer,
}
...
struct FlashinferPackedFp8MlaFullGraphBuffers {
    ...
    q_nope: Ds41rtDeviceBuffer,
    q_absorbed: Ds41rtDeviceBuffer,
    q_rope: Ds41rtDeviceBuffer,
    q_rope_staging: Ds41rtDeviceBuffer,
    physical_page_table: Option<Ds41rtDeviceBuffer>,
    ...
    hidden_projection_w4a16: Option<Ds41rtB12xCoordinatorW4a16Buffers>,
    hidden_projection_w8a16_packed_o: Option<Ds41rtB12xCoordinatorW4a16Buffers>,
}
...
fn flashinfer_direct_packed_fp8_mla_capacity_supported(bytes: usize) -> bool {
    bytes % FLASHINFER_PACKED_FP8_MLA_ROW_BYTES == 0
        && bytes / FLASHINFER_PACKED_FP8_MLA_ROW_BYTES % DS41RT_CUDA_GENERIC_KV_PAGE_SIZE == 0
}
...
fn flashinfer_packed_fp8_mla_decode_device_buffers(
    ...
    let packed_visible_kv = if let Some(page_table) = physical_page_table {
        let required_page_bytes = rows
            ...
            .map(|rows| rows / DS41RT_CUDA_GENERIC_KV_PAGE_SIZE)
    ...
    with_coordinator_cuda_graph_slot(&graph_key, |library, slot| {
        ...
        let stable_physical_page_table = if let Some(page_table) = physical_page_table {
            let physical_pool_pages = packed_kv
                ...
                .and_then(|rows| {
                    rows.checked_add(DS41RT_CUDA_GENERIC_KV_PAGE_SIZE - 1)
                        .map(|rows| rows / DS41RT_CUDA_GENERIC_KV_PAGE_SIZE)
                })
...
fn stage_flashinfer_hidden_projection(
    ...
    staging_output: Ds41rtDeviceBuffer,
    direct_output: bool,
) -> (
    Option<Ds41rtDeviceBuffer>,
...
fn coordinator_w4a16_o_proj_tn64_enabled() -> bool {
    std::env::var("DS41RT_B12X_COORDINATOR_W4A16_O_PROJ_TN64")
        .map(|value| value != "0")
        .unwrap_or(true)
}
...
fn b12x_mla_capture_module() -> String {
    env::var(DS41RT_B12X_MLA_MODULE_ENV).unwrap_or_else(|_| B12X_MLA_CAPTURE_MODULE.to_owned())
}

fn b12x_mla_capture_function() -> String {
    env::var(DS41RT_B12X_MLA_FUNCTION_ENV).unwrap_or_else(|_| B12X_MLA_CAPTURE_FUNCTION.to_owned())
}
...
pub(in crate::commands::real_full) fn capture_or_update_layer_causal_attention_bf16_graph(
    ...
    slot: &mut CoordinatorCudaGraphWorkspaceSlot,
    signature: CoordinatorCudaGraphSignature,
    query_buffer: Ds41rtDeviceBuffer,
    key_buffer: Ds41rtDeviceBuffer,
    value_buffer: Ds41rtDeviceBuffer,
    output_buffer: Ds41rtDeviceBuffer,
    rows: usize,
    heads: usize,
...
mod flashinfer_capture_shape_tests {
    ...
    fn buffer_at(address: usize) -> Ds41rtDeviceBuffer {
        Ds41rtDeviceBuffer {
