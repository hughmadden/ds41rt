...
pub(in crate::commands::real_full) struct PendingNvfp4RowPayload {
    staging: Ds41rtHostBuffer,
    payload_bytes: usize,
    ready_event: Arc<CoordinatorCudaEvent>,
}

pub(in crate::commands::real_full) struct PendingBf16RowPayload {
    staging: Ds41rtHostBuffer,
    payload_bytes: usize,
    ready_event: Arc<CoordinatorCudaEvent>,
}
...
#[derive(Default)]
struct Bf16RowPayloadWorkspace {
    staging: ReusableHostBuffer,
}
...
thread_local! {
    static BF16_ROW_PAYLOAD_WORKSPACES: RefCell<HashMap<usize, Bf16RowPayloadWorkspace>> =
        RefCell::new(HashMap::new());
}

pub(in crate::commands::real_full) fn begin_read_device_bf16_row_payload(
    input: &DeviceBf16Output,
) -> Result<PendingBf16RowPayload> {
    anyhow::ensure!(
        input.rows > 0 && input.values_per_row > 0,
        "BF16 hidden exchange requires a nonempty device input"
    );
    let payload_bytes = input
        .rows
        .checked_mul(input.values_per_row)
        .and_then(|values| values.checked_mul(std::mem::size_of::<u16>()))
        .context("BF16 hidden exchange payload byte count overflow")?;
    anyhow::ensure!(
        payload_bytes == input.bytes,
        "BF16 hidden exchange payload bytes {payload_bytes} do not match input bytes {}",
        input.bytes
    );
    let graph_key =
        coord_sparse_a_graph_key_for_full_hidden_rows(input.rows, input.values_per_row)?
            .context("asynchronous BF16 hidden exchange requires a Coord-Sparse-A graph slot")?;
    let library = cuda_native_library()?;
    let staging = BF16_ROW_PAYLOAD_WORKSPACES.with(|workspaces| {
        ...
        workspace.staging.ensure_capacity(
            library,
            payload_bytes,
            "BF16 hidden exchange payload",
        )?;
        Ok::<_, anyhow::Error>(workspace.staging.buffer)
    })?;
    with_coordinator_cuda_graph_slot(&graph_key, |library, slot| {
        let cuda_stream = slot.stream_ptr();
        input
            .wait_ready_on_stream(cuda_stream)
            .context("waiting for BF16 hidden rows before exchange readback")?;
        unsafe {
            library
                .copy_d2h_host_buffer_async(staging, input.buffer(), payload_bytes, cuda_stream)
                .context("reading coordinator BF16 hidden exchange payload")?;
        }
        let ready_event = slot.record_output_ready_event(library)?;
        Ok(PendingBf16RowPayload {
            staging,
            payload_bytes,
            ready_event,
        })
...
pub(in crate::commands::real_full) fn finish_read_device_bf16_row_payload(
    pending: PendingBf16RowPayload,
    payload: &mut Vec<u8>,
) -> Result<()> {
    pending
        .ready_event
        .synchronize()
        .context("waiting for coordinator BF16 hidden exchange payload")?;
