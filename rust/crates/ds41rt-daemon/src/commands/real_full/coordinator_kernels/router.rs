...
const DS4_FLASH_ROUTER_SHORTLIST_ENV: &str = "DS41RT_DS4_FLASH_ROUTER_SHORTLIST";
...
pub(in crate::commands::real_full) struct Ds4FlashRouterTopKDeviceBuffers {
    pub(in crate::commands::real_full) indices: Ds41rtDeviceBuffer,
    pub(in crate::commands::real_full) scores: Ds41rtDeviceBuffer,
    pub(in crate::commands::real_full) weights: Ds41rtDeviceBuffer,
}
...
fn preloaded_ds4_flash_router_device_weights(
    ...
    router_aux_name: &str,
    hash_routing: bool,
) -> Result<(&'static NativeLibrary, Ds41rtDeviceBuffer, Ds41rtDeviceBuffer)> {
...
mod ds4_flash_device_handoff_tests {
    ...
    fn pro_router_matches_sqrt_softplus_bias_oracle_on_gpu0() {
        ...
        let resident_weight_name = "ds41rt.test.pro.router.weight";
        let resident_bias_name = "ds41rt.test.pro.router.bias";
...
pub(in crate::commands::real_full) fn validate_router_topk_inputs(
    ...
    if top_k == 0 || top_k > experts || top_k > DS41RT_CUDA_ROUTER_TOPK_MAX_K {
        anyhow::bail!(
            "real full router top-k invalid top_k={top_k} for experts={experts}; max supported top_k={DS41RT_CUDA_ROUTER_TOPK_MAX_K}"
        );
    }
...
pub(in crate::commands::real_full) fn validate_router_topk_bf16_inputs(
    ...
    if top_k == 0 || top_k > experts || top_k > DS41RT_CUDA_ROUTER_TOPK_MAX_K {
        anyhow::bail!(
            "real full BF16 router top-k invalid top_k={top_k} for experts={experts}; max supported top_k={DS41RT_CUDA_ROUTER_TOPK_MAX_K}"
        );
    }
...
pub(in crate::commands::real_full) fn validate_router_topk_bf16_preloaded_resident_inputs(
    ...
    if top_k == 0 || top_k > experts || top_k > DS41RT_CUDA_ROUTER_TOPK_MAX_K {
        anyhow::bail!(
            "real full preloaded BF16 router top-k invalid top_k={top_k} for experts={experts}; max supported top_k={DS41RT_CUDA_ROUTER_TOPK_MAX_K}"
        );
    }
    let expected_hidden = rows
        .checked_mul(hidden_dim)
...
pub(in crate::commands::real_full) fn validate_router_topk_bf16_preloaded_resident_device_input(
    ...
    if top_k == 0 || top_k > experts || top_k > DS41RT_CUDA_ROUTER_TOPK_MAX_K {
        anyhow::bail!(
            "real full preloaded BF16 router top-k device-input invalid top_k={top_k} for experts={experts}; max supported top_k={DS41RT_CUDA_ROUTER_TOPK_MAX_K}"
        );
    }
