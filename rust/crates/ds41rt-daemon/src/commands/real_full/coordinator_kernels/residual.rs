...
use ds41rt_core::{
    ...
    GLM52_NUM_HIDDEN_LAYERS, GLM52_ROUTED_SCALING_FACTOR,
};
use ds41rt_ffi::{
    Ds41rtCudaGraphCaptureInfo, Ds41rtDeviceBuffer, Ds41rtHostBuffer, NativeLibrary,
    DS41RT_CUDA_ROUTER_TOPK_MAX_K, DS41RT_CUDA_SAMPLE_TOPK_MAX_K,
};
...
pub(in crate::commands::real_full) const CUDA_REFERENCE_SHARED_ROUTED_DELTA_BF16_BACKEND: &str =
    "cuda-reference-shared-routed-delta-bf16";
...
pub(in crate::commands::real_full) fn residual_add_bf16_device_input_delta_view_device_output(
    ...
    residual_add_bf16_device_input_delta_view_device_output_impl(
        residual,
        Some(delta_source),
        delta,
    )
...
/// Residual-add a raw BF16 device view whose producer has already established
/// a synchronization boundary. This is the handoff from the fixed TP4 arena:
/// the arena owns the delta allocation, while the returned residual remains an
/// ordinary request-owned device output.
pub(in crate::commands::real_full) fn residual_add_bf16_device_input_synchronized_delta_view_device_output(
    residual: &DeviceBf16Output,
    delta: Ds41rtDeviceBuffer,
) -> Result<DeviceBf16Output> {
    residual_add_bf16_device_input_delta_view_device_output_impl(residual, None, delta)
}

fn residual_add_bf16_device_input_delta_view_device_output_impl(
    residual: &DeviceBf16Output,
    delta_source: Option<&DeviceBf16Output>,
    delta: Ds41rtDeviceBuffer,
) -> Result<DeviceBf16Output> {
    ...
    if let Some(graph_key) = coord_sparse_b_graph_key_for_full_hidden_rows(rows, values_per_row)? {
...
fn cuda_sparse_b_single_row_fp8_shared_residual_add_device_output(
    ...
    let device_output_buffer = with_coordinator_cuda_graph_slot(&graph_key, |library, slot| {
        ...
        residual
            ...
            .context("waiting for single-row FP8 Sparse-B residual input")?;
        shared_delta
            .wait_ready_on_stream(cuda_stream)
            .context("waiting for single-row FP8 Sparse-B shared-expert input")?;
...
pub(in crate::commands::real_full) fn flatten_sparse_b_bf16_partials(
    ...
    let total_rows = validate_ds4_tp4_partial_row_maps(global_row_indices_by_host, dst_rows)?;
...
fn validate_sparse_b_low_precision_partial_layout(
    ...
    let total_rows = validate_ds4_tp4_partial_row_maps(global_row_indices, dst_rows)?;
    let src_bytes = total_rows
        .checked_mul(output_row_stride_bytes)
...
fn validate_ds4_tp4_partial_row_maps(
    global_row_indices: &[impl AsRef<[usize]>],
    dst_rows: usize,
) -> Result<usize> {
    let mut contributions = vec![0_usize; dst_rows];
    let mut total_rows = 0_usize;
    for (chunk_index, rows) in global_row_indices.iter().enumerate() {
        let rows = rows.as_ref();
        total_rows = total_rows
            .checked_add(rows.len())
            .context("DS4 TP4 partial row count overflows usize")?;
        for &row in rows {
            let count = contributions.get_mut(row).with_context(|| {
                format!(
                    "DS4 TP4 partial chunk {chunk_index} row {row} is out of bounds for {dst_rows} destination rows"
                )
            })?;
            *count += 1;
            anyhow::ensure!(
                *count <= DS4_EXPERT_TP_WORLD_SIZE,
                "DS4 TP4 row {row} received more than {DS4_EXPERT_TP_WORLD_SIZE} TP rank contributions"
            );
        }
    }
    Ok(total_rows)
...
pub(in crate::commands::real_full) fn cuda_residual_add_bf16_device_inputs_output(
    ...
    if let Some(graph_key) =
        coord_sparse_b_graph_key_for_full_hidden_rows(residual.rows, residual.values_per_row)?
    {
...
pub(in crate::commands::real_full) fn capture_or_update_sparse_b_residual_add_bf16_graph(
    ...
    residual_buffer: Ds41rtDeviceBuffer,
    delta_buffer: Ds41rtDeviceBuffer,
...
pub(in crate::commands::real_full) fn capture_or_update_sparse_b_scatter_add_bf16_to_f32_graph(
    ...
    slot: &mut CoordinatorCudaGraphWorkspaceSlot,
    signature: CoordinatorCudaGraphSignature,
    src_buffer: Ds41rtDeviceBuffer,
    index_buffer: Ds41rtDeviceBuffer,
    dst_buffer: Ds41rtDeviceBuffer,
    dst_rows: usize,
    rows: usize,
    row_width: usize,
    label: &'static str,
...
fn cuda_sparse_b_scatter_shared_residual_add_low_precision_device_output(
    ...
    let device_output_buffer = with_coordinator_cuda_graph_slot(&graph_key, |library, slot| {
        ...
        let launch_scatter = |src_buffer: Ds41rtDeviceBuffer,
                              index_buffer: Ds41rtDeviceBuffer,
                              partial_rows: usize|
         -> Result<()> {
            unsafe {
...
impl CudaStreamedSparseBAccumulator {
    pub(in crate::commands::real_full) fn new(dst_rows: usize, row_width: usize) -> Result<Self> {
        ...
        let (mut routed_f32, zero_ready) =
            with_coordinator_cuda_graph_slot(&graph_key, |library, slot| {
                ...
                let zero_ready = slot
                    .record_output_ready_event(library)
                    .context("recording incremental Sparse-B initialization")?;
                Ok((routed_f32, zero_ready))
            })?;
        // Every accumulator consumer is queued on this graph-key stream, so
        // the zero-fill remains ordered without a host wait. Retain its event
        // on the owned buffer for early-error/drop safety; finalization later
        // replaces it with the event for the accumulator's last consumer.
        routed_f32.set_ready_event(zero_ready);
        Ok(Self {
            graph_key,
            routed_f32,
    ...
    fn push_chunks_inner(
        ...
        anyhow::ensure!(
            next_total_partial_rows <= self.dst_rows.saturating_mul(DS4_EXPERT_TP_WORLD_SIZE),
            "incremental Sparse-B partial rows {} exceed routed contribution capacity {}",
            next_total_partial_rows,
            self.dst_rows.saturating_mul(DS4_EXPERT_TP_WORLD_SIZE)
        );
    ...
    pub(in crate::commands::real_full) fn finalize_segment(
        ...
        self.finalize_segment_inner(segment, true, true)
    }

    pub(in crate::commands::real_full) fn finalize_delta_segment_async(
        &mut self,
        segment: &StreamedSparseBResidualSegment<'_>,
    ) -> Result<DeviceBf16Output> {
        self.finalize_segment_inner(segment, false, false)
    }

    fn finalize_segment_inner(
        &mut self,
        segment: &StreamedSparseBResidualSegment<'_>,
        add_residual: bool,
        synchronize: bool,
            ...
            with_coordinator_cuda_graph_slot(&self.graph_key, |library, slot| {
                ...
                if add_residual {
                    segment
                        .residual
                        .wait_ready_on_stream(stream)
                        .context("waiting for incremental Sparse-B residual input")?;
                }
                segment
                    .shared_delta
                    .wait_ready_on_stream(stream)
                    .context("waiting for incremental Sparse-B shared-expert input")?;
                unsafe {
                    if add_residual {
                        library
                            .cuda_residual_add_shared_f32_delta_bf16_async(
                        ...
                        library
                            ...
                            .context(
                                "executing incremental Sparse-B shared+routed FFN delta add",
                            )?;
                    }
                    if synchronize {
                        library
                ...
                let ready_event = (!synchronize)
                    .then(|| slot.record_output_ready_event(library))
                    .transpose()?;
                Ok((
                    DeviceBf16Output {
                        buffer: output,
        ...
        if let Some(ready_event) = ready_event {
            ...
            // The accumulator can be returned to its pool as soon as the
            // transport reports complete. Fence its last consumer rather than
            // relying on the former host-side stream synchronization.
            self.routed_f32.set_ready_event(ready_event);
        }
...
where
    ...
    Payload: AsRef<[u8]>,
{
    let segments = streamed_sparse_b_device_segments(segments, dst_rows, row_width, true)?;
    cuda_stream_sparse_b_scatter_shared_bf16_device_outputs(
        &segments, dst_rows, row_width, true, next_chunk,
    )
}

pub(in crate::commands::real_full) fn cuda_stream_sparse_b_scatter_shared_delta_bf16_device_outputs<
    NextChunk,
    Payload,
>(
    segments: &[StreamedSparseBResidualSegment<'_>],
...
where
    ...
    Payload: AsRef<[u8]>,
{
    let segments = streamed_sparse_b_device_segments(segments, dst_rows, row_width, false)?;
    cuda_stream_sparse_b_scatter_shared_bf16_device_outputs(
        &segments, dst_rows, row_width, false, next_chunk,
    )
}

pub(in crate::commands::real_full) fn cuda_stream_sparse_b_scatter_shared_delta_bf16_device_output_from_buffer<
    NextChunk,
    Payload,
>(
    ...
    Payload: AsRef<[u8]>,
{
    let expected_bytes = dst_rows
        .checked_mul(row_width)
        .and_then(|values| values.checked_mul(std::mem::size_of::<u16>()))
        .context("CUDA streamed Sparse-B shared delta byte count overflows usize")?;
    anyhow::ensure!(
        dst_rows > 0
            && row_width > 0
            && !shared_delta.ptr.is_null()
            && shared_delta.device_id == 0
            && shared_delta.bytes == expected_bytes,
        "CUDA streamed Sparse-B shared delta buffer does not describe {dst_rows}x{row_width} BF16 values on coordinator GPU0"
    );
    let segments = [StreamedSparseBDeviceSegment {
        residual: None,
        residual_ready_event: None,
        shared_delta,
        shared_delta_ready_event: None,
        row_start: 0,
        row_count: dst_rows,
    }];
    let mut outputs = cuda_stream_sparse_b_scatter_shared_bf16_device_outputs(
        &segments, dst_rows, row_width, false, next_chunk,
    )?;
    anyhow::ensure!(
        outputs.len() == 1,
        "CUDA streamed Sparse-B single shared delta produced {} outputs",
        outputs.len()
    );
    Ok(outputs.pop().expect("one streamed Sparse-B output exists"))
}

#[derive(Clone)]
struct StreamedSparseBDeviceSegment {
    residual: Option<Ds41rtDeviceBuffer>,
    residual_ready_event: Option<Arc<CoordinatorCudaEvent>>,
    shared_delta: Ds41rtDeviceBuffer,
    shared_delta_ready_event: Option<Arc<CoordinatorCudaEvent>>,
    row_start: usize,
    row_count: usize,
}

fn streamed_sparse_b_device_segments(
    segments: &[StreamedSparseBResidualSegment<'_>],
    dst_rows: usize,
    row_width: usize,
    add_residual: bool,
) -> Result<Vec<StreamedSparseBDeviceSegment>> {
    anyhow::ensure!(
        !segments.is_empty(),
        "CUDA streamed Sparse-B requires at least one residual segment"
    );
    let mut expected_row_start = 0_usize;
    let mut device_segments = Vec::with_capacity(segments.len());
    for segment in segments {
        ...
        device_segments.push(StreamedSparseBDeviceSegment {
            residual: add_residual.then(|| segment.residual.buffer()),
            residual_ready_event: add_residual
                .then(|| segment.residual.ready_event())
                .flatten(),
            shared_delta: segment.shared_delta.buffer(),
            shared_delta_ready_event: segment.shared_delta.ready_event(),
...
fn cuda_stream_sparse_b_scatter_shared_bf16_device_outputs<NextChunk, Payload>(
    segments: &[StreamedSparseBDeviceSegment],
    dst_rows: usize,
    row_width: usize,
    add_residual: bool,
    ...
    Payload: AsRef<[u8]>,
{
    anyhow::ensure!(
        !segments.is_empty(),
        "CUDA streamed Sparse-B requires at least one device segment"
    );
    let mut expected_row_start = 0_usize;
    for segment in segments {
        anyhow::ensure!(
        ...
        anyhow::ensure!(
            !segment.shared_delta.ptr.is_null()
                && segment.shared_delta.device_id == 0
                && (!add_residual
                    || segment
                        .residual
                        .is_some_and(|residual| !residual.ptr.is_null() && residual.device_id == 0)),
            "CUDA streamed Sparse-B segment buffers must reside on coordinator GPU0"
        );
        expected_row_start = expected_row_start
            .checked_add(segment.row_count)
            .context("CUDA streamed Sparse-B segment row range overflows usize")?;
    ...
    with_coordinator_cuda_graph_slot(&graph_key, |library, slot| {
        let cuda_stream = slot.stream_ptr();
        for segment in segments {
            if let Some(event) = segment.residual_ready_event.as_deref() {
                event
                    .wait_on_stream(cuda_stream)
                    .context("waiting for streamed Sparse-B residual input")?;
            }
            if let Some(event) = segment.shared_delta_ready_event.as_deref() {
                event
                    .wait_on_stream(cuda_stream)
                    .context("waiting for streamed Sparse-B shared-expert input")?;
            }
        }
        let src_buffer = slot.buffer(
            library,
        ...
        )) = next_chunk()?
        {
            ...
            anyhow::ensure!(
                total_partial_rows <= dst_rows.saturating_mul(DS4_EXPERT_TP_WORLD_SIZE),
                "streamed Sparse-B partial rows {total_partial_rows} exceed routed contribution capacity {}",
                dst_rows.saturating_mul(DS4_EXPERT_TP_WORLD_SIZE)
            );
            ...
            unsafe {
                if add_residual {
                    let residual = segment
                        .residual
                        .context("streamed Sparse-B residual segment is missing its residual")?;
                    library
                        .cuda_residual_add_shared_f32_delta_bf16_async(
                            residual,
                            segment.shared_delta,
                        ...
                        .context(
                            "executing streamed Sparse-B shared+routed residual segment add",
                        )?;
                } else {
                    library
                        .cuda_residual_add_f32_delta_bf16_async(
                        ...
                        .context("executing streamed Sparse-B shared+routed FFN delta add")?;
                }
            }
...
pub(in crate::commands::real_full) fn capture_or_update_sparse_b_scatter_residual_add_bf16_graph(
    ...
    src_buffer: Ds41rtDeviceBuffer,
    index_buffer: Ds41rtDeviceBuffer,
    dst_f32_buffer: Ds41rtDeviceBuffer,
    residual_bf16_buffer: Ds41rtDeviceBuffer,
    output_bf16_buffer: Ds41rtDeviceBuffer,
    dst_rows: usize,
    rows: usize,
    row_width: usize,
    values: usize,
...
pub(in crate::commands::real_full) fn capture_or_update_sparse_b_scatter_shared_residual_add_bf16_graph(
    ...
    index_buffer: Ds41rtDeviceBuffer,
    dst_f32_buffer: Ds41rtDeviceBuffer,
    shared_delta_bf16_buffer: Ds41rtDeviceBuffer,
    residual_bf16_buffer: Ds41rtDeviceBuffer,
    output_bf16_buffer: Ds41rtDeviceBuffer,
    dst_rows: usize,
    rows: usize,
