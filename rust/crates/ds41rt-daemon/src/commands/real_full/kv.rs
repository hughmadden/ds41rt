...
use ds41rt_core::{KvBlockDescriptor, KvCacheBackingStore, KvCacheConfig, LayerId, PositionId};
...
pub(super) use attention_io::real_full_attention_kv_io_dry_run_for_layers;
use device::real_full_device_kv_block_ios;

pub(super) fn real_full_kv_backing_store_dry_run_for_layers(
    ...
    let indexer_layer_id = store
        .config()
        .dsa_indexer_layer_ids()
        .first()
        .copied()
        .unwrap_or_default();
    let non_indexer_layer_id = (0..layer_count)
        .find(|layer_id| {
            !store
                .config()
                .layer_has_dsa_indexer(LayerId(*layer_id as u32))
        })
        .unwrap_or_default();
    ...
    Ok(RealFullKvBackingStoreDryRun {
        ...
        dsa_layer_bytes_per_token: store
            .config()
            .layer_bytes_per_token(LayerId(indexer_layer_id as u32)),
        non_dsa_layer_bytes_per_token: store
            .config()
            .layer_bytes_per_token(LayerId(non_indexer_layer_id as u32)),
        layer_count,
        backed_prefill_writes: layer_count,
        backed_decode_writes: layer_count,
        backed_tentative_mtp_writes: layer_count * REAL_FULL_PREFLIGHT_MTP_ROWS,
