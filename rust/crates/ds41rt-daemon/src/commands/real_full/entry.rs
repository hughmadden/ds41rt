...
use ds41rt_loader::{decode_tokenizer_ids, LoadedTokenizer};
use ds41rt_transport::{expert_protocol_v2_compact_id, TcpProtocolV2HostBatchTarget};
...
fn request_lm_head_sampling_options(
    request: &ds41rt_api::RealFullRequest,
...
const REAL_FULL_MAX_EXECUTION_LANES_ENV: &str = "DS41RT_REAL_FULL_MAX_EXECUTION_LANES";
...
const REAL_FULL_SERVE_PREWARM_REQUEST_ENV: &str = "DS41RT_REAL_FULL_SERVE_PREWARM_REQUEST";
...
const REAL_FULL_SERVE_PREFIX_PREFILL_PROBE_ENV: &str = "DS41RT_REAL_FULL_SERVE_PREFIX_PREFILL_PROBE";
...
const REAL_FULL_REQUEST_THREAD_PINNED_ENV: &str = "DS41RT_REAL_FULL_REQUEST_THREAD_PINNED";
const REAL_FULL_REQUEST_THREAD_PINNED_WORKERS_ENV: &str =
    "DS41RT_REAL_FULL_REQUEST_THREAD_PINNED_WORKERS";
const REAL_FULL_REQUEST_WORKER_CPUS_ENV: &str = "DS41RT_REAL_FULL_REQUEST_WORKER_CPUS";
const REAL_FULL_SCHEDULER_WORKER_CPU_ENV: &str = "DS41RT_REAL_FULL_SCHEDULER_WORKER_CPU";
const REAL_FULL_REQUEST_MTP_VERIFY_ENV: &str = "DS41RT_REAL_FULL_REQUEST_MTP_VERIFY";
const REAL_FULL_DSPARK_ENV: &str = "DS41RT_REAL_FULL_DSPARK";
const REAL_FULL_DSPARK_SHADOW_ENV: &str = "DS41RT_REAL_FULL_DSPARK_SHADOW";
const REAL_FULL_DSPARK_CACHE_MODE_ENV: &str = "DS41RT_REAL_FULL_DSPARK_CACHE_MODE";
const REAL_FULL_DSPARK_TAIL_CACHE_BYTES_ENV: &str = "DS41RT_REAL_FULL_DSPARK_TAIL_CACHE_BYTES";
const REAL_FULL_DSPARK_TRACE_ENV: &str = "DS41RT_REAL_FULL_DSPARK_TRACE";
const REAL_FULL_DSPARK_CONFIDENCE_POLICY_ENV: &str = "DS41RT_REAL_FULL_DSPARK_CONFIDENCE_POLICY";
const REAL_FULL_DSPARK_FIXED_DRAFTS_ENV: &str = "DS41RT_REAL_FULL_DSPARK_FIXED_DRAFTS";
const REAL_FULL_DSPARK_PROFILE_AT_STARTUP_ENV: &str = "DS41RT_REAL_FULL_DSPARK_PROFILE_AT_STARTUP";
const REAL_FULL_EXPERT_READY_TIMEOUT_SECS_ENV: &str =
    "DS41RT_REAL_FULL_SERVE_EXPERT_READY_TIMEOUT_SECS";
const REAL_FULL_EXPERT_WARMUP_STATUS_FILE_ENV: &str =
    "DS41RT_REAL_FULL_SERVE_EXPERT_WARMUP_STATUS_FILE";
const DEFAULT_REAL_FULL_EXPERT_READY_TIMEOUT_SECS: u64 = 900;
const REAL_FULL_KV_POOL_TOKENS_ENV: &str = "DS41RT_REAL_FULL_KV_POOL_TOKENS";
...
const REAL_FULL_KV_SNAPSHOT_LOAD_ENV: &str = "DS41RT_REAL_FULL_KV_SNAPSHOT_LOAD";
const REAL_FULL_KV_SNAPSHOT_SAVE_ENV: &str = "DS41RT_REAL_FULL_KV_SNAPSHOT_SAVE";
const REAL_FULL_KV_SNAPSHOT_SAVE_TOKENS_ENV: &str = "DS41RT_REAL_FULL_KV_SNAPSHOT_SAVE_TOKENS";
const REAL_FULL_KV_SNAPSHOT_SAVE_POINTS_ENV: &str = "DS41RT_REAL_FULL_KV_SNAPSHOT_SAVE_POINTS";
const REAL_FULL_ENGINE_COMMIT_ENV: &str = "DS41RT_ENGINE_COMMIT";

pub(crate) struct LoadedRealFullServing {
    pub(crate) info: ds41rt_api::RealFullInfo,
    pub(crate) kv_config: KvCacheConfig,
    pub(crate) executor: Arc<dyn ds41rt_api::RealFullRequestExecutor>,
}
...
struct RealFullSchedulerRequestExecutor {
    base_info: ds41rt_api::RealFullInfo,
    catalog: TensorCatalog,
...
impl RealFullDsparkRuntime {
    fn trace_shadow_window(
        ...
        eprintln!(
            ...
            serde_json::json!({
                "schema": "ds41rt-dspark-shadow-policy-trace-v1",
    ...
    fn take_scheduler_state(
            ...
            && target_radix.is_none()
        {
            let snapshot = self.kv_snapshot_load.as_ref().ok_or_else(|| {
                "an external cached-prefix request requires DS41RT_REAL_FULL_KV_SNAPSHOT_LOAD"
                    .to_owned()
            })?;
...
impl RealFullSchedulerRequestExecutor {
                ...
                for sample_index in 0..=MEASURED_SAMPLES {
                    ...
                    let requests = lane_indices
                        ...
                        .map(|(request_index, (lane_index, draft_tokens))| {
                            ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
                    ...
                    let cycles =
                        ds41rt_api::RealFullRequestExecutor::execute_real_full_decode_cycle_batch(
                            self, requests,
                        )
                        .into_iter()
    ...
    fn batched_dspark_cycles_eligible(&self, requests: &[ds41rt_api::RealFullRequest]) -> bool {
    ...
    fn complete_batched_dspark_finish(
    ...
    ) -> std::result::Result<ds41rt_api::RealFullDecodeCycle, String> {
        ...
        let generated_tokens = cycle_token_ids
            .into_iter()
            .map(|token_id| ds41rt_api::RealFullGeneratedToken {
                token_id,
                text: self.decode_sampled_token_text_cached(Some(token_id)),
            })
            .collect();
        Ok(ds41rt_api::RealFullDecodeCycle {
            info,
            generated_tokens,
        })
    }
    ...
    fn execute_real_full_decode_cycle_inner(
        &self,
        mut request: ds41rt_api::RealFullRequest,
    ) -> std::result::Result<ds41rt_api::RealFullDecodeCycle, String> {
        prewarm_flashinfer_cudnn_mla_suffix_graphs_for_worker().map_err(format_error_chain)?;
        let request_start = Instant::now();
...
impl ds41rt_api::RealFullRequestExecutor for RealFullSchedulerRequestExecutor {
    fn execute_real_full_request(
        &self,
        request: ds41rt_api::RealFullRequest,
    ) -> std::result::Result<ds41rt_api::RealFullInfo, String> {
    ...
    fn real_full_decode_cycle_batch_max_size(
        &self,
        _request: &ds41rt_api::RealFullRequest,
    ) -> usize {
        self.max_execution_lanes
    }
    ...
    fn execute_real_full_decode_cycle_batch(
        &self,
        requests: Vec<ds41rt_api::RealFullRequest>,
    ) -> Vec<std::result::Result<ds41rt_api::RealFullDecodeCycle, String>> {
...
fn real_full_request_prefill_chunk_tokens() -> usize {
    env::var("DS41RT_REAL_FULL_REQUEST_PREFILL_CHUNK_TOKENS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .filter(|tokens| *tokens > 0)
...
pub(crate) fn load_real_full_serving(
    ...
    anyhow::ensure!(
        ...
        "real-ds4-full serving requires DS41RT_B12X=1 for startup-captured production kernels"
    );
    anyhow::ensure!(
    ...
    let executor: Arc<dyn ds41rt_api::RealFullRequestExecutor> =
        if real_full_request_thread_pinned_enabled() {
            ...
            let executor =
                ds41rt_api::ThreadPinnedRealFullRequestExecutor::spawn_pool_with_cpu_affinity(
                    "ds41rt-real-full-request-worker",
                    scheduler_executor,
                    worker_count,
                    &worker_cpus,
                )
            ...
            if let Some(prompts) = prewarm_prompts.as_ref() {
                if max_execution_lanes > 1 && real_full_dspark_enabled() {
                    ds41rt_api::RealFullRequestExecutor::finish_real_full_sequence(
                        &scheduler_executor,
                ...
                prewarm_real_full_serving_requests(
                    |request| {
                        ds41rt_api::RealFullRequestExecutor::execute_real_full_decode_cycle(
                            &scheduler_executor,
                            request,
                        )
                    },
...
fn prewarm_real_full_serving_requests(
    ...
    let mut max_chunk_request = ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
        0,
    ...
    if canonical_chunk_tokens < configured_prefill_chunk_tokens {
        ...
        let mut canonical_request = ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
            0,
    ...
    for (prompt_index, (prompt, prompt_tokens)) in prompts.iter().enumerate() {
            ...
            && *prompt_tokens == REAL_FULL_SERVE_NO_SELECTOR_DSA_BOUNDARY_PROMPT_TOKENS
        {
            ...
            let mut request = ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
                0,
                &sequence_id,
                &boundary_prompt,
                *prompt_tokens,
        ...
        let request = ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
            0,
            &sequence_id,
            prompt,
            *prompt_tokens,
    ...
    let recurrent = ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
    ...
    if let Some(probe) = prefix_prefill_probe {
        ...
        for (case_index, case) in probe.cases.iter().enumerate() {
            ...
            for repeat_index in 0..probe.repeats {
                ...
                let seed_info = execute(ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
                    20_000_u64
    ...
    if canonical_workspace_complete {
        ...
        for (prompt_index, (prompt, prompt_tokens)) in prompts.iter().rev().enumerate() {
            ...
            let capture_info = execute(ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
    ...
    let dsa_selector_seed = if dsa_selector_query_rows.is_empty() {
        ...
        let mut seed_request = ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
            29_999,
            &sequence_id,
            prompt,
            *prompt_tokens,
    ...
    if let Some((sequence_id, mut prompt, mut prompt_tokens)) = dsa_selector_seed {
        for (step_index, query_rows) in dsa_selector_query_rows.iter().copied().enumerate() {
            ...
            let mut sweep_request = ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
                30_000_u64
                    .checked_add(
...
fn prewarm_real_full_dspark_width_cohort(
    ...
    let initial_cycle = execute(ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
    ...
    for (width_index, draft_tokens) in draft_widths.iter().copied().enumerate() {
        ...
        let verify_cycle = execute(ds41rt_api::RealFullRequest::new_decode_step_for_sequence(
            request_index,
            &sequence_id,
            dspark_prompt,
            dspark_prompt_tokens,
...
fn real_full_sparse_tcp_targets_from_args(
    ...
    if dispatch_transport == RealFullSchedulerSparseDispatchTransport::VerbsHost {
        ds41rt_transport::verbs_host_preflight()
...
fn initialize_sparse_tcp_dispatch_status(
    info: &mut ds41rt_api::RealFullInfo,
...
) -> Result<ds41rt_api::RealFullInfo> {
...
mod tests {
    ...
    use anyhow::Result;
    use ds41rt_core::{KvCacheDType, DEFAULT_MODEL_ID, DS4_KV_SOURCE_PAGE_TOKENS, EXPERT_HOSTS};
    use ds41rt_loader::LoadedTokenizer;
    use std::fs;
    use std::sync::Arc;
    ...
    fn speculative_terminal_report_preserves_active_model_hidden_width() {
        let sample = RealFullSpeculativeTerminalSample {
            hidden_dim: ds41rt_core::DS4_FLASH_HIDDEN_SIZE,
            vocab_size: 129_280,
            top_token_id: 17,
            sampled_token_id: 19,
            sample_top_k: 8,
        ...
        assert_eq!(report.hidden_dim, ds41rt_core::DS4_FLASH_HIDDEN_SIZE);
    ...
    fn target_radix_reuse_waits_for_compressor_state_replay() {
        let flash = ds41rt_core::ModelFacts::default();
    ...
    fn real_full_sparse_verbs_host_targets_and_plan_follow_rdma_preflight() {
        ...
        let preflight_ok = ds41rt_transport::verbs_host_preflight().is_ok();
    ...
    fn real_full_request_mtp_rows_default_to_disabled_for_live_serve() {
        let first_decode_loop_step =
            ds41rt_api::RealFullRequest::new_decode_step(1, "user: hi", 3, 4, Vec::new(), 0, 4);
    ...
    fn real_full_request_mtp_rows_are_opt_in_and_disabled_for_recurrent_or_single_token_decode() {
        ...
        let later_decode_loop_step =
            ds41rt_api::RealFullRequest::new_decode_step(2, "user: hi", 3, 4, vec![13], 1, 4);
        let standalone_single_step =
            ds41rt_api::RealFullRequest::new_decode_step(3, "user: hi", 3, 1, Vec::new(), 0, 1);
    ...
    fn request_prompt_token_ids_tokenizes_prompt_with_loaded_tokenizer() {
        ...
        let request = ds41rt_api::RealFullRequest::new_decode_step(
            9,
            "user: Use real full.",
            1,
            1,
