...
fn scheduler_smoke_reports_prefill_rows_per_expert_distribution() {
    ...
    assert_eq!(report.prefill_active_experts, DS4_FLASH_ROUTED_EXPERTS);
    assert_eq!(report.prefill_rows_per_expert_min, 12);
    assert_eq!(report.prefill_rows_per_expert_max, 12);
    assert!((report.prefill_rows_per_expert_avg - 12.0).abs() < 1.0e-9);
}
