...
use ds41rt_core::{
    ...
    Priority, DS4_FLASH_ROUTED_EXPERTS, DS4_FLASH_TOP_K,
};
use serde::Serialize;
...
fn scheduler_smoke_report(args: SchedulerSmokeArgs) -> SchedulerSmokeReport {
    ...
    let mut rows_per_expert = vec![0_usize; DS4_FLASH_ROUTED_EXPERTS];
        ...
        || decode_created < args.decode_arrivals
    {
        ...
        if admission
            .deferred
            .iter()
            .any(|wave| matches!(wave.mode, ds41rt_core::LayerWaveMode::Prefill))
        {
        ...
        for selected in admission.selected {
            match selected.mode {
                ds41rt_core::LayerWaveMode::Decode => {
                ...
                ds41rt_core::LayerWaveMode::Prefill => {
    ...
    SchedulerSmokeReport {
        benchmark: "deepseek_v4_flash_scheduler_prefill_decode_interleaving".to_owned(),
        ...
        prefill_rows_per_expert_avg: total_prefill_expert_rows as f64
            / DS4_FLASH_ROUTED_EXPERTS as f64,
...
fn accumulate_prefill_expert_rows(wave: &LayerWave, rows_per_expert: &mut [usize]) {
    for source in &wave.row_sources {
        for row_offset in 0..source.row_count {
            ...
            for slot in 0..DS4_FLASH_TOP_K {
                let expert_id =
                    (token_position * DS4_FLASH_TOP_K + slot) % DS4_FLASH_ROUTED_EXPERTS;
