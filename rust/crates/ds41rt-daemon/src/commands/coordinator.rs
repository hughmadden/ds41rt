...
use ds41rt_core::{KvCacheAllocator, KvCacheConfig, KvCacheDType, ModelFacts};
...
fn coordinator_backend(value: &str) -> Result<ds41rt_api::ApiBackend> {
    match value {
        "tiny" | "synthetic-ds4-layer" | "real-ds4-full" => {
            Ok(ds41rt_api::ApiBackend::parse(value).expect("matched coordinator backend parses"))
        }
...
pub(crate) async fn run_coordinator(args: CoordinatorArgs) -> Result<()> {
    ...
    let transport = ds41rt_api::ApiTransport::parse(&args.transport)
    ...
    let mut python_capture_task = Some(
        std::thread::Builder::new()
            .name("ds41rt-python-capture-init".to_owned())
    ...
    let real_full_serving = if backend == ds41rt_api::ApiBackend::RealDs4Full {
    ...
    let api_config = ds41rt_api::ApiConfig {
        backend,
        transport,
    ...
    axum::serve(listener, ds41rt_api::router_with_config(api_config)).await?;
    Ok(())
}
...
mod tests {
    ...
    use ds41rt_core::{KvCacheDType, KvLayout, MlaKvCacheRepresentation, ModelFacts};
