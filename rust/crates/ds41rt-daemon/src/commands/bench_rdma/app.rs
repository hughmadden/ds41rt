...
use ds41rt_ffi::{
    c_char_array_to_string, Ds41rtRdmaRcCompletionStats, Ds41rtRdmaRcEndpointInfo, NativeLibrary,
    DS41RT_STATUS_RDMA_UNAVAILABLE,
};
...
pub(super) struct Ds41rtVerbsAppBenchmarkAttempt {
    ...
    pub(super) protocol_v2_control_plane_dry_run: Ds41rtVerbsAppControlPlaneDryRun,
    ...
    pub(super) app_transport_runs: Vec<Ds41rtVerbsAppRun>,
    pub(super) app_transport_error: Option<String>,
    pub(super) payloads: Vec<Ds41rtVerbsAppPayload>,
    pub(super) chain_hops: usize,
    pub(super) protocol_v2_chain_plans: Vec<Ds41rtVerbsAppChainPlan>,
    pub(super) native_rdma_probe: Ds41rtVerbsNativeRdmaProbe,
}

#[derive(Debug, Serialize)]
pub(super) struct Ds41rtVerbsAppPayload {
...
#[derive(Debug, Serialize)]
pub(super) struct Ds41rtVerbsAppChainPlan {
...
#[derive(Debug, Serialize)]
pub(super) struct Ds41rtVerbsAppControlPlaneDryRun {
...
#[derive(Debug, Serialize)]
pub(super) struct Ds41rtVerbsNativeRdmaProbe {
    pub(super) attempted: bool,
...
pub(super) struct Ds41rtVerbsAppNativeEndpointDescriptor {
...
pub(super) struct Ds41rtVerbsAppRun {
    ...
    pub(super) local_endpoint: Ds41rtVerbsAppNativeEndpointDescriptor,
    pub(super) peer_endpoint: Ds41rtVerbsAppNativeEndpointDescriptor,
}

#[derive(Debug, Serialize, Deserialize)]
struct Ds41rtVerbsAppRunStart {
    message: String,
    run_kind: String,
    payload_index: usize,
    row_count: usize,
    ...
    client_native_endpoint: Ds41rtVerbsAppNativeEndpointDescriptor,
}

#[derive(Debug, Serialize, Deserialize)]
struct Ds41rtVerbsAppRunReady {
    message: String,
    payload_index: usize,
    server_endpoint: VerbsHostRcEndpointDescriptor,
    server_native_endpoint: Ds41rtVerbsAppNativeEndpointDescriptor,
}

#[derive(Debug, Serialize, Deserialize)]
struct Ds41rtVerbsAppRecvReady {
    message: String,
    payload_index: usize,
}

pub(super) fn ds41rt_verbs_app_benchmark_attempt(
    resolved_mode: &str,
    peer: Option<String>,
    payload_bytes: &[usize],
) -> Result<Ds41rtVerbsAppBenchmarkAttempt> {
    ...
    Ok(Ds41rtVerbsAppBenchmarkAttempt {
        attempted: true,
...
fn protocol_v2_control_plane_dry_run(
...
) -> Result<Ds41rtVerbsAppControlPlaneDryRun> {
    ...
    let (local_endpoint_role, peer_endpoint_required) = match app_role {
        ...
        other => anyhow::bail!("unsupported DS41RT verbs app role: {other}"),
    };
    ...
    Ok(Ds41rtVerbsAppControlPlaneDryRun {
        ...
        data_plane_blocker: ds41rt_transport::verbs_host_app_transport_blocker().to_owned(),
    })
}
...
fn app_role_for_resolved_mode(resolved_mode: &str) -> Result<&'static str> {
    match resolved_mode {
        ...
        other => anyhow::bail!("unsupported DS41RT verbs app resolved mode: {other}"),
    }
}
...
fn native_rdma_probe(
    ...
    request_frame: &[u8],
    response_frame: &[u8],
) -> Ds41rtVerbsNativeRdmaProbe {
    let native_library_path = native_library_path();
    let mut probe = Ds41rtVerbsNativeRdmaProbe {
        attempted: true,
    ...
    let Some(path) = native_library_path else {
        probe.native_library_error = Some(
            "native library not found; set DS41RT_NATIVE_LIB or run just test-native".to_owned(),
        );
        return probe;
    };
...
fn native_library_path() -> Option<PathBuf> {
    ...
    let cwd_path = PathBuf::from("native/build/libds41rt_native.so");
...
pub(super) fn run_ds41rt_verbs_app_benchmark(
    resolved_mode: &str,
    peer: Option<&str>,
    port: u16,
    payloads: &[Ds41rtVerbsAppPayload],
    duration_secs: u64,
) -> Result<Vec<Ds41rtVerbsAppRun>> {
    match app_role_for_resolved_mode(resolved_mode)? {
        "server" => run_ds41rt_verbs_app_server(peer, port, payloads),
        "client" => {
            let peer = peer.context("DS41RT verbs app client requires --peer")?;
            run_ds41rt_verbs_app_client(peer, port, payloads, duration_secs)
        }
...
impl<'a> NativeRdmaEndpoint<'a> {
    ...
    fn native_descriptor(&self) -> Ds41rtVerbsAppNativeEndpointDescriptor {
        Ds41rtVerbsAppNativeEndpointDescriptor {
    ...
    fn connect(&self, peer: &Ds41rtVerbsAppNativeEndpointDescriptor) -> Result<()> {
    ...
    fn poll(&self, send: u32, recv: u32) -> Result<Ds41rtRdmaRcCompletionStats> {
        self.library
...
fn run_ds41rt_verbs_app_server(
    peer: Option<&str>,
    port: u16,
    payloads: &[Ds41rtVerbsAppPayload],
) -> Result<Vec<Ds41rtVerbsAppRun>> {
    let native_path = native_library_path().context(
        "native library not found; set DS41RT_NATIVE_LIB or run a native RDMA-enabled build",
    )?;
        ...
        match listener.accept() {
            ...
            Err(err) if err.kind() == std::io::ErrorKind::WouldBlock => {
                if Instant::now() >= deadline {
                    anyhow::bail!("timed out waiting for DS41RT verbs app client on {bind_addr}");
                }
                thread::sleep(Duration::from_millis(25));
            }
            Err(err) => return Err(err).context("accepting DS41RT verbs app client"),
        }
    };
    ...
    for _ in 0..expected_runs {
        let start: Ds41rtVerbsAppRunStart = read_control(&mut reader)?;
        if start.message != "run_start" {
            anyhow::bail!(
                "DS41RT verbs app server expected run_start, got {}",
                start.message
            );
        }
        let payload = payloads.get(start.payload_index).with_context(|| {
            format!(
                "DS41RT verbs app server payload index {} is out of range",
                start.payload_index
            )
        })?;
...
fn run_ds41rt_verbs_app_client(
    peer: &str,
    port: u16,
    payloads: &[Ds41rtVerbsAppPayload],
    duration_secs: u64,
...
fn execute_client_app_run(
    ...
    let start = Ds41rtVerbsAppRunStart {
    ...
    let ready: Ds41rtVerbsAppRunReady = read_control(reader)?;
    if ready.message != "run_ready" || ready.payload_index != payload_index {
        anyhow::bail!("DS41RT verbs app client received invalid run_ready message");
    }
    ...
    let recv_ready: Ds41rtVerbsAppRecvReady = read_control(reader)?;
    if recv_ready.message != "recv_ready" || recv_ready.payload_index != payload_index {
        anyhow::bail!("DS41RT verbs app client received invalid recv_ready message");
    }
...
fn execute_server_app_run(
    ...
    {
        anyhow::bail!("DS41RT verbs app server run_start payload metadata did not match local plan");
    }
    ...
    let ready = Ds41rtVerbsAppRunReady {
...
fn build_app_run_report(
    ...
    poll_iterations: u64,
    local_endpoint: Ds41rtVerbsAppNativeEndpointDescriptor,
    peer_endpoint: Ds41rtVerbsAppNativeEndpointDescriptor,
) -> Ds41rtVerbsAppRun {
...
fn connect_control_stream(peer: &str, port: u16) -> Result<TcpStream> {
    ...
    let mut addrs = addr
        .to_socket_addrs()
        .with_context(|| format!("resolving DS41RT verbs app peer {addr}"))?;
    let first = addrs
        .next()
        .with_context(|| format!("DS41RT verbs app peer {addr} resolved no addresses"))?;
    TcpStream::connect_timeout(&first, timeout)
        .with_context(|| format!("connecting DS41RT verbs app control plane to {addr}"))
}
...
fn read_control<T: DeserializeOwned>(reader: &mut BufReader<TcpStream>) -> Result<T> {
    ...
    if bytes == 0 {
        anyhow::bail!("DS41RT verbs app control plane closed");
    }
...
fn validate_payload_matches_frames(
    ...
    if payload.request_wire_bytes != request_frame.len() {
        anyhow::bail!(
            "DS41RT verbs app request frame bytes {} did not match payload {}",
            request_frame.len(),
            payload.request_wire_bytes
        );
    }
    if payload.response_wire_bytes != response_frame.len() {
        anyhow::bail!(
            "DS41RT verbs app response frame bytes {} did not match payload {}",
            response_frame.len(),
            payload.response_wire_bytes
        );
    }
...
fn verbs_app_control_timeout() -> Duration {
    let secs = parse_env_u64("DS41RT_VERBS_APP_CONTROL_TIMEOUT_SECS", 60).unwrap_or(60);
...
fn verbs_app_roundtrip_iterations(
    ...
    if let Some(value) = parse_optional_env_usize("DS41RT_VERBS_APP_ROUNDTRIP_ITERATIONS")? {
...
fn verbs_app_chain_iterations() -> Result<usize> {
    parse_optional_env_usize("DS41RT_VERBS_APP_CHAIN_ITERATIONS")
        .map(|value| value.unwrap_or(1).max(1))
}

fn verbs_app_chain_hops() -> Result<usize> {
    parse_optional_env_usize("DS41RT_VERBS_APP_CHAIN_HOPS")
...
fn protocol_v2_payloads_for_benchmark(
    payload_bytes: &[usize],
) -> Result<Vec<Ds41rtVerbsAppPayload>> {
    ...
    payload_bytes
        ...
        .map(|(idx, payload)| {
            if payload % hidden_row_bytes != 0 {
                anyhow::bail!(
                    "DS41RT app payload {payload} is not an exact multiple of hidden row bytes {hidden_row_bytes}"
                );
            }
            let row_count = payload / hidden_row_bytes;
            if row_count == 0 {
                anyhow::bail!("DS41RT app payload must contain at least one hidden row");
            }
...
fn protocol_v2_chain_plans_for_payloads(
    payloads: &[Ds41rtVerbsAppPayload],
    hops: usize,
) -> Result<Vec<Ds41rtVerbsAppChainPlan>> {
    payloads
        .iter()
...
fn protocol_v2_chain_plan_for_payload(
    payload: &Ds41rtVerbsAppPayload,
    hops: usize,
) -> Result<Ds41rtVerbsAppChainPlan> {
...
fn frame_arena_evidence(
    ...
    let registration_alignment = ds41rt_transport::verbs_host_capabilities().preferred_alignment;
...
mod tests {
    ...
    fn app_protocol_v2_payloads_match_flash_hidden_rows() {
        ...
        let attempt =
            ds41rt_verbs_app_benchmark_attempt("app-client", Some("emu".to_owned()), &payloads)
                .unwrap();
    ...
    fn app_protocol_v2_control_plane_dry_run_tracks_roles() {
        ...
        let server =
            ds41rt_verbs_app_benchmark_attempt("app-server", Some("emu".to_owned()), &payloads)
                .unwrap();
        let client =
            ds41rt_verbs_app_benchmark_attempt("app-client", Some("emu".to_owned()), &payloads)
                .unwrap();
        let capability =
            ds41rt_verbs_app_benchmark_attempt("app-capability", None, &payloads).unwrap();
