...
/// unit-testable. `host_cache` and `host_cache_config` are explicit nulls when
/// the cache is off, so clients can tell "cache off" from "no stats yet".
fn stats_json(
    host_cache: Option<ds41rt_hostcache::metrics::Snapshot>,
    host_cache_config: Option<&ds41rt_hostcache::config::Config>,
    queue_len: usize,
    admission: &AdmissionStats,
) -> serde_json::Value {
    serde_json::json!({
        "host_cache": host_cache,
        // Deliberately exports the cache's whole effective `Config` (packet
        // HC-9), not just `store_pace_ns`: fleet operators tune several of
        // these knobs, and one key keeps the export forward-compatible as new
        // knobs land.
        "host_cache_config": host_cache_config,
    ...
    loop {
        ...
        if stats_published.elapsed() >= std::time::Duration::from_secs(1) {
            ...
            if let Ok(mut slot) = stats.lock() {
                *slot = stats_json(prefixes.host_metrics(), prefixes.host_config(),
                    deferred.len(), &admission_stats);
            }
        }
...
mod admission_stats_tests {
    ...
    fn stats_json_publishes_null_host_cache_and_every_counter() {
        ...
        // them): `host_cache` and `host_cache_config` are explicit nulls when
        // the cache is off, and every admission counter — including queue
        // rejects — is present.
        ...
        let value = stats_json(None, None, 5, &stats);
        assert_eq!(value["host_cache"], serde_json::Value::Null);
        assert_eq!(value["host_cache_config"], serde_json::Value::Null);
