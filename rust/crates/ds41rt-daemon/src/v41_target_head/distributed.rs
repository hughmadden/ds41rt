...
impl<'w, 'a> Rank<'w, 'a> {
    fn new(library: &'a NativeLibrary, weights: &'w VocabularyShard<'a>, capacity: usize) -> Result<Self> {
        ...
        Ok(Self {
            ...
            weights, capacity, graphs: [None; 80],
        })
    }
    ...
    async unsafe fn execute(&mut self, input: Ds41rtDeviceBuffer, rows: usize) -> Result<()> {
        let queued = (|| -> Result<()> { unsafe {
            ...
            if let Some(graph) = self.graphs[rows - 1] { lib.cuda_graph_launch(graph, self.stream.raw) }
...
impl<'w, 'a> DistributedVocabularyWave<'w, 'a> {
    ...
    pub async unsafe fn execute(&mut self, normalized: Ds41rtDeviceBuffer, rows: usize) -> Result<()> {
        ...
        let first_work = async {
            first_device.future(unsafe { first.get_mut().execute(normalized, rows) }).await?;
        ...
        let second_work = second_device.future(unsafe { second.get_mut().execute(normalized, rows) });
...
mod tests {
    ...
    fn distributed_vocabulary_real_weights_match_full_and_cancel_safely() -> Result<()> {
        ...
        let full_logits = Allocation::new(devices[1], 16 * 129280 * 4)?;
        let input = Allocation::new(devices[1], 16 * 10240)?;
        let values: Vec<u8> = (0..16 * 5120).flat_map(|i| {
            let value = ((i * 17 % 127) as f32 - 63.) / 128.;
            ((value.to_bits() >> 16) as u16).to_ne_bytes()
        }).collect();
        devices[1].run(|| lib.copy_h2d(input.buffer, &values))?;
        let other_input = Allocation::new(devices[1], 16 * 10240)?;
        let other_values: Vec<u8> = (0..16 * 5120).flat_map(|i| {
            let value = ((i * 19 % 127) as f32 - 61.) / 64.;
            ((value.to_bits() >> 16) as u16).to_ne_bytes()
        }).collect();
        devices[1].run(|| lib.copy_h2d(other_input.buffer, &other_values))?;
        let budgets = DistributedVocabularyWave::device_bytes(16, 64640)?;
        let mut first = DistributedVocabularyWave::new(devices, [&a, &b], 16, budgets)?;
        let mut second = DistributedVocabularyWave::new(devices, [&a, &b], 16, budgets)?;
        let runtime = tokio::runtime::Builder::new_current_thread().build()?;
        for rows in [1, 3, 16, 3] {
