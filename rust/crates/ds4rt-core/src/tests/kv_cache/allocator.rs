...
fn mtp_tentative_resolution_rejects_accepted_count_beyond_draft() {
    ...
    assert!(matches!(
        err,
        Ds4rtError::MtpAcceptedTokensExceedDraft {
            accepted_tokens: 3,
            draft_tokens: 2
        }
    ));
...
fn direct_mtp_transaction_rejects_nonconsecutive_writes_without_mutation() {
    ...
    let error = allocator
        ...
        .unwrap_err();
    assert!(matches!(error, Ds4rtError::InvalidMtpKvTransaction { .. }));
    assert!(write_ids
        .iter()
