...
#[test]
fn deepseek_v4_flash_kv_cache_tracks_hybrid_index_layers() {
    let facts = ModelFacts::default();
    let config = KvCacheConfig::deepseek_v4_hybrid_bf16(128, &facts);

    assert_eq!(config.layout, KvLayout::DeepseekV4HybridBf16);
    assert_eq!(config.layers, DS4_FLASH_NUM_HIDDEN_LAYERS);
    assert_eq!(config.key_value_width, DS4_HEAD_DIM);
    assert_eq!(config.dsa_indexer_layers, 21);
    assert_eq!(config.dsa_indexer_layer_ids().first(), Some(&2));
    assert_eq!(config.dsa_indexer_layer_ids().last(), Some(&42));
    assert_eq!(config.layer_bytes_per_token(LayerId(0)), 1_024);
    assert_eq!(config.layer_bytes_per_token(LayerId(2)), 1_280);
    assert_eq!(config.bytes_per_token(), 49_408);
    assert_eq!(
        (0..facts.num_hidden_layers)
...
#[test]
fn deepseek_v4_kv_cache_fails_closed_on_invalid_attention_schedule() {
    let mut facts = ModelFacts::default();
    facts.compress_ratios.pop();

    let error = KvCacheConfig::try_deepseek_v4_hybrid_bf16(128, &facts)
        .unwrap_err()
        .to_string();
...
fn kv_cache_dtype_parser_accepts_runtime_cache_labels() {
    assert_eq!(
        KvCacheDType::parse_cache_dtype("bf16"),
        Some(KvCacheDType::Bf16)
    );
    assert_eq!(
        KvCacheDType::parse_cache_dtype("FP8"),
        Some(KvCacheDType::Fp8)
    );
    assert_eq!(
        KvCacheDType::parse_cache_dtype("nvfp4"),
        Some(KvCacheDType::Nvfp4)
    );
    assert_eq!(KvCacheDType::parse_cache_dtype("int4"), None);
}
