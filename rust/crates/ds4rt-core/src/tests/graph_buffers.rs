...
#[test]
fn flash_decode_graph_contract_matches_protocol_v2_row_shape() {
    let contract = ExpertGraphBufferContract::ds4_flash_bf16(
        LayerId(3),
    ...
    assert_eq!(contract.hidden_rows.hidden_dim, DS4_FLASH_HIDDEN_SIZE);
    assert_eq!(
    ...
    assert_eq!(contract.route_metadata.route_capacity(), DS4_FLASH_TOP_K);
    assert_eq!(
        contract.route_metadata.bytes(),
        DS4_FLASH_TOP_K * EXPERT_GRAPH_ROUTE_ENTRY_BYTES
            + EXPERT_GRAPH_ROW_ROUTE_COUNT_BYTES
            + EXPERT_GRAPH_HOST_ROW_GLOBAL_INDEX_BYTES
            + EXPERT_GRAPH_ACTIVE_COUNTS_BYTES
    );
    assert_eq!(contract.workspace.max_expert_tiles, DS4_FLASH_TOP_K);
    contract
        .validate_active_counts(ExpertGraphActiveCounts {
            rows: 1,
            routes: DS4_FLASH_TOP_K,
            expert_tiles: DS4_FLASH_TOP_K,
        })
        .unwrap();
}
...
fn graph_keys_do_not_split_compatible_rows_by_source_mode() {
    ...
    let decode = ExpertGraphBufferContract::ds4_flash_bf16(
        LayerId(3),
        LayerWaveMode::Decode,
        GraphBucket::new(16),
        recipe.clone(),
    ...
    .unwrap();
    let prefill = ExpertGraphBufferContract::ds4_flash_bf16(
        LayerId(3),
        LayerWaveMode::Prefill,
        GraphBucket::new(16),
        recipe.clone(),
    ...
    .unwrap();
    let mtp = ExpertGraphBufferContract::ds4_flash_bf16(
        LayerId(3),
...
#[test]
fn flash_prefill_graph_contract_keeps_fixed_bucket_for_smaller_active_counts() {
    let contract = ExpertGraphBufferContract::ds4_flash_bf16(
        LayerId(9),
        LayerWaveMode::Prefill,
        GraphBucket::new(512),
    ...
    assert_eq!(
        contract.request_payload_bytes(),
        512 * DS4_FLASH_HIDDEN_BF16_BYTES
    );
    assert_eq!(
        contract.response_payload_bytes(),
        512 * DS4_FLASH_HIDDEN_BF16_BYTES
    ...
    assert_eq!(
        contract.route_metadata.route_capacity(),
        512 * DS4_FLASH_TOP_K
    );
    assert_eq!(
        contract.route_metadata.bytes(),
        512 * DS4_FLASH_TOP_K * EXPERT_GRAPH_ROUTE_ENTRY_BYTES
            + 512 * EXPERT_GRAPH_ROW_ROUTE_COUNT_BYTES
            + 512 * EXPERT_GRAPH_HOST_ROW_GLOBAL_INDEX_BYTES
            + EXPERT_GRAPH_ACTIVE_COUNTS_BYTES
    );
    assert_eq!(
        contract.workspace.max_expert_tiles,
...
fn graph_instance_pool_acquires_and_releases_mixed_source_bucket() {
    ...
    assert!(lease.fixed_buffer_bytes > host_batch.num_rows() * DS4_FLASH_HIDDEN_BF16_BYTES);
    assert_eq!(
        pool.stats(),
...
#[test]
fn flash_dspark_graph_contract_rejects_active_counts_outside_bucket() {
    let contract = ExpertGraphBufferContract::ds4_flash_bf16(
        LayerId(11),
        LayerWaveMode::MtpVerify,
        GraphBucket::new(8),
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::GraphBufferActiveCountOutOfBounds { field: "rows", .. })
    ));
    assert!(matches!(
        contract.validate_active_counts(ExpertGraphActiveCounts {
            rows: 2,
            routes: 2 * DS4_FLASH_TOP_K + 1,
            expert_tiles: 1,
        }),
        Err(Ds4rtError::GraphBufferActiveCountOutOfBounds {
