...
fn expert_batch_mixes_decode_and_prefill_rows_with_fixed_envelope() {
    ...
    assert_eq!(batch.route_count(), 16 * DS4_FLASH_TOP_K);
...
fn expert_batch_reconstructs_partials_in_batch_row_order() {
    ...
    assert!(matches!(
        err,
        Ds4rtError::ExpertBatchPartialRowCountMismatch {
            expected: 3,
            actual: 1
        }
    ));
...
fn expert_host_batch_scatters_partials_back_to_global_rows() {
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchPartialRowCountMismatch {
            expected: 1,
            actual: 0
        })
    ));
...
fn expert_host_batch_set_partitions_with_explicit_owner_lookup() {
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchSetRouteCountMismatch {
            expected: 12,
            actual: 11,
        })
    ));
...
fn expert_host_batch_accumulator_validates_shapes() {
    let batch = small_prefill_batch(2);
    let hosts = expert_hosts();
    let routes = routes_with_experts(&batch, &[&[0, 4, 8, 12, 16, 20], &[1, 5, 9, 13, 0, 4]]);
    let host_batch = ExpertHostBatch::from_expert_batch(
        &batch,
        "spark-1",
        &routes,
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchPartialOutputWidthMismatch {
            row_index: 0,
            expected_width: 2,
            actual_width: 1,
        })
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchPartialAccumulatorSizeMismatch {
            expected_values: 4,
            actual_values: 3,
        })
    ));
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchContributionCountMismatch {
            expected: 2,
            actual: 1,
        })
    ));
...
fn expert_tp4_batch_replicates_every_route_and_requires_four_sparks() {
    ...
    assert!(matches!(
        error,
        Ds4rtError::ExpertTensorParallelHostCountMismatch {
...
fn expert_host_batch_set_accumulates_partials_and_validates_host_count() {
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchSetPartialHostCountMismatch {
            expected: 4,
            actual: 3,
        })
    ));
...
fn expert_host_batch_set_validates_reconstruction_plan_before_accumulating() {
    ...
    assert!(matches!(
        err,
        Ds4rtError::ExpertHostBatchSetDuplicateHost { host } if host == duplicate_host
    ));
    ...
    assert!(matches!(
        bad.accumulate_partial_outputs_f32(&partials_by_host, 1),
        Err(Ds4rtError::ExpertHostBatchGlobalRowOutOfBounds {
            row_index: 2,
            row_count: 2,
        })
    ));
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchSetReconstructionPlanMissingGlobalRow { row_index: 0 })
    ));
}
...
fn expert_host_batch_rejects_bad_routes_and_unknown_hosts() {
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchUnknownHost { .. })
    ));
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchRouteCountMismatch {
            expected: 12,
            actual: 11
        })
    ));
    ...
    assert!(matches!(
        ...
        Err(Ds4rtError::ExpertHostBatchRouteRowMismatch {
            expected: 1,
            actual: 0
        })
    ));
