...
use crate::{
    owner_for_expert, DType, Ds4rtError, ExpertBatch, ExpertOwnerLookup, GraphBucket, LayerId,
...
impl PartialReconstructionPlan {
    pub fn validate_for_batches(
        ...
        if self.global_row_count != global_row_count {
            return Err(
                Ds4rtError::ExpertHostBatchSetReconstructionPlanGlobalRowCountMismatch {
        ...
        if self.host_row_maps.len() != batches.len() {
            return Err(
                Ds4rtError::ExpertHostBatchSetReconstructionPlanHostCountMismatch {
            ...
            batches.iter().zip(self.host_row_maps.iter()).enumerate()
        {
            if host_map.host != batch.host {
                return Err(
                    Ds4rtError::ExpertHostBatchSetReconstructionPlanHostMismatch {
                        map_index,
            ...
            if host_map.global_row_indices.len() != batch.rows.len() {
                return Err(
                    Ds4rtError::ExpertHostBatchSetReconstructionPlanRowCountMismatch {
                ...
                .enumerate()
            {
                if *actual >= global_row_count {
                    return Err(Ds4rtError::ExpertHostBatchGlobalRowOutOfBounds {
                        row_index: *actual,
                ...
                if *actual != row.global_row_index {
                    return Err(
                        Ds4rtError::ExpertHostBatchSetReconstructionPlanGlobalRowMismatch {
                            host: host_map.host.clone(),
                            host_row_index,
    ...
    pub fn accumulate_partial_outputs_f32<T: AsRef<[f32]>>(
        ...
        output_dim: usize,
    ) -> Result<ExpertHostBatchSetAccumulation, Ds4rtError> {
        if partial_outputs_by_host.len() != self.host_row_maps.len() {
            return Err(Ds4rtError::ExpertHostBatchSetPartialHostCountMismatch {
        ...
        let expected_values = self
            ...
            .ok_or_else(|| Ds4rtError::GraphBufferContractInvalid {
            ...
            .zip(partial_outputs_by_host.iter())
        {
            if partial_outputs.len() != host_map.global_row_indices.len() {
                return Err(Ds4rtError::ExpertHostBatchPartialRowCountMismatch {
                ...
                .enumerate()
            {
                ...
                if partial.len() != output_dim {
                    return Err(Ds4rtError::ExpertHostBatchPartialOutputWidthMismatch {
        ...
        for (row_index, count) in contribution_counts.iter().enumerate() {
            if *count == 0 {
                return Err(
                    Ds4rtError::ExpertHostBatchSetReconstructionPlanMissingGlobalRow { row_index },
                );
            }
        }
...
impl ExpertHostBatch {
    ...
    fn from_expert_batch_with_route_owner(
        ...
        if !expert_hosts.iter().any(|candidate| candidate == &host) {
            return Err(Ds4rtError::ExpertHostBatchUnknownHost { host });
        }
        if routes.len() != batch.route_count() {
            return Err(Ds4rtError::ExpertHostBatchRouteCountMismatch {
        ...
        for (global_row_index, row) in batch.rows.iter().enumerate() {
            ...
            let row_routes = routes.get(row.route_offset..end).ok_or(
                Ds4rtError::ExpertHostBatchRouteRangeOutOfBounds {
                    row_index: global_row_index,
            ...
            for route in row_routes {
                if route.row_index != global_row_index {
                    return Err(Ds4rtError::ExpertHostBatchRouteRowMismatch {
    ...
    pub fn scatter_partial_outputs<T: Clone>(
        ...
        partial_outputs: &[T],
        global_row_count: usize,
    ) -> Result<Vec<Option<T>>, Ds4rtError> {
...
impl ExpertHostBatchSet {
    ...
    pub fn tp4_from_expert_batch(
        ...
        if expert_hosts.len() != DS4_EXPERT_TP_WORLD_SIZE {
            return Err(Ds4rtError::ExpertTensorParallelHostCountMismatch {
    ...
    fn from_batches_with_route_replicas(
        ...
        if total_routes != expected_routes {
            return Err(Ds4rtError::ExpertHostBatchSetRouteCountMismatch {
...
fn validate_unique_hosts<'a>(hosts: impl IntoIterator<Item = &'a str>) -> Result<(), Ds4rtError> {
    let mut seen = BTreeSet::new();
    for host in hosts {
        if !seen.insert(host) {
            return Err(Ds4rtError::ExpertHostBatchSetDuplicateHost {
                host: host.to_owned(),
            });
        }
    }
