...
use crate::{Ds4rtError, KvBlockDescriptor, LayerId, PositionId};
...
impl KvCacheAllocator {
    ...
    pub fn pause(&mut self, id: u64) -> Result<(), Ds4rtError> {
        self.transition(id, KvReservationState::Paused)
    }

    pub fn resume(&mut self, id: u64) -> Result<(), Ds4rtError> {
        self.transition(id, KvReservationState::Active)
    }

    pub fn release(&mut self, id: u64) -> Result<(), Ds4rtError> {
    ...
    fn record_write_with_state(
        ...
        let reservation = self
            ...
            .ok_or(Ds4rtError::UnknownKvReservation(descriptor.reservation_id))?;
        ...
        if token_end > reservation.tokens {
            return Err(Ds4rtError::KvWriteOutOfBounds {
                token_start,
    ...
    pub fn mark_write_written(&mut self, id: u64) -> Result<(), Ds4rtError> {
    ...
    ) -> Result<(Vec<u64>, Vec<u64>), Ds4rtError> {
        if accepted_tokens > draft_tokens {
            return Err(Ds4rtError::MtpAcceptedTokensExceedDraft {
                accepted_tokens,
                draft_tokens,
            });
        }
    ...
    pub fn resolve_mtp_tentative_write_ids(
        ...
        let first = self
            .writes
            .get(&write_ids[0])
            .ok_or(Ds4rtError::UnknownKvWrite(write_ids[0]))?;
        ...
        for (offset, write_id) in write_ids.iter().copied().enumerate() {
            let write = self
                .writes
                .get(&write_id)
                .ok_or(Ds4rtError::UnknownKvWrite(write_id))?;
            let expected_token_start = token_start.checked_add(offset as u64).ok_or_else(|| {
                Ds4rtError::InvalidMtpKvTransaction {
            ...
            {
                return Err(Ds4rtError::InvalidMtpKvTransaction {
                    reason: format!(
    ...
    pub fn pause_write(&mut self, id: u64) -> Result<(), Ds4rtError> {
        self.transition_write(id, KvWriteState::Paused)
    }

    pub fn resume_write(&mut self, id: u64) -> Result<(), Ds4rtError> {
    ...
    fn transition(&mut self, id: u64, state: KvReservationState) -> Result<(), Ds4rtError> {
        let reservation = self
            .reservations
            .get_mut(&id)
            .ok_or(Ds4rtError::UnknownKvReservation(id))?;
        reservation.state = state;
        Ok(())
    }

    fn transition_write(&mut self, id: u64, state: KvWriteState) -> Result<(), Ds4rtError> {
        let write = self
            .writes
            .get_mut(&id)
            .ok_or(Ds4rtError::UnknownKvWrite(id))?;
        write.state = state;
        Ok(())
    }
}
