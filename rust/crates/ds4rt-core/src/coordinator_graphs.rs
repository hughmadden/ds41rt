...
use crate::{
    DType, Ds4rtError, GraphBucket, LayerId, LayerWaveMode, GLM52_FIRST_K_DENSE_REPLACE,
...
impl CoordinatorGraphShape {
    ...
    pub fn validate_layer(self, layer_id: LayerId) -> Result<(), Ds4rtError> {
        if self.accepts_layer(layer_id) {
            return Ok(());
        }
        Err(Ds4rtError::GraphBufferContractInvalid {
            reason: format!(
