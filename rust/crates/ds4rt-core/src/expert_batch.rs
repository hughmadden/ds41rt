...
use crate::{
    DType, Ds4rtError, GraphBucket, LayerId, LayerWave, PlacementVersion, PositionId, RequestId,
    RowSourceKind,
};
...
impl ExpertBatch {
    ...
    pub fn from_wave_with_envelope(
        ...
        if source_rows != wave.num_rows() {
            return Err(Ds4rtError::ExpertBatchRowCountMismatch {
                source_rows,
                hidden_rows: wave.num_rows(),
            });
        }
        if wave.num_rows() > graph_bucket.row_capacity {
            return Err(Ds4rtError::ExpertBatchMixRejected {
                reason: format!(
        ...
        Ok(Self {
            ...
            hidden_dtype,
            routed_experts: wave.route_metadata.routed_experts,
            graph_bucket,
    ...
    pub fn try_merge(&self, other: &Self) -> Result<Self, Ds4rtError> {
        if let Some(reason) = self.mix_rejection_reason(other) {
            return Err(Ds4rtError::ExpertBatchMixRejected { reason });
        }
    ...
    pub fn reconstruct_partial_outputs<T: Clone>(
        &self,
        partial_outputs: &[T],
    ) -> Result<Vec<(ExpertBatchRow, T)>, Ds4rtError> {
        if partial_outputs.len() != self.rows.len() {
            return Err(Ds4rtError::ExpertBatchPartialRowCountMismatch {
    ...
    fn mix_rejection_reason(&self, other: &Self) -> Option<String> {
        ...
        if self.routed_experts != other.routed_experts {
            return Some("different routed expert counts".to_owned());
        }
