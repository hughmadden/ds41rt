...
use crate::{Ds4rtError, LayerId, ModelFacts, PlacementVersion, PositionId, Priority, RequestId};
...
impl LayerWave {
    ...
    pub fn try_merge(&self, other: &Self) -> Result<Self, Ds4rtError> {
        if let Some(reason) = self.mix_rejection_reason(other) {
            return Err(Ds4rtError::LayerWaveMixRejected { reason });
        }
