...
impl RollingExpertRowPackAccumulator {
    pub fn new(config: RollingExpertRowPackConfig) -> Result<Self, Ds4rtError> {
    ...
    pub fn push_chunk(
        ...
        row_count: usize,
    ) -> Result<Vec<RollingExpertRowPackEmission>, Ds4rtError> {
    ...
    pub fn finish(&mut self) -> Result<Vec<RollingExpertRowPackEmission>, Ds4rtError> {
        ensure(!self.finished, "rolling row pack accumulator is finished")?;
        self.finished = true;
    ...
    fn emit_one(&mut self) -> Result<RollingExpertRowPackEmission, Ds4rtError> {
...
pub fn plan_rolling_expert_row_packs(
    ...
    row_count: usize,
    config: RollingExpertRowPackConfig,
) -> Result<RollingExpertRowPackPlan, Ds4rtError> {
...
) -> Result<CompletionFirstRoutePlan, Ds4rtError> {
...
fn ensure(condition: bool, reason: impl Into<String>) -> Result<(), Ds4rtError> {
    if condition {
        Ok(())
    } else {
        Err(rejected(reason))
...
fn rejected(reason: impl Into<String>) -> Ds4rtError {
    Ds4rtError::ExpertRoutePlanRejected {
        reason: reason.into(),
    }
}
