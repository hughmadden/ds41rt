...
use crate::{
    DType, Ds4rtError, ExpertHostBatch, ExpertHostBatchSet, GraphBucket, LayerId, LayerWaveMode,
    ModelFacts,
};
...
impl ExpertGraphInstancePool {
    ...
    pub fn register_contract(
        ...
        if instances == 0 {
            return Err(Ds4rtError::GraphBufferContractInvalid {
    ...
    pub fn register_ds4_flash_bf16(
        ...
        instances: usize,
    ) -> Result<ExpertGraphKey, Ds4rtError> {
    ...
    ) -> Result<ExpertGraphPoolLease, Ds4rtError> {
    ...
    ) -> Result<ExpertGraphHostBatchSetLease, Ds4rtError> {
    ...
    pub fn release(&mut self, lease: ExpertGraphPoolLease) -> Result<(), Ds4rtError> {
    ...
    fn best_entry_for_host_batch(
        &self,
        batch: &ExpertHostBatch,
    ) -> Result<Option<(usize, ExpertGraphActiveCounts)>, Ds4rtError> {
...
impl ExpertGraphBufferContract {
    ...
    pub fn for_model(
        ...
        let row_stride_bytes = hidden_exchange_row_stride_bytes(&dtype, facts.hidden_size)?;
        let max_rows = row_bucket.row_capacity;
        let route_metadata = RouteMetadataBufferContract {
            max_rows,
            max_local_routes_per_row: facts.top_k,
        ...
        let max_expert_tiles = route_metadata.route_capacity().min(facts.routed_experts);
    ...
    pub fn validate_active_counts(
        ...
        if counts.rows > self.hidden_rows.max_rows {
            return Err(Ds4rtError::GraphBufferActiveCountOutOfBounds {
                field: "rows",
                actual: counts.rows,
    ...
    pub fn active_counts_for_host_batch(
        &self,
        batch: &ExpertHostBatch,
    ) -> Result<ExpertGraphActiveCounts, Ds4rtError> {
...
fn hidden_exchange_row_stride_bytes(dtype: &DType, hidden_dim: usize) -> Result<usize, Ds4rtError> {
    match dtype {
        DType::Bf16 | DType::F16 => Ok(hidden_dim * std::mem::size_of::<u16>()),
        other => Err(Ds4rtError::GraphBufferContractInvalid {
