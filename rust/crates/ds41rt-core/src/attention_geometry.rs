...
use crate::{AttentionKind, Ds41rtError, ModelFacts, ModelVariant};
...
impl DeepseekV4AttentionPlan {
    pub fn from_model_facts(facts: &ModelFacts) -> Result<Self, Ds41rtError> {
    ...
    pub fn validate_sparkinfer_sm120_contract(&self) -> Result<(), Ds41rtError> {
...
fn validate_model_facts(facts: &ModelFacts) -> Result<(), Ds41rtError> {
    ensure_geometry(
...
fn ensure_geometry(condition: bool, reason: impl Into<String>) -> Result<(), Ds41rtError> {
    if condition {
        Ok(())
    } else {
        Err(Ds41rtError::InvalidDeepseekV4AttentionGeometry {
            reason: reason.into(),
        })
    }
}
