...
fn mtp_tentative_resolution_rejects_accepted_count_beyond_draft() {
    ...
    assert!(matches!(
        err,
        Ds41rtError::MtpAcceptedTokensExceedDraft {
            accepted_tokens: 3,
            draft_tokens: 2
        }
    ));
...
fn direct_mtp_transaction_rejects_nonconsecutive_writes_without_mutation() {
    ...
    let error = allocator
        ...
        .unwrap_err();
    assert!(matches!(error, Ds41rtError::InvalidMtpKvTransaction { .. }));
    assert!(write_ids
        .iter()
...
fn kv_cache_rejects_out_of_bounds_prefill_write() {
    ...
    assert!(matches!(
        err,
        Ds41rtError::KvWriteOutOfBounds {
            token_start: 4,
            token_count: 8,
...
fn kv_cache_capacity_is_enforced() {
    ...
    assert!(matches!(
        err,
        Ds41rtError::KvCapacityExceeded {
            requested_tokens: 2,
            available_tokens: 1
        }
    ));
