...
fn expert_host_batch_scatters_partials_back_to_global_rows() {
    ...
    assert!(matches!(
        ...
        Err(Ds41rtError::ExpertHostBatchPartialRowCountMismatch {
            expected: 1,
            actual: 0
        })
    ));
...
fn expert_host_batch_accumulator_validates_shapes() {
    ...
    assert!(matches!(
        ...
        Err(Ds41rtError::ExpertHostBatchPartialOutputWidthMismatch {
            row_index: 0,
            expected_width: 2,
            actual_width: 1,
        })
    ...
    assert!(matches!(
        ...
        Err(Ds41rtError::ExpertHostBatchPartialAccumulatorSizeMismatch {
            expected_values: 4,
            actual_values: 3,
        })
    ));
    ...
    assert!(matches!(
        ...
        Err(Ds41rtError::ExpertHostBatchContributionCountMismatch {
            expected: 2,
            actual: 1,
        })
    ));
...
fn expert_host_batch_set_accumulates_partials_and_validates_host_count() {
    ...
    assert!(matches!(
        ...
        Err(Ds41rtError::ExpertHostBatchSetPartialHostCountMismatch {
            expected: 4,
            actual: 3,
        })
    ));
...
fn expert_host_batch_set_validates_reconstruction_plan_before_accumulating() {
    ...
    assert!(matches!(
        err,
        Ds41rtError::ExpertHostBatchSetDuplicateHost { host } if host == duplicate_host
    ));
    ...
    assert!(matches!(
        bad.accumulate_partial_outputs_f32(&partials_by_host, 1),
        Err(Ds41rtError::ExpertHostBatchGlobalRowOutOfBounds {
            row_index: 2,
            row_count: 2,
        })
    ));
    ...
    assert!(matches!(
        ...
        Err(Ds41rtError::ExpertHostBatchSetReconstructionPlanMissingGlobalRow { row_index: 0 })
    ));
}
...
fn expert_host_batch_rejects_bad_routes_and_unknown_hosts() {
    ...
    assert!(matches!(
        ...
        Err(Ds41rtError::ExpertHostBatchUnknownHost { .. })
    ));
    ...
    assert!(matches!(
        ...
        Err(Ds41rtError::ExpertHostBatchRouteCountMismatch {
            expected: 12,
            actual: 11
        })
    ));
    ...
    assert!(matches!(
        ...
        Err(Ds41rtError::ExpertHostBatchRouteRowMismatch {
            expected: 1,
            actual: 0
        })
    ));
