...
use crate::{
    DType, Ds41rtError, ExpertHostBatch, ExpertHostBatchSet, GraphBucket, LayerId, LayerWaveMode,
    ModelFacts,
};
...
impl ExpertGraphInstancePool {
    ...
    fn best_entry_for_host_batch(
        &self,
        batch: &ExpertHostBatch,
    ) -> Result<Option<(usize, ExpertGraphActiveCounts)>, Ds41rtError> {
...
impl ExpertGraphBufferContract {
    ...
    pub fn validate_active_counts(
        ...
        if counts.rows > self.hidden_rows.max_rows {
            return Err(Ds41rtError::GraphBufferActiveCountOutOfBounds {
                field: "rows",
                actual: counts.rows,
    ...
    pub fn active_counts_for_host_batch(
        &self,
        batch: &ExpertHostBatch,
    ) -> Result<ExpertGraphActiveCounts, Ds41rtError> {
...
fn hidden_exchange_row_stride_bytes(dtype: &DType, hidden_dim: usize) -> Result<usize, Ds41rtError> {
    match dtype {
        DType::Bf16 | DType::F16 => Ok(hidden_dim * std::mem::size_of::<u16>()),
        other => Err(Ds41rtError::GraphBufferContractInvalid {
