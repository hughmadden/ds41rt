...
impl DeepseekV4CompressorLayerExecutionPlan {
    ...
    pub fn prefill(
        &self,
        source_tokens: usize,
    ) -> Result<Option<DeepseekV4CompressorPrefillPlan>, Ds41rtError> {
        let Some(main) = self.main.as_ref() else {
            return Ok(None);
        };
        if source_tokens == 0 {
...
pub fn deepseek_v4_compressor_execution_plans(
    facts: &ModelFacts,
) -> Result<Vec<DeepseekV4CompressorLayerExecutionPlan>, Ds41rtError> {
...
fn compressor_spec(
    ...
    head_dim: usize,
) -> Result<DeepseekV4CompressorSpec, Ds41rtError> {
...
fn projection_pair_width(spec: &DeepseekV4CompressorSpec) -> Result<usize, Ds41rtError> {
    spec.projected_width
        .checked_mul(2)
        .ok_or_else(|| invalid_compressor("compressor KV/gate projection width overflow"))
}

fn invalid_compressor(reason: impl Into<String>) -> Ds41rtError {
    Ds41rtError::InvalidDeepseekV4KvLayout {
        reason: reason.into(),
    }
}
