...
use crate::{Ds41rtError, KvBlockDescriptor, LayerId, PositionId};
...
impl KvCacheAllocator {
    ...
    pub fn reserve(
        ...
        if resident_tokens + tokens > self.config.max_tokens {
            return Err(Ds41rtError::KvCapacityExceeded {
    ...
    pub fn release(&mut self, id: u64) -> Result<(), Ds41rtError> {
    ...
    fn record_write_with_state(
        ...
        let reservation = self
            ...
            .ok_or(Ds41rtError::UnknownKvReservation(descriptor.reservation_id))?;
        ...
        if token_end > reservation.tokens {
            return Err(Ds41rtError::KvWriteOutOfBounds {
                token_start,
    ...
    pub fn mark_write_written(&mut self, id: u64) -> Result<(), Ds41rtError> {
    ...
    ) -> Result<(Vec<u64>, Vec<u64>), Ds41rtError> {
        if accepted_tokens > draft_tokens {
            return Err(Ds41rtError::MtpAcceptedTokensExceedDraft {
                accepted_tokens,
                draft_tokens,
            });
        }
    ...
    pub fn resolve_mtp_tentative_write_ids(
        ...
        let first = self
            .writes
            .get(&write_ids[0])
            .ok_or(Ds41rtError::UnknownKvWrite(write_ids[0]))?;
        ...
        for (offset, write_id) in write_ids.iter().copied().enumerate() {
            let write = self
                .writes
                .get(&write_id)
                .ok_or(Ds41rtError::UnknownKvWrite(write_id))?;
            let expected_token_start = token_start.checked_add(offset as u64).ok_or_else(|| {
                Ds41rtError::InvalidMtpKvTransaction {
            ...
            {
                return Err(Ds41rtError::InvalidMtpKvTransaction {
                    reason: format!(
    ...
    fn transition(&mut self, id: u64, state: KvReservationState) -> Result<(), Ds41rtError> {
        let reservation = self
            .reservations
            .get_mut(&id)
            .ok_or(Ds41rtError::UnknownKvReservation(id))?;
        reservation.state = state;
        Ok(())
    }

    fn transition_write(&mut self, id: u64, state: KvWriteState) -> Result<(), Ds41rtError> {
        let write = self
            .writes
            .get_mut(&id)
            .ok_or(Ds41rtError::UnknownKvWrite(id))?;
        write.state = state;
        Ok(())
    }
}
