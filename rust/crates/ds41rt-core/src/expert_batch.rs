...
use crate::{
    DType, Ds41rtError, GraphBucket, LayerId, LayerWave, PlacementVersion, PositionId, RequestId,
    RowSourceKind,
};
...
impl ExpertBatch {
    ...
    pub fn from_wave_with_envelope(
        ...
        if source_rows != wave.num_rows() {
            return Err(Ds41rtError::ExpertBatchRowCountMismatch {
                source_rows,
                hidden_rows: wave.num_rows(),
            });
        }
        if wave.num_rows() > graph_bucket.row_capacity {
            return Err(Ds41rtError::ExpertBatchMixRejected {
                reason: format!(
    ...
    pub fn try_merge(&self, other: &Self) -> Result<Self, Ds41rtError> {
        if let Some(reason) = self.mix_rejection_reason(other) {
            return Err(Ds41rtError::ExpertBatchMixRejected { reason });
        }
