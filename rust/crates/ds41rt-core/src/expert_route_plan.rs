...
impl RollingExpertRowPackAccumulator {
    pub fn new(config: RollingExpertRowPackConfig) -> Result<Self, Ds41rtError> {
    ...
    pub fn push_chunk(
        ...
        row_count: usize,
    ) -> Result<Vec<RollingExpertRowPackEmission>, Ds41rtError> {
    ...
    pub fn finish(&mut self) -> Result<Vec<RollingExpertRowPackEmission>, Ds41rtError> {
        ensure(!self.finished, "rolling row pack accumulator is finished")?;
        self.finished = true;
    ...
    fn emit_one(&mut self) -> Result<RollingExpertRowPackEmission, Ds41rtError> {
...
pub fn plan_rolling_expert_row_packs(
    ...
    row_count: usize,
    config: RollingExpertRowPackConfig,
) -> Result<RollingExpertRowPackPlan, Ds41rtError> {
...
) -> Result<CompletionFirstRoutePlan, Ds41rtError> {
...
fn ensure(condition: bool, reason: impl Into<String>) -> Result<(), Ds41rtError> {
    if condition {
        Ok(())
    } else {
        Err(rejected(reason))
