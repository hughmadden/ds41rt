...
use crate::{Ds41rtError, LayerId, ModelFacts, PlacementVersion, PositionId, Priority, RequestId};
...
impl LayerWave {
    pub fn decode(step: DecodeStep) -> Self {
        Self::decode_with_model(step, &ModelFacts::default())
    }

    pub fn decode_with_model(step: DecodeStep, facts: &ModelFacts) -> Self {
        ...
        Self {
            ...
            layer_id: step.layer_id,
            hidden_shape: HiddenShape::for_model_bf16(1, facts),
            ...
            route_metadata: RouteMetadataPlaceholder::for_model(facts),
    ...
    pub fn prefill(chunk: PrefillChunk) -> Self {
        Self::prefill_with_model(chunk, &ModelFacts::default())
    }

    pub fn prefill_with_model(chunk: PrefillChunk, facts: &ModelFacts) -> Self {
    ...
    pub fn mtp_verify(block: MtpVerifyBlock) -> Self {
        Self::mtp_verify_with_model(block, &ModelFacts::default())
    }

    pub fn mtp_verify_with_model(block: MtpVerifyBlock, facts: &ModelFacts) -> Self {
        let mut kv_reads = Vec::new();
        ...
        Self {
            ...
            hidden_shape: HiddenShape::for_model_bf16(block.token_count, facts),
            row_sources: vec![RowSource {
    ...
    pub fn benchmark_with_model(
        ...
        Self {
            ...
            hidden_shape: HiddenShape::for_model_bf16(rows, facts),
            row_sources: vec![RowSource {
    ...
    pub fn try_merge(&self, other: &Self) -> Result<Self, Ds41rtError> {
        if let Some(reason) = self.mix_rejection_reason(other) {
            return Err(Ds41rtError::LayerWaveMixRejected { reason });
        }
