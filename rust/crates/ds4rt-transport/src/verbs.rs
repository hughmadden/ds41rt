...
use ds4rt_core::{ExpertRequest, ExpertResponse};
use ds4rt_ffi::{
    c_char_array_to_string, Ds4rtDeviceBuffer, Ds4rtHostBuffer, Ds4rtRdmaRcCompletionStats,
    Ds4rtRdmaRcEndpointBufferView, Ds4rtRdmaRcEndpointInfo, NativeLibrary,
    DS4RT_DEVICE_BUFFER_FLAG_MAPPED_HOST, DS4RT_HOST_BUFFER_FLAG_MAPPED,
    DS4RT_HOST_BUFFER_FLAG_PINNED,
};
...
const VERBS_HOST_RDMA_DEVICE_MAP_ENV: &str = "DS4RT_PROTOCOL_V2_VERBS_HOST_DEVICE_MAP";
...
struct VerbsHostProtocolV2CqPollRequest {
    ...
    poll_iterations: u32,
    deadline: Instant,
    response_tx: mpsc::SyncSender<Result<Ds4rtRdmaRcCompletionStats>>,
}

struct VerbsHostProtocolV2CqWaiter {
    response_tx: mpsc::SyncSender<Result<Ds4rtRdmaRcCompletionStats>>,
    response_rx: mpsc::Receiver<Result<Ds4rtRdmaRcCompletionStats>>,
}
...
impl VerbsHostProtocolV2PinnedResponseFrame {
    ...
    fn payload_host_buffer(&self, start: usize, end: usize) -> Option<Ds4rtHostBuffer> {
        if self.buffer.flags & DS4RT_HOST_BUFFER_FLAG_PINNED == 0 {
            return None;
        }
        Some(Ds4rtHostBuffer {
...
impl VerbsHostProtocolV2ResponsePayload {
    ...
    pub fn pinned_host_buffer(&self) -> Option<Ds4rtHostBuffer> {
        self.pinned_frame
            .as_ref()?
...
fn verbs_response_payload_exposes_and_recycles_pinned_subview() -> Result<()> {
    ...
    assert_ne!(pinned.flags & DS4RT_HOST_BUFFER_FLAG_PINNED, 0);
    drop(payload);
...
fn is_verbs_host_rdma_completion_error(error: &anyhow::Error) -> bool {
    error.chain().any(|cause| {
        let message = cause.to_string();
        message.contains("ds4rt_rdma_rc_endpoint_poll returned status")
...
impl VerbsHostProtocolV2PersistentClientSession {
    fn connect(
        ...
        let native_path = verbs_host_native_library_path().context(
            "native library not found; set DS4RT_NATIVE_LIB or build native/libds4rt_native.so with RDMA",
        )?;
...
fn protocol_v2_request_device_payload(
    ...
    frame: Ds4rtDeviceBuffer,
    response_slot: Option<Ds4rtDeviceBuffer>,
    execution_lane: u32,
...
fn device_buffers_match(left: Ds4rtDeviceBuffer, right: Ds4rtDeviceBuffer) -> bool {
    left.ptr == right.ptr
...
impl NativeRdmaEndpoint {
    ...
    fn send_buffer_view(&self) -> Result<Ds4rtRdmaRcEndpointBufferView> {
        self.library
            .rdma_rc_endpoint_buffer_view(self.info.handle, false)
    }

    fn recv_buffer_view(&self) -> Result<Ds4rtRdmaRcEndpointBufferView> {
        self.library
    ...
    fn try_poll(&self, send: u32, recv: u32) -> Result<Ds4rtRdmaRcCompletionStats> {
        self.library
...
impl VerbsHostMappedRdmaPollStats {
    fn from_native(stats: Ds4rtRdmaRcCompletionStats) -> Self {
        Self {
...
pub struct VerbsHostMappedRdmaSlot {
    ...
    pub host_ptr: *mut u8,
    pub device_buffer: Ds4rtDeviceBuffer,
}
...
pub struct VerbsHostMappedRdmaRing {
    ...
    send_view: Ds4rtRdmaRcEndpointBufferView,
    recv_view: Ds4rtRdmaRcEndpointBufferView,
    send_sequence: u64,
...
fn validate_mapped_endpoint_buffer_view(
    view: Ds4rtRdmaRcEndpointBufferView,
    layout: VerbsHostRdmaRing,
    label: &str,
) -> Result<()> {
    anyhow::ensure!(
    ...
    let expected_flags = DS4RT_HOST_BUFFER_FLAG_PINNED | DS4RT_HOST_BUFFER_FLAG_MAPPED;
    anyhow::ensure!(
...
fn verbs_host_native_library_path() -> Option<PathBuf> {
    ...
    for candidate in [
        "native/build-cuda-sm120/libds4rt_native.so",
        "native/build-cuda/libds4rt_native.so",
        "native/build/libds4rt_native.so",
    ] {
...
fn verbs_host_persistent_rings(
    ...
    let requested_slot_bytes = parse_env_usize(
        "DS4RT_VERBS_HOST_RING_SLOT_BYTES",
        VERBS_HOST_RDMA_RING_SLOT_BYTES,
    )?;
    let depth = parse_env_usize("DS4RT_VERBS_HOST_RING_DEPTH", VERBS_HOST_RDMA_RING_DEPTH)?;
...
mod persistent_tests {
    ...
    fn persistent_retryable_error_detects_rdma_completion_failure() {
        let err = anyhow::anyhow!(
            "ds4rt_rdma_rc_endpoint_poll returned status 7: RDMA RC endpoint completion returned non-success status"
        )
    ...
    fn mapped_request_device_payload_preserves_hidden_offset() {
        ...
        assert_eq!(
            ...
            DS4RT_DEVICE_BUFFER_FLAG_MAPPED_HOST
        );
    }
