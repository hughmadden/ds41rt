...
const PROTOCOL_V2_TCP_TIMING_ENV: &str = "DS4RT_PROTOCOL_V2_TCP_TIMING";
const SHARED_HIDDEN_PAYLOAD_MIN_ROWS: usize = 1_024;
const MIN_ROUND_ROBIN_STREAMING_ROWS: usize = 1024;
const PROTOCOL_V2_EXPERT_QUEUE_STATS_ENV: &str = "DS4RT_PROTOCOL_V2_EXPERT_QUEUE_STATS";
const PROTOCOL_V2_EXPERT_QUEUE_ROW_ROUTES_ENV: &str = "DS4RT_PROTOCOL_V2_EXPERT_QUEUE_ROW_ROUTES";
const PROTOCOL_V2_EXPERT_QUEUE_ROW_ROUTES_GATE_FILE_ENV: &str =
    "DS4RT_PROTOCOL_V2_EXPERT_QUEUE_ROW_ROUTES_GATE_FILE";
const PROTOCOL_V2_STREAM_INGRESS_ROWS_ENV: &str = "DS4RT_PROTOCOL_V2_STREAM_INGRESS_ROWS";
...
const PROTOCOL_V2_VERBS_HOST_ADDITIONAL_RAILS_ENV: &str =
    "DS4RT_PROTOCOL_V2_VERBS_HOST_ADDITIONAL_RAILS";
...
fn partition_protocol_v2_request_for_rails(
    ...
    for row_start in (0..row_count).step_by(rows_per_part) {
        ...
        let hidden_payload = if request.header.row_count as usize >= SHARED_HIDDEN_PAYLOAD_MIN_ROWS
        {
            request.hidden_payload.slice(hidden_start..hidden_end)
        } else {
            Bytes::copy_from_slice(&request.hidden_payload[hidden_start..hidden_end])
        };
        let mut partition = ExpertProtocolV2Request::new_with_hidden_stride_bytes(
...
fn shared_or_compact_hidden_payload(
    host_batch: &ExpertHostBatch,
    global_hidden_payload: &Bytes,
    global_row_count: usize,
) -> Result<Bytes> {
    let expected_bytes = global_row_count
        .checked_mul(host_batch.hidden_bytes_per_row)
        .context("host-batch global hidden payload byte count overflow")?;
    anyhow::ensure!(
        global_hidden_payload.len() == expected_bytes,
        "host-batch global hidden payload bytes {} did not match expected {expected_bytes}",
        global_hidden_payload.len()
    );
    if global_row_count >= SHARED_HIDDEN_PAYLOAD_MIN_ROWS
        && host_batch.num_rows() == global_row_count
        && host_batch.global_row_indices().eq(0..global_row_count)
    {
        return Ok(global_hidden_payload.clone());
    }
    Ok(Bytes::from(host_batch.compact_hidden_payload(
...
impl VerbsHostProtocolV2HostBatchSetPersistentClient {
    ...
    pub async fn dispatch_bf16_payload_streaming(
        ...
        self.dispatch_bf16_payload_streaming_with_ingress_policy(
            set,
            Bytes::copy_from_slice(global_hidden_payload),
            request_id_base,
            response_dtype,
            reduced_root_host_index,
            owner_fanout,
            row_sharded_reduction,
            StreamIngressPolicy::Environment,
            chunk_tx,
        )
        .await
    }

    /// Shared-backing variant for a coordinator payload that already owns its
    /// lifetime. Identity-row TP4 requests retain this backing through the
    /// persistent RDMA send instead of materializing one payload per Spark.
    pub async fn dispatch_bf16_payload_streaming_shared(
        &self,
        set: &ExpertHostBatchSet,
        global_hidden_payload: Bytes,
        request_id_base: u64,
    ...
    /// Stream response chunks while keeping every request partition a complete
    /// regular frame. Multi-rail row striping remains enabled, but the global
    /// streamed-ingress environment knob is deliberately ignored.
    pub async fn dispatch_bf16_payload_streaming_complete_frames(
        &self,
        set: &ExpertHostBatchSet,
        global_hidden_payload: &[u8],
        request_id_base: u64,
        ...
        self.dispatch_bf16_payload_streaming_with_ingress_policy(
            ...
            owner_fanout,
            row_sharded_reduction,
            StreamIngressPolicy::CompleteFrames,
            chunk_tx,
        )
        .await
    }

    async fn dispatch_bf16_payload_streaming_with_ingress_policy(
        &self,
        set: &ExpertHostBatchSet,
        global_hidden_payload: Bytes,
        request_id_base: u64,
        ...
        owner_fanout: bool,
        row_sharded_reduction: bool,
        stream_ingress_policy: StreamIngressPolicy,
        chunk_tx: std_mpsc::Sender<VerbsHostProtocolV2HostBatchSetBf16PayloadChunk>,
    ) -> Result<TcpProtocolV2HostBatchSetDispatchStats> {
        let (lane, _lane_guard) = if set.global_row_count >= MIN_ROUND_ROBIN_STREAMING_ROWS {
            self.acquire_round_robin_execution_lane().await
        } else {
            self.acquire_execution_lane().await
        };
    ...
    async fn acquire_round_robin_execution_lane(
        &self,
    ) -> (usize, Option<tokio::sync::OwnedMutexGuard<()>>) {
        debug_assert!(!self.lane_locks.is_empty());
        let preferred = self.next_lane.fetch_add(1, Ordering::Relaxed) % self.lane_locks.len();
        for lane_offset in 0..self.lane_locks.len() {
            let lane = (preferred + lane_offset) % self.lane_locks.len();
            if let Ok(guard) = Arc::clone(&self.lane_locks[lane]).try_lock_owned() {
                return (lane, Some(guard));
            }
        }
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum StreamIngressPolicy {
    Environment,
    CompleteFrames,
}

impl StreamIngressPolicy {
    fn chunk_rows(self) -> Result<Option<usize>> {
        match self {
            Self::Environment => protocol_v2_stream_ingress_rows(),
            Self::CompleteFrames => Ok(None),
        }
    }
}
...
async fn verbs_host_protocol_v2_host_batch_set_bf16_payload_dispatch_persistent_stream(
    ...
    let stream_ingress_rows = stream_ingress_policy.chunk_rows()?;
    let striped_row_sharded_reduction =
        row_sharded_reduction && protocol_v2_verbs_host_stripe_spark_reduction_enabled();
    ...
    for (host_index, host_batch) in set.batches.iter().enumerate() {
        ...
        let compact_hidden = shared_or_compact_hidden_payload(
            host_batch,
            &global_hidden_payload,
            set.global_row_count,
        )?;
        ...
        let request = ExpertProtocolV2Request::from_expert_host_batch_bytes(
            request_id_base + host_index as u64,
            host_batch,
            compact_hidden,
        )?;
...
mod tests {
    ...
    #[test]
    fn complete_frame_ingress_policy_never_splits_a_partition() {
        assert_eq!(
            StreamIngressPolicy::CompleteFrames.chunk_rows().unwrap(),
            None
        );
    }

    #[test]
    fn large_identity_host_batch_reuses_shared_hidden_payload_backing() {
        let row_count = SHARED_HIDDEN_PAYLOAD_MIN_ROWS;
        let host_batch = ExpertHostBatch {
            ...
            routes: (0..row_count).map(|index| route(index, index)).collect(),
        };
        let global_hidden = Bytes::from(vec![7_u8; row_count * 8]);

        let compact =
            shared_or_compact_hidden_payload(&host_batch, &global_hidden, row_count).unwrap();
        let request =
            ExpertProtocolV2Request::from_expert_host_batch_bytes(99, &host_batch, compact)
                .unwrap();

        assert_eq!(request.hidden_payload, global_hidden);
        assert_eq!(request.hidden_payload.as_ptr(), global_hidden.as_ptr());
    }

    #[test]
    fn small_identity_host_batch_keeps_the_latency_oriented_copy_path() {
        let host_batch = ExpertHostBatch {
            host: "spark0".to_owned(),
            layer_id: LayerId(3),
            placement_version: PlacementVersion::from("test"),
        ...
        let global_hidden = Bytes::from((0_u8..32).collect::<Vec<_>>());

        let compact = shared_or_compact_hidden_payload(&host_batch, &global_hidden, 4).unwrap();

        assert_eq!(compact, global_hidden);
        assert_ne!(compact.as_ptr(), global_hidden.as_ptr());
    }

    #[test]
    fn nonidentity_host_batch_still_compacts_hidden_rows() {
        let host_batch = ExpertHostBatch {
            host: "spark0".to_owned(),
            layer_id: LayerId(3),
            placement_version: PlacementVersion::from("test"),
            ...
            quantization_recipe: "test".to_owned(),
            rows: vec![row(2, 0, 0), row(0, 0, 0)],
            routes: Vec::new(),
        };
        let global_hidden = Bytes::from((0_u8..24).collect::<Vec<_>>());

        let compact = shared_or_compact_hidden_payload(&host_batch, &global_hidden, 3).unwrap();

        assert_eq!(
            compact,
            [&global_hidden[16..24], &global_hidden[0..8]].concat()
        );
        assert_ne!(compact.as_ptr(), global_hidden.as_ptr());
    }
    ...
    async fn execution_lanes_use_aux_for_overlap_and_reuse_primary_after_release() {
        ...
        let (serial, serial_guard) = client.acquire_execution_lane().await;
        assert_eq!(serial, 0);
        drop(serial_guard);

        let (next_serial, _next_serial_guard) = client.acquire_execution_lane().await;
        assert_eq!(next_serial, 0);
    }

    #[tokio::test]
    async fn round_robin_execution_lanes_rotate_after_release() {
        let client = VerbsHostProtocolV2HostBatchSetPersistentClient {
            targets: Vec::new(),
            clients_by_lane: vec![Vec::new(), Vec::new()],
            additional_clients_by_lane: vec![Vec::new(), Vec::new()],
        ...
        let (first, first_guard) = client.acquire_round_robin_execution_lane().await;
        assert_eq!(first, 0);
        drop(first_guard);

        let (second, _second_guard) = client.acquire_round_robin_execution_lane().await;
        assert_eq!(second, 1);
    }
