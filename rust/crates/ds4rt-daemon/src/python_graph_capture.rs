...
pub(crate) const DS4RT_B12X_ENV: &str = "DS4RT_B12X";
pub(crate) const DS4RT_B12X_SPARK_ENV: &str = "DS4RT_B12X_SPARK_PYTHON_CAPTURE";
...
static DS4RT_PYTHON_REFERENCE_PATH_READY: AtomicBool = AtomicBool::new(false);
...
pub(crate) fn initialize_spark_python_capture_from_env() -> Result<Option<PythonCaptureStatus>> {
    ...
    initialize_python_capture(DS4RT_B12X_SPARK_ENV, SPARK_PYTHON_CAPTURE_MODULES, "Spark")
}
...
fn initialize_python_capture(
    ...
    Python::with_gil(|py| -> PyResult<Vec<String>> {
        ...
        ensure_ds4rt_python_reference_path(py)?;
        eprintln!(
...
fn add_ds4rt_python_reference_path(py: Python<'_>) -> PyResult<()> {
    let sys = PyModule::import_bound(py, "sys")?;
    let sys_path = sys.getattr("path")?;
    for path in ds4rt_python_reference_paths() {
        add_python_path(&sys_path, path)?;
    }
    for path in ds4rt_python_dynload_paths(py)? {
        add_python_path(&sys_path, path)?;
    }
    Ok(())
}

fn ensure_ds4rt_python_reference_path(py: Python<'_>) -> PyResult<()> {
    if DS4RT_PYTHON_REFERENCE_PATH_READY.load(Ordering::Acquire) {
        return Ok(());
    }
    ...
    add_ds4rt_python_reference_path(py)?;
    DS4RT_PYTHON_REFERENCE_PATH_READY.store(true, Ordering::Release);
    Ok(())
}
...
fn ds4rt_python_reference_paths() -> [PathBuf; 2] {
    let root = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../..")
        .join("python")
        .join("reference");
    [root.clone(), root.join("ds4rt_reference")]
}

fn ds4rt_python_dynload_paths(py: Python<'_>) -> PyResult<Vec<PathBuf>> {
    let sys = PyModule::import_bound(py, "sys")?;
...
mod tests {
    ...
    fn env_gate_defaults_to_disabled() {
        ...
        env::remove_var(DS4RT_B12X_ENV);
        env::remove_var(DS4RT_B12X_SPARK_ENV);
    ...
    fn env_gates_parse_true_values() {
        let _guard = ENV_MUTEX.lock().expect("env test mutex poisoned");
        env::set_var(DS4RT_B12X_ENV, "on");
        env::set_var(DS4RT_B12X_SPARK_ENV, "1");
        assert!(coordinator_python_capture_enabled());
        assert!(spark_python_capture_enabled());
        env::remove_var(DS4RT_B12X_ENV);
        env::remove_var(DS4RT_B12X_SPARK_ENV);
    ...
    fn legacy_spark_b12x_env_does_not_enable_python_capture() {
        let _guard = ENV_MUTEX.lock().expect("env test mutex poisoned");
        env::remove_var(DS4RT_B12X_SPARK_ENV);
        env::set_var("DS4RT_B12X_SPARK", "1");
        assert!(!spark_python_capture_enabled());
        env::remove_var("DS4RT_B12X_SPARK");
    }
...
"#,
                "ds4rt_test_capture.py",
                "ds4rt_test_capture",
            )?;
            let sys = PyModule::import_bound(py, "sys")?;
            sys.getattr("modules")?
                .set_item("ds4rt_test_capture", module)?;
            Ok(())
        })
        .map_err(|err| anyhow::anyhow!(format_python_error(err)))?;
        ...
        Python::with_gil(|py| -> PyResult<()> {
            let module = PyModule::import_bound(py, "ds4rt_test_capture")?;
...
"#,
                "ds4rt_test_bool_query.py",
                "ds4rt_test_bool_query",
            )?;
            let sys = PyModule::import_bound(py, "sys")?;
            sys.getattr("modules")?
                .set_item("ds4rt_test_bool_query", module)?;
            Ok(())
        })
        .map_err(|err| anyhow::anyhow!(format_python_error(err)))?;
    ...
    #[test]
    fn launch_imports_ds4rt_reference_b12x_adapter_with_target_override() -> Result<()> {
        let _guard = ENV_MUTEX.lock().expect("env test mutex poisoned");
        pyo3::prepare_freethreaded_python();
        Python::with_gil(|py| -> PyResult<()> {
            let module = PyModule::from_code_bound(
...
"#,
                "ds4rt_test_b12x_capture_target.py",
                "ds4rt_test_b12x_capture_target",
            )?;
            let sys = PyModule::import_bound(py, "sys")?;
            sys.getattr("modules")?
                .set_item("ds4rt_test_b12x_capture_target", module)?;
            let os = PyModule::import_bound(py, "os")?;
            os.getattr("environ")?.set_item(
                "DS4RT_B12X_MLA_CAPTURE_TARGET",
                "ds4rt_test_b12x_capture_target:capture",
            )?;
            Ok(())
        })
        .map_err(|err| anyhow::anyhow!(format_python_error(err)))?;
        ...
        env::remove_var("DS4RT_B12X_MLA_CAPTURE_TARGET");
        Python::with_gil(|py| -> PyResult<()> {
            let os = PyModule::import_bound(py, "os")?;
            os.getattr("environ")?
                .call_method1("pop", ("DS4RT_B12X_MLA_CAPTURE_TARGET", py.None()))?;
            Ok(())
        })
        .map_err(|err| anyhow::anyhow!(format_python_error(err)))?;
        result?;
        ...
        Python::with_gil(|py| -> PyResult<()> {
            let module = PyModule::import_bound(py, "ds4rt_test_b12x_capture_target")?;
            let captured = module.getattr("captured")?;
            assert_eq!(
                captured.get_item("phase")?.extract::<String>()?,
                "cuda_graph_capture"
    ...
    #[test]
    fn launch_imports_ds4rt_reference_b12x_spark_adapter_with_target_override() -> Result<()> {
        let _guard = ENV_MUTEX.lock().expect("env test mutex poisoned");
        pyo3::prepare_freethreaded_python();
        Python::with_gil(|py| -> PyResult<()> {
            let module = PyModule::from_code_bound(
...
"#,
                "ds4rt_test_b12x_spark_capture_target.py",
                "ds4rt_test_b12x_spark_capture_target",
            )?;
            let sys = PyModule::import_bound(py, "sys")?;
            sys.getattr("modules")?
                .set_item("ds4rt_test_b12x_spark_capture_target", module)?;
            let os = PyModule::import_bound(py, "os")?;
            os.getattr("environ")?.set_item(
                "DS4RT_B12X_SPARK_CAPTURE_TARGET",
                "ds4rt_test_b12x_spark_capture_target:capture",
            )?;
            Ok(())
        })
        .map_err(|err| anyhow::anyhow!(format_python_error(err)))?;
        ...
        env::remove_var("DS4RT_B12X_SPARK_CAPTURE_TARGET");
        Python::with_gil(|py| -> PyResult<()> {
            let os = PyModule::import_bound(py, "os")?;
            os.getattr("environ")?
                .call_method1("pop", ("DS4RT_B12X_SPARK_CAPTURE_TARGET", py.None()))?;
            Ok(())
        })
        .map_err(|err| anyhow::anyhow!(format_python_error(err)))?;
        result?;
        ...
        Python::with_gil(|py| -> PyResult<()> {
            let module = PyModule::import_bound(py, "ds4rt_test_b12x_spark_capture_target")?;
            let captured = module.getattr("captured")?;
            assert_eq!(
                captured.get_item("phase")?.extract::<String>()?,
                "cuda_graph_capture"
