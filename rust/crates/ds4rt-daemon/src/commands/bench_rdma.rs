...
use app::{
    ds4rt_verbs_app_benchmark_attempt, run_ds4rt_verbs_app_benchmark, Ds4rtVerbsAppBenchmarkAttempt,
};
...
const DS4RT_VERBS_APP_BENCHMARK_KIND: &str = "ds4rt-verbs-app-protocol-v2";
...
struct RdmaBenchReport {
    ...
    ds4rt_app_benchmark: Option<Ds4rtVerbsAppBenchmarkAttempt>,
}
...
pub(crate) fn run_bench_rdma(args: BenchRdmaArgs) -> Result<()> {
    ...
    let app_mode = is_ds4rt_app_mode(&resolved_mode);
    ...
    let benchmark_kind = if app_mode {
        DS4RT_VERBS_APP_BENCHMARK_KIND
    } else {
    ...
    let mut report = RdmaBenchReport {
        ...
        skip_reason: None,
        runs: Vec::new(),
        ds4rt_app_benchmark: None,
    };

    if app_mode {
        let mut attempt = ds4rt_verbs_app_benchmark_attempt(
            &resolved_mode,
            args.peer.clone(),
        ...
        if resolved_mode == "app-capability" {
            report.skipped = true;
            report.skip_reason = Some("DS4RT verbs app capability mode only".to_owned());
        } else if !attempt.preflight_ok {
            report.skipped = true;
            report.skip_reason = Some(match attempt.preflight_error.as_deref() {
                Some(err) => format!("DS4RT verbs app benchmark preflight failed: {err}"),
                None => "DS4RT verbs app benchmark preflight failed".to_owned(),
            });
        } else {
            match run_ds4rt_verbs_app_benchmark(
                &resolved_mode,
                args.peer.as_deref(),
                args.port,
                &attempt.payloads,
                ...
                Err(err) => {
                    ...
                    report.skipped = true;
                    report.skip_reason = Some(format!("DS4RT verbs app benchmark failed: {err}"));
                }
            }
        }
        report.ds4rt_app_benchmark = Some(attempt);
...
fn is_ds4rt_app_mode(resolved_mode: &str) -> bool {
    matches!(
        resolved_mode,
