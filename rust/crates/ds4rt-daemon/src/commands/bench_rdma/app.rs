...
use ds4rt_core::{DS4_FLASH_HIDDEN_SIZE, DS4_FLASH_NUM_HIDDEN_LAYERS};
use ds4rt_ffi::{
    c_char_array_to_string, Ds4rtRdmaRcCompletionStats, Ds4rtRdmaRcEndpointInfo, NativeLibrary,
    DS4RT_STATUS_RDMA_UNAVAILABLE,
};
use ds4rt_transport::{
...
pub(super) struct Ds4rtVerbsAppBenchmarkAttempt {
    ...
    pub(super) protocol_v2_control_plane_dry_run: Ds4rtVerbsAppControlPlaneDryRun,
    ...
    pub(super) app_transport_runs: Vec<Ds4rtVerbsAppRun>,
    pub(super) app_transport_error: Option<String>,
    pub(super) payloads: Vec<Ds4rtVerbsAppPayload>,
    pub(super) chain_hops: usize,
    pub(super) protocol_v2_chain_plans: Vec<Ds4rtVerbsAppChainPlan>,
    pub(super) native_rdma_probe: Ds4rtVerbsNativeRdmaProbe,
}

#[derive(Debug, Serialize)]
pub(super) struct Ds4rtVerbsAppPayload {
...
#[derive(Debug, Serialize)]
pub(super) struct Ds4rtVerbsAppChainPlan {
...
#[derive(Debug, Serialize)]
pub(super) struct Ds4rtVerbsAppControlPlaneDryRun {
...
#[derive(Debug, Serialize)]
pub(super) struct Ds4rtVerbsNativeRdmaProbe {
    pub(super) attempted: bool,
...
pub(super) struct Ds4rtVerbsAppNativeEndpointDescriptor {
...
pub(super) struct Ds4rtVerbsAppRun {
    ...
    pub(super) local_endpoint: Ds4rtVerbsAppNativeEndpointDescriptor,
    pub(super) peer_endpoint: Ds4rtVerbsAppNativeEndpointDescriptor,
}

#[derive(Debug, Serialize, Deserialize)]
struct Ds4rtVerbsAppRunStart {
    message: String,
    run_kind: String,
    payload_index: usize,
    row_count: usize,
    ...
    client_native_endpoint: Ds4rtVerbsAppNativeEndpointDescriptor,
}

#[derive(Debug, Serialize, Deserialize)]
struct Ds4rtVerbsAppRunReady {
    message: String,
    payload_index: usize,
    server_endpoint: VerbsHostRcEndpointDescriptor,
    server_native_endpoint: Ds4rtVerbsAppNativeEndpointDescriptor,
}

#[derive(Debug, Serialize, Deserialize)]
struct Ds4rtVerbsAppRecvReady {
    message: String,
    payload_index: usize,
}

pub(super) fn ds4rt_verbs_app_benchmark_attempt(
    resolved_mode: &str,
    peer: Option<String>,
    payload_bytes: &[usize],
) -> Result<Ds4rtVerbsAppBenchmarkAttempt> {
    let payloads = protocol_v2_payloads_for_benchmark(payload_bytes)?;
    let chain_hops = ds4_flash_sparse_moe_chain_hops();
    ...
    let capabilities = ds4rt_transport::verbs_host_capabilities();
    ...
    Ok(Ds4rtVerbsAppBenchmarkAttempt {
        attempted: true,
...
fn protocol_v2_control_plane_dry_run(
...
) -> Result<Ds4rtVerbsAppControlPlaneDryRun> {
    ...
    let (local_endpoint_role, peer_endpoint_required) = match app_role {
        ...
        other => anyhow::bail!("unsupported DS4RT verbs app role: {other}"),
    };
    ...
    Ok(Ds4rtVerbsAppControlPlaneDryRun {
        ...
        data_plane_blocker: ds4rt_transport::verbs_host_app_transport_blocker().to_owned(),
    })
}
...
fn app_role_for_resolved_mode(resolved_mode: &str) -> Result<&'static str> {
    match resolved_mode {
        ...
        other => anyhow::bail!("unsupported DS4RT verbs app resolved mode: {other}"),
    }
}
...
fn native_rdma_probe(
    ...
    request_frame: &[u8],
    response_frame: &[u8],
) -> Ds4rtVerbsNativeRdmaProbe {
    let native_library_path = native_library_path();
    let mut probe = Ds4rtVerbsNativeRdmaProbe {
        attempted: true,
    ...
    let Some(path) = native_library_path else {
        probe.native_library_error = Some(
            "native library not found; set DS4RT_NATIVE_LIB or run just test-native".to_owned(),
        );
        return probe;
    };
    ...
    match library.rdma_plan_host_buffer_registration(
        ...
        host_buffer.len(),
        ds4rt_transport::verbs_host_capabilities().preferred_alignment,
    ) {
        Ok(plan) => {
...
fn native_library_path() -> Option<PathBuf> {
    ...
    let cwd_path = PathBuf::from("native/build/libds4rt_native.so");
...
pub(super) fn run_ds4rt_verbs_app_benchmark(
    resolved_mode: &str,
    peer: Option<&str>,
    port: u16,
    payloads: &[Ds4rtVerbsAppPayload],
    duration_secs: u64,
) -> Result<Vec<Ds4rtVerbsAppRun>> {
    match app_role_for_resolved_mode(resolved_mode)? {
        "server" => run_ds4rt_verbs_app_server(peer, port, payloads),
        "client" => {
            let peer = peer.context("DS4RT verbs app client requires --peer")?;
            run_ds4rt_verbs_app_client(peer, port, payloads, duration_secs)
        }
...
impl<'a> NativeRdmaEndpoint<'a> {
    ...
    fn native_descriptor(&self) -> Ds4rtVerbsAppNativeEndpointDescriptor {
        Ds4rtVerbsAppNativeEndpointDescriptor {
    ...
    fn connect(&self, peer: &Ds4rtVerbsAppNativeEndpointDescriptor) -> Result<()> {
    ...
    fn poll(&self, send: u32, recv: u32) -> Result<Ds4rtRdmaRcCompletionStats> {
        self.library
...
fn run_ds4rt_verbs_app_server(
    peer: Option<&str>,
    port: u16,
    payloads: &[Ds4rtVerbsAppPayload],
) -> Result<Vec<Ds4rtVerbsAppRun>> {
    let native_path = native_library_path().context(
        "native library not found; set DS4RT_NATIVE_LIB or run a native RDMA-enabled build",
    )?;
        ...
        match listener.accept() {
            ...
            Err(err) if err.kind() == std::io::ErrorKind::WouldBlock => {
                if Instant::now() >= deadline {
                    anyhow::bail!("timed out waiting for DS4RT verbs app client on {bind_addr}");
                }
                thread::sleep(Duration::from_millis(25));
            }
            Err(err) => return Err(err).context("accepting DS4RT verbs app client"),
        }
    };
    ...
    for _ in 0..expected_runs {
        let start: Ds4rtVerbsAppRunStart = read_control(&mut reader)?;
        if start.message != "run_start" {
            anyhow::bail!(
                "DS4RT verbs app server expected run_start, got {}",
                start.message
            );
        }
        let payload = payloads.get(start.payload_index).with_context(|| {
            format!(
                "DS4RT verbs app server payload index {} is out of range",
                start.payload_index
            )
        })?;
...
fn execute_client_app_run(
    ...
    let start = Ds4rtVerbsAppRunStart {
    ...
    let ready: Ds4rtVerbsAppRunReady = read_control(reader)?;
    if ready.message != "run_ready" || ready.payload_index != payload_index {
        anyhow::bail!("DS4RT verbs app client received invalid run_ready message");
    }
    ...
    let recv_ready: Ds4rtVerbsAppRecvReady = read_control(reader)?;
    if recv_ready.message != "recv_ready" || recv_ready.payload_index != payload_index {
        anyhow::bail!("DS4RT verbs app client received invalid recv_ready message");
    }
...
fn execute_server_app_run(
    ...
    {
        anyhow::bail!("DS4RT verbs app server run_start payload metadata did not match local plan");
    }
    ...
    let ready = Ds4rtVerbsAppRunReady {
...
fn build_app_run_report(
    ...
    poll_iterations: u64,
    local_endpoint: Ds4rtVerbsAppNativeEndpointDescriptor,
    peer_endpoint: Ds4rtVerbsAppNativeEndpointDescriptor,
) -> Ds4rtVerbsAppRun {
...
fn connect_control_stream(peer: &str, port: u16) -> Result<TcpStream> {
    ...
    let mut addrs = addr
        .to_socket_addrs()
        .with_context(|| format!("resolving DS4RT verbs app peer {addr}"))?;
    let first = addrs
        .next()
        .with_context(|| format!("DS4RT verbs app peer {addr} resolved no addresses"))?;
    TcpStream::connect_timeout(&first, timeout)
        .with_context(|| format!("connecting DS4RT verbs app control plane to {addr}"))
}
...
fn read_control<T: DeserializeOwned>(reader: &mut BufReader<TcpStream>) -> Result<T> {
    ...
    if bytes == 0 {
        anyhow::bail!("DS4RT verbs app control plane closed");
    }
...
fn validate_payload_matches_frames(
    ...
    if payload.request_wire_bytes != request_frame.len() {
        anyhow::bail!(
            "DS4RT verbs app request frame bytes {} did not match payload {}",
            request_frame.len(),
            payload.request_wire_bytes
        );
    }
    if payload.response_wire_bytes != response_frame.len() {
        anyhow::bail!(
            "DS4RT verbs app response frame bytes {} did not match payload {}",
            response_frame.len(),
            payload.response_wire_bytes
        );
    }
...
fn accumulate_completion_stats(
    ...
    recv_total: &mut u64,
    poll_total: &mut u64,
    stats: &Ds4rtRdmaRcCompletionStats,
) {
...
fn verbs_app_ib_port_num() -> Result<u32> {
    parse_env_u32("DS4RT_VERBS_APP_IB_PORT_NUM", 1)
}

fn verbs_app_control_timeout() -> Duration {
    let secs = parse_env_u64("DS4RT_VERBS_APP_CONTROL_TIMEOUT_SECS", 60).unwrap_or(60);
...
fn verbs_app_roundtrip_iterations(
    ...
    if let Some(value) = parse_optional_env_usize("DS4RT_VERBS_APP_ROUNDTRIP_ITERATIONS")? {
        return Ok(value.max(1));
    }
    let multiplier = duration_secs.max(1) as usize;
    let logical = payload.request_logical_payload_bytes;
    let base = if logical <= 8_192 {
        1_000
    } else if logical <= 131_072 {
        200
    } else if logical <= 524_288 {
        50
    } else {
        10
    };
...
fn verbs_app_chain_iterations() -> Result<usize> {
    parse_optional_env_usize("DS4RT_VERBS_APP_CHAIN_ITERATIONS")
        .map(|value| value.unwrap_or(1).max(1))
}

fn verbs_app_chain_hops() -> Result<usize> {
    parse_optional_env_usize("DS4RT_VERBS_APP_CHAIN_HOPS")
        .map(|value| value.unwrap_or_else(ds4_flash_sparse_moe_chain_hops).max(1))
}
...
fn protocol_v2_payloads_for_benchmark(
    payload_bytes: &[usize],
) -> Result<Vec<Ds4rtVerbsAppPayload>> {
    let hidden_row_bytes = DS4_FLASH_HIDDEN_SIZE * ExpertV2Dtype::Bf16.bytes_per_element();
    payload_bytes
        .iter()
        .enumerate()
        .map(|(idx, payload)| {
            if payload % hidden_row_bytes != 0 {
                anyhow::bail!(
                    "DS4RT app payload {payload} is not an exact multiple of hidden row bytes {hidden_row_bytes}"
                );
            }
            let row_count = payload / hidden_row_bytes;
            if row_count == 0 {
                anyhow::bail!("DS4RT app payload must contain at least one hidden row");
            }
...
fn protocol_v2_request_response_for_row_count(
    ...
    let routes = (0..row_count)
        ...
        .collect::<Vec<_>>();
    let hidden_payload = deterministic_hidden_payload_bf16(row_count, DS4_FLASH_HIDDEN_SIZE)?;
    let request = ExpertProtocolV2Request::new(
        request_id,
        1,
        0,
        DS4_FLASH_HIDDEN_SIZE as u32,
        ExpertV2Dtype::Bf16,
        rows,
        routes,
        hidden_payload,
...
fn protocol_v2_chain_plans_for_payloads(
    payloads: &[Ds4rtVerbsAppPayload],
    hops: usize,
) -> Result<Vec<Ds4rtVerbsAppChainPlan>> {
    payloads
        .iter()
...
fn ds4_flash_sparse_moe_chain_hops() -> usize {
...
fn frame_arena_evidence(
    ...
    let registration_alignment = ds4rt_transport::verbs_host_capabilities().preferred_alignment;
...
mod tests {
    ...
    #[test]
    fn app_protocol_v2_payloads_match_flash_hidden_rows() {
        let payloads =
            parse_payload_bytes(super::super::DEFAULT_APP_PROTOCOL_V2_PAYLOAD_BYTES).unwrap();
        let attempt =
            ds4rt_verbs_app_benchmark_attempt("app-client", Some("emu".to_owned()), &payloads)
                .unwrap();
        ...
        assert_eq!(attempt.native_rdma_probe.rc_send_recv_loopback_bytes, 8_192);
        assert_eq!(
        ...
        assert_eq!(attempt.chain_hops, 43);
        assert_eq!(
            ...
            .zip(attempt.protocol_v2_chain_plans.iter())
        {
            ...
            assert_eq!(
                ...
                payload.total_registered_span_bytes * 43
            );
    ...
    fn app_protocol_v2_payloads_reject_non_hidden_row_sizes() {
        let err = protocol_v2_payloads_for_benchmark(&[4096])
            .unwrap_err()
            .to_string();
        assert!(err.contains("not an exact multiple of hidden row bytes 8192"));
    }

    #[test]
    fn app_protocol_v2_control_plane_dry_run_tracks_roles() {
        let payloads = parse_payload_bytes("8192").unwrap();
        let server =
            ds4rt_verbs_app_benchmark_attempt("app-server", Some("emu".to_owned()), &payloads)
                .unwrap();
        let client =
            ds4rt_verbs_app_benchmark_attempt("app-client", Some("emu".to_owned()), &payloads)
                .unwrap();
        let capability =
            ds4rt_verbs_app_benchmark_attempt("app-capability", None, &payloads).unwrap();
