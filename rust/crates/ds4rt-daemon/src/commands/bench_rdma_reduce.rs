...
use ds4rt_ffi::{
    Ds4rtDeviceBuffer, Ds4rtRouteShardReductionBuffers, NativeLibrary,
    DS4RT_ROUTE_SHARD_LOCAL_BF16, DS4RT_ROUTE_SHARD_WIRE_BF16,
    DS4RT_ROUTE_SHARD_WIRE_FP8_E4M3_ROW_SCALED, DS4RT_ROUTE_SHARD_WIRE_NVFP4_E2M1_FP8_E4M3,
};
use ds4rt_transport::{
...
fn reduction_buffers(
    ...
    Ok(Ds4rtRouteShardReductionBuffers {
        local: local_bf16,
        peers,
        output_f32,
    })
...
fn pack_reduction_payload(
    ...
    unsafe {
        ...
        match codec.wire_dtype {
            ...
            DS4RT_ROUTE_SHARD_WIRE_FP8_E4M3_ROW_SCALED => native
            ...
            DS4RT_ROUTE_SHARD_WIRE_NVFP4_E2M1_FP8_E4M3 => native
...
fn reduction_codec(codec: &str, row_width: usize) -> Result<ReductionCodec> {
    match codec {
        "bf16" => Ok(ReductionCodec {
            wire_dtype: DS4RT_ROUTE_SHARD_WIRE_BF16,
        ...
        "fp8" => {
            ...
            Ok(ReductionCodec {
                wire_dtype: DS4RT_ROUTE_SHARD_WIRE_FP8_E4M3_ROW_SCALED,
                row_stride_bytes: row_width
        ...
        "nvfp4" => {
            ...
            Ok(ReductionCodec {
                wire_dtype: DS4RT_ROUTE_SHARD_WIRE_NVFP4_E2M1_FP8_E4M3,
...
fn load_native_library(path: Option<&Path>) -> Result<NativeLibrary> {
    let path = path
        ...
        .context("RDMA reduction requires --native-lib or DS4RT_NATIVE_LIB")?;
...
mod tests {
    use super::*;
    use ds4rt_core::DS4_FLASH_HIDDEN_SIZE;
    ...
    fn reduction_header_roundtrips_across_generations() -> Result<()> {
        let header = ReductionFrameHeader {
            ...
            destination_rank: 0,
            rows: 16,
            row_width: DS4_FLASH_HIDDEN_SIZE,
            row_stride_bytes: DS4_FLASH_HIDDEN_SIZE + std::mem::size_of::<f32>(),
            wire_dtype: DS4RT_ROUTE_SHARD_WIRE_FP8_E4M3_ROW_SCALED,
            frame_bytes: REDUCTION_HEADER_BYTES
                + 16 * (DS4_FLASH_HIDDEN_SIZE + std::mem::size_of::<f32>()),
            world_size: 3,
        };
