...
use ds4rt_core::GLM52_ROUTED_SCALING_FACTOR;
use ds4rt_ffi::{
...
const KERNEL_FILTERS: &[&str] = &[
    ...
    "router",
    "router_topk_bf16",
    "ds4_flash_router_topk_bf16_shortlist",
...
pub(crate) fn run_bench_cuda_kernels(args: BenchCudaKernelsArgs) -> Result<()> {
    ...
    if kernel_selected_explicit(&args, "ds4_flash_router_topk_bf16_shortlist", &["router"]) {
        bench_ds4_flash_router_topk_bf16_shortlist(&library, &stream, &native_lib, &args)?;
    }
...
fn router_top_k(args: &BenchCudaKernelsArgs, experts: usize, label: &str) -> Result<usize> {
    if args.top_k > experts || args.top_k > DS4RT_CUDA_ROUTER_TOPK_MAX_K {
        anyhow::bail!(
            "{label} invalid top_k={} for experts={experts}; max supported top_k={DS4RT_CUDA_ROUTER_TOPK_MAX_K}",
            args.top_k
        );
    }
    Ok(args.top_k)
...
fn sample_top_k(args: &BenchCudaKernelsArgs, label: &str) -> Result<usize> {
    if args.top_k > args.vocab || args.top_k > DS4RT_CUDA_SAMPLE_TOPK_MAX_K {
        anyhow::bail!(
            "{label} invalid top_k={} for vocab={}; max supported top_k={DS4RT_CUDA_SAMPLE_TOPK_MAX_K}",
            args.top_k,
            args.vocab
        );
    }
...
fn native_library_path(args: &BenchCudaKernelsArgs) -> PathBuf {
    args.native_lib
        .clone()
        .or_else(|| env::var_os("DS4RT_NATIVE_LIB").map(PathBuf::from))
        .unwrap_or_else(|| PathBuf::from("native/build-cuda/libds4rt_native.so"))
}
...
fn bench_ds4_flash_router_topk_bf16_shortlist(
    library: &NativeLibrary,
    stream: &CudaStream<'_>,
    native_lib: &Path,
    args: &BenchCudaKernelsArgs,
    ...
    const TOP_K: usize = 6;
    anyhow::ensure!(
        args.hidden_dim == HIDDEN && args.routes == EXPERTS && args.top_k == TOP_K,
        "Flash shortlist router benchmark requires hidden_dim={HIDDEN}, routes={EXPERTS}, top_k={TOP_K}"
    );
    let hidden_values = args
        .rows
        .checked_mul(HIDDEN)
        .context("Flash shortlist hidden value count overflow")?;
    let weight_values = EXPERTS
        .checked_mul(HIDDEN)
        .context("Flash shortlist weight value count overflow")?;
    let output_values = args
        .rows
        .checked_mul(TOP_K)
        .context("Flash shortlist output value count overflow")?;
    let approximate_values = args
        .rows
        .checked_mul(EXPERTS)
        .context("Flash shortlist approximate-logit count overflow")?;
    let hidden = bf16_pattern(hidden_values);
    let router_weight = bf16_pattern(weight_values);
    let correction_bias = (0..EXPERTS)
        .map(|idx| ((idx % 7) as f32 - 3.0) * 0.03125)
        .collect::<Vec<_>>();
    let hidden_buffer = upload(library, u16_bytes(&hidden), "Flash shortlist hidden")?;
    let weight_buffer = upload(
        library,
        u16_bytes(&router_weight),
        "Flash shortlist router weight",
    )?;
    let bias_buffer = upload(
        library,
        f32_bytes(&correction_bias),
        "Flash shortlist correction bias",
    )?;
    let index_buffer =
        DeviceAllocation::new(library, output_values * 4, "Flash shortlist indices")?;
    let score_workspace = DeviceAllocation::new(
        library,
        (output_values + approximate_values) * 4,
        "Flash shortlist score workspace",
    )?;
    let weight_out_buffer =
        DeviceAllocation::new(library, output_values * 4, "Flash shortlist weights")?;
    let approximate_logits = device_buffer_view(
        score_workspace.buffer(),
        output_values * 4,
        approximate_values * 4,
        "Flash shortlist approximate logits",
    )?;

    let timing = time_kernel(args, || unsafe {
        library.cuda_linear_bf16_f32_cublas_async(
            hidden_buffer.buffer(),
            weight_buffer.buffer(),
            approximate_logits,
            args.rows,
        ...
        library.cuda_ds4_flash_router_refine_topk_bf16_async(
            hidden_buffer.buffer(),
            weight_buffer.buffer(),
            bias_buffer.buffer(),
            approximate_logits,
    ...
    emit_timing(
        ...
        &json!({
            ...
            "logical_payload_bytes": hidden_values * 2
                + weight_values * 2
                + EXPERTS * 4
                + (output_values * 3 + approximate_values) * 4,
        }),
        args,
        timing,
    )
...
fn device_buffer_view(
    ...
    let end = byte_offset
        .checked_add(bytes)
        .with_context(|| format!("{label} byte range overflows usize"))?;
    anyhow::ensure!(
        end <= buffer.bytes,
        "{label} byte range {byte_offset}..{end} exceeds buffer bytes {}",
        buffer.bytes
    );
    Ok(Ds4rtDeviceBuffer {
        ptr: unsafe { buffer.ptr.cast::<u8>().add(byte_offset).cast() },
        bytes,
...
impl<'a> BenchCudaGraph<'a> {
    fn new(library: &'a NativeLibrary, capture: Ds4rtCudaGraphCaptureInfo) -> Result<Self> {
...
fn python_buffer(name: &'static str, buffer: Ds4rtDeviceBuffer) -> PythonDeviceBufferArg<'static> {
    PythonDeviceBufferArg {
        name,
        ptr: buffer.ptr,
        bytes: buffer.bytes,
