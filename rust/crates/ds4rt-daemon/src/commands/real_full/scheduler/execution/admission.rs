...
const SPECULATIVE_TARGET_ATTENTION_FUSION_ENV: &str =
    "DS4RT_REAL_FULL_SPECULATIVE_TARGET_ATTENTION_FUSION";
...
impl CachedResidentDeviceBuffer {
    fn device_buffer(self) -> Ds4rtDeviceBuffer {
        Ds4rtDeviceBuffer {
...
impl From<Ds4rtDeviceBuffer> for CachedResidentDeviceBuffer {
    fn from(buffer: Ds4rtDeviceBuffer) -> Self {
        Self {
...
fn scheduler_current_kv_resident_layer(
    ...
    let dsa = if ds4rt_core::GLM52_DSA_INDEXER_LAYER_IDS_WITH_MTP.contains(&layer_id) {
...
fn scheduler_device_kv_readback_validation_enabled() -> bool {
    match std::env::var("DS4RT_REAL_FULL_VALIDATE_DEVICE_KV_READBACK") {
        Ok(value) => matches!(
            value.as_str(),
...
fn real_full_launch_fused_mtp_target_attention(
    ...
    fused_wave.hidden_shape = ds4rt_core::HiddenShape::ds4_flash_bf16_rows(total_rows);
...
fn real_full_launch_sequential_mtp_target_attention(
    ...
    for (row, descriptor) in current_descriptors.iter().enumerate() {
        ...
        row_wave.hidden_shape = ds4rt_core::HiddenShape::ds4_flash_bf16_rows(1);
        row_wave.row_sources[0].token_start = ds4rt_core::PositionId(token_start);
...
fn real_full_launch_scheduler_attention_from_device_kv_with_query_override(
    ...
    let launch = if let Some(hidden) = query_hidden {
        ...
        if let Some(real_attention) = real_attention {
            if env::var_os("DS4RT_REAL_FULL_DIAGNOSTIC_LAYER_DUMP_DIR").is_some()
                && wave.layer_id.0 == 0
            {
                eprintln!(
...
fn single_wave_source(wave: &LayerWave) -> Result<&ds4rt_core::RowSource> {
...
mod tests {
    use super::*;
    use ds4rt_core::{DecodeStep, LayerId, MtpVerifyBlock, PositionId, Priority};
