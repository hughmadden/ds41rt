...
fn real_full_scheduler_execution_accepts_request_shaped_prefill_decode_mtp_rows() {
    ...
    let report = real_full_scheduler_execution_for_shape(
        ds4rt_core::KvCacheConfig::glm52_phase0(shape.reservation_tokens()),
        &catalog,
        shape.clone(),
    )
    ...
    assert_eq!(
        ...
        report.sparse_expert_batches * ds4rt_core::EXPERT_HOSTS.len()
    );
    assert_eq!(
        report.sparse_expert_host_batch_rows,
        report.sparse_expert_batch_rows * ds4rt_core::EXPERT_HOSTS.len()
    );
    assert_eq!(
        report.sparse_expert_host_batch_routes,
        report.sparse_expert_batch_routes * ds4rt_core::DS4_EXPERT_TP_WORLD_SIZE
    );
    ...
    assert_eq!(
        ...
        shape.reservation_tokens() * ds4rt_core::KvCacheConfig::glm52_phase0(1).bytes_per_token()
    );
