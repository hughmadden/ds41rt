...
pub(crate) const REAL_NVFP4_CUDA_REFERENCE_KERNELS_ENV: &str =
    "DS4RT_REAL_FULL_CUDA_REFERENCE_KERNELS";
...
) -> Result<Option<Ds4rtDeviceBuffer>> {
...
fn native_flash_expert_catalog(catalog: &TensorCatalog) -> bool {
    ...
    let supported_variant = (native_fp4
        && catalog.facts.variant == ds4rt_core::ModelVariant::Flash)
        || (exl3
            && matches!(
                catalog.facts.variant,
                ds4rt_core::ModelVariant::Flash | ds4rt_core::ModelVariant::Pro
            ));
...
mod tests {
    ...
    fn streamed_ingress_state_restores_logical_hidden_row_order() {
        ...
        let plan_view = ds4rt_transport::ExpertProtocolV2RequestView::parse(&plan_frame).unwrap();
        ...
        assert!(!state
            .accept_data(
                &ds4rt_transport::ExpertProtocolV2RequestView::parse(&first_frame).unwrap()
            )
            .unwrap());
        assert!(state
            .accept_data(
                &ds4rt_transport::ExpertProtocolV2RequestView::parse(&second_frame).unwrap()
            )
            .unwrap());
    ...
    fn real_nvfp4_protocol_v2_streaming_fallback_emits_one_complete_response() {
        ...
        let view = ds4rt_transport::ExpertProtocolV2RequestView::parse(&frame).unwrap();
    ...
    fn real_nvfp4_protocol_v2_streamed_ingress_matches_regular_execution() {
        ...
        let plan_view = ds4rt_transport::ExpertProtocolV2RequestView::parse(&plan_frame).unwrap();
        let data_view = ds4rt_transport::ExpertProtocolV2RequestView::parse(&data_frame).unwrap();
    ...
    fn real_nvfp4_protocol_v2_executor_preloads_cuda_projection_cache_when_available() {
        ...
        let preload = match executor.preload_assigned_projections_with_cuda(true) {
            ...
            Err(error) => {
                ...
                if message.contains("requires DS4RT_NATIVE_LIB")
    ...
    fn exl3_pro_plan_catalog() -> TensorCatalog {
        ...
        catalog.facts.variant = ds4rt_core::ModelVariant::Pro;
    ...
    fn gptqmodel_exl3_flash_selects_rank_local_k2_backend() {
        ...
        assert!(native_flash_expert_catalog(&catalog));
        let expert = ds4rt_loader::exl3_expert(&catalog, 0, 0).unwrap();
        assert!(expert
            .gate
            .trellis
            .name
    ...
    fn exl3_pro_resident_plan_is_rank_local_k2_without_widening() {
        ...
        native_fp4_pro.facts.variant = ds4rt_core::ModelVariant::Pro;
    ...
    fn real_checkpoint_topk_host_batch_set_fixture(
        ...
        let batch = ExpertBatch {
            ...
            routed_experts: ModelFacts::default().routed_experts,
            quantization_recipe: ModelFacts::default().quantization_recipe,
            rows: vec![ds4rt_core::ExpertBatchRow {
                row_id: 0,
                source_kind: RowSourceKind::PrefillChunk,
    ...
    fn local_host_partial_bf16_accumulation(
        ...
        global_hidden: &[u8],
        owner_lookup: &ExpertOwnerLookup,
    ) -> anyhow::Result<ds4rt_core::ExpertHostBatchSetAccumulation> {
    ...
    fn execute_request(
        ...
        request: &ExpertProtocolV2Request,
    ) -> anyhow::Result<ds4rt_transport::ExpertProtocolV2Response> {
        let frame = request.encode().unwrap();
        let view = ds4rt_transport::ExpertProtocolV2RequestView::parse(&frame).unwrap();
    ...
    fn load_real_owner_lookup_or_skip() -> Option<ExpertOwnerLookup> {
        ...
        let loadplan_path =
            repo_root().join(".ds4rt-cache/model-artifacts/diagnostic/loadplan.spark-0.json");
