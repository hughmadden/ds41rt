...
use ds4rt_ffi::{
    Ds4rtDeviceBuffer, Ds4rtHostBuffer, NativeLibrary, DS4RT_CUDA_GENERIC_KV_PAGE_SIZE,
};
use std::{
...
use crate::commands::real_full::coordinator_kernels::{
    ...
    FlashinferCompressedMlaKvInput, FlashinferMlaHiddenProjection, FlashinferTargetKvPageTable,
...
const ADMISSION_STAGE_TIMING_ENV: &str = "DS4RT_REAL_FULL_ADMISSION_STAGE_TIMING";
const PACKED_FP8_MLA_BATCHED_SUFFIX_ENV: &str = "DS4RT_REAL_FULL_PACKED_FP8_MLA_BATCHED_SUFFIX";
const DSPARK_TRACE_ENV: &str = "DS4RT_REAL_FULL_DSPARK_TRACE";
const PACKED_MLA_TRACE_DIR_ENV: &str = "DS4RT_REAL_FULL_PACKED_MLA_TRACE_DIR";
const PACKED_MLA_TRACE_LAYERS_ENV: &str = "DS4RT_REAL_FULL_PACKED_MLA_TRACE_LAYERS";
const PACKED_MLA_TRACE_ROWS_ENV: &str = "DS4RT_REAL_FULL_PACKED_MLA_TRACE_ROWS";
const PACKED_MLA_TRACE_QUERY_ROWS_ENV: &str = "DS4RT_REAL_FULL_PACKED_MLA_TRACE_QUERY_ROWS";
const PACKED_MLA_TRACE_LIMIT_ENV: &str = "DS4RT_REAL_FULL_PACKED_MLA_TRACE_LIMIT";
...
fn maybe_dump_packed_mla_trace(
    ...
    q_projected: Ds4rtDeviceBuffer,
    q_nope: Ds4rtDeviceBuffer,
    q_rope_rotated: Ds4rtDeviceBuffer,
        ...
        if !weight_manifest.is_file() {
            ...
            fs::write(
                ...
                serde_json::to_vec_pretty(&serde_json::json!({
                    "format": "ds4rt-packed-mla-trace-weights-v1",
                    "layer": layer,
                    "heads": heads,
    ...
    let metadata = serde_json::json!({
        "format": "ds4rt-packed-mla-trace-v1",
        "layer": layer,
        "rows": span.rows,
...
struct RealFullDeviceMlaKvRopeDeviceBuffers {
    ...
    k_rope_rotated: Ds4rtDeviceBuffer,
}
...
impl RealFullDeviceMlaKvDeviceParts {
    ...
    pub(in crate::commands::real_full) fn from_projected_kv_a_device_bf16(
        ...
        projected_kv_a_buffer: Ds4rtDeviceBuffer,
        layer_id: LayerId,
        rows: usize,
        kv_lora_rank: usize,
        rope_dim: usize,
    ...
    pub(in crate::commands::real_full) fn kv_latent_buffer(&self) -> Ds4rtDeviceBuffer {
        self.kv_latent.buffer
    }

    pub(in crate::commands::real_full) fn k_rope_buffer(&self) -> Ds4rtDeviceBuffer {
        self.k_rope.buffer
    }

    pub(in crate::commands::real_full) fn dsa_key_buffer(&self) -> Option<Ds4rtDeviceBuffer> {
    ...
    pub(in crate::commands::real_full) fn rotate_k_rope_bf16_with_position_buffer(
        &self,
        positions: &[u32],
        positions_device: Ds4rtDeviceBuffer,
        theta: f32,
    ) -> Result<RealFullDeviceMlaKvRopeParts> {
        if positions.len() != self.rows {
            anyhow::bail!(
...
impl RealFullDeviceMlaKvProjectedParts {
    ...
    pub(in crate::commands::real_full) fn normalized_buffer(&self) -> Ds4rtDeviceBuffer {
        self.normalized.buffer
    }

    pub(in crate::commands::real_full) fn projected_buffer(&self) -> Ds4rtDeviceBuffer {
        self.projected.buffer
    }

    pub(in crate::commands::real_full) fn k_nope_buffer(&self) -> Ds4rtDeviceBuffer {
        self.k_nope.buffer
    }

    pub(in crate::commands::real_full) fn values_buffer(&self) -> Ds4rtDeviceBuffer {
        self.values.buffer
    }
...
impl RealFullDeviceMlaKvRopeParts {
    ...
    pub(in crate::commands::real_full) fn k_rope_rotated_buffer(&self) -> Ds4rtDeviceBuffer {
...
impl RealFullDeviceMlaQueryParts {
    ...
    pub(in crate::commands::real_full) fn from_projected_suffix_device_bf16(
        library: &'static NativeLibrary,
        projected_query_buffer: Ds4rtDeviceBuffer,
        prefix_rows: usize,
        suffix_positions: &[u32],
        heads: usize,
        nope_dim: usize,
...
impl RealFullDeviceMlaKvProjectedDeviceBuffers {
    ...
    fn run_mla_rope_attention_with_uploaded_suffix_bf16(
        ...
        q_nope: Ds4rtDeviceBuffer,
        q_rope: Ds4rtDeviceBuffer,
        suffix_k_nope: Ds4rtDeviceBuffer,
        suffix_k_nope_bytes: usize,
        suffix_k_rope: Ds4rtDeviceBuffer,
        suffix_k_rope_bytes: usize,
        suffix_values: Ds4rtDeviceBuffer,
...
impl RealFullDeviceMlaQueryDeviceBuffers {
    fn q_nope_buffer(&self) -> Ds4rtDeviceBuffer {
        self.q_nope
    }

    fn q_rope_rotated_buffer(&self) -> Ds4rtDeviceBuffer {
        self.q_rope_rotated
    }
}
...
impl RealFullDeviceMlaAttentionParts {
    ...
    pub(in crate::commands::real_full) fn output_buffer(&self) -> Ds4rtDeviceBuffer {
        self.output.buffer
    }
...
fn real_full_device_main_mla_row_bytes(config: &KvCacheConfig) -> Result<usize> {
    match config.layout {
        KvLayout::DeepseekV4HybridBf16 | KvLayout::Glm52CompressedBf16 => config
            .key_value_width
            .checked_mul(std::mem::size_of::<u16>())
...
impl RealFullDeviceKvExecutionMirror {
    fn live(cache: RealFullDeviceKvCache<'static>, status: &'static str) -> Self {
        Self {
            cache: Some(cache),
            status,
            writes: 0,
    ...
    pub(in crate::commands::real_full) fn new(config: KvCacheConfig) -> Result<Self> {
        ...
        match RealFullDeviceKvCache::new(library, config) {
            Ok(cache) => Ok(Self::live(cache, "cuda-kv-cache-live-scheduler")),
            Err(error) => {
                if cuda_required {
                    return Err(error)
    ...
    pub(in crate::commands::real_full) fn new_native_target_mapping(
        config: KvCacheConfig,
    ) -> Result<Self> {
        let cuda_required = coordinator_cuda_reference_kernels_enabled();
        let library = match cuda_native_library() {
            ...
            Err(error) => {
                if cuda_required {
                    return Err(error).context(
                        "real-full native target page mapping requires CUDA reference execution but no CUDA-enabled native library is available",
                    );
                }
                return Ok(Self::unavailable("cuda-native-target-page-map-unavailable"));
            }
        };
        match RealFullDeviceKvCache::new_native_target_mapping_only(library, config) {
            Ok(cache) => Ok(Self::live(cache, "cuda-native-target-page-map")),
            Err(error) => {
                if cuda_required {
                    return Err(error)
                        .context("allocating live scheduler native target page mapping");
                }
                Ok(Self::unavailable(
                    real_full_device_kv_roundtrip_error_status(&error),
                ))
    ...
    pub(in crate::commands::real_full) fn new_with_storage(
        ...
        let has_payload_storage = storage.has_payload_storage();
        ...
        let status = if has_payload_storage {
            "cuda-kv-cache-live-scheduler-shared-storage"
        } else {
            "cuda-native-target-page-map-shared-storage"
        };
        Ok(Self::live(cache, status))
    }
    ...
    pub(in crate::commands::real_full) fn physical_token_positions(
        &self,
        ...
        let cache = self
            .cache
            .as_ref()
            .context("live scheduler device KV cache is unavailable for physical slot lookup")?;
        let logical_token_end = logical_token_start
            .checked_add(token_count)
            .context("target KV physical slot lookup range overflows usize")?;
        (logical_token_start..logical_token_end)
            .map(|logical_token| {
                u32::try_from(cache.physical_token_position(logical_token)?)
                    .context("target KV physical token position exceeds u32")
            })
            .collect()
    }
    ...
    pub(in crate::commands::real_full) fn write_projected_mla_kv_a_device_blocks_bf16(
        ...
        descriptors: &[KvBlockDescriptor],
        projected_kv_a_buffer: Ds4rtDeviceBuffer,
        kv_norm_weight: Option<Ds4rtDeviceBuffer>,
    ...
    pub(in crate::commands::real_full) fn write_mla_decode_kv_device_block_bf16(
        ...
        input_norm_weight: Ds4rtDeviceBuffer,
        kv_a_weight: Ds4rtDeviceBuffer,
        kv_norm_weight: Ds4rtDeviceBuffer,
    ...
    pub(in crate::commands::real_full) fn write_projected_mla_kv_a_and_dsa_key_device_blocks_bf16(
        ...
        descriptors: &[KvBlockDescriptor],
        projected_kv_a_buffer: Ds4rtDeviceBuffer,
        dsa_key_buffer: Ds4rtDeviceBuffer,
        kv_norm_weight: Option<Ds4rtDeviceBuffer>,
    ) -> Result<Option<Vec<RealFullDeviceKvBlockIo>>> {
        if descriptors.is_empty() {
            return Ok(Some(Vec::new()));
    ...
    pub(in crate::commands::real_full) fn run_mla_rope_attention_parts_from_device_prefix_with_device_weights_and_projected_query_device_suffix_bf16(
        ...
        kv_norm_weight: Ds4rtDeviceBuffer,
        kv_b_weight: Ds4rtDeviceBuffer,
        q_projected_buffer: Ds4rtDeviceBuffer,
        q_suffix_positions: &[u32],
        suffix_k_nope_bf16: &[u8],
        suffix_k_rope_rotated_bf16: &[u8],
        suffix_values_bf16: &[u8],
    ...
    pub(in crate::commands::real_full) fn run_mla_rope_attention_parts_from_device_prefix_with_device_weights_and_projected_query_device_kv_suffix_bf16(
        ...
        kv_b_weight: Ds4rtDeviceBuffer,
        q_projected_buffer: Ds4rtDeviceBuffer,
        kv_a_projected_buffer: Ds4rtDeviceBuffer,
        suffix_positions: &[u32],
        heads: usize,
        nope_dim: usize,
        v_dim: usize,
    ...
    pub(in crate::commands::real_full) fn run_mla_rope_attention_parts_from_device_kv_with_device_weights_and_projected_query_device_bf16(
        ...
        kv_b_weight: Ds4rtDeviceBuffer,
        q_projected_buffer: Ds4rtDeviceBuffer,
        dsa_query: Option<(Ds4rtDeviceBuffer, Ds4rtDeviceBuffer)>,
        ...
        anyhow::ensure!(
            dsa_query.is_none(),
            "generic device KV attention no longer accepts inherited GLM DSA query inputs; DeepSeek V4 serving must use the native target arena"
        );
        let rows = descriptors
            .iter()
            .try_fold(0_usize, |acc, descriptor| {
            ...
            && q_suffix_positions.len() <= REAL_FULL_COMPRESSED_MLA_MAX_QUERY_ROWS
        {
            ...
            let last_token_end = descriptors
                ...
                .unwrap_or(0);
            eprintln!(
                "real_full_attention_route layer_id={} rows={} prefix_rows={} query_rows={} descriptors={} first_token_start={} last_token_end={} compressed_suffix={} dense_packed_eligible={}",
                layer_id.0,
                rows,
                prefix_rows,
                q_suffix_positions.len(),
    ...
    fn run_scheduler_mla_attention_from_device_kv_bf16(
        ...
        descriptors: &[KvBlockDescriptor],
        q_suffix_positions: &[u32],
        query_hidden: Option<Ds4rtDeviceBuffer>,
...
struct SchedulerAttentionResidentBuffers {
    kv_norm_weight: Ds4rtDeviceBuffer,
    kv_b_weight: Ds4rtDeviceBuffer,
    query_projection_weight: Option<Ds4rtDeviceBuffer>,
    output_projection_weight: Option<Ds4rtDeviceBuffer>,
}
...
impl DeviceKvLayerRoundTripPlan {
    ...
    fn descriptor(&self) -> Result<KvBlockDescriptor> {
        ...
        Ok(KvBlockDescriptor {
            ...
            token_start: ds4rt_core::PositionId(self.token_start),
...
impl<'a> DeviceKvReusableHostBuffer<'a> {
    ...
    fn buffer(&mut self, bytes: usize, label: &'static str) -> Result<Ds4rtHostBuffer> {
        if bytes == 0 {
...
impl<'a> DeviceKvReusableDeviceBuffer<'a> {
    fn new(library: &'a NativeLibrary) -> Self {
        Self {
            library,
            buffer: Ds4rtDeviceBuffer::default(),
            capacity: 0,
            label: "",
        }
    }

    fn buffer(&mut self, bytes: usize, label: &'static str) -> Result<Ds4rtDeviceBuffer> {
        if bytes == 0 {
...
fn target_kv_physical_spans(
    ...
    while logical_token < logical_token_end {
        let logical_page = logical_token / DS4RT_CUDA_GENERIC_KV_PAGE_SIZE;
        let page_token = logical_token % DS4RT_CUDA_GENERIC_KV_PAGE_SIZE;
        ...
        let tokens_here =
            (DS4RT_CUDA_GENERIC_KV_PAGE_SIZE - page_token).min(logical_token_end - logical_token);
        let physical_token_start = physical_page
            .checked_mul(DS4RT_CUDA_GENERIC_KV_PAGE_SIZE)
...
pub(in crate::commands::real_full) struct RealFullDeviceKvStorage<'a> {
    ...
    payload_mode: RealFullDeviceKvPayloadMode,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum RealFullDeviceKvPayloadMode {
    Allocated,
    NativeTargetMappingOnly,
}

impl RealFullDeviceKvPayloadMode {
    fn capacity_bytes(self, config: &KvCacheConfig) -> usize {
        match self {
            Self::Allocated => config.capacity_bytes(),
            Self::NativeTargetMappingOnly => 0,
        }
    }
}
...
impl<'a> RealFullDeviceKvStorage<'a> {
    fn new(library: &'a NativeLibrary, config: KvCacheConfig) -> Result<Self> {
        let payload_mode = RealFullDeviceKvPayloadMode::Allocated;
        let capacity_bytes = payload_mode.capacity_bytes(&config);
        if capacity_bytes == 0 {
    ...
    fn native_target_mapping_only(library: &'a NativeLibrary, config: KvCacheConfig) -> Self {
        Self {
            library,
            config,
            cache: Ds4rtDeviceBuffer::default(),
            payload_mode: RealFullDeviceKvPayloadMode::NativeTargetMappingOnly,
        }
    }

    fn has_payload_storage(&self) -> bool {
        self.payload_mode == RealFullDeviceKvPayloadMode::Allocated
    }
}
...
impl<'a> RealFullDeviceKvCache<'a> {
    ...
    fn new_native_target_mapping_only(
        library: &'a NativeLibrary,
        config: KvCacheConfig,
    ) -> Result<Self> {
        let storage = Arc::new(RealFullDeviceKvStorage::native_target_mapping_only(
            library,
            config.clone(),
        ));
        let logical_capacity_tokens = config.max_tokens;
    ...
    fn new_with_storage(
        ...
        library
            .cuda_generic_kv_page_table_init_base(
    ...
    fn physical_token_position(&self, logical_token: usize) -> Result<usize> {
        ...
        if let Some(physical_pages) = self.physical_pages.as_ref() {
            ...
            return (*physical_page as usize)
                .checked_mul(DS4RT_CUDA_GENERIC_KV_PAGE_SIZE)
                .and_then(|base| base.checked_add(logical_token % DS4RT_CUDA_GENERIC_KV_PAGE_SIZE))
    ...
    fn physical_page_table(&self) -> Option<(Ds4rtDeviceBuffer, u64)> {
    ...
    fn copy_target_kv_boundary_page(
        ...
        anyhow::ensure!(
            source_page != destination_page
                && (1..DS4RT_CUDA_GENERIC_KV_PAGE_SIZE).contains(&valid_tokens),
        ...
        if !self.storage.has_payload_storage() {
            // Native DeepSeek target serving owns the authoritative 256-token
            // KV/index pages in DeepseekV4TargetDeviceStorage. This wrapper
            // retains only the expanded 64-token logical-to-physical mapping;
            // its 64-token compatibility copy must never be selected.
            anyhow::bail!(
                "generic 64-token KV boundary copy reached a native-target mapping-only cache; copy the authoritative 256-token page through native target storage"
            );
        }
        let copy_plane = |library: &NativeLibrary,
                          storage: Ds4rtDeviceBuffer,
        ...
        let page_token = |page: u32| -> Result<PositionId> {
            (page as u64)
                .checked_mul(DS4RT_CUDA_GENERIC_KV_PAGE_SIZE as u64)
                .map(PositionId)
        ...
        for layer in 0..self.config.layers {
            let source_descriptor = KvBlockDescriptor {
                ...
                token_count: DS4RT_CUDA_GENERIC_KV_PAGE_SIZE,
            };
    ...
    fn main_mla_cache_for_layer(&self, layer_id: LayerId) -> Result<Ds4rtDeviceBuffer> {
        let layer_base = self
            .config
    ...
    fn attention_ready_mla_frontier_append_target(
        ...
        descriptors: &[KvBlockDescriptor],
        row_stride_bytes: usize,
    ) -> Result<Option<(Ds4rtDeviceBuffer, DeviceKvAttentionReadyFrontier)>> {
    ...
    fn attention_ready_mla_frontier_payload_for_descriptors(
        &mut self,
        descriptors: &[KvBlockDescriptor],
    ) -> Result<Option<(Ds4rtDeviceBuffer, usize, usize)>> {
        let Some(first) = descriptors.first() else {
            return Ok(None);
        };
        let layer_index = usize::try_from(first.layer_id.0)
    ...
    fn write_mla_dsa_planes_from_contiguous_device(
        &self,
        descriptors: &[KvBlockDescriptor],
        main_src: Ds4rtDeviceBuffer,
        main_row_bytes: usize,
        dsa_src: Ds4rtDeviceBuffer,
    ) -> Result<Vec<RealFullDeviceKvBlockIo>> {
        anyhow::ensure!(
    ...
    fn split_projected_query_suffix_to_reusable_buffers(
        ...
        prefix_rows: usize,
        suffix_positions: &[u32],
        positions_device: Option<Ds4rtDeviceBuffer>,
        heads: usize,
        nope_dim: usize,
        rope_dim: usize,
        theta: f32,
    ...
    fn attention_combined_buffers(
        ...
        values_bytes: usize,
        label: &'static str,
    ) -> Result<(Ds4rtDeviceBuffer, Ds4rtDeviceBuffer, Ds4rtDeviceBuffer)> {
        let k_nope = self
...
impl<'a> DeviceBufferGuard<'a> {
    ...
    fn borrowed(library: &'a NativeLibrary, buffer: Ds4rtDeviceBuffer) -> Self {
        Self {
            library,
            buffer,
            owned: false,
    ...
    fn into_buffer(mut self) -> Result<Ds4rtDeviceBuffer> {
        if !self.owned {
...
fn scheduler_attention_project_query_from_hidden_bf16_into(
    layer_id: LayerId,
    query_hidden: Ds4rtDeviceBuffer,
    query_rows: usize,
    query_projection_weight: Ds4rtDeviceBuffer,
    projected_query: Ds4rtDeviceBuffer,
) -> Result<()> {
    if query_rows == 0 {
...
fn scheduler_attention_project_output_to_hidden_bf16(
    layer_id: LayerId,
    library: &'static NativeLibrary,
    attention_output: Ds4rtDeviceBuffer,
    rows: usize,
    input_width: usize,
    output_projection_weight: Ds4rtDeviceBuffer,
) -> Result<DeviceBufferGuard<'static>> {
    if rows == 0 {
...
mod tests {
    ...
    #[test]
    fn native_target_page_mapping_allocates_no_generic_kv_payload() {
        let config = KvCacheConfig::glm52_compressed_nvfp4(8);
        assert_eq!(
            RealFullDeviceKvPayloadMode::Allocated.capacity_bytes(&config),
            config.capacity_bytes()
        );
        assert_eq!(
            RealFullDeviceKvPayloadMode::NativeTargetMappingOnly.capacity_bytes(&config),
            0
        );
    }
    ...
    #[test]
    fn device_kv_execution_mirror_dsa_plane_writes_preserve_overlapping_rows() -> Result<()> {
        let config = KvCacheConfig::glm52_phase0(8);
        let first_descriptor = KvBlockDescriptor {
            reservation_id: 1,
    ...
    fn device_kv_roundtrip_status_labels_cuda_unavailable_errors() {
        let error = anyhow::anyhow!(
            "allocating real-full device KV roundtrip cache: ds4rt_alloc_device_buffer returned status 3: CUDA unavailable"
        );
    ...
    fn device_kv_roundtrip_cuda_errors_are_soft_only_when_cuda_is_not_required() {
        let soft = device_kv_roundtrip_failed(
            anyhow::anyhow!("ds4rt_cuda_kv_cache_write_blocks returned status 3: CUDA unavailable"),
            false,
        )
        ...
        let hard = device_kv_roundtrip_failed(
            anyhow::anyhow!("ds4rt_cuda_kv_cache_write_blocks returned status 7"),
            true,
        )
        .unwrap_err()
        .to_string();
