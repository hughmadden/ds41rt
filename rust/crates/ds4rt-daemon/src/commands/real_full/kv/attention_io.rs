...
pub(in crate::commands::real_full) fn real_full_attention_kv_io_dry_run_for_layers(
    ...
    let layerwave_payload_count_mismatch_guard =
        layerwave_payload_count_mismatch_guard(store.config());
...
fn layerwave_payload_count_mismatch_guard(kv_config: &KvCacheConfig) -> bool {
    let mut guard_store = KvCacheBackingStore::new(KvCacheConfig {
        max_tokens: 1,
        ..kv_config.clone()
    });
...
mod tests {
    ...
    #[test]
    fn payload_count_guard_uses_deepseek_v4_kv_geometry() {
        let facts = ModelFacts::default();
        let config = KvCacheConfig::deepseek_v4_hybrid_bf16(16, &facts);

        assert_eq!(config.layout_label(), "deepseek-v4-hybrid-bf16");
        assert!(layerwave_payload_count_mismatch_guard(&config));
    }
}
