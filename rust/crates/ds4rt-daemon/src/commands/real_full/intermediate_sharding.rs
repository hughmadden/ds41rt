...
use ds4rt_ffi::{Ds4rtNcclComm, NativeLibrary};
use std::{
...
pub(crate) const EXPERT_INTERMEDIATE_SHARDS_ENV: &str = "DS4RT_EXPERT_INTERMEDIATE_SHARDS";
pub(crate) const EXPERT_INTERMEDIATE_SHARD_RANK_ENV: &str = "DS4RT_EXPERT_INTERMEDIATE_SHARD_RANK";
pub(crate) const EXPERT_INTERMEDIATE_REDUCTION_ENV: &str = "DS4RT_EXPERT_INTERMEDIATE_REDUCTION";
...
pub(crate) const EXPERT_INTERMEDIATE_REDUCTION_ROOT_ENV: &str =
    "DS4RT_EXPERT_INTERMEDIATE_REDUCTION_ROOT";
pub(crate) const EXPERT_INTERMEDIATE_REDUCTION_PORT_ENV: &str =
    "DS4RT_EXPERT_INTERMEDIATE_REDUCTION_PORT";
...
pub(crate) const EXPERT_INTERMEDIATE_OWNER_PEERS_ENV: &str =
    "DS4RT_EXPERT_INTERMEDIATE_OWNER_PEERS";
pub(crate) const EXPERT_INTERMEDIATE_OWNER_PORT_ENV: &str = "DS4RT_EXPERT_INTERMEDIATE_OWNER_PORT";
pub(crate) const EXPERT_INTERMEDIATE_RDMA_PEERS_ENV: &str = "DS4RT_EXPERT_INTERMEDIATE_RDMA_PEERS";
...
pub(crate) const EXPERT_INTERMEDIATE_RDMA_PORT_ENV: &str = "DS4RT_EXPERT_INTERMEDIATE_RDMA_PORT";
pub(crate) const EXPERT_INTERMEDIATE_RDMA_SLOT_BYTES_ENV: &str =
    "DS4RT_EXPERT_INTERMEDIATE_RDMA_SLOT_BYTES";
pub(crate) const EXPERT_INTERMEDIATE_RDMA_RING_DEPTH_ENV: &str =
    "DS4RT_EXPERT_INTERMEDIATE_RDMA_RING_DEPTH";
pub(crate) const EXPERT_INTERMEDIATE_RDMA_STRIPE_MIN_BYTES_ENV: &str =
    "DS4RT_EXPERT_INTERMEDIATE_RDMA_STRIPE_MIN_BYTES";
pub(crate) const EXPERT_WEIGHT_PRELOAD_NCCL_ENV: &str = "DS4RT_EXPERT_WEIGHT_PRELOAD_NCCL";
pub(crate) const EXPERT_WEIGHT_PRELOAD_NCCL_PORT_ENV: &str =
    "DS4RT_EXPERT_WEIGHT_PRELOAD_NCCL_PORT";
...
// Keep startup-only weight exchange separate from long-lived Spark reduction
// and other collective bootstrap ports.
...
impl SparkExpertReduction {
    pub(crate) fn communicator(&self) -> &Ds4rtNcclComm {
        &self.communicator
    }
...
mod tests {
    ...
    #[test]
    fn rdma_reduction_cutover_keeps_small_waves_on_the_coordinator() {
        let mode = ExpertIntermediateReductionMode::SparkRdma;
        assert!(
            (1..16).all(|rows| reduction_dispatch_for_config(mode, rows, 16, 8, true).is_none())
        );
        let distributed = reduction_dispatch_for_config(mode, 16, 16, 8, true).unwrap();
        assert!(!distributed.owner_fanout);
        assert!(distributed.row_sharded);
        assert!(reduction_dispatch_for_config(mode, 31, 32, 8, true).is_none());
        let tuned = reduction_dispatch_for_config(mode, 32, 32, 8, true).unwrap();
        assert!(!tuned.owner_fanout);
        assert!(tuned.row_sharded);
    }
