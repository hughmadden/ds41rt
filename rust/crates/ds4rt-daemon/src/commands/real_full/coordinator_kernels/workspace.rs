...
pub(in crate::commands::real_full) fn require_cuda_enabled_native_library(
    ...
    anyhow::bail!(
        "{purpose} require a CUDA-enabled native library, but {} reports `{version}`. Rebuild with `cmake -S native -B native/build-cuda -G Ninja -DDS4RT_ENABLE_CUDA=ON -DDS4RT_ENABLE_RDMA=OFF -DDS4RT_CUDA_ARCHITECTURES=120 && cmake --build native/build-cuda`, or set DS4RT_NATIVE_LIB to a CUDA-enabled libds4rt_native.so",
        path.display()
    );
}
