...
use ds4rt_ffi::DS4RT_HOST_BUFFER_FLAG_PINNED;
use ds4rt_loader::{read_tensor_bytes_into, DEEPSEEK_V4_EXL3_RECIPE};
...
pub(in crate::commands::real_full) struct Ds4FlashTp4ReductionDeviceBuffers {
    ...
    pub(in crate::commands::real_full) rank_partials: [Ds4rtDeviceBuffer; DS4_FLASH_TP],
    pub(in crate::commands::real_full) reduction_f32: Ds4rtDeviceBuffer,
...
pub(in crate::commands::real_full) struct Ds4FlashTp4HostPartialChunk<'a> {
    ...
    pub(in crate::commands::real_full) pinned_payload: Option<Ds4rtHostBuffer>,
            ...
            for chunk in chunks {
                ...
                anyhow::ensure!(
                    !pinned.ptr.is_null()
                        ...
                        && pinned.flags & DS4RT_HOST_BUFFER_FLAG_PINNED != 0,
...
mod tests {
    ...
    use ds4rt_loader::build_catalog_for_snapshot;
    use std::path::Path;
    ...
    fn buffer(ptr: usize, bytes: usize, device_id: i32) -> Ds4rtDeviceBuffer {
        Ds4rtDeviceBuffer {
    ...
    fn flash_tp4_host_barrier_stages_into_fixed_gpu0_graph_inputs() {
        ...
        let pinned_chunks = (0..striped_ranks.len())
            .map(|chunk| Ds4FlashTp4HostPartialChunk {
                rank: striped_ranks[chunk],
                payload_bf16: &striped_payloads[chunk],
                pinned_payload: Some(Ds4rtHostBuffer {
    ...
    fn real_flash_decode_sparse_block_preserves_tp4_split_graph_on_gpu0() {
        ...
        let mut rank_allocations = Vec::<Vec<Ds4rtDeviceBuffer>>::new();
        ...
        let assert_rank_close = |rank: usize, actual_buffer: Ds4rtDeviceBuffer| {
