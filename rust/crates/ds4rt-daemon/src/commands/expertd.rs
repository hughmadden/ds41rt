...
pub(crate) async fn run_expertd(args: ExpertDaemonArgs) -> Result<()> {
    ...
    if !args.synthetic_weights {
        ...
        if !real_nvfp4_cuda_reference_kernels_enabled() {
            anyhow::bail!(
                "phase0 real-weight expertd serving requires {REAL_NVFP4_CUDA_REFERENCE_KERNELS_ENV}=1 and DS4RT_NATIVE_LIB pointing at native/build-cuda/libds4rt_native.so; refusing CPU fallback"
            );
        }
        ...
        return match args.transport.as_str() {
            "tcp" => {
                ds4rt_transport::serve_protocol_v2_tcp_with_executor(&args.listen, executor).await
            }
            "verbs-host" => {
                ds4rt_transport::serve_protocol_v2_verbs_host_with_executor(&args.listen, executor)
                    .await
            }
    ...
    match args.transport.as_str() {
        "tcp" => ds4rt_transport::serve_synthetic_protocol_v2_tcp(&args.listen).await,
        "tcp-debug-json" | "debug-json" => {
            ds4rt_transport::serve_synthetic_debug_json_tcp(&args.listen).await
        }
        "verbs-host" => ds4rt_transport::serve_synthetic_verbs_host(&args.listen).await,
    ...
    let (capabilities, preflight_ok, preflight_error, infiniband_path, frame_protocol) =
        match args.transport.as_str() {
            "tcp" => (
                ...
                None,
                None,
                Some(ds4rt_transport::EXPERT_PROTOCOL_V2_FRAME_PROTOCOL.to_owned()),
            ),
            "tcp-debug-json" | "debug-json" => (
                ds4rt_transport::tcp_capabilities(),
                true,
                None,
                None,
                Some(ds4rt_transport::DEBUG_JSON_FRAME_PROTOCOL.to_owned()),
            ),
            "verbs-host" => {
...
fn require_real_tp4_intermediate_shard(
    ...
    shard.context(
        "real DeepSeek V4 expert serving requires DS4RT_EXPERT_INTERMEDIATE_SHARDS=4 and \
DS4RT_EXPERT_INTERMEDIATE_SHARD_RANK=0..3; logical owner metadata is only for ingestion and \
...
mod tests {
    ...
    fn real_weight_serving_rejects_owner_partition_fallback() {
        ...
        assert!(error.contains("requires DS4RT_EXPERT_INTERMEDIATE_SHARDS=4"));
    ...
    fn real_weight_serving_catalog_filters_role_and_layer_from_owner_lookup() {
        let catalog = TensorCatalog {
            ...
            snapshot_path: "/tmp/ds4rt-filtered-serving-catalog".to_owned(),
    ...
    fn tp4_serving_keeps_every_expert_independent_of_logical_owner() {
        let catalog = TensorCatalog {
            model_id: "test/model".to_owned(),
            snapshot_path: "/tmp/ds4rt-tp4-serving-catalog".to_owned(),
            facts: ModelFacts::default(),
            tensors: vec![
                serving_filter_tensor("model.layers.3.mlp.experts.0.gate_proj.weight", 3, 0),
                serving_filter_tensor("model.layers.3.mlp.experts.1.gate_proj.weight", 3, 1),
    ...
    async fn tcp_real_weight_entrypoint_refuses_cpu_fallback_without_cuda_env() {
        ...
        assert!(error.contains("requires DS4RT_REAL_FULL_CUDA_REFERENCE_KERNELS=1"));
