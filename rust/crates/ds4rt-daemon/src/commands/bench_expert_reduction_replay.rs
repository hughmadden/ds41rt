...
pub(crate) async fn run_bench_expert_reduction_replay(
    ...
    write_record(
        ...
        &json!({
            ...
            "response_dtype": "bf16",
            "spark_reduction_mode": env_value("DS4RT_EXPERT_INTERMEDIATE_REDUCTION"),
            "spark_reduction_dtype": env_value("DS4RT_EXPERT_INTERMEDIATE_REDUCTION_DTYPE"),
            "spark_row_sharded": env_value("DS4RT_EXPERT_INTERMEDIATE_ROW_SHARDED_REDUCTION"),
            "stripe_spark_reduction": env_value("DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_SPARK_REDUCTION"),
            "stripe_min_rows": env_value("DS4RT_PROTOCOL_V2_VERBS_HOST_STRIPE_MIN_ROWS"),
        }),
    )?;
    output.flush()?;
...
fn validate_replay_plan(manifest: &ReplayPlanManifest, chains: &[ReplayChain]) -> Result<()> {
    anyhow::ensure!(
        manifest.schema == "ds4rt-expert-reduction-replay-plan-v2",
        "replay plan schema {:?} is not ds4rt-expert-reduction-replay-plan-v2; regenerate it from a DeepSeek route bank",
        manifest.schema
    );
...
mod tests {
    ...
    fn flash_manifest() -> ReplayPlanManifest {
        ReplayPlanManifest {
            schema: "ds4rt-expert-reduction-replay-plan-v2".to_owned(),
    ...
    fn replay_rejects_unversioned_glm_geometry() {
        ...
        manifest.schema = "ds4rt-expert-reduction-replay-plan-v1".to_owned();
