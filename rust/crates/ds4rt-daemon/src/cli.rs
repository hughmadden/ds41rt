...
use ds4rt_core::{DEFAULT_MODEL_ID, DS4_FLASH_HIDDEN_SIZE};
use std::path::PathBuf;

pub(crate) const DEFAULT_REAL_FULL_MAX_CONTEXT_TOKENS: usize = 128 * 1024;

#[derive(Debug, Parser)]
#[command(name = "ds4rt", about = "DS4RT phase0 runtime CLI")]
...
mod tests {
    ...
    fn transport_benchmark_defaults_use_flash_geometry() {
        let cli = Cli::try_parse_from(["ds4rt", "bench-rdma-ring"]).unwrap();
        ...
        let cli =
            Cli::try_parse_from(["ds4rt", "bench-protocol-v2-tcp", "--addr", "127.0.0.1:9100"])
                .unwrap();
