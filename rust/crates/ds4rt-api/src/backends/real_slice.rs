...
pub(crate) async fn real_ds4_slice_completion(
    state: &ApiState,
    ...
    let slice = state
        ...
        .real_slice
        .as_ref()
        .ok_or_else(|| runtime_error("real-ds4-slice backend has no loaded tensor summary"))?;
    let first = slice
        .tensors
        .first()
        .map(|tensor| {
    ...
    let mut response = format!(
        "real ds4 slice loaded tensors={} bytes={} first={}",
