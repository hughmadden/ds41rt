...
use ds4rt_core::ExpertRequest;
#[cfg(test)]
use ds4rt_core::DEFAULT_MODEL_ID;
...
async fn models(State(state): State<Arc<ApiState>>) -> Json<ModelList> {
    Json(ModelList {
        ...
        data: vec![
            ModelInfo {
                id: "ds4rt-tiny".to_owned(),
                object: "model",
                owned_by: "ds4rt",
            },
            ModelInfo {
                id: "ds4rt-synthetic-ds4-layer".to_owned(),
                object: "model",
                owned_by: "ds4rt",
            },
            ModelInfo {
...
) -> Result<ds4rt_core::ExpertResponse, ApiError> {
    match state.config.transport {
        ApiTransport::Inproc => ds4rt_transport::inproc_roundtrip(request)
            .await
        ...
        ApiTransport::Tcp => {
            ...
            ds4rt_transport::tcp_protocol_v2_expert_request_roundtrip(
                addr,
                request,
                Default::default(),
            )
        ...
        ApiTransport::TcpDebugJson => {
            ...
            let addr = parse_expert_target(target)?;
            ds4rt_transport::debug_json_tcp_roundtrip(addr, request, Default::default())
                .await
                .map_err(runtime_error)
        }
        ApiTransport::VerbsHost => {
            ...
            let addr = parse_expert_target(target)?;
            ds4rt_transport::verbs_host_protocol_v2_expert_request_roundtrip(
                addr,
                request,
                Default::default(),
            )
