...
pub(crate) fn selected_backend(
    ...
    let backend = if request.model == "ds4rt-tiny" {
        ApiBackend::Tiny
    } else if request.model == "ds4rt-synthetic-ds4-layer" {
        ApiBackend::SyntheticDs4Layer
        ...
        return Err(crate::invalid_request(
            format!(
                "model {} is not served by this DS4RT instance; query /v1/models for the exact identity",
                request.model
            ),
            Some("model"),
        ));
...
pub(crate) fn prompt_token_ids(
    ...
    (backend == ApiBackend::RealDs4Full)
        ...
        .and_then(|snapshot_path| {
            ds4rt_loader::encode_tokenizer_text(Path::new(snapshot_path), prompt, false).ok()
        })
        .map(|summary| {
            summary
                .token_ids
