...
## Matched-input service follow-up

Repeating the host probe with the wire probe's recorded layer-0 input and six actual routed experts gave capacity-80 medians of 393.691 µs GPU-only, 419.547 µs with host transfers, and 427.163 µs through production `execute_host_chunks`, owned-response copying and encoding. Capacities 1/16/80 again produced finite, byte-identical route partials. The earlier synthetic routing choice does not explain the service gap in this input.

A temporary one-layer worker used the production service and transport with only the external fixture's loading loops restricted to layer 0; it listened on port 19442. A release client measured one peer at a time: the existing full worker returned in 1336.017 µs median, and the one-layer worker in 1422.506 µs. Both used 40 warmups and 120 measured requests. The one-layer worker's internal request interval was about 0.50 ms, including parsing, binding, execution and sending the owned response to its consumer. The interval excludes incoming queue wait, the outer transport task and actual socket writes. This narrows the remaining measurement scope; it does not prove which queue or network operation is responsible. The temporary worker was stopped after measurement.

Production workers now expose opt-in `ds41rt::timing` / `native expert service` fields for queue wait, response construction/handoff, execution excluding that handoff, and total worker time. Info-level per-layer loading events report resident bytes and elapsed milliseconds. These events do not claim to measure NVMe bandwidth or GPU-only time. The subsequent [direct-submission rollout](ds41-native-direct-submission.md) deployed this instrumentation and optimized builds to all four workers; use that record for the newer full-model results.
