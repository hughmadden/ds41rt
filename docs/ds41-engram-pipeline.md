...
`EngramLayerWeights` additionally packs the native scales into 4,915,200 bytes of MMA scale storage, bringing per-layer residency to 162,437,120 bytes without changing FP8 weight bytes. `EngramGate::execute` now owns K32 activation scratch, alpha, projected KV and residual output, enqueues native quantization/GEMM followed by the fused gate on one stream, and synchronizes before exposing output. M16 execution workspace is 2,452,500 bytes including stable residual, embedding and mask inputs, separate from gathered-row storage. Native handles bind the compiled device/SM geometry and reject invalid capacities, pointers, overlaps and undersized scratch; module loading and initialization occur before execution.
...
`EngramGate` owns fixed input destinations and a CUDA graph for a captured row count. Capture warms the native launch sequence before recording quantization, projection and gating; replay stages new inputs and requires the same row count. Staging drains submitted copies before returning, including copy errors. Producers can write directly to the corresponding stable input destinations. Graph destruction follows stream synchronization and precedes buffer destruction. The budget excludes CUDA graph/driver allocations, resident weights and the separate gathered-row owner. Daemon compilation passed; capture/replay numerical equivalence and scheduler integration remain unqualified.
