...
The generic revision-three protocol now assigns dtype 7 to `Fp8E4m3Ue8m0K32`: each row contains its E4M3 payload followed by its K32 scale bytes. Size computation rejects zero/non-K32 widths and overflow. Owned and borrowed request decoders preserve the exact bytes at 1, 16 and 80 rows; truncated frames are rejected. The native BF16 worker still explicitly rejects this representation until its GPU binding supports it. The transport suite passes 149 tests (one ignored), and the workspace check passes using Python 3.12 for PyO3.
...
[Candidate patches, exact probe and results](ds41-fp8-expert-consumer-probe.json). These are external overlays, not a shipped b12x input API. Remaining work includes planned input-layout ownership, native export/ABI and Rust buffer binding, RTX GPU encoding/staging, official-weight qualification and live rollout. The 576-wide tile candidates remain separate and unmeasured.
