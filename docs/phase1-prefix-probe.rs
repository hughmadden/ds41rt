...
pub(super) fn run<'w, 'a>(lib: &'a NativeLibrary, runtime: &tokio::runtime::Runtime,
    ...
    let shapes: Vec<_>=(1..=5).flat_map(|a| (1..=5).map(move |b| [a,b])).collect();
    ...
    for sweep in 0..4 {
        let mut order=shapes.clone(); order.rotate_left((snapshot+sweep*7)%25);
        if (snapshot+sweep)%2!=0 { order.reverse(); }
        order.insert(0,[5,5]); order.push([5,5]);
        for (ordinal, lengths) in order.iter().enumerate() {
            ...
            samples.push(serde_json::json!({"sweep":sweep,"ordinal":ordinal,"lengths":lengths,
                "sentinel":ordinal==0 || ordinal==26,"warmup":sweep==0,"prepare_us":prepare_us,
