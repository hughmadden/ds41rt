...
## Split semantics in the old engines and current V4.1 decode

The checked packed W4A16 AOT exports in both pinned sibling engines instantiate a fused kernel with separate FC1 and FC2 persistent GEMM stages. GLM's `b12x/moe/_shared/kernels/w4a16/kernel.py` calls FC1 around line 7125, activation around 7181 and FC2 around 7231; DS4 has corresponding calls around 6527, 6583 and 6633. Intermediate buffers and grid barriers let FC2 reuse FC1/activation output. These are global buffers, although physical DRAM traffic depends on cache behavior. They do not recompute all of FC1 for each FC2 output tile.

Their scheduler contains split-K machinery. The special small-row `B12X_W4A16_SMALL_M_SPLITK` switch defaults to zero; this is distinct from both ordinary tiled scheduling and V4.1's current five-way output split. Do not describe all these mechanisms simply as “split kernel.”

Current V4.1 uses W4A8: native FP4 weights and FP8 E4M3 activations with UE8M0 K32 scales. BF16 transport and intermediate rounding boundaries do not make its matrix computation W4A16. The [deployed one-row output split](ds41-decode-dispatch-rollout.md) repeats FC1 per output partition to increase parallelism. The next candidate is splitting intermediate slices and reducing ordered partial outputs, to retain parallelism while avoiding repeated FC1 weight reads. That candidate remains unimplemented and unmeasured. [TP scaling diagnostics](ds41-expert-tp-efficiency.md) report effective unique-weight throughput, not measured DRAM bandwidth.
