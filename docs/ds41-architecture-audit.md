...
The TCP listener now honors an explicit executor chunking capability, calls synchronous streaming execution on a blocking worker, and passes responses through a one-entry channel to timed asynchronous socket writes. Header identity, frame size and final-marker sequencing are checked before enqueueing, while native row coverage remains the native receiver's responsibility. The persistent client's `roundtrip_chunks` method bounds frame count and size, invokes a frame sink, and restores its reusable socket only after successful final-frame consumption; errors and cancellation close the socket without automatic partial-wave replay. Single-response executors retain their previous path. Daemon compilation passes with 447 warnings, including unused native entry points; no runtime tests were run, and native executor registration, GPU worker startup and coordinator dispatch still remain to be connected.

The `expertd-native` command now registers a native streaming executor and owns all 40 backbone layers on a dedicated thread, using one rebindable execution workspace and bounded request/response queues. Startup admits resident weights plus loading staging and execution workspace before payload reads, and preserves configured TP-rank identity through the TCP listener. The command and remaining deployment/coordinator/runtime limitations are documented in `ds41-native-expert-service.md`; compilation passes, with no GPU execution qualification claimed for this integration.

Coordinator expert dispatch now has a concurrent four-peer TCP owner and an RTX wave owner that copies validated chunks into persistent FP32 planes before completion-gated native reduction. Each connection is checked against its expected TP identity, and failed or cancelled waves cannot expose output. The M16 device allocation budget is 8,192,000 bytes for four route planes and shared/final BF16 buffers; synchronous host ingress/copies are a fallback, with scheduler, RDMA and runtime qualification still open.

Engram staging now concatenates up to 16 request-owned address batches into preallocated token-major FP8/E8M0 rows and a text mask, reads each distinct mapped table row once in ascending address order, and scatters duplicate rows into their original token/head slots without modifying request histories. Image rows perform no table reads and produce zero embeddings with unit scales. `EngramDeviceRows` owns bounded upload/dequantization buffers, synchronizes before exposing BF16 `[token,24,256]` output, and invalidates output on errors; M16 needs 298,000 device bytes excluding driver and host storage. Generic native allocation/stream ownership is shared with expert execution. The mapped gather must run on an I/O worker because prefetched pages can still fault; background gather scheduling, projection/gate execution and full scheduler integration remain open, with no new GPU qualification claimed.

The engram staging integration passes daemon `cargo check` and `cargo build` through the configured Python environment, with 436 warnings including unused staging APIs pending scheduler wiring; no tests or model loads were run for this change.

`EngramGatherer` now runs mapped gathers on a dedicated I/O thread with 1–32 preallocated staging slots under an explicit host payload budget. The pool bounds queued, active and completed-but-unconsumed work; immutable batches stay associated with each result, and exhausted admission returns backpressure without waiting. Request-owned tickets support nonblocking completion polling, immediate disposal of queued results on cancellation, and discarding results after an uninterruptible in-progress page fault finishes. `EngramDeviceRows::poll_upload` consumes completed leases on the CUDA thread and returns staging to the pool after synchronized upload/dequantization. Shutdown skips queued gathers and joins the active worker. The daemon builds with 437 warnings; no runtime tests were run, and the scheduler still must submit early gathers and cancel tickets on rollback before consuming results.

The request-owned `EngramPipeline` now prepares compressed token/image batches and submits both layers' early advice/gathers before model execution, retries bounded admission during polling, and validates pipeline identity plus current request generations before upload. Whole-wave acceptance validates all histories before mutation and cancels remaining I/O, preserving rejected suffix rollback. Daemon compilation/build pass with 437 warnings; model scheduler call sites, projection/gate execution and runtime qualification remain open in `ds41-engram-pipeline.md`.

Native coordinator tensor range reads now support bounded pinned uploads without eager engram table reads or routed-expert loading through the dense path. Engram projection/scales and gate weights retain their native representations in owned RTX allocations, while an owned residual gate consumes completed projected KV and image masks through the existing fused CUDA kernel. Each layer's four resident tensors require 157,521,920 bytes; M16 gate output requires 655,360 bytes. The daemon builds with 443 warnings, but the native FP8 projection export, complete scheduler invocation and runtime qualification remain open.

Native engram FP8 AOT now exports K32 activation quantization and 32x32-scale GEMM at all six capacities, validates generated ABIs, and links all twelve objects into the coordinator library with source/artifact hashes recorded in `ds41-native-fp8-aot-build.json`; native launch bindings, scale packing and runtime qualification remain open.

The native FP8 bridge now initializes checked module handles, uses generated b12x launch-grid tables, packs native 32x32 scales without modifying FP8 weights, and exposes library-borrowing Rust launches to the owned engram projection/gate sequence. Native and daemon builds pass, public symbols are present, and coordinator release/WIP scripts enable and preserve FP8 artifacts with shell syntax checked; full container and numerical/graph qualification remain open in `ds41-native-fp8-bridge-build.json`. SparkInfer main is pinned to `02fea5c504f75f80fe3ec5e4e22ff20338e66803`, which shares quantizer grid calculation between tensor and native export paths without changing its policy.
...
Four physical GB10 hosts now pass current native expert ownership/execution and complete-plane TP4 TCP arithmetic to an RTX reducer with bitwise full-width-reference results; this removes the two-physical-host limitation of the earlier fixture without establishing production service or full-model readiness.
